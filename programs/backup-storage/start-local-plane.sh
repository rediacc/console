...
#      reach it, carrying BACKUP_S3_* so `createBackupPlane` selects the
...
#
# Same reasoning for BACKUP_S3_ENDPOINT: the presigned URLs the server signs are
...
echo "==> account dev gateway on :$GATEWAY_PORT (BACKUP_S3_* -> $PROBE_BUCKET)"
cd "$ACCOUNT_DIR"
# `set -a` + source: the gateway needs the .env crypto keys. BACKUP_S3_* are
...
export GATEWAY_PORT
export BACKUP_S3_ENDPOINT="http://${BRIDGE_HOST}:${RUSTFS_PORT}"
export BACKUP_S3_BUCKET="$PROBE_BUCKET"
