...
const ConnectivityTestModal: React.FC<ConnectivityTestModalProps> = ({
  ...
  // Table columns
  const statusVariantMap: Record<TestResult['status'], StatusVariant> = {
    pending: 'neutral',
    testing: 'processing',
    success: 'success',
    failed: 'error',
  ...
  const columns: ColumnsType<TestResult> = [
      ...
      render: (name: string, record: TestResult) => {
        const renderIcon = () => {
          switch (record.status) {
            ...
            case 'success':
              return <CheckCircleOutlined data-testid={`connectivity-status-icon-success-${name}`} />
            case 'failed':
              return <CloseCircleOutlined data-testid={`connectivity-status-icon-failed-${name}`} />
            case 'pending':
            default:
              return <ClockCircleOutlined data-testid={`connectivity-status-icon-pending-${name}`} />
          }
        }
    ...
    },
    {
      ...
      dataIndex: 'teamName',
      key: 'teamName',
      render: (name: string) => <ResourceTag>{name}</ResourceTag>
    },
    {
    ...
    },
    {
      ...
      render: (status: TestResult['status'], record: TestResult) => {
        const copy = {
          pending: t('machines:pending'),
          testing: t('machines:testing'),
          success: t('machines:connected'),
          failed: t('machines:failed'),
        }

        return (
          <StatusPill
            $variant={statusVariantMap[status]}
            data-testid={`connectivity-status-tag-${record.machineName}-${status}`}
          >
            {copy[status]}
          </StatusPill>
        )
    ...
    },
    {
      ...
      render: (message: string | undefined, record: TestResult) => {
        if (!message) return '-'
        return <MessageText $isError={record.status === 'failed'}>{message}</MessageText>
      }
    }
  ]
