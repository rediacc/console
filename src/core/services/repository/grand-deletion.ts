...
import { type RepositoryWithRelations, isCredential, isFork } from '../repository'
 ...
 */
export function canDeleteGrandRepository(
 ...
 * @param grandRepository - The grand repository
 * @param allRepositories - Array of all repositories
 * @returns Array of child clones
 */
export function findChildClones(
  grandRepository: RepositoryWithRelations,
  allRepositories: RepositoryWithRelations[]
): ChildClone[] {
  const grandGuid = grandRepository.repositoryGuid

  return allRepositories
    .filter(r =>
      r.grandGuid === grandGuid &&
      r.repositoryGuid !== grandGuid // Exclude the grand repository itself
    )
    .map(r => ({
