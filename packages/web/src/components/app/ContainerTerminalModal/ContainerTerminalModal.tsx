/**
 * Container Terminal Modal Component
...
import { ContainerOutlined, ExpandOutlined, ReloadOutlined } from '@/utils/optimizedIcons';
...
interface ContainerTerminalModalProps {
  ...
  /** Shell command to run (default: /bin/sh) */
  command?: string;
}

/**
 * Container Terminal Modal for docker exec in Electron desktop app
 *
 * Opens an xterm.js terminal connected via SSH to run docker exec.
...
export const ContainerTerminalModal: React.FC<ContainerTerminalModalProps> = ({
  ...
  // Use the container hook for docker exec session management
  ...
  const title = (
    <Space>
      <ContainerOutlined />
      <Text>
        {t('resources:localActions.containerTerminal')} - {displayName}
      </Text>
      ...
      footer={
        <Flex justify="space-between" align="center">
          <Space>
            {container.isConnected && (
              <Text type="secondary">
                {t('resources:localActions.containerConnectedTo', {
      ...
      >
        ...
        {container.isConnecting && (
          <Alert
            ...
            description={t('resources:localActions.containerEstablishingConnection', {
              container: displayName,
            })}
            showIcon
            // eslint-disable-next-line no-restricted-syntax
