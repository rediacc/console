...
export const useMachineExclusivity = (teamName?: string) => {
  const { t: _t } = useTranslation(['machines', 'ceph']);
  ...
  const { data: availableMachinesData = [], isLoading: loadingAvailable } =
    useAvailableMachinesForClone(teamName || '', !!teamName);
  ...
  const checkAvailability = useCallback(
    async (machineNames: string[]): Promise<MachineAvailability> => {
      ...
      machineNames.forEach((machineName) => {
        ...
        if (cached && now - cached.timestamp < CACHE_DURATION) {
          if (cached.result.isValid) {
            ...
            const exclusivityError = cached.result.errors.find(
              (e) => e.code === 'MACHINE_ALREADY_ASSIGNED'
            );
  ...
  const validateExclusivity = useCallback(
    async (
      _machines: Machine[],
  ...
  const getAvailableMachines = useCallback(
    (assignmentType?: 'cluster' | 'image' | 'clone'): AvailableMachine[] => {
      if (!assignmentType) {
  ...
  const suggestAlternatives = useCallback(
    (unavailableMachines: string[], count: number): string[] => {
  ...
  const getCachedValidation = useCallback(
    (machineName: string): ValidationResult | null => {
