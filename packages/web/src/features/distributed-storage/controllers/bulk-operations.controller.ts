...
export class BulkOperationsController {
    ...
    const batchInfo: BulkOperationBatch = {
      ...
      error:
        batchResult.failed.length > 0
          ? new Error(batchResult.failed.map((f) => f.error).join('; '))
          : undefined,
    };
  ...
  async performBulkValidation(
    ...
    if (machines.length > 100) {
      const batchResult = await BatchApiService.batchValidateMachines(machines, targetType);
    ...
    const validationResult = MachineValidationService.validateBulkAssignment(machines, targetType);
    ...
    if (checkAvailability && validationResult.validMachines.length > 0) {
      ...
      if (unavailableMachines.length > 0) {
        ...
        validationResult.validMachines = validationResult.validMachines.filter(
          (m) => !unavailableMachines.includes(m)
        );
  ...
  generateBulkOperationReport(
    ...
    const report: BulkOperationReport = {
      ...
      summary: {
        ...
        skipped:
          result.totalMachines - result.successfulMachines.length - result.failedMachines.length,
      },
