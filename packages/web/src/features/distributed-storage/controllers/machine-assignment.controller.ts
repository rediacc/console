...
import type { useMachineAssignment, useMachineExclusivity } from '../hooks';
import type { PoolIdentifier } from '../hooks';
import type { AssignmentConflict, ConflictResolution } from '../models';
import {
  WorkflowStep,
...
const mapToValidationTarget = (type: MachineAssignmentType): 'cluster' | 'image' | 'clone' => {
  switch (type) {
    case 'CLUSTER':
      return 'cluster';
    case 'IMAGE':
...
export class MachineAssignmentController {
  ...
  private emit<TEvent extends WorkflowEvent>(
    event: TEvent,
    workflowId: string,
    ...payload: WorkflowEventPayloadMap[TEvent] extends undefined
      ? []
      : [WorkflowEventPayloadMap[TEvent]]
  ) {
        ...
        steps.push({
          ...
          execute: async () => {
            ...
            if (availability.unavailable.length > 0) {
              result.conflicts = Object.entries(availability.conflicts).map(
                ([machine, conflict]) => ({
      ...
      if (result.conflicts.length > 0 && options.autoResolveConflicts) {
        steps.push({
          ...
          execute: async () => {
            ...
            const finalMachines = machineNames.filter(
              (m) => !result.skippedMachines.includes(m) || resolvedMachines.includes(m)
            );
      ...
      steps.push({
        ...
        execute: async () => {
          ...
          if (assignResult.success) {
            result.assignedMachines = machineNames.filter(
              (m) => !result.skippedMachines.includes(m)
            );
          } else {
            result.assignedMachines = machineNames.filter(
              (m) =>
                !result.skippedMachines.includes(m) && !assignResult.failedMachines?.includes(m)
            );
   ...
   */
  async validateAndAssign(context: ControllerValidationContext): Promise<AssignmentWorkflowResult> {
