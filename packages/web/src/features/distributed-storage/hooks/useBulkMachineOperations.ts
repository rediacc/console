...
export const useBulkMachineOperations = (teamName?: string) => {
  ...
  const [lastOperationContext, setLastOperationContext] = useState<LastOperationContext | null>(null)
    ...
    try {
      switch (lastOperationContext.type) {
        case 'assign-cluster': {
          ...
          if (result.success) {
            showMessage('success', t('machines:bulkOperations.rollbackSuccess', {
