...
export const useBackgroundRefresh = (
  ...
  const getMachinesToRefresh = useCallback(() => {
    ...
    if (onlySelectedMachines && selectedMachines.length > 0) {
      machinesToRefresh = machines.filter((m) => selectedMachines.includes(m.machineName));
    }

    // Limit number of machines
    if (machinesToRefresh.length > maxMachines) {
      // Prioritize selected machines, then take first N
      const selected = machinesToRefresh.filter((m) => selectedMachines.includes(m.machineName));
      const unselected = machinesToRefresh.filter((m) => !selectedMachines.includes(m.machineName));
 ...
 */
export const useSmartRefresh = (machines: Machine[], teamName: string) => {
