...
// Base selectors
export const selectMachineAssignmentState = (state: RootState) => state.machineAssignment;

export const selectSelectedMachines = (state: RootState) =>
  state.machineAssignment.selectedMachines;

export const selectSelectionMode = (state: RootState) => state.machineAssignment.selectionMode;
...
export const selectActiveFilters = (state: RootState) => state.machineAssignment.activeFilters;

export const selectExpandedGroups = (state: RootState) => state.machineAssignment.expandedGroups;
...
export const selectFilteredMachines = createSelector(
  [(_: RootState, machines: Machine[]) => machines, selectActiveFilters],
  (machines, filters) => {
    ...
    // Filter by team name
    if (filters.teamName) {
      filtered = filtered.filter((machine) => machine.teamName === filters.teamName);
    }
...
export const selectRecentSuccessfulOperations = createSelector(
  ...
  (history): OperationHistoryEntry[] => history.filter((op) => op.result === 'success').slice(0, 5)
);
...
export const selectMachinesBySelectionStatus = createSelector(
  [(_: RootState, machines: Machine[]) => machines, selectSelectedMachines],
