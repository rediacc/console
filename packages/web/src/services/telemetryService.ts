...
import { BatchSpanProcessor, SimpleSpanProcessor, TraceIdRatioBasedSampler } from '@opentelemetry/sdk-trace-web'
...
class TelemetryService {
  ...
  async initialize(config: TelemetryConfig): Promise<void> {
    ...
    try {
      ...
      const spanProcessor = config.environment === 'development'
  ...
  trackApiCall(method: string, url: string, statusCode?: number, duration?: number, error?: string): void {
  ...
  private getUserAttributes(): Record<string, string> {
    ...
    if (this.userContext.email) attributes['user.email_domain'] = this.userContext.email.split('@')[1] || ''
