...
export const useCreateRepo = createMutation<{
  ...
  successMessage: (vars) => `Repo "${vars.repoName}:${vars.repoTag || 'latest'}" created successfully`,
...
export const useDeleteRepo = createMutation<{
  ...
  request: ({ teamName, repoName, repoTag }) =>
    api.repos.delete(teamName, repoName, repoTag),
  invalidateKeys: ['repos', 'teams', 'machines'],
  successMessage: (vars) => `Repo "${vars.repoName}:${vars.repoTag || 'latest'}" deleted successfully`,
