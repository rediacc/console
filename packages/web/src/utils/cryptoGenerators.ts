...
async function exportRSAPrivateKeyToPEM(privateKey: CryptoKey): Promise<string> {
  ...
  const pemExported = formatPem(exportedAsBase64, '-----BEGIN PRIVATE KEY-----', '-----END PRIVATE KEY-----')
...
async function exportEd25519PublicKeyToSSH(publicKey: CryptoKey, comment: string = ''): Promise<string> {
  const exported = await crypto.subtle.exportKey('spki', publicKey)
  const exportedAsBase64 = arrayBufferToBase64(exported)
