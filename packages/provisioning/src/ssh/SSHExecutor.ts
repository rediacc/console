...
export class SSHExecutor {
  ...
  private readonly sshConfigOptions?: SSHConfigOptions;

  /**
   * Create an SSHExecutor with optional configuration.
   *
   * @param sshConfigOptions - Optional SSH configuration (data dir, key path)
   */
  constructor(sshConfigOptions?: SSHConfigOptions) {
    this.sshConfigOptions = sshConfigOptions;
  }
  ...
  getSSHOptions(config?: SSHConfig): string {
    ...
    let opts = getSSHOptions(this.sshConfigOptions);
  ...
  getSCPOptions(config?: SSHConfig): string {
    ...
    let opts = getSCPOptions(this.sshConfigOptions);
  ...
  private logKeyStatusOnce(): void {
    ...
    const keyPath = getSSHPrivateKeyPath(this.sshConfigOptions);
    if (isSSHKeyAvailable(this.sshConfigOptions)) {
...
/**
 * Get the default singleton SSHExecutor instance.
 *
...
/**
 * Create a new SSHExecutor with custom configuration.
 *
 * Use this when you need a non-default SSH configuration.
 */
export function createSSHExecutor(options?: SSHConfigOptions): SSHExecutor {
  return new SSHExecutor(options);
}
