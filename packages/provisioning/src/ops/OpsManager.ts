 ...
 *
 * NOTE: This version uses injectable configuration instead of reading from environment.
 * For environment-based initialization, use loadConfigFromEnv() or createOpsManagerFromEnv().
 */
export class OpsManager {
   ...
   * @param vmExecutor - Optional custom VM executor (for testing or custom SSH config)
   */
  constructor(provisioningConfig: ProvisioningConfig, vmExecutor?: OpsVMExecutor) {
    ...
    this.commandRunner = new OpsCommandRunner(
      provisioningConfig.renet.binaryPath,
      provisioningConfig.renet.rootPath
    );
    this.vmExecutor = vmExecutor ?? new OpsVMExecutor();
  ...
  /**
   * Get the underlying command runner for advanced operations
   */
  getCommandRunner(): OpsCommandRunner {
    return this.commandRunner;
  }

  /**
   * Get the underlying VM executor for advanced operations
   */
  getVMExecutor(): OpsVMExecutor {
