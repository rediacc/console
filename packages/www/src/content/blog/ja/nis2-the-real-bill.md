...
**バックアップ**は2つのモードで動作します。ホットはクラッシュ整合性のある BTRFS スナップショットです。ダウンタイムなし。コールドは停止、スナップショット、起動のサイクルです。両方とも systemd タイマーでスケジュールされます。両方とも rclone で多くの宛先に配信されます。ボリュームは LUKS2 暗号化されています。オペレーターが鍵を保持します。Rediacc 社は平文を見ることはありません。[バックアップとリストア](/ja/docs/backup-restore)および[クロスバックアップ戦略](/ja/docs/cross-backup)を参照してください。
...
**テストデータとフルスタッククローン**は BTRFS リフリンクで動作します。フォークはリポジトリサイズに関係なく一定時間です。フルスタックとはデータ、設定、コンテナ、サービスを意味します。[PocketOS テスト](/ja/blog/i-tested-rediacc-against-the-pocketos-incident)では 128 GB のリポジトリを 7.2 秒でフォークしました。フォークは縮小されたステージングコピーではなく、現在の本番そのものです。[リスクフリーアップグレード](/ja/docs/risk-free-upgrades)を参照してください。

**インスタントリストア**: 任意の rclone ターゲットから `rdc repo pull` で新鮮なフォークに取り込み、親リポジトリのワイルドカード証明書でカバーされるフォーク固有のサブドメインで起動します。DNS の混乱も、証明書のダンスも必要ありません。インシデント対応の演習で「回復にかかった実際の時間」を記録する際、この手順の再現性と速度がそのまま監査証拠になります。

**統一された監査ログ。** コントロールプレーン全体で 70 以上のイベントタイプを記録します。サインイン、API トークン、設定書き込み、リポジトリのライフサイクル、バックアップ、同期、ターミナルセッション、マシン操作をカバーします。チェーンはオペレーターのワークステーションでハッシュリンクされています。`rdc config audit verify` がエンドツーエンドで検証します。
...
さらに詳しく知りたい場合は3つのリンクがあります。パブリックマッピングは[NIS2 と DORA](/ja/docs/legal-nis2-dora)にあります。より広い枠組みは[コンプライアンス概要](/ja/docs/legal-overview)にあります。Rediacc 側の商用情報は[サブスクリプションとライセンシング](/ja/docs/subscription-licensing)にあります。
...
データプレーンの統合：5つのコスト項目が1つ（Rediacc）と既存の IaaS 項目に集約されます。サプライヤー登録簿のデータプレーンセクションは5エントリから2エントリに減少します。継続的有効性の説明はハッシュチェーンされた監査ログエビデンスを伴う週次ドリルに変わります。回復テストの説明は `rdc backup status` の出力と週次のリストアドリルによって裏付けられます。GRC プラットフォームへの入力となるエビデンスの質も改善します。バックアップベンダー個別のレポートではなく、コントロールプレーン全体の統一した監査ログをエビデンスとして提出できるからです。
...
さらに深く知りたい場合は3つのドキュメントがあります。[ゼロコストバックアップ](/ja/docs/zero-cost-backup)は既存のサービスよりもストレージが軽い理由を説明しています。[クロスバックアップ戦略](/ja/docs/cross-backup)は大陸間 DR をカバーしています。[サブスクリプションとライセンシング](/ja/docs/subscription-licensing)は商用面です。
