...
**統一された監査ログ。** コントロールプレーン全体で 70 以上のイベントタイプを記録します。サインイン、API トークン、設定書き込み、リポジトリのライフサイクル、バックアップ、同期、ターミナルセッション、マシン操作をカバーします。チェーンはオペレーターのワークステーションでハッシュリンクされています。`rdc audit verify` がエンドツーエンドで検証します。
