...
description: "PocketOS ha perso il suo database di produzione per colpa di un agente Cursor in 9 secondi. Ho eseguito lo stesso test, cronometrato ogni passo, e annotato cosa ha retto e cosa tocca a te sistemare."
...
Esamina con attenzione la timeline di Jer. Quattro guasti separati si sono accumulati l'uno sull'altro.
...
Soffermati sul fallimento 4 per un momento. Le regole di sistema di Cursor dicevano all'agente di non eseguire mai comandi git distruttivi senza una richiesta esplicita dell'utente. Dopo l'eliminazione, l'agente è stato invitato a spiegarsi. Ha prodotto una confessione scritta. Ha ammesso che eliminare un volume del database è "l'azione più distruttiva e irreversibile possibile: molto peggio di un force push" e ha elencato ogni regola di sicurezza che ha violato.
...
Ho una macchina di produzione reale che esegue Rediacc, chiamata `hostinger`. Tredici repository ci vivono: un server mail, un GitLab self-hosted, uno stack di osservabilità e una demo da 128 GB di StackOverflow che usiamo per i benchmark. Il disco è pieno all'87%. Spazio libero: praticamente zero. Il tipo di macchina dove gli errori fanno male.
...
Questa è la linea di responsabilità condivisa. Rediacc gestisce la metà dell'infrastruttura. La metà del servizio esterno vive nel tuo codice applicativo.
