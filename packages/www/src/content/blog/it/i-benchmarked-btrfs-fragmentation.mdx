---
title: La Tua Frammentazione Sembra Terrificante. Ho Misurato il Costo.
description: >-
  Il report sulla salute dell'archiviazione segnala repository a quasi 20.000
  estensioni/GB. Ho misurato entrambi gli estremi: penalizzazione in lettura
  zero. Perché Rediacc non ha un comando di deframmentazione.
author: Rediacc
...
> **TL;DR.** Il comando `rdc machine status --storage-health` di Rediacc riporta un indice di frammentazione per ogni repository. Su una macchina di produzione segnala GitLab a circa 19.650 estensioni per gigabyte. L'istinto e' di deframmentare. Ho misurato invece.
>
...
Francamente, il report sulla salute dell'archiviazione esiste per rispondere a una domanda: dove sta andando il mio disco. Mostra le dimensioni di ogni repository, quanti dati condivide con i suoi fork, e un indice di frammentazione. Quest'ultimo numero sembra spaventoso. Sulla macchina che gestisco, l'immagine del repository di GitLab riporta 268.771 estensioni su 14,6 GB. Sono circa 19.650 estensioni per gigabyte, e lo strumento la etichetta come "alta".
...
I file all'interno del volume non sono frammentati. Ho controllato: il binario gitaly di GitLab è in 10 estensioni, un file pack di git in 17. Il numero di 19.650 per gigabyte descrive il contenitore copy-on-write. È esattamente come dovrebbe apparire, e non dice nulla sul fatto che le letture siano lente.

## Il benchmark

Ho scelto due repository agli estremi opposti della scala. Uno poco frammentato, uno molto. Ho letto da ciascuno con IO diretto, che salta la page cache e forza una lettura reale dal disco.
...
La divergenza risponde alla domanda che la frammentazione non può: quante risorse disco reali e recuperabili ha questo repository. Su un pool in difficoltà, un repository a bassa divergenza è un obiettivo di pulizia scarso. I suoi byte sono condivisi, quindi cancellarlo libera quasi nulla. I byte vivono dove la divergenza è alta.
...
Quindi Rediacc riporta la frammentazione e la spiega. Non offre nessun pulsante per agire su di essa. La risposta onesta era misurare l'assunzione, non automatizzare il riflesso.
