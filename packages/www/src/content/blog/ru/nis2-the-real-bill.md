...
**Backup** работает в двух режимах. Горячий режим создаёт crash-consistent BTRFS-снимок без downtime. Холодный делает цикл остановки, снимка и запуска. Оба планируются через systemd timers. Оба отправляют данные на множество адресов через rclone. Тома зашифрованы с помощью LUKS. Ключи у оператора. Rediacc как компания никогда не видит данные в открытом виде. Смотрите [Backup & Restore](/ru/docs/backup-restore) и [Cross Backup Strategy](/ru/docs/cross-backup).
...
**Тестовые данные и полное клонирование стека** работают на BTRFS reflink. Форк выполняется за константное время, независимо от размера репозитория. Полный стек: данные, конфигурации, контейнеры и сервисы. В нашем [тесте PocketOS](/ru/blog/i-tested-rediacc-against-the-pocketos-incident) мы сделали форк репозитория 128 ГБ за 7,2 секунды. Форк является актуальным продакшеном, а не урезанной копией для стейджинга. Смотрите [Risk-Free Upgrades](/ru/docs/risk-free-upgrades).

**Мгновенное восстановление**: `rdc repo pull` из любой rclone-цели в свежий fork, запускаемый на поддомене, специфичном для fork, покрытом wildcard-сертификатом родительского репозитория. Никакой возни с DNS и сертификатами.

**Унифицированный журнал аудита.** Более 70 типов событий по всему control plane. Охватывают входы, API-токены, записи конфигурации, жизненный цикл репозитория, backup, синхронизацию, терминальные сессии и операции с машинами. Цепочка связана хешами на рабочей станции оператора. `rdc config audit verify` проверяет её от начала до конца.
...
Ещё три ссылки для глубокого погружения. Публичное соответствие: [NIS2 and DORA](/ru/docs/legal-nis2-dora). Более широкое обоснование: [Compliance Overview](/ru/docs/legal-overview). Коммерческая сторона Rediacc: [Subscription & Licensing](/ru/docs/subscription-licensing).
...
Ещё три документа для тех, кто хочет погрузиться глубже. [Бесплатное резервное копирование](/ru/docs/zero-cost-backup) объясняет, почему мы работаем легче конкурентов на стороне хранения. [Cross Backup Strategy](/ru/docs/cross-backup) охватывает межконтинентальный DR. Коммерческая сторона описана в [Subscription & Licensing](/ru/docs/subscription-licensing).
