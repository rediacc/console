...
description: "PocketOS perdió su base de datos de producción a manos de un agente de Cursor en 9 segundos. Ejecuté el mismo tipo de prueba en mi propia plataforma y cronometré cada paso. Esto es lo que aguantó y lo que sigue siendo responsabilidad del desarrollador."
...
El cómo-hacerlo completo vive en [Repositorios § Secretos](/es/docs/repositories#secrets). La receta es corta: `rdc repo secret set --name <repo> --key <KEY> --value <val> --current ""` para sembrar, `${REDIACC_SECRET_<KEY>}` en compose para consumir, `rdc repo fork` y comprobar que la lista de secretos del fork está vacía.
