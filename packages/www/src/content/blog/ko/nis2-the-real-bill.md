...
**백업**: 핫(크래시 일관성 있는 BTRFS 스냅샷, 무중단) 및 콜드(앱 일관성 있는 중단-스냅샷-재시작) 백업 전략. systemd 타이머로 예약되고 rclone으로 다중 목적지로 전송됩니다. LUKS 암호화 볼륨으로, 운영자가 자격증명을 보유하며 Rediacc 회사는 평문 데이터를 볼 수 없습니다. 운영 형태는 [백업 및 복원](/ko/docs/backup-restore)과 [크로스 백업 전략](/ko/docs/cross-backup)을 참조하십시오.
...
**테스트 데이터 및 전체 스택 복제**: BTRFS reflink 기반 fork로, 저장소 크기에 관계없이 일정한 시간이 소요되며 전체 스택(데이터, 구성, 컨테이너 상태, 서비스)을 복제합니다. 저희 [PocketOS 테스트](/ko/blog/i-tested-rediacc-against-the-pocketos-incident)에서 128 GB 저장소를 7.2초에 fork했습니다. Fork는 간소화된 스테이징 환경이 아닌 현재 운영 환경입니다. [위험 없는 업그레이드](/ko/docs/risk-free-upgrades)를 참조하십시오.
...
**통합 감사 로그**: 전체 컨트롤 플레인(인증, API 토큰, 구성 쓰기, 저장소 생명주기, 백업, 동기화, 터미널 세션, 머신 작업)을 커버하는 70개 이상의 이벤트 유형. 운영자 워크스테이션에서 해시 체인으로 연결되며, `rdc audit verify`로 종단간 무결성을 검증합니다.
...
NIS2 조항에 대한 기능 매핑은 [NIS2 및 DORA](/ko/docs/legal-nis2-dora)를 참조하십시오. 더 넓은 아키텍처 프레임은 [컴플라이언스 개요](/ko/docs/legal-overview)를 참조하십시오. Rediacc 측 상업적 세부사항은 [구독 및 라이선싱](/ko/docs/subscription-licensing)을 참조하십시오.
...
[제로 비용 백업](/ko/docs/zero-cost-backup)(저장소 측면에서 기존 업체보다 가볍게 운영하는 아키텍처 논거), [크로스 백업 전략](/ko/docs/cross-backup)(대륙 간 DR), [구독 및 라이선싱](/ko/docs/subscription-licensing)(상업적 측면)은 연결된 문서를 참조하십시오.
