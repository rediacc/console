...
Construo o Rediacc. A plataforma existe para tornar exatamente este tipo de falha mais difícil. Por isso sentei-me e executei o teste.
...
Percorra atentamente a linha do tempo do Jer. Quatro falhas separadas acumularam-se umas sobre as outras.
...
Tenho uma máquina de produção real a correr o Rediacc, chamada `hostinger`. Treze repositórios vivem nela: um servidor de correio, um GitLab auto-hospedado, uma stack de observabilidade e uma demo de 128 GB do StackOverflow que usamos para benchmarks. O disco está a 87% de capacidade. O espaço livre é basicamente zero. O tipo de máquina onde os erros doem.
...
A primeira coisa que o agente (eu, neste caso) tentaria: abrir uma shell no repositório de produção e olhar em volta.
...
Por isso o agente faz o que qualquer agente tentaria. Define o override por conta própria.
...
Um fork de 128 GB em 2,3 segundos. A razão é um reflink BTRFS. O fork são apenas metadados que apontam para os blocos do pai até que um lado escreva. Não se movem dados.
...
Um fork é um reflink BTRFS byte-a-byte do pai. O que quer que viva em `data/`, `.env` ou `secrets/` do pai vem com ele. Se o seu repositório contiver uma `STRIPE_LIVE_KEY`, um `AWS_ACCESS_KEY_ID` ou um token de API do Railway, o agente no fork pode lê-los. Pode chamar `api.stripe.com` ou `s3.amazonaws.com` ou `backboard.railway.app` com esses tokens. Do exterior, essas chamadas parecem ser de produção. O Stripe ou AWS não consegue distinguir o fork.
...
A lacuna do lado do programador é sua para fechar.
...
O guia completo está em [Repositórios - Segredos](/pt/docs/repositories#secrets). Três passos. Primeiro, semeie o valor: `rdc repo secret set <repo> --key <KEY> --value <val> --current ""`. Depois referencie `${REDIACC_SECRET_<KEY>}` no seu ficheiro compose. Por fim, execute `rdc repo fork` e confirme que o fork arranca com uma lista de segredos vazia.
