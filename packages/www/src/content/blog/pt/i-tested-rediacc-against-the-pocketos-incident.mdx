...
description: "O PocketOS perdeu a base de dados de produção para um agente Cursor em 9 segundos. Executei o mesmo teste, cronometrei cada passo, e anotei o que aguentou e o que é da sua responsabilidade corrigir."
...
O guia completo está em [Repositórios - Segredos](/pt/docs/repositories#secrets). Três passos. Primeiro, semeie o valor: `rdc repo secret set --name <repo> --key <KEY> --value <val> --current ""`. Depois referencie `${REDIACC_SECRET_<KEY>}` no seu ficheiro compose. Por fim, execute `rdc repo fork` e confirme que o fork arranca com uma lista de segredos vazia.
