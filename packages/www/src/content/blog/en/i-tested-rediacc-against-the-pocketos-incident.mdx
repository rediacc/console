...
The full how-to lives in [Repositories § Secrets](/en/docs/repositories#secrets). Three steps. First, seed the value: `rdc repo secret set <repo> --key <KEY> --value <val> --current ""`. Then reference `${REDIACC_SECRET_<KEY>}` in your compose file. Finally, run `rdc repo fork` and confirm the fork starts with an empty secret list.
