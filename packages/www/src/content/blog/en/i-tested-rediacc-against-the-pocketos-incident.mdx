...
The full how-to lives in [Repositories § Secrets](/en/docs/repositories#secrets). The recipe is short: `rdc repo secret set --name <repo> --key <KEY> --value <val> --current ""` to seed, `${REDIACC_SECRET_<KEY>}` in compose to consume, `rdc repo fork` and check that the fork's secret list is empty.
