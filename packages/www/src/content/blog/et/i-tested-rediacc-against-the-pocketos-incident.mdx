...
description: "PocketOS kaotas oma tootmisandmebaasi Cursor-agendi tõttu 9 sekundiga. Käitasin sama testi, ajatasin iga sammu ja märkisin, mis pidas vastu ja mis on sinu parandada."
...
Ehitan Rediacc-i. Platvorm eksisteerib selleks, et muuta täpselt selline ebaõnnestumine raskemaks. Seega istusin maha ja käitasin testi.
...
Käi Jer-i ajaskaala hoolikalt läbi. Neli eraldi ebaõnnestumist kuhjusid üksteisele peale.
...
Peatu hetkeks ebaõnnestumise 4 juures. Cursor-i süsteemireeglid käskisid agendil mitte kunagi käitada hävitavaid git-käske ilma kasutaja selgesõnalise taotluseta. Pärast kustutamist paluti agendil end selgitada. See koostas kirjaliku ülestunnistuse. Ta tunnistas, et andmebaasimahu kustutamine on "kõige hävitavam, pöördumatum toiming: palju hullem kui force push" ja loetles kõik ohutuseeskirjad, mida rikkus.
...
Mul on tõeline tootmismasin, mis käitab Rediacc-i, nimega `hostinger`. Sellel elab kolmteist repositooriumi: meilinserver, isemajutatud GitLab, vaatlustaristik ja 128 GB StackOverflow demo, mida kasutame võrdlusmõõtmisteks. Ketas on 87% täis. Vaba ruumi: sisuliselt null. Selline masin, kus vead teevad haiget.
...
**Saladused ei ela LUKS-i pildi sees.** Need elavad kettal eraldi tasandil: env-režiimi saladused jõuavad konteineriteni `${REDIACC_SECRET_<KEY>}` interpoleerimise kaudu sinu compose-failis, faili-režiimi saladused jõuavad konteineriteni tmpfs-failina `/run/secrets/<key>` juures Dockeri compose-i `secrets:` ploki kaudu. Mõlemal juhul ei puutu väärtus repositooriumi krüpteeritud pildiga. Hargi BTRFS reflink kopeerib pildi. See ei kopeeri seda, mis pildi sees kunagi polnud. Värske hargi salajuste nimekiri on tühi. Selle konteinerid käivituvad ilma tootmismandaatideta. Tootmiskõnesid pole isegi võimalik teha.
...
Täielik juhend elab [Repositooriumid § Saladused](/en/docs/repositories#secrets) lehel. Kolm sammu. Esiteks, seemne väärtus: `rdc repo secret set --name <repo> --key <KEY> --value <val> --current ""`. Seejärel viita `${REDIACC_SECRET_<KEY>}` oma compose-failis. Lõpuks käivita `rdc repo fork` ja kinnita, et hargi salajuste nimekiri on tühi.
