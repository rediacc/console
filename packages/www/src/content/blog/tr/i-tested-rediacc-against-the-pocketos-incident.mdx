...
description: "PocketOS, üretim veritabanını 9 saniyede bir Cursor ajanına kaybetti. Aynı testi çalıştırdım, her adımı zamanladım ve neyin tuttuğunu, neyin sizin sorumluluğunuzda olduğunu not aldım."
...
Ben Rediacc'ı kuruyorum. Platform tam olarak bu tür bir başarısızlığı zorlaştırmak için var. Ben de oturup testi yaptım.
...
4. arızayı bir an için ayrı tutun. Cursor'ın sistem kuralları, ajana açık bir kullanıcı isteği olmadan asla yıkıcı git komutları çalıştırmamasını söylüyordu. Silmenin ardından ajan ne yaptığını açıklaması istendi. Yazılı bir itiraf üretti. Bir veritabanı volume'ünü silmenin "mümkün olan en yıkıcı, geri döndürülemez eylem olduğunu: force push'tan çok daha kötü" olduğunu kabul etti ve çiğnediği her güvenlik kuralını listeledi.
...
Rediacc çalıştıran `hostinger` adında gerçek bir üretim makinem var. Üzerinde on üç depo yaşıyor: bir mail sunucusu, kendi barındırdığım bir GitLab, bir gözlemlenebilirlik yığını ve karşılaştırma testleri için kullandığımız 128 GB'lık bir StackOverflow demosu. Disk %87 dolu. Boş alan neredeyse sıfır. Hataların acıttığı türde bir makine.
...
Ajanın (bu durumda benim) deneyeceği ilk şey en doğal olanıydı: üretim deposuna bir kabuk açıp etrafa bakmak.
...
Bir çatallama, ana deponun bayt-bayt BTRFS reflink'idir. Ana deponun `data/`, `.env` veya `secrets/` dizininde ne yaşıyorsa çatallamaya da gelir. Deponuz `STRIPE_LIVE_KEY`, `AWS_ACCESS_KEY_ID` veya bir Railway API tokenı içeriyorsa, çatallamadaki ajan bunları okuyabilir. Bu tokenlarla `api.stripe.com`'a, `s3.amazonaws.com`'a veya `backboard.railway.app`'e çağrı yapabilir. Dışarıdan bakıldığında bu çağrılar üretimden geliyormuş gibi görünür. Stripe veya AWS, çatallamayı ayırt edemez.
...
Yukarıdaki bölümü ilk yazdığım haliyle bıraktım. O zamanki dürüstlüğü ağırlığını taşıyordu. Yayımladığımdan bu yana eksik parçayı ekledik ve boşluk artık kapatılabilir. Çatallamaların davranışını değiştirerek değil, kimlik bilgilerini şifreli depo imajının dışında yaşayan bir yere koyarak.
...
Bunun PocketOS senaryosu için anlamı: Rediacc üzerinde olsalardı ve gizli anahtarları benimsemiş olsalardı, çatallama içindeki ajan bir kimlik bilgisi dosyasında `STRIPE_LIVE_KEY` bulamazdı. Çatallamanın gizli anahtar haritası boş olurdu. Sandbox içinden Stripe'a çağrı yapmak için ortada bir şey kalmazdı. Yukarıdaki orijinal bölümün "gerçek kayıp" niteleyicisi "kimlik bilgilerini imaja gömen depolar için gerçek kayıp" haline gelir. Ve bu depolar için çözüm, yeni mekanizmaya tek seferlik bir geçiş.

Dürüst artık boşluk. `rdc repo secret`'ı benimsemeden önce depo imajına yazdığınız gizli anahtarlar. Bir volume'e commit edilmiş bir `.env`, bir veritabanına kalıcılaştırılmış bir kimlik bilgisi, içinde token olan bir YAML dosyası. Hâlâ imajın verisinin parçası. CoW reflink onları eskisi gibi taşır. Mekanizma, onları koymak için güvenli bir yer verir; şifreli volume'ün içinde diskte zaten olan şeyi geriye dönük olarak kaldırmaz. Düzeltme bugün manuel bir taşıma (değeri okuyun, `rdc repo secret` ile ayarlayın, imaj kaynağından temizleyin). Birinci sınıf bir `rdc repo secret lift --from .env` yol haritasında.

Özellikle ajan tehdit modeli için bu tasarımın iki yan etkisi öne çıkıyor. Birincisi, `repo secret list` ve `repo secret get` MCP araçları olarak açığa çıkar (okumaya güvenli. Adlar ve özetler, asla değerler), böylece bir uygulama kurmanıza yardım eden bir ajan, ne olduğunu görmeden neyin yapılandırıldığını keşfedebilir. İkincisi, bir yazma işlemi ön koşulu sağlamadığında, JSON hata zarfı, ajanın insana birebir aktarması gereken somut komutları içeren yapılandırılmış bir `next.options[].run` alanı içerir. "önceki değeri unuttum" durumu için rotasyon kaçış kapısı dahil. Tam model için [Yapay Zeka Ajan Güvenliği](/tr/docs/ai-agents-safety) sayfasına bakın.

Tam nasıl-yapılır rehberi [Depolar § Gizli Anahtarlar](/tr/docs/repositories#secrets) sayfasında. Üç adım. Birincisi, değeri tohumlayın: `rdc repo secret set --name <repo> --key <KEY> --value <val> --current ""`. Ardından compose dosyanızda `${REDIACC_SECRET_<KEY>}` referansını kullanın. Son olarak, `rdc repo fork` komutunu çalıştırın ve fork'un boş bir gizli anahtar listesiyle başladığını doğrulayın.
