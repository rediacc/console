...
Tam nasıl yapılır rehberi [Depolar § Gizli Anahtarlar](/tr/docs/repositories#secrets) sayfasında. Üç adım. Önce değeri ayarlayın: `rdc repo secret set <repo> --key <KEY> --value <val> --current ""`. Ardından compose dosyanızda `${REDIACC_SECRET_<KEY>}` referansını kullanın. Son olarak, `rdc repo fork` komutunu çalıştırın ve fork'un boş bir gizli anahtar listesiyle başladığını doğrulayın.
