...
**Gizli anahtarlar LUKS imajının içinde yaşamaz.** Diskte ayrı bir düzlemde yaşıyorlar: env modundaki gizli anahtarlar konteynerlere compose dosyanızdaki `${REDIACC_SECRET_<KEY>}` interpolasyonu olarak ulaşır, file modundaki gizli anahtarlar ise Docker compose'un `secrets:` bloğu üzerinden `/run/secrets/<key>` yolundaki bir tmpfs dosyası olarak konteynerlere ulaşır. Her iki durumda da değer asla deponun şifreli imajına yazılmaz. Çatallamanın BTRFS reflink'i imajı kopyalar. İmajda hiç olmayan şeyi kopyalamaz. Yeni bir çatallamanın gizli anahtar listesi boştur. Konteynerleri üretim kimlik bilgisi olmadan başlar, yani üretim çağrılarını yapmak bile mümkün değildir.
...
Tam nasıl-yapılır rehberi [Depolar § Gizli Anahtarlar](/tr/docs/repositories#secrets) sayfasında. Tarif kısa: tohumlamak için `rdc repo secret set --name <repo> --key <KEY> --value <val> --current ""`, tüketmek için compose'da `${REDIACC_SECRET_<KEY>}`, sonra `rdc repo fork` ve çatallamanın gizli anahtar listesinin boş olduğunu doğrulayın.
