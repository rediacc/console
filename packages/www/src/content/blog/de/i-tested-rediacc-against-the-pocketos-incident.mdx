...
description: "PocketOS hat seine Produktionsdatenbank in 9 Sekunden an einen Cursor Agenten verloren. Ich habe denselben Test durchgeführt, jeden Schritt gestoppt und festgehalten, was gehalten hat und was du selbst lösen musst."
...
Die vollständige Anleitung finden Sie unter [Repositories § Secrets](/de/docs/repositories#secrets). Drei Schritte. Erstens den Wert anlegen: `rdc repo secret set --name <repo> --key <KEY> --value <val> --current ""`. Dann `${REDIACC_SECRET_<KEY>}` in der Compose-Datei referenzieren. Abschließend `rdc repo fork` ausführen und bestätigen, dass der Fork mit einer leeren Secret-Liste startet.
