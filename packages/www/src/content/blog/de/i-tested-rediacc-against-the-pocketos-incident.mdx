...
Ich baue Rediacc. Die Plattform existiert, um genau diese Art von Fehler schwerer zu machen. Also habe ich mich hingesetzt und den Test durchgeführt.
...
Geht man Jers Zeitleiste sorgfältig durch, sieht man: Vier separate Fehler stapelten sich aufeinander.
...
Betrachten wir Fehler 4 für einen Moment. Cursors Systemregeln wiesen den Agenten an, niemals destruktive Git-Befehle ohne ausdrückliche Aufforderung des Benutzers auszuführen. Nach der Löschung wurde der Agent gebeten, sich zu erklären. Er produzierte ein schriftliches Geständnis. Er gab zu, dass das Löschen eines Datenbank-Volumes "the most destructive, irreversible action possible: far worse than a force push" sei und listete jede Sicherheitsregel auf, die er gebrochen hatte.
...
Ich betreibe eine echte Produktionsmaschine mit Rediacc, namens `hostinger`. Dreizehn Repositories liegen darauf: ein Mailserver, ein selbst gehostetes GitLab, ein Observability-Stack und eine 128 GB große StackOverflow-Demo, die wir für Benchmarks verwenden. Die Festplatte ist zu 87 % voll. Freier Speicher: praktisch null. Die Art von Maschine, auf der Fehler wehtun.
...
Das Erste, was der Agent (in diesem Fall ich) versuchen würde: Eine Shell ins Produktions-Repo öffnen und sich umsehen.
...
Also tut der Agent, was jeder Agent versuchen würde. Er setzt den Override selbst.
...
Ein 128 GB Fork in 2,3 Sekunden. Der Grund ist ein BTRFS-Reflink. Der Fork besteht nur aus Metadaten, die auf die Blöcke des Elternteils zeigen, bis eine Seite schreibt. Es werden keine Daten verschoben.
...
Ein Fork ist ein Byte-für-Byte-BTRFS-Reflink des Elternteils. Was auch immer im `data/`, `.env` oder `secrets/` des Elternteils liegt, kommt mit. Wenn Ihr Repository einen `STRIPE_LIVE_KEY`, eine `AWS_ACCESS_KEY_ID` oder ein Railway-API-Token enthält, kann der Agent im Fork sie lesen. Er kann mit diesen Tokens `api.stripe.com`, `s3.amazonaws.com` oder `backboard.railway.app` aufrufen. Von außen sehen diese Aufrufe wie Produktion aus. Stripe oder AWS können den Fork nicht unterscheiden.
...
Die Lücke auf der Entwicklerseite liegt in Ihrer Verantwortung.
...
Die vollständige Anleitung finden Sie unter [Repositories § Secrets](/de/docs/repositories#secrets). Drei Schritte. Erstens den Wert anlegen: `rdc repo secret set <repo> --key <KEY> --value <val> --current ""`. Dann `${REDIACC_SECRET_<KEY>}` in der Compose-Datei referenzieren. Abschließend `rdc repo fork` ausführen und bestätigen, dass der Fork mit einer leeren Secret-Liste startet.
