---
title: "Votre fragmentation semble terrifiante. J'ai mesuré le coût."
description: "Notre rapport de santé du stockage signale des dépôts à près de 20 000 extents/Go. J'ai mesuré les deux extrêmes : pénalité de lecture nulle. Pourquoi Rediacc n'a pas de commande defrag."
...
Franchement, le rapport de santé du stockage existe pour répondre à une seule question : où va mon disque ? Il affiche la taille de chaque dépôt, la quantité de données qu'il partage avec ses forks, et un chiffre de fragmentation. Ce dernier chiffre paraît effrayant. Sur la machine que j'exploite, l'image du dépôt GitLab déclare 268 771 extents sur 14,6 Go. Soit environ 19 650 extents par gigaoctet, et l'outil le qualifie d'"élevé".
...
Les fichiers à l'intérieur du volume ne sont pas fragmentés. J'ai vérifié : le binaire gitaly de GitLab est en 10 extents, un fichier git pack en 17. Le chiffre de 19 650 par gigaoctet décrit le conteneur copy-on-write. C'est exactement à quoi il devrait ressembler, et ça ne vous dit rien sur la lenteur des lectures.

## Le benchmark

J'ai choisi deux dépôts aux extrémités opposées de l'échelle. L'un peu fragmenté, l'autre très fragmenté. J'ai lu depuis chacun avec des E/S directes, ce qui ignore le cache de pages et force une vraie lecture disque.
...
La divergence répond à la question que la fragmentation ne peut pas poser : combien de disque réel et récupérable ce dépôt coûte-t-il ? Sur un pool serré, un dépôt à faible divergence est une mauvaise cible de nettoyage. Ses octets sont partagés, le supprimer libère donc presque rien. Les octets vivent là où la divergence est élevée.
...
Alors Rediacc rapporte la fragmentation et l'explique. Il n'offre aucun bouton pour agir dessus. La réponse honnête était de mesurer l'hypothèse, pas d'automatiser le réflexe.
