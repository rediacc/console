...
> **Mise à jour. Mai 2026.** Depuis la publication de cet article, Rediacc a livré des **secrets par dépôt** de première classe (`rdc repo secret`). La « lacune honnête » décrite ci-dessous (les identifiants SaaS externes intégrés dans votre dépôt) dispose désormais d'une réponse native qui garde les secrets entièrement hors de l'image LUKS, de sorte que les forks n'en héritent par défaut d'aucun. Rendez-vous à la nouvelle section finale [Mise à jour : la lacune est désormais comblable](#mise-à-jour--la-lacune-est-désormais-comblable) pour découvrir ce qui a été livré, ce qui a changé et le risque résiduel qui subsiste.
...
Le mode d'emploi complet vit dans [Dépôts § Secrets](/fr/docs/repositories#secrets). Trois étapes. Premièrement, amorcez la valeur : `rdc repo secret set <repo> --key <KEY> --value <val> --current ""`. Ensuite, référencez `${REDIACC_SECRET_<KEY>}` dans votre fichier compose. Enfin, exécutez `rdc repo fork` et confirmez que le fork démarre avec une liste de secrets vide.
