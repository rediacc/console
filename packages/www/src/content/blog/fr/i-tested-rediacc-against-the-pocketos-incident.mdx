...
description: "PocketOS a perdu sa base de données de production à cause d'un agent Cursor en 9 secondes. J'ai effectué le même type de test sur ma propre plateforme et chronométré chaque étape. Voici ce qui a tenu et ce qui reste de la responsabilité du développeur."
...
**Les secrets ne vivent pas à l'intérieur de l'image LUKS.** Ils vivent sur un plan séparé du disque : les secrets en mode env atteignent les conteneurs sous forme d'interpolation `${REDIACC_SECRET_<KEY>}` dans votre fichier compose, les secrets en mode file atteignent les conteneurs sous forme de fichier tmpfs à `/run/secrets/<key>` via le bloc `secrets:` de Docker compose. Dans les deux cas, la valeur n'est jamais écrite dans l'image chiffrée du dépôt. Le reflink BTRFS du fork copie l'image. Il ne copie pas ce qui n'a jamais été dans l'image. La liste de secrets d'un fork frais est vide. Ses conteneurs démarrent sans aucun identifiant de production, donc aucun appel de production n'est même possible.
...
Le mode d'emploi complet vit dans [Dépôts § Secrets](/fr/docs/repositories#secrets). La recette est courte : `rdc repo secret set --name <repo> --key <KEY> --value <val> --current ""` pour amorcer, `${REDIACC_SECRET_<KEY>}` dans compose pour consommer, `rdc repo fork` puis vérifier que la liste de secrets du fork est vide.
