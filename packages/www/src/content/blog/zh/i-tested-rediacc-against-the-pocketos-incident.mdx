...
完整的操作指引见 [仓库 § 密钥](/zh/docs/repositories#secrets)。分三步：首先播种值，运行 `rdc repo secret set <repo> --key <KEY> --value <val> --current ""`；然后在 compose 文件里引用 `${REDIACC_SECRET_<KEY>}`；最后运行 `rdc repo fork`，确认 fork 的密钥列表是空的。
