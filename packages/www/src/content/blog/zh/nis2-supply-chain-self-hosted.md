...
**3. GRC 层仍属于你。** Rediacc 为操作员提供包含 70 余个事件的哈希链审计日志（`rdc config audit verify` 可端到端验证链的完整性）。但它不提供供应商登记册、控制框架或证据收集工作流。这些仍需借助 Drata、Vanta、OneTrust 或欧洲本土竞争者。配套的[实际账单文章](/zh/blog/nis2-the-real-bill)详细介绍了这种互补关系的成本结构。
...
如果你在未来十二个月内面临备份或灾难恢复续约，且 Article 21(2)(d) 已纳入采购评分表，有三个具体动作：

1. 在白板上画出你当前供应商的四层数据路径。如果你连第三层次级处理商都说不出来，那你就存在一个早于 NIS2 的登记册完整性问题，续约正是修复它的合适时机。
2. 用上面的六道问题清单审视现有供应商。把答案发给 DPO 和审计员，问他们这些差距是否已被接受。如果差距涉及第 3 层（密钥托管）或第 4 层（你没有枚举到的次级处理商），那就是切入点。
3. 看一看换成自托管控制平面后，替代方案的供应商登记册会是什么样子。比较的是登记册条目，而不是许可证成本。许可证成本通常只差一两倍，但登记册条目的形态完全不同。（配套文章 [NIS2 合规栈的结构性成本](/zh/blog/nis2-the-real-bill) 详细梳理了哪些可以消除、哪些还会留下。）

如果我们是你的候选方案之一，这个承诺是具体的。把你们的供应商问卷发给我们，我们会基于一个已部署的实例来填写，给出我们对你这些问题的真实答案，包括差距。如果你想在提交材料前先过一遍架构，我们可以安排一次 30 分钟的工程团队架构评审。通往可防御登记册条目的路，不是一本光鲜的宣传册，而是答案本身，包括那些让人不舒服的答案。
