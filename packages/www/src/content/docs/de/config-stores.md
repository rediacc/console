---
title: "Konfiguration & Stores"
description: "CLI-Konfigurationsdateien verwalten und mit externen Backends wie Git, Bitwarden, S3, HashiCorp Vault oder lokalen Dateien synchronisieren. Unterstützt Verschlüsselung für sichere Übertragung."
category: "Guides"
order: 8
language: de
...
Die Standard-Konfigurationsdatei ist `~/.config/rediacc/rediacc.json`. Sie können benannte Konfigurationen für verschiedene Umgebungen erstellen:
...
Speichert jede Konfiguration als separate **sichere Notiz** in Ihrem Bitwarden-Tresor. Elemente werden mit dem Präfix `rdc:` benannt (z. B. `rdc:rediacc`, `rdc:production`).
