...
Wenn Sie Secrets im Fork zum Testen benötigen, setzen Sie sie explizit mit Sandbox-Werten auf dem Fork, zum Beispiel `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`. Jetzt spricht der Fork mit der Stripe-Sandbox und startet sauber. Produktionszugangsdaten haben die Produktion nie verlassen.
