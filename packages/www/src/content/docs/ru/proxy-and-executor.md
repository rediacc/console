...
Executor — это сам CLI, запущенный как сервер через `rdc serve`. Тот же самый бинарник, который оператор запускает на ноутбуке, становится тем, что выполняет команды от его имени. У него два варианта размещения:
