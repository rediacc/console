...
description: "Использование веб-консоли Rediacc для управления машинами, репозиториями и резервными копиями"
category: Reference
order: 1
language: ru
...
**Rediacc** является самостоятельно размещаемым программным обеспечением, которое вы устанавливаете на собственных серверах. С его помощью вы создаёте резервные копии, разветвляете и восстанавливаете целые живые системы. Приложения, базы данных и конфигурацию вместе, а не по файлам. Именно это и является сутью: большинство инструментов резервного копирования копируют файлы и теряют связи между ними.

Это руководство описывает работу с веб-консолью на [https://www.rediacc.com/](https://www.rediacc.com/).
...
Чтобы использовать Rediacc, вам сначала нужна учетная запись.
...
Управление пользователями: здесь вы контролируете, кто из вашей организации получает доступ.
...
Команды группируют пользователей, чтобы вы могли предоставлять доступ оптом.
...
Контроль доступа централизует разрешения через группы, а не для каждого пользователя отдельно.
...
В разделе Машины вы управляете своими серверами и репозиториями на них.
...
Вы можете запускать удалённые операции на машине из веб-интерфейса.
...
Репозиторий является одним изолированным развертыванием приложения. Его приложения, его данные, его конфигурация, его собственный Docker daemon. Поэтому почти каждая кнопка на этой странице действует на репозиторий, а не на машину, на которой он находится. Это стоит знать перед нажатием Delete.
...
Вы подключаетесь к репозиторию двумя способами. Настольное приложение даёт вам настоящий терминал и SSH. Браузер подходит для осмотра и управления кликами, но не может вести длинные сессии оболочки. Выбирайте настольное приложение, если выполняете реальную работу.
...
В разделе Хранилище вы определяете физические расположения, куда попадают ваши данные резервных копий.
...
В разделе Учетные данные вы управляете секретами, которые используют ваши репозитории для доступа к ресурсам.
...
Раздел Очередь отслеживает ожидающие и завершенные операции во всей системе.
...
Раздел Аудит хранит запись каждой операции, выполненной в системе.
