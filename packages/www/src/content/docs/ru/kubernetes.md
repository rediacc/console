...
- **Kubernetes-репозиторий представляет собой пространство имён.** `rdc repo create --cluster <name>` создаёт репозиторий, рабочим домом которого является пространство имён Kubernetes `<repo>` внутри этого кластера.
...
Узлы кластера также материализуются в `resources.machines`, поэтому вы можете подключиться по SSH к конкретному узлу обычной командой `rdc term connect -m <cluster>-<pool>-<n>`.
