...
Репозитории Rediacc поддерживают версионирование в стиле git на основе форков с копированием при записи. Каждый неизменяемый форк является **коммитом**: побайтово стабильным, замороженным образом, который отказывается монтироваться. Ветви являются именованными ссылками, указывающими на коммит. `rdc repo checkout` клонирует коммит через reflink обратно в перезаписываемый рабочий форк, а `rdc repo merge` объединяет две линии истории без изменения живого репозитория на месте.
