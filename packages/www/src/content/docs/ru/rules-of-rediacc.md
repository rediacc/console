...
- Форк-репозитории используют плоские авто-маршруты под wildcard-сертификатом машины. Пользовательские домены (`rediacc.domain`) игнорируются в форках — домен принадлежит grand-репозиторию.
...
- **Bind-монтирование в `${REPOSITORY_PATH}/...` рекомендуется** для ясности, но именованные тома также работают безопасно.
  ```yaml
  volumes:
    - ${REPOSITORY_PATH}/data:/data        # bind-монтирование (рекомендуется)
