...
- **Обязательные функции**: `prep()`, `up()`, `down()`. Необязательная: `info()`.
- `prep()` выполняется при **каждом развёртывании** — используйте для загрузки образов, генерации конфигурации и создания каталогов.
...
- **`rdc repo up`** выполняет `prep()`, затем `up()` — всегда.
