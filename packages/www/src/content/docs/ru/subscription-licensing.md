...
Отслеживание слотов машин проверяется на сервере. Когда CLI выпускает лицензию репозитория, сервер account проверяет квоту слотов машин подписки (например, 2 машины для Community, 5 для Professional). Слот удерживается в течение 1 часа с момента последнего выпуска лицензии репозитория на этой машине и автоматически освобождается после неактивности. План с 5 слотами может поэтому охватывать десятки машин со временем, поскольку слоты удерживаются только во время активного управления ресурсами.
