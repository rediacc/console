...
О разделении между workstation и сервером см. [rdc vs renet](/en/docs/rdc-vs-renet), а о жизненном цикле репозитория см. [Repositories](/en/docs/repositories).
...
Лицензия репозитория является подписанной лицензией для одного репозитория на одной машине. Это единственный файл лицензии, хранящийся на машине, размещённый по ключу подписи:
...
- `rdc repo push`, `rdc repo pull`, `rdc repo sync`, **полная проверка, включая срок действия**: для передачи резервных копий требуется активное право на использование
...
Ограничения остаются мягкими там, где это важнее всего: работающие репозитории (`up`, `down`, `delete`, автозапуск) продолжают функционировать даже после окончания подписки. А вот новые операции (создание, форк, изменение размера и обновление лицензии) и передача резервных копий (`push`, `pull`, `sync`) требуют активного права на использование.
...
О первоначальной настройке машины см. [Machine Setup](/en/docs/setup).
...
Об эксплуатационной настройке см. [On-Premise Installation - Licensing for Air-Gapped Deployments](/en/docs/on-premise), а о криптографическом дизайне см. [License Chain & Delegation](/en/docs/license-chain).
