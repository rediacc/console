...
Приложение запущено, но пока вы видели его только через `docker ps`. Три команды покрывают повседневную работу: **tunnel** — чтобы открыть приложение в браузере, **term** — чтобы выполнять команды внутри песочницы, **sync** — чтобы перемещать файлы между ноутбуком и репозиторием.
...
Tunnel, term, sync. Три команды — полный рабочий цикл.
