...
Эти три команды независимы. Вы можете выполнить любую из них без остальных. Они используют общую модель безопасности, описанную в разделе [Модель безопасности](#safety-model) ниже.

Очистка удаляет состояние, оставшееся после удалённых ресурсов. Чтобы освободить пространство, занятое *живыми* репозиториями (блоки, которые их файловые системы освободили, но пул ещё удерживает), используйте вместо этого [`rdc repo trim`](/ru/docs/repositories#reclaim-space-trim); эти два механизма дополняют друг друга.
