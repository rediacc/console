...
# Выполнить команду в песочнице Landlock (используется renet внутри)
renet sandbox-exec --allow-rw /path --allow-ro /usr --allow-exec /bin -- command
```

`sandbox-exec` применяет к команде ограничения файловой системы Landlock LSM. Изолированный процесс может обращаться только к явно разрешённым путям; любой другой доступ к файловой системе блокируется ядром. renet использует это внутри, чтобы изолировать выполнение Rediaccfile, операции compose и SSH-команды пределами точки монтирования соответствующего репозитория.
