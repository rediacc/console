...
Форк — это побайтовая копия образа родителя. Что есть в `.env` родителя, то есть и в `.env` форка. Контейнеры форка запускаются. Они читают тот же ключ Stripe. Они вызывают тот же Stripe API с продакшен-учётными данными. Для Stripe этот вызов — это *вы*.
...
`${REDIACC_SECRET_DB_HOST}` — режим `env`: обёртка compose в `renet` раскрывает его из хранилища секретов во время развёртывания.

Блок `secrets:` — режим `file`, с использованием стандартного механизма Docker. В пути хоста используется `${REDIACC_NETWORK_ID}`, поэтому один и тот же compose-файл работает для родителей и форков. У каждого форка свой сетевой ID.
