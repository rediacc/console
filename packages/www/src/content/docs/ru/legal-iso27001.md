...
|-----------------|-------------------|-------------------|
| **A.8**, Управление активами | A.8.1 Инвентаризация активов | Каждый репозиторий это отдельный идентифицируемый актив с уникальным GUID. `rdc machine status <machine> --repositories` выводит список всех репозиториев с размером, состоянием монтирования и количеством контейнеров. |
...
| **A.12**, Операционная безопасность | A.12.3 Резервное копирование | `rdc repo push/pull` с зашифрованным удалённым хранилищем на несколько пунктов назначения (SSH, S3, B2, Azure, GDrive). CoW-снимки для восстановления на определённый момент времени. `rdc repo admin validate` проверяет состояние резервных копий и целостность репозитория. |
| **A.12**, Операционная безопасность | A.12.4 Журналирование и мониторинг | 70+ типов событий (аутентификация, API-токены, конфигурация, лицензирование, операции с машинами). Мониторинг состояния машин через `rdc machine status`. Мониторинг статуса контейнеров и ресурсов. |
...
- Репозитории перечисляются по машинам (`rdc machine status --repositories`)
- Статус шифрования каждого репозитория, состояние монтирования, количество контейнеров и использование диска видимы
...
3. **Продвижение**: Использование `rdc repo promote` для замены форка на продакшен
...
- Проверки состояния машин (`rdc machine status --system`) поддерживают операционный мониторинг
- `rdc repo admin validate` проверяет состояние резервных копий после каждой операции
