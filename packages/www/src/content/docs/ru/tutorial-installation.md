...
> Префикс `time` — это приём оболочки, который выводит время выполнения команды. Мы используем его на протяжении всей серии, чтобы вы видели реальную скорость каждого шага. Он необязателен: уберите его, если он вам не нужен.
...
`rdc` — это CLI на вашем ноутбуке. На сервере работает отдельный компонент `renet`, которым `rdc` управляет по SSH. Вам никогда не придётся вручную подключаться к серверу по SSH: `rdc` сделает это за вас.
