...
Показывает все репозитории на машине с их размером, состоянием монтирования, состоянием шифрования и колонкой `Type`, которая классифицирует каждый как `grand` (верхний уровень), `fork` (CoW-копия другого репозитория) или `unknown` (происхождение не удалось определить). Полные правила классификации см. в разделе [Репозитории. Колонка Type и зеркало состояния](/ru/docs/repositories#колонка-type-и-зеркало-состояния).
