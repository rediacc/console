...
> **Примечание:** После удаления запись конфигурации сохраняется (репозиторий может существовать на других машинах). Используйте `rdc config repository remove <name>` для её удаления или `--archive-config` для сохранения учётных данных для восстановления.
