...
description: "Пошаговое руководство по развертыванию зашифрованной изолированной инфраструктуры на собственных серверах с использованием Rediacc в локальном режиме."
...
```
Ваша рабочая станция                 Удаленный сервер
┌──────────────┐        SSH        ┌──────────────────────────┐
│              │ ──────────────▶   │  renet (Go binary)       │
...
- macOS или Linux с SSH-клиентом
- Пара SSH-ключей (например, `~/.ssh/id_ed25519` или `~/.ssh/id_rsa`)

**На удаленном сервере:**
- Ubuntu 20.04+ или Debian 11+ (другие дистрибутивы Linux могут работать, но не протестированы)
...
Установите CLI Rediacc на вашу рабочую станцию:

```bash
curl -fsSL https://get.rediacc.com | sh
```

Эта команда загружает бинарный файл `rdc` в `$HOME/.local/bin/`. Убедитесь, что эта директория добавлена в PATH. Проверьте установку:

```bash
rdc --help
```

> Для обновления в дальнейшем выполните `rdc update`.

## Шаг 2: Создание локального контекста
...
- **Запуск (`up`):** Сначала корневой Rediaccfile, затем поддиректории в **алфавитном порядке** (от А до Я).
- **Остановка (`down`):** Поддиректории в **обратном алфавитном порядке** (от Я до А), затем корневой последним.
...
Переменные `{SERVICE}_IP` автоматически генерируются из `.rediacc.json` (см. Шаг 7). Соглашение об именовании преобразует имя сервиса в верхний регистр с заменой дефисов на подчеркивания, а затем добавляет `_IP`. Например, `listmonk-app` становится `LISTMONK_APP_IP`.
...
Порядок выполнения для `up`: корневой, затем `backend`, `database`, `monitoring` (А-Я).
Порядок выполнения для `down`: `monitoring`, `database`, `backend`, затем корневой (Я-А).

## Шаг 7: Сетевое взаимодействие сервисов (.rediacc.json)
...
IP-адрес сервиса вычисляется из идентификатора сети репозитория и слота сервиса:
...
Первые два смещения (0 и 1) зарезервированы для сетевого адреса и шлюза. Слоты сервисов начинаются со смещения 2.
...
# Создание зашифрованного репозитория (10 ГБ)
...
Вся конфигурация контекста хранится в `~/.rediacc/config.json`. Вот аннотированный пример того, как выглядит этот файл после выполнения описанных выше шагов:
...
- **Справочник CLI** — Полный справочник команд см. на странице [CLI-приложение](/ru/docs/cli-application).
- **Резервное копирование и восстановление** — Настройте внешнее резервное копирование в S3-совместимое хранилище для аварийного восстановления.
- **Обратный прокси** — Настройте Traefik для HTTPS с автоматическими сертификатами Let's Encrypt.
- **Контрольные точки/восстановление CRIU** — Создавайте контрольные точки запущенных контейнеров для мгновенной миграции или отката.
