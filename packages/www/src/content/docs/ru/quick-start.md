...
rdc config machine setup my-server        # Устанавливает renet + создаёт хранилище данных
```
...
rdc repo template apply app-postgres -m my-server -r my-app   # Развернуть docker-compose.yml + Rediaccfile
```
...
rdc repo status my-app -m my-server                  # Состояние монтирования, Docker, размер, шифрование
```
...
```bash
rdc vscode my-server my-app              # Открывает VS Code через SSH внутри песочницы репозитория
```
...
**Демонстрационный процесс:** `rdc repo template apply` -> `rdc vscode my-server my-app` -> редактирование `docker-compose.yml` -> `renet dev up` -> приложение работает -> итерация.
...
**Терминал:**
```bash
rdc term my-server my-app                            # SSH в песочницу репозитория
rdc term my-server my-app -c "curl localhost:3000"   # Выполнить команду и выйти
rdc term my-server                                   # SSH на машину (без песочницы)
```
...
```bash
rdc repo tunnel my-server my-app                     # Автоопределение контейнера и порта
rdc repo tunnel my-server my-app --port 5432         # Туннель к Postgres
rdc repo tunnel my-server my-app --port 5432 --local 15432  # Свой локальный порт
```
...
```bash
rdc repo fork my-app -m my-server --tag experiment --up     # Мгновенный CoW-клон + запуск
rdc repo list -m my-server                                  # Показывает: my-app (grand) + my-app:experiment (fork)
rdc repo delete my-app:experiment -m my-server              # Удалить форк, гранд не затронут
```
...
```bash
rdc config infra set my-server           # Настроить: базовый домен, публичные IP, диапазоны портов
rdc config infra show my-server          # Просмотр конфигурации
rdc config infra push my-server          # Отправить конфигурацию прокси на удалённый сервер
```
...
```bash
rdc repo template apply proxy -m my-server -r infra     # Развернуть прокси в репозитории
