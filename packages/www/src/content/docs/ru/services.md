...
| `up()` | После завершения всех `prep()` | Запуск сервисов (например, `renet compose -- up -d`) | Ошибка корневого Rediaccfile — **критическая** (останавливает всё). Ошибки в поддиректориях — **некритические** (логируются, продолжение к следующему) |
