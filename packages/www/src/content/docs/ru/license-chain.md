...
Каждая машина, работающая с Renet, хранит последнее известное состояние цепочки в `{licenseDir}/chain-state.json` (то есть `/var/lib/rediacc/license/chain-state.json`, рядом с директорией `repos/` для каждого репозитория). Состояние цепочки привязано к ключу подписи и подписке и индексируется как `"<keyId>:<subscriptionId>"`, поэтому вселенные, подписанные разными ключами, отслеживают свои порядковые номера независимо. При каждой проверке лицензии Renet выполняет:
