...
Имя назначения должно совпадать с настроенным провайдером хранения. Выполните `rdc config storages`, чтобы увидеть доступные имена. Добавляйте новых провайдеров через конфигурацию rclone.

**"Infrastructure config incomplete" при развёртывании**
Все три поля обязательны: `--public-ipv4`, `--base-domain` и `--cert-email`. Выполните `rdc config show-infra <machine>`, чтобы проверить, какие поля отсутствуют.
