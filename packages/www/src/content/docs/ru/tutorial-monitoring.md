...
description: >-
  Проверка состояния машины, инспекция контейнеров, обзор служб systemd,
  сканирование ключей хоста и запуск диагностики среды.
category: Tutorials
order: 4
language: ru
...
Контейнеры от предыдущих развёртываний могут оставаться, если `repo down` не был выполнен корректно. Остановите их с помощью `rdc repo down --name <repo> -m <machine>` или проверьте напрямую через `rdc term connect -m <machine> -r <repo> -c "docker ps -a"`.
