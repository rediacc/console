...
> Репозиторий должен быть отмонтирован перед изменением размера. `repo expand` работает в режиме онлайн. Изменение размера меняет максимальный размер репозитория; чтобы вернуть освободившиеся блоки в пул без изменения максимума, используйте вместо этого [`repo trim`](#reclaim-space-trim).
