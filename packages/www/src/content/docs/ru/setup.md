...
Эта команда создает конфигурацию с именем `my-infra` и сохраняет её в `~/.rediacc/my-infra.json`. Конфигурация по умолчанию (без указания имени) хранится как `~/.rediacc/rediacc.json`.
