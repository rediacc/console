...
Docker-демон для каждого репозитория и сами контейнеры репозитория запускаются с **метками контейнеров по умолчанию** на каждой поддерживаемой ОС. `rdc machine setup` не устанавливает пользовательские политики SELinux или профили AppArmor. Поведение по ОС:
...
Команда `rdc machine setup` устанавливает на удаленном сервере следующее:
