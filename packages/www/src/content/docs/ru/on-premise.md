...
- `ENVIRONMENT=production` (по умолчанию): стандартные лимиты ресурсов; CLI, подключающиеся к этому серверу, по умолчанию используют канал обновлений **stable**. Значение `production` — это устаревший идентификатор развёртывания. Оба режима `production` и `edge` являются production-качеством.
