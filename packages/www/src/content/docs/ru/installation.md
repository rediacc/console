...
Требуется Node.js версии 22 или новее. Чтобы установить определённую версию:
...
Команды, которые изменяют машину (развертывание, резервное копирование, `rdc machine setup` и подобные), автоматически устанавливают соответствующий бинарный файл `renet`. Если бинарный файл обновляется, маршрутный сервер (`rediacc-router`) автоматически перезапускается, чтобы подхватить новую версию. Команды только для чтения, например `rdc machine status` и `rdc repo list`, никогда не заменяют бинарный файл: они используют версию, уже установленную на машине, и выводят предупреждение, если она отличается от версии CLI.
...
Чтобы пропустить автоматический перезапуск, передайте `--skip-router-restart` команде, которая устанавливает renet, или задайте переменную окружения `REDIACC_SKIP_ROUTER_RESTART=1`.
