...
- **Блокировка соединений между репозиториями**: Если сервис пытается подключиться к loopback IP за пределами подсети `/26` своего репозитория, ядро блокирует это. Процесс в репозитории А не может достичь сервисов в репозитории Б. Единственное исключение — форки: их соединения в подсеть родителя перенаправляются на собственные сервисы форка (см. [Ограничения](/ru/docs/limits)); сам родитель остаётся недоступным.
