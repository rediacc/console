...
Порты открываются только после настройки публичного IP с помощью `rdc machine infra set <machine> --public-ipv4`. До этого момента машина не имеет открытых портов. После настройки:
...
| Лимит | Значение |
|-------|-------|
| Снимков на репозиторий | Без ограничений, кроме вашей квоты хранилища. Каждый снимок восстанавливается независимо, поэтому хранение большего их числа стоит только места в хранилище, и больше ничего |
| Одновременные снимки одного репозитория | 1. Второй запуск обнаруживает, что блокировка подготовки репозитория уже удерживается, и **пропускается**, а не ставится в очередь: он сообщает, что блокировку держит другой запуск, и завершается. Запустите снова позже |
| Одновременные холодные снимки | 1 на хранилище данных; второй `renet backup snapshot --cold` отклоняется сразу и ничего не останавливает |
| Частота резервного копирования | Минимальный интервал не установлен; ограничен пропускной способностью вашего хранилища. Используйте `rdc backup strategy set <name> --bwlimit "6M"` для ограничения скорости загрузки |
| Хранение | Задаётся с помощью `rdc backup retention set` (`--keep-last`, `--keep-hourly`, `--keep-daily`, `--keep-weekly`, `--keep-monthly`, `--keep-yearly`) и применяется на стороне сервера; `rdc backup retention clear` удаляет политику, после чего сохраняются все снимки |
| Резервное копирование между машинами | Поддерживается; целевая машина должна иметь достаточно места в хранилище данных |

### Квота на хранилище резервных копий

Снимки отправляются в хранилище чанков, которое измеряется для каждой подписки в **физически уникально сохранённых байтах**: то, что реально хранится после дедупликации, а не сумма того, что снимки логически представляют. Снимки, разделяющие одни и те же данные, а также репозитории из одного семейства форков, засчитывают эти данные только один раз.

| План | Квота | Как показывает `rdc backup usage` |
|------|-------|-------------------------------|
| Community | 10 ГБ | `10G` |
| Professional | 100 ГБ | `100G` |
| Business | 500 ГБ | `500G` |
| Enterprise | 2 ТБ | `2T` |

Это значения по умолчанию для плана. У подписки может быть собственное переопределение, поэтому проверяйте `rdc backup usage`, а не полагайтесь на строку выше.

Квота применяется до того, как данные начинают перемещаться: снимок, который превысил бы её, отклоняется в момент, когда машина запрашивает разрешение на загрузку, а не в середине передачи. Попадание точно в квоту разрешено; превышение на один байт отклоняется. В учтённый объём входят и загрузки, ещё выполняющиеся в данный момент, поэтому два одновременных запуска не могут оба протиснуться под лимит.

Превышение квоты никогда ничего не удаляет. Оно останавливает новые загрузки, а снимки, которые у вас уже есть, остаются на месте.

Если подписка истекает, хранилище резервных копий переходит в режим только для чтения: существующие снимки остаются доступными для чтения, а новые загрузить нельзя. Эти данные хранятся ещё **60 дней** после истечения подписки, после чего одна очистка их удаляет. Открытый возврат средств у организации замораживает эту очистку вместо её выполнения.

Копии между машинами, сделанные с помощью `rdc repo push`, не затрагивают эту квоту. Они попадают в хранилище данных целевой машины и ограничены его свободным местом.
