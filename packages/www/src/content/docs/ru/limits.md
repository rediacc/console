...
| Хранение | Задаётся с помощью `rdc backup retention set` (параметры GFS: `--keep-last`, `--keep-hourly`, `--keep-daily`, `--keep-weekly`, `--keep-monthly`, `--keep-yearly`) и применяется на стороне сервера; `rdc backup retention clear` удаляет его. Хранилище чанков учитывается в рамках квоты плана как объём физически уникально сохранённых байт, поэтому дедупликация между снимками и в рамках всего семейства форков засчитывается только один раз: Community 10 GiB, Professional 100 GiB, Business 500 GiB, Enterprise 2 TiB, проверяется командой `rdc backup usage`. Прежний путь storage-push не имеет собственного хранения и управляется вашим поставщиком хранилища. |
