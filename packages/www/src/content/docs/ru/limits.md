...
На этой странице перечислены жёсткие и мягкие лимиты, применяемые к развёртываниям Rediacc. Прочитайте её перед планированием ёмкости, чтобы знать, какие потолки существуют, а какие нет.
...
Wildcard-сертификаты TLS выпускаются для каждого репозитория при первом развёртывании через Let's Encrypt (вызов Cloudflare DNS-01). Let's Encrypt ограничивает выдачу **50 сертификатами на зарегистрированный домен в неделю**. Поскольку Rediacc использует один wildcard-сертификат на репозиторий (а не на сервис), развёртывание, создающее более 50 новых репозиториев за одну неделю, достигнет этого предела.
...
Читайте матрицу как единый взгляд на то, что каждая CI-протестированная ОС предоставляет из коробки. Все пять удовлетворяют каждому требованию, поэтому это операторская справочная информация, а не критерий отбора.
