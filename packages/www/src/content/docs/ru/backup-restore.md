...
**Отправка в хранилище** (`rdc repo push`) копирует весь файл резервной копии в совместимого с rclone провайдера, который вы регистрируете самостоятельно. Это путь, который выводится из употребления в пользу чанкового хранилища, и запланированные стратегии больше его не используют. Разделы ниже, которые его описывают, по-прежнему работают сегодня, но рассматривайте их как устаревший путь.
...
## Получение и восстановление резервной копии
...
| Опция | Описание |
|--------|-------------|
| `--to <remote>` | Целевое хранилище или машина (отправка) |
| `--to-machine <machine>` | Целевая машина для отправки между машинами |
| `--from <remote>` | Исходное хранилище или машина (получение) |
