...
description: >-
  Создайте профиль конфигурации, зарегистрируйте удалённую машину, проверьте
  подключение по SSH и настройте параметры инфраструктуры.
category: Tutorials
order: 2
language: ru
...
Разверните сгенерированную конфигурацию прокси Traefik на сервере с помощью `rdc config infra push -m bridge-vm`.
...
Сохранённый ключ хоста не совпадает с текущим ключом сервера. Это происходит после пересборки сервера или переназначения IP. Запустите `rdc config machine scan-keys --name <machine>` для обновления ключа.
