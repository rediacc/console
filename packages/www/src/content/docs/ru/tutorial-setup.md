...
Сохранённый ключ хоста не совпадает с текущим ключом сервера. Это происходит после пересборки сервера или переназначения IP. Запустите `rdc config machine scan-keys -m <machine>` для обновления ключа.
