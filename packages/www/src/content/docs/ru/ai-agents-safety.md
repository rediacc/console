...
Пользователи не на Linux не имеют эвакуационного люка из подхода «сначала форк». Это сделано намеренно. Нет способа для агента обойти песочницу, независимо от того, как его попросили. Если вам нужно переопределение, запустите вашего агента внутри WSL, Linux-контейнера или Linux-VM. Иначе работайте с форком.
