...
Всё состояние хранится в файле конфигурации на вашей рабочей станции (например, `~/.config/rediacc/rediacc.json`).
...
- Опциональная синхронизация зашифрованной конфигурации сохраняет тот же файл в хранилище конфигураций с разграничением по командам
...
Хранилище данных (datastore) представляет собой пул хранения для каждой машины, содержащий образы репозиториев. У него два бэкенда, выбираемых во время `datastore init`:

- **Локальный (по умолчанию)**: файловая система BTRFS на loop-устройстве на собственном диске машины. Образы репозиториев являются LUKS-зашифрованными файлами внутри неё; форк представляет собой один вызов `cp --reflink=always`. Это бэкенд, который использует каждое развёртывание на одной машине, и он не требует ничего, кроме диска сервера.
- **Ceph RBD**: хранилище данных располагается на RBD-образе, смонтированном из внешнего кластера Ceph, с обычной BTRFS поверх него (без LUKS на этом уровне, поскольку узлы Ceph никогда не открывают LUKS). Форк и архитектура множественного доступа только для чтения используют собственные примитивы copy-on-write RBD (snapshot, protect, clone) и пространства имён RADOS для изоляции на уровне арендатора.

Оба бэкенда представляют одну и ту же модель репозитория для всего, что находится над ними, поэтому команды `repo`, резервное копирование и форки работают одинаково. Разница в том, где живут байты и какой механизм copy-on-write использует форк (reflink BTRFS против клона RBD). См. [Настройку машины](/en/docs/setup) для инициализации каждого бэкенда и [Справочник по серверу](/en/docs/server-reference) для команд хранилища данных на уровне renet.

## Репозитории Kubernetes

Наряду с Docker-репозиториями машина может размещать **кластеры**. Rediacc сохраняет мышление репозитория, инвертируя привычную модель объектов: кластер является контейнером, а Kubernetes-репозиторий является пространством имён внутри него.

- Состояние кластера (каталог данных k3s на узел) хранится в поддерживаемых хранилищем данных copy-on-write файлах-образах, по одному на узел, поэтому кластер форкается и мигрирует как набор образов.
- Kubernetes-репозиторий представляет собой пространство имён `<repo>` плюс его тома. Постоянные тома являются **отдельными** copy-on-write единицами (RBD-образы на Ceph, или небольшие файлы-образы хранилища данных через локальный провайдер PV), никогда не каталогами внутри одного непрозрачного образа кластера. Именно это разделение делает форки на уровне репозитория независимо copy-on-write.
- `KUBECONFIG` внедряется как аналог `DOCKER_HOST`, а обёртка `renet kube` применяет манифесты из `up()` так же, как `renet compose` запускает Docker.

Клонирование и перенос кластера целиком находятся в `rdc cluster fork` и `rdc cluster migrate`. Это отличительная возможность: форкнуть или перенести работающий кластер, включая его данные, на другую машину или в другой дата-центр с коротким переключением. См. руководство [Kubernetes](/en/docs/kubernetes) для полной модели, команд и измеренных показателей переключения.
