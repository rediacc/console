...
Всё состояние CLI хранится в плоских JSON-файлах конфигурации в `~/.rediacc/`. Хранилища позволяют синхронизировать эти конфигурации с внешними бэкендами для резервного копирования, совместного использования или доступа с нескольких устройств. Учётные данные хранилищ хранятся отдельно в `~/.rediacc/.credentials.json`.

![Config & Stores](/img/arch-operating-modes.svg)

### Локальный адаптер (по умолчанию)

Режим по умолчанию для самостоятельного размещения. Всё состояние хранится в файле конфигурации на вашей рабочей станции (например, `~/.rediacc/rediacc.json`).
...
Каждая конфигурация — это плоский JSON-файл, хранящийся в `~/.rediacc/`. Конфигурация по умолчанию — `rediacc.json`; именованные конфигурации используют имя в качестве имени файла (например, `production.json`). Вот аннотированный пример:
