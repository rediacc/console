...
| **処理の完全性** (PI1) | 正確で完全な処理 | 決定論的なRediaccfileライフサイクルフック（`up`/`down`）が一貫したデプロイメントを保証。`rdc repo admin validate`が予期しないシャットダウンやバックアップ操作後のリポジトリの完全性とバックアップの健全性を検証。 |
...
これらのログを取得する方法は2つあります。ユーザー、チーム、日付によるフィルタリング付き管理ダッシュボード。組織管理者向けのポータルアクティビティページで、タイプと日付によるフィルタリングに加え、独自のツールにパイプできるJSONエクスポート。マシンオペレーションもシステムログに記録されるため、多層防御が実現されます。
