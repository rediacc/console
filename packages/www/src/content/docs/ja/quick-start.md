...
rdc config machine setup my-server        # renetのプロビジョニング + データストアの作成
```
...
```bash
rdc repo create my-app -m my-server --size 2G       # 2 GBの暗号化リポジトリを作成
```
...
rdc repo template apply app-postgres -m my-server -r my-app   # docker-compose.yml + Rediaccfileをデプロイ
```
...
```bash
rdc repo up my-app -m my-server                      # Rediaccfileのup()を実行
rdc repo list -m my-server                           # マシン上のすべてのリポジトリを表示
rdc repo status my-app -m my-server                  # マウント状態、Docker、サイズ、暗号化
```
...
```bash
rdc vscode my-server my-app              # VS Code SSHを開き、リポジトリサンドボックス内に移動
```
...
**デモフロー:** `rdc repo template apply` → `rdc vscode my-server my-app` → `docker-compose.yml` を編集 → `renet dev up` → アプリの動作確認 → 反復開発。
...
**ターミナル:**
```bash
rdc term my-server my-app                            # リポジトリサンドボックスにSSH接続
rdc term my-server my-app -c "curl localhost:3000"   # コマンドを実行して終了
rdc term my-server                                   # マシンにSSH接続（サンドボックスなし）
```
...
```bash
rdc repo tunnel my-server my-app                     # コンテナとポートを自動検出
rdc repo tunnel my-server my-app --port 5432         # Postgresをトンネル
rdc repo tunnel my-server my-app --port 5432 --local 15432  # カスタムローカルポート
```
...
```bash
rdc repo fork my-app -m my-server --tag experiment --up     # 即座にCoWクローン + 起動
rdc repo list -m my-server                                  # 表示: my-app (grand) + my-app:experiment (fork)
rdc repo delete my-app:experiment -m my-server              # フォークを削除、グランドは影響なし
```
...
```bash
rdc config infra set my-server           # 設定: ベースドメイン、パブリックIP、ポート範囲
rdc config infra show my-server          # 設定を確認
rdc config infra push my-server          # プロキシ設定をリモートにプッシュ
```
...
```bash
rdc repo template apply proxy -m my-server -r infra     # リポジトリにプロキシをデプロイ
rdc repo up infra -m my-server                           # Traefikを起動
```
