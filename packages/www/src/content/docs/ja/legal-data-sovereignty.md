...
## ホスト型サービス: 地域データレジデンシー

ホスト型のRediaccサービス（セルフホストではない）を利用するユーザーの場合、データレジデンシーは地域インフラストラクチャによって担保されます。EU（フランクフルト）、米国（バージニア）、アジア太平洋（東京）の3つのリージョンが利用可能です。各リージョンは独立したデータベースとストレージを運用し、リージョンをまたぐデータフローは発生しません。トランザクションメールはAWS SES経由で配信されます。EUと米国は専用の地域エンドポイントを使用し、アジア太平洋はEUのエンドポイント（eu-central-1）を使用します。EUリージョンはR2ストレージに管轄権の強制を適用しています。技術的な詳細については[データリージョン](/ja/docs/data-regions)を参照してください。
