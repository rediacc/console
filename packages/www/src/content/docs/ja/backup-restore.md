...
どちらの戦略からも除外されないリポジトリは両方に取り込まれるため、毎時のクラッシュ整合性スナップショットと毎週のアプリケーション整合性スナップショットの両方を持つことになります。`rdc backup snapshot list <repo>`はそれらをまとめて表示し、共有されるブロックは1回だけ保存されます。
