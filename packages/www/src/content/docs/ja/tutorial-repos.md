...
- プロビジョニングされたマシン（[チュートリアル: マシンセットアップ](/en/docs/tutorial-setup)を参照）
...
マシン上のすべてのリポジトリをサイズ、マウント状態、暗号化状態とともに表示し、各リポジトリを `grand`（最上位）、`fork`（別リポジトリの CoW コピー）、`unknown`（系統を特定できない）に分類する `Type` カラムも表示します。完全な分類ルールについては [リポジトリ. Type カラムと状態ミラー](/ja/docs/repositories#type-column-and-the-state-mirror) を参照してください。
...
- [サービス](/en/docs/services), Rediaccfileリファレンス、サービスネットワーキング、自動起動、マルチサービスレイアウト
- [チュートリアル: 監視と診断](/en/docs/tutorial-monitoring), ヘルスチェック、コンテナ検査、診断
- [ツール](/en/docs/tools), ターミナル、ファイル同期、VS Code連携
