...
# 特定のマシンのアクティベーション詳細を表示

rdc subscription activation-status -m hostinger
```

アカウントサーバーからサブスクリプションの詳細を表示します。マシン単位のアクティベーション詳細は `rdc subscription activation-status -m` を使用します。
...
|----------|-----------|--------------|----------|------------|
| フローティングライセンス要求数 | 500 | 5,000 | 20,000 | 100,000 |
