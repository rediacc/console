...
`{SERVICE}_IP`変数は必要な場合に利用可能ですが、明示的なバインドはもはや必要なく、自動的に処理されます。命名規則：大文字、ハイフンをアンダースコアに置換、`_IP`サフィックス。例えば、`listmonk-app`は`LISTMONK_APP_IP`になります。
...
各リポジトリは一意のループバックIPを取得し、カーネルが`bind()`呼び出しを正しいIPに自動的に書き換えます。リポジトリ間のポート競合は発生しないはずです。予期しない動作が見られる場合は、サービスが`docker compose`ではなく`renet compose`経由で起動されていることを確認してください。他のサービスへの接続には、生のIPではなくサービス名（例：`postgres`）を使用してください。サービス名はすべてのフォークで正しく解決されます。
