...
`rdc repo sync upload`を使用して、プロジェクトファイルをリポジトリに転送します。
...
- **コンテナ起動後**、Dockerボリュームディレクトリを自動除外したい場合。コンテナがまだ起動されていなければ、除外するボリュームがないため、すべてのディレクトリが変更されます（問題ありません、コンテナは初回起動時にデータを再作成します）
...
プロジェクトのルートに`Rediaccfile`を作成します。このBashスクリプトは、サービスの起動と停止方法を定義します。
...
> **重要：** Rediaccfileでは`docker compose`の代わりに常に`renet compose --`を使用してください。`renet compose`ラッパーはホストネットワーク、CRIUチェックポイント/リストア機能、IPの割り当て、およびrenet-proxyに必要なサービスディスカバリを強制します。`docker compose`を直接使用すると、これらすべてがバイパスされ、バリデーション時に拒否されます。
>
> `sudo docker`も使用しないでください、`sudo`は`DOCKER_HOST`を含む環境変数をリセットするため、コンテナがリポジトリの分離されたデーモンではなくシステムのDockerデーモン上に作成されてしまいます。Rediaccfile関数はすでに十分な権限で実行されています。
...
1. **`ports:`マッピングを削除** - `renet compose`はホストネットワークを使用し、ポートマッピングを自動的に除去します
2. **`network_mode: host`を削除** - `renet compose`が自動的に追加します
3. **再起動ポリシーはそのまま維持しても安全** - renetはCRIU互換性のために自動的に除去し、ルーターwatchdogが停止したコンテナを自動回復します
4. **サービス間接続にはサービス名を使用**（例：`postgres`、`redis`）- renetはすべてのサービス名を解決可能なホスト名として注入します。データベースや設定ファイルに保存される接続文字列に生のIPを埋め込まないでください；フォーク分離を維持するためにサービス名を使用してください
5. **バインドは自動** - カーネルが`bind()`を正しいループバックIPに書き換えます。サービスは`0.0.0.0`または`localhost`を使用できます

`{SERVICE}_IP`変数は必要な場合に利用可能ですが、明示的なバインドはもはや必要ありません。バインドは自動的に行われます。命名規則：大文字、ハイフンをアンダースコアに置換、`_IP`サフィックス。例えば、`listmonk-app`は`LISTMONK_APP_IP`になります。
...
1. プロジェクトをアップロード（`node_modules`を除外して`up()`で取得することを検討）
2. コンテナ起動後に所有権修正を実行
...
各リポジトリは一意のループバックIPを取得し、カーネルが`bind()`呼び出しを正しいIPに自動的に書き換えます。リポジトリ間のポート競合は発生しません。予期しない動作が見られる場合は、サービスが`docker compose`ではなく`renet compose`経由で起動されていることを確認してください。他のサービスへの接続には、生のIPではなくサービス名（例：`postgres`）を使用してください。サービス名はすべてのフォークで正しく解決されます。
