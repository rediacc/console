...
| [第17条](https://gdpr-info.eu/art-17-gdpr/)、消去権 | 要求に応じて個人データを削除 | `rdc repo delete`がLUKSボリュームを暗号学的に消去します。フォークの削除でクローンされたコピーが完全に除去されます。 |
...
| [第30条](https://gdpr-info.eu/art-30-gdpr/)、処理記録 | 処理活動の記録を維持 | 監査ログがアカウントレベルの40以上のイベントタイプを追跡：認証、APIトークン、設定ストア操作、ライセンス。管理ダッシュボードまたはポータルアクティビティページからエクスポート(JSONエクスポート対応)。 |
...
マシン間操作の場合、`rdc repo push/pull`がSSH経由でデータを転送します。バックアップ先はオペレーターの認証情報なしでは読み取れないLUKS暗号化ボリュームを受け取ります。
