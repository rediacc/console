...
Oracle Linux、AlmaLinux、Rocky Linuxはすべて同じDNFフローを使用します。`dnf` を持つRHEL互換ディストリビューションであれば上記のリポジトリを利用できます。注意: **Oracle Linux 10はRediaccサーバーターゲットとして公式にサポートされている唯一のRHELファミリーディストリビューションです** ([要件](/en/docs/requirements)を参照)。Rocky/Alma 10にはrenetデータプレーンが必要とするbtrfsカーネルモジュールが含まれていませんが、`rdc` CLIはそれらにもインストール可能です。
...
デプロイやバックアップ、`rdc machine setup` など、マシンに変更を加えるコマンドは、対応する `renet` バイナリを自動的にプロビジョニングします。バイナリが更新されると、新しいバージョンを読み込むためにルートサーバー (`rediacc-router`) が自動的に再起動されます。`rdc machine status` や `rdc repo list` などの読み取り専用コマンドがバイナリを置き換えることはありません。マシン上の既存のバージョンをそのまま使い、CLI のバージョンと異なる場合は警告を表示します。
...
自動再起動をスキップするには、renet をプロビジョニングするコマンドに `--skip-router-restart` を渡すか、環境変数 `REDIACC_SKIP_ROUTER_RESTART=1` を設定してください。
