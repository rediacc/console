...
- **stable**（デフォルト）：十分にテストされたリリース、本番環境に推奨
- **edge**：最新の機能とバグ修正、リリースごとに更新

```bash
rdc update --channel edge      # edgeチャンネルに切り替え
rdc update --channel stable    # stableに戻す
rdc update --status            # 現在のチャンネルとバージョンを表示
```

edgeチャンネルから直接インストールするには：
```bash
