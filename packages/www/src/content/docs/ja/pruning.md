...
プルーニングは削除済みリソースが残した状態を取り除きます。*ライブ*リポジトリが占有している容量（ファイルシステムが解放したがプールがまだ保持しているブロック）を回収するには、代わりに [`rdc repo trim`](/ja/docs/repositories#reclaim-space-trim) を使用してください。2 つは補完的な関係にあります。
...
- `knownHosts` エントリ（自動更新パスは `rdc machine scan-keys`）。
...
- **すべての状態保存時**（`rdc repo up` および任意のリポジトリ状態を更新する操作）. ミラーコードが導入される前に作成されたリポジトリ向け。
...
- **machine prune を backup schedule と組み合わせてください。** バックアップスケジュールのデプロイ（`rdc backup schedule`）後に、定期的なマシンプルーニングを追加して、古いスナップショットと孤立したデータストアアーティファクトをクリーンアップしてください。
