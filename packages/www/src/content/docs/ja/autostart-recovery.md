...
自動起動が有効なリポジトリはブート時に自動的に起動します。その後停止した場合、定期リコンサイラーが復帰させます。プロンプトもなく、手動再起動も不要です。
...
キーファイルのスロットには高速な PBKDF2 KDF を採用しています。256 バイトのランダムなキーファイルそのものが十分なセキュリティ強度を持つため、メモリハードな KDF を使っても保護が向上するわけではなく、アンロックのレイテンシが増えるだけです。マウントは 1 秒以内に完了します。この最適化より前に作成されたリポジトリは、マウントのたびに数秒の Argon2id 導出が発生します。マシン上でリポジトリをアンマウントした状態で、オペレーターコマンド `renet repository kdf-migrate --name <guid>` を実行するとその場で変換できます。スロット 0 は引き続き Argon2id を使用します。人間が入力するパスフレーズには適切な選択です。

起動時に、`rediacc-autostart.service`という名前のワンショットsystemdサービスが自動起動有効なリポジトリのリストを読み込み、それぞれのキーファイルを使用してマウントし、リポジトリごとのDockerデーモンを起動して、Rediaccfileの`up()`フックを実行します。シャットダウン時には、サービスが`down()`を実行し、Dockerを停止して、LUKSボリュームを閉じます。
