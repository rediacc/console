...
- **リポジトリ間接続はカーネルでブロックされます**: カーネルはリポジトリの `/26` サブネット外のループバック IP への接続を自動的にブロックします。あるリポジトリのサービスは、別のリポジトリのサービスに到達できません。唯一の例外として、フォークから親リポジトリのサブネットへの接続は、フォーク自身のサービス（同じサービス、フォーク側のデータコピー）へ透過的にリダイレクトされ、親に届くことはありません。関係のないリポジトリ間は引き続きブロックされます。
- **サービス間通信**: **サービス名**（例: `db`、`redis`）を使用してください。renet は各サービス名を正しい IP に解決されるホスト名として自動的に注入します。Docker の DNS 名は host モードでは機能しませんが、`/etc/hosts` 経由のサービス名は機能します。`${DB_IP}` などを永続的な設定ファイル（例: データベースに保存された接続文字列）に埋め込むのは避けてください。フォークは直接の親から引き継いだ IP を自身のサービスへリダイレクトしますが、このリダイレクトは一世代分のみです。フォークのフォークには、到達できない祖父母の IP が残ります。サービス名はどのリポジトリでも常に正しく解決されます。
...
- データストアはシステムディスク上の固定サイズのBTRFSプールファイルです。`rdc machine status <name> --system`で実効的な空き容量を確認できます。`rdc datastore resize`で拡張できます。
...
- **Docker のブリッジネットワーキングは、リポジトリごとのデーモンで無効化されています。** 各リポジトリの `daemon.json`（`FlavorRediacc`）には `"bridge": "none"` と `"iptables": false` が設定されているため、単純な `docker run <image>` で作成されるコンテナはループバックインターフェースのみを持ち、外向きの接続性を持ちません。これはバグではなく、リポジトリ間の分離を強制するためのものです。あるリポジトリが別のリポジトリのループバック IP に到達することを防ぐカーネルレベルの eBPF フックは、ホストのネットワーク名前空間に属するコンテナにのみ適用されるからです。本番サービスでは `renet compose` を使用してください。これは自動的に `network_mode: host` を注入します。シェル内でアドホックに単発のコンテナを実行する場合は、明示的に `--network host` を指定してください。（ユーザーごとの Hub デーモン（`FlavorHub`、開発環境）は例外で、`bridge="docker0"` と `iptables=true` を有効にするため、ユーザーが実行するコンテナは通常のアウトバウンドネットワーク接続を得られます。）
...
- **テイクオーバー**（`rdc repo promote <fork>`）はgrandリポジトリのデータをフォークのデータで置き換えます。grandはその識別情報（GUID、networkId、ドメイン、自動起動、バックアップチェーン）を保持します。古い本番データはバックアップフォークとして保存されます。使用例: フォークでアップグレードをテストし、確認後に本番にテイクオーバー。`rdc repo promote <backup-fork>`で元に戻せます。
