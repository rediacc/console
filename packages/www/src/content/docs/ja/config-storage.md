...
- **PRFサポート付きのパスキープロバイダー**: FIDO2セキュリティキー（例：YubiKey）、iCloud Keychain、Google Password Manager、1Password、またはDashlane
...
2. パスキーを登録します（YubiKey/iCloud Keychain/1Passwordのポップアップ）
...
- FIDO2セキュリティキー（例：YubiKey）
- iCloud Keychain（macOS/iOSのSafari）
- Google Password Manager（Android/ChromeOSのChrome）
- 1Password
- Dashlane
