...
| `rdc config machine set <machine> --backup-strategies <s1,s2>` | 戦略をマシンに紐付ける |
...
| `rdc config machine setup <machine>` | マシンの初期プロビジョニングを実行する |
...
| `rdc repo sync upload -m <machine> -r <repo> --local <path>` | ローカルファイルをリポジトリにアップロードする |
| `rdc repo sync download -m <machine> -r <repo> --local <path>` | リポジトリのファイルをローカルにダウンロードする |
