...
> **ホットとコールドのレイアウト。** ストラテジーが `--mode hot` を使用する場合、バックアップはストレージ上の `<bucket>/<folder>/hot/<guid>` に書き込まれます。`--mode cold` のストラテジーは `<bucket>/<folder>/cold/<guid>` に書き込みます。標準的なパターンは、1 つの毎時ホットストラテジーと 1 つの毎週コールドストラテジーの組み合わせです。詳細は [バックアップとリストア. ストレージレイアウト](/ja/docs/backup-restore#storage-layout) を参照してください。
