...
3. 各ユーザー行には、メール、ステータス(アクティブ/非アクティブ)、権限グループ、最終アクティビティ時間が表示されます。
...
5. **{{t:common.actions.export}}**ボタンをクリックし、形式を選択します: **CSV**または**JSON**。
...
2. **Current Members**タブで既にチームに割り当てられているメンバーを表示します。
3. **Add Member**タブに切り替えます。
...
1. トレースするチームを選択します。
2. 時計/履歴アイコンをクリックします。
3. **Audit History**モーダルで、合計レコード、表示されたレコード、最終アクティビティのカウントを確認します。
4. **{{t:common.actions.export}}**ボタンをクリックしてCSVまたはJSON形式でエクスポートします。
...
2. **「+」**ボタンをクリックします。
3. **Enter group name**フィールドに意味のある名前を入力します。
4. **OK**ボタンをクリックしてグループを作成します。
...
1. 権限グループを選択し、**Manage Permissions**オプションをクリックします。
2. **Current Permissions**タブでグループのアクセス権を表示します。
3. 各アクションの横にある赤い**{{t:common.delete}}**ボタンをクリックすることで、権限を取り消すことができます。
4. **Add Permission**タブをクリックして、グループに新しい権限を追加します。
...
6. ステージを確認します: **{{t:queue.statusAssigned}}** → **{{t:queue.statusProcessing}}** → **{{t:queue.statusCompleted}}**
...
1. マシンを選択し、**Remote**ボタンをクリックします。
2. ドロップダウンメニューのオプションを確認します:
   - **Run on Server**: マシン上で関数を実行
   - **Test Connection**: マシンにpingを送信

![リモートメニュー - Run on ServerとTest Connection](/assets/images/UserGuideEng/37_remote_button.png)
*(図37: リモートボタン - 選択したマシン上での関数実行メニュー)*

> **ヒント**: 関数を実行する前に、「Test Connection」オプションを使用してマシンにアクセス可能であることを確認してください。

#### セットアップ

1. **Run on Server**オプションを選択します。
2. **Available Functions**リストで**setup**関数を見つけます。
3. 関数名をクリックして選択します。
...
1. **Run on Server** > **hello**関数を選択します。
...
4. **Response (Console)**セクションでマシンの出力を確認します。
...
1. **Remote** > **Run on Server** > **Advanced**パスに従います。
...
3. フォームに入力します:
   - **Repository Name**: リポジトリ名を入力(例: postgresql)
   - **Size**: リポジトリサイズを入力(例: 2GB)
   - **Credential ID**: 自動生成された認証情報を確認
   - **Select Template**: テンプレートを選択(例: databases_postgresql)
...
5. タスク追跡ウィンドウでステージを確認します: **{{t:queue.statusAssigned}}** → **{{t:queue.statusProcessing}}** → **{{t:queue.statusCompleted}}**
...
4. **Tag ID**フィールドに新しいタグを入力します(例: 2025-12-06-20-37-08)。
...
2. **Tag ID**フィールドにデプロイするバージョンを入力します。
3. **Target Machines**フィールドでターゲットマシンを選択します。
4. **Override existing file**オプションをチェックします(該当する場合)。
5. **{{t:common.actions.addToQueue}}**ボタンをクリックします。
...
2. フォームに入力します:
   - **Backup Tag**: 説明的な名前を入力(例: backup01012025)
   - **Target Storage Systems**: バックアップ場所を選択
   - **Override Existing File**: オプションを有効または無効にする
   - **Enable Checkpoint**: 設定を確認
...
> **ヒント**: 検索ボックスを使用してテンプレートを素早く見つけてください。テンプレートの機能について学ぶには「View Details」を使用してください。
...
1. リポジトリを選択し、**fx** > **Advanced** > **Unmount**パスに従います。
...
1. リポジトリを選択し、**fx** > **Delete Repository**パスに従います。
...
2. ドロップダウンメニューからアクセス方法を選択します:
   - **Desktop Application**: グラフィカルインターフェースでアクセス
   - **VS Code**: コードエディタで開く
   - **Terminal**: コマンドライン経由でアクセス
   - **CLI Commands**: コマンドラインツール

![接続オプションメニュー](/assets/images/UserGuideEng/74_repo_connection.png)
*(図74: リポジトリ接続メニュー - 異なるアクセスパス)*

> **ヒント**: VS Codeで作業している場合、「VS Code」オプションが最速の統合を提供します。

3. ブラウザが許可を要求したら、**Open**ボタンをクリックします。
...
4. **Confirm New Password**フィールドに同じパスワードを再入力します。
5. **Change Password**ボタンをクリックします。
...
2. **Add Storage**ボタンをクリックします。

![ストレージ追加ボタン](/assets/images/UserGuideEng/79_storage_add_button.png)
*(図79: ストレージ管理ページ - 「Add Storage」ボタン)*

3. フォームに入力します:
   - **Storage Name**: 説明的な名前を入力
   - **Storage Provider**: 選択(例: s3)
   - **Description**: オプションの説明を追加
   - **Disable Versioning**: オプション
   - **Additional Parameters**: rcloneフラグ(例: --transfers 4)
...
4. 必要に応じて**Repository Name**を変更します。
...
3. 監査履歴を確認します。
4. **{{t:common.actions.export}}**ボタンから形式を選択します: **CSV**または**JSON**。
...
> **ヒント**: 「Only Stale Items」オプションは、長時間処理されているタスクを見つけるのに役立ちます。タスク実行のトレンドを分析するために、キュー履歴を定期的にエクスポートしてください。
