---
title: Webアプリケーション
description: "Rediaccウェブコンソールを使用してマシン、リポジトリ、バックアップを管理する"
category: Reference
order: 1
language: ja
...
**Rediacc**は、自社サーバーにインストールするセルフホスト型ソフトウェアです。稼働中のシステム全体をバックアップ、フォーク、リストアできます。アプリ、データベース、設定をまとめて、ファイル単位ではなく。この点が核心です。多くのバックアップツールはファイルをコピーしますが、それらの関係性は失われます。

このガイドでは、[https://www.rediacc.com/](https://www.rediacc.com/)のウェブコンソールについて説明します。
...
ユーザー管理は、組織内のどのメンバーにアクセスを付与するかを管理する場所です。
...
チームはユーザーをグループ化し、まとめてアクセス権を付与できます。
...
アクセス制御は、ユーザー単位ではなくグループを通じて権限を一元管理します。
...
マシンセクションは、サーバーとその上のリポジトリを管理する場所です。
...
リポジトリは独立したアプリケーションデプロイメントの単位です。アプリ、データ、設定、専用の Docker デーモンをそれぞれ持ちます。このページのほぼすべてのボタンは、マシンではなくリポジトリに対して作用します。削除をクリックする前に、この点を理解しておいてください。
...
リポジトリへの接続方法は2種類あります。デスクトップアプリは本格的なターミナルと SSH を提供します。ブラウザは確認やクリック操作に向いていますが、長時間のシェルセッションには対応できません。本格的な作業をする場合はデスクトップアプリを選択してください。
...
ストレージセクションは、バックアップデータの保存先となる物理的な場所を定義する場所です。
...
認証情報セクションは、リポジトリが各種サービスへアクセスするために使用するシークレットを管理する場所です。
...
キューセクションは、システム全体の保留中および完了した操作を追跡します。
...
監査セクションは、システムで実行されたすべての操作の記録を保管します。
