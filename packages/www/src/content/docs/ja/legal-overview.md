...
|-------------|---------|------------------|
| [GDPR](/ja/docs/legal-gdpr) | EUのデータ保護とプライバシー | 同一マシン上でのCoWクローン、LUKS2暗号化、ゼロ知識設定ストア、監査ログ、`rdc repo destroy`による消去権 |
...
- **暗号化バックアップ**: `rdc repo backup push/pull`はSSH経由でデータを転送します。バックアップ先はLUKS暗号化ボリュームを受け取ります。
