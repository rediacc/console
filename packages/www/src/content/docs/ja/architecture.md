...
自己ホスト使用用のデフォルト設定です。すべての状態はワークステーション上の設定ファイル(例:`~/.config/rediacc/rediacc.json`)に存在します。
...
設定に`apiUrl`と`token`フィールドが含まれている場合、自動的に有効化されます。状態管理とチームコラボレーションにRediacc APIを使用します。

- クラウドAPIに保存された状態
- ロールベースアクセスを備えたマルチユーザーチーム
- 視覚的管理用Webコンソール
- `rdc auth login`で設定

> **注**: クラウドアダプタコマンドは実験的です。`REDIACC_EXPERIMENTAL=1`を設定して有効にしてください。

両方のアダプタは同じCLIコマンドを使用します。アダプタは状態の保存場所と認証の仕組みにのみ影響します。
