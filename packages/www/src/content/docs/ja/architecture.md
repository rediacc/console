...
- マシンへの直接SSH接続
- 外部サービスは不要
- デフォルト設定はCLI初回使用時に自動作成されます。名前付き設定は`rdc config init <name>`で作成されます
...
データストアは、リポジトリイメージを保持するマシンごとのストレージプールです。`datastore init` の時点で選択する2つのバックエンドがあります。

- **ローカル(デフォルト)**: マシン自身のディスク上の loop デバイスによる BTRFS ファイルシステム。リポジトリイメージはその内部の LUKS 暗号化ファイルであり、フォークは単なる `cp --reflink=always` です。あらゆる単一マシン構成が使用するバックエンドで、サーバーのディスク以外は何も必要としません。
- **Ceph RBD**: データストアは外部の Ceph クラスターからマップされた RBD イメージ上にあり、その上に素の BTRFS が乗ります(Ceph ノードは LUKS を一切開かないため、このレイヤーに LUKS はありません)。フォークと読み取り専用のマルチクライアントアーキテクチャは、RBD 自体の copy-on-write プリミティブ(snapshot、protect、clone)とテナントごとの分離のための RADOS namespace を使用します。

両方のバックエンドは、その上位にあるすべてのものに対して同じリポジトリモデルを提示するため、`repo` コマンド、バックアップ、フォークはまったく同じように動作します。違いはバイトがどこに存在するか、そしてフォークがどの copy-on-write の仕組みを使うか(BTRFS の reflink か RBD の clone か)です。各バックエンドの初期化方法については [マシンセットアップ](/en/docs/setup) を、renet レベルのデータストアコマンドについては [サーバーリファレンス](/en/docs/server-reference) を参照してください。

## Kubernetesリポジトリ

Dockerリポジトリに加えて、マシンは**クラスター**をホストできます。Rediaccは通常のオブジェクトモデルを逆転させることでリポジトリの考え方を維持します。クラスターがコンテナであり、Kubernetesリポジトリはその内部の namespace です。

- クラスターの状態(ノードごとの k3s データディレクトリ)はデータストアに支えられた copy-on-write イメージファイルとしてノードごとに存在するため、クラスターはイメージの集合としてフォーク・マイグレーションされます。
- Kubernetesリポジトリは namespace `<repo>` とそのボリュームです。永続ボリュームは**別個の** copy-on-write ユニット(Ceph 上の RBD イメージ、またはローカル PV プロビジョナーによる小さなデータストアイメージファイル)であり、1つの不透明なクラスターイメージ内のディレクトリになることは決してありません。この分離こそが、リポジトリごとのフォークを独立して copy-on-write にする理由です。
- `KUBECONFIG` は `DOCKER_HOST` の類似物として注入され、`renet kube` ラッパーは `renet compose` が Docker を実行するのと同じ方法で `up()` からマニフェストを適用します。

クラスター全体のクローンと再配置は `rdc cluster fork` と `rdc cluster migrate` にあります。これが差別化能力です。実行中のクラスターを、そのデータごと、短いカットオーバーで別のマシンやデータセンターへフォークまたは移動できます。完全なモデル、コマンド、測定されたカットオーバーの数値については [Kubernetes](/en/docs/kubernetes) ガイドを参照してください。
