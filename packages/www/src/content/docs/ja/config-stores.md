---
title: "Config & Stores"
description: "CLI設定ファイルを管理し、Git、Bitwarden、S3、HashiCorp Vault、ローカルファイルなどの外部バックエンドに同期します。"
category: "Guides"
order: 8
language: ja
...
デフォルトの設定ファイルは`~/.config/rediacc/rediacc.json`です。さまざまな環境用に名前付き設定を作成できます：
...
各設定をBitwardenボールトに個別の**セキュアノート**として保存します。アイテムは`rdc:`プレフィックスで命名されます（例：`rdc:rediacc`、`rdc:production`）。
