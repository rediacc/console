...
Renetを実行する各マシンは`{licenseDir}/chain-state.json`（つまり`/var/lib/rediacc/license/chain-state.json`、リポジトリごとの`repos/`ディレクトリと同じ階層）に最後に確認したチェーン状態を保存します。チェーン状態は`"<keyId>:<subscriptionId>"`という形式のキーで署名キーとサブスクリプションごとにスコープされるため、異なるキーで署名されたユニバースはそれぞれ独立してシーケンスを追跡します。ライセンス検証のたびにRenetは以下を確認します：
