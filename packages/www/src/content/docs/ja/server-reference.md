...
このページでは、RediaccサーバーにSSH接続した際に見つかるものについて説明します：ディレクトリ構成、`renet`コマンド、systemdサービス、およびワークフロー。
...
`sandbox-exec`はLandlock LSMのファイルシステム制限を適用し、その後指定されたコマンドを実行します。すべてのリポジトリレベル接続に対して、`sandbox-gateway`（SSH ForceCommandハンドラー）によって自動的に呼び出されます。

### ユーザーごとのHub（開発環境）

Hubは各ユーザーに開発環境用の独自のDockerデーモンを提供し、リポジトリごとの`FlavorRediacc`デーモンとは独立しています。

```bash
# ユーザーごとのHub systemdユニットをインストール / 削除
sudo renet hub install
sudo renet hub uninstall

# アイドル状態のユーザーごとのHubデーモンをガベージコレクト
sudo renet hub gc
```

デーモンは`--flavor`で選択される2つのフレーバーのいずれかで動作します：

```bash
# リポジトリ隔離デーモン（bridge=none、iptables=false）— デフォルト
sudo renet daemon start-foreground --flavor=rediacc ...

# ユーザーごとのHubデーモン（bridge=docker0、iptables=true、live-restore=true）
sudo renet daemon start-foreground --flavor=hub ...
```

`hub`フレーバーは通常のブリッジネットワーキングを有効にし、ユーザーが実行するコンテナに外部への接続性を提供します。`rediacc`フレーバーはリポジトリ間のループバック隔離を強制します。HubのAuditログは`/var/log/rediacc/hub/<user>.log`に書き込まれます。

**フラグ：**
- `--allow-rw`、`--allow-ro`、`--allow-exec`：Landlockパスルール
- `--home-overlay`：リポジトリごとの書き込み分離のためにホームディレクトリ上にOverlayFSをマウント
- `--sandbox-dir`：リポジトリごとのワークスペース（`<datastore>/.interim/sandbox/<name>/`）
- `--work-dir`：作業ディレクトリを設定し、リポジトリ環境の`.envrc`を読み込む
- `--run-as`：セットアップ後に対象ユーザーへ権限を降格
- `--reset-home`：クリーンな状態で再起動するためにリポジトリごとのホームオーバーレイをクリア

**`sandbox-gateway`** は `authorized_keys` の `command=` で設定されるSSH ForceCommandハンドラーです。各リポジトリのSSHキーはリポジトリ名を組み込んだ状態でゲートウェイを起動し、クライアントが偽造することはできません。ゲートウェイはsandbox-execの引数を構築し、sudoを介して実行します。
...
> ほとんどのユーザーは代わりに`rdc repo push/pull`を使用すべきです。`rdc`コマンドは資格情報とマシンの解決を自動的に処理します。
...
| `rediacc-autostart-reconcile.service` | 定期的な自動起動リコンサイラー（以下のタイマーによって実行） |
| `rediacc-autostart-reconcile.timer` | 起動後に停止した自動起動リポジトリを回復するため、約3分ごとに`renet repository reconcile`を実行 |
