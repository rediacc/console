...
# Landlock サンドボックス内でコマンドを実行（内部使用）
renet sandbox-exec --allow-rw /path --allow-ro /usr --allow-exec /bin -- command
```

`sandbox-exec` は Landlock LSM のファイルシステム制限を適用し、その後指定されたコマンドを実行します。すべてのリポジトリレベル接続に対して、`sandbox-gateway`（SSH ForceCommandハンドラー）によって自動的に呼び出されます。
