...
# Landlock サンドボックス内でコマンドを実行（renet が内部で使用）
renet sandbox-exec --allow-rw /path --allow-ro /usr --allow-exec /bin -- command
```

`sandbox-exec` はコマンドに Landlock LSM のファイルシステム制限を適用します。サンドボックス化されたプロセスは明示的に許可されたパスにしかアクセスできず、それ以外のファイルシステムアクセスはカーネルによってブロックされます。これは renet が内部で使用し、Rediaccfile の実行、compose 操作、SSH コマンドをそのリポジトリのマウントパスに隔離します。
