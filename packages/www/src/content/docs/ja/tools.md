...
このコマンドは以下を実行します。
1. マシン上にオープンソースのエディタサーバーを一度だけインストール（読み取り専用共有パス、チェックサム検証済み）
2. リポジトリのサンドボックス内でサーバーを起動（ファイルツリー、統合ターミナル、子プロセスすべてがリポジトリと同じ環境で動作）
3. ローカルポートへの SSH トンネルを開き、セッションごとのトークン URL でブラウザを起動
...
| オプション | 説明 |
|--------|-------------|
| `--no-open` | ブラウザを起動せず URL を表示 |
| `--url-only` | URL を1行だけ stdout に出力してトンネルを保持（スクリプト用） |
| `--local <port>` | ローカルのトンネルポートを指定 |
| `--server-provider <id>` | エディタサーバーの実装: `openvscode`（デフォルト）または `code-server` |
| `--server-archive <file>` | マシン上の事前配置済みの tarball からインストール（インターネット不要） |

Linux、macOS、Windows、タブレットのどこからでも使えます。ローカルに必要なのはブラウザだけです。
