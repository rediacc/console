...
- **クロスリポジトリ接続ブロック**: サービスがリポジトリの`/26`サブネット外のループバックIPに接続しようとすると、カーネルがブロックします。リポジトリAのプロセスはリポジトリBのサービスに到達できません。唯一の例外はフォークです。親のサブネットへの接続はフォーク自身のサービスへリダイレクトされます（[制限](/ja/docs/limits)を参照）。親そのものには到達できません。
