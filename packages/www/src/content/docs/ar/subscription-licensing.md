...
- `rdc repo push` و`rdc repo pull` و`rdc repo sync`، **يُتحقق بالكامل بما في ذلك انتهاء الصلاحية**: نقل النسخ الاحتياطية يتطلب استحقاقاً نشطاً
...
يبقى التطبيق مرناً حيث يهم ذلك أكثر: تستمر المستودعات قيد التشغيل في العمل حتى بعد انتهاء الاشتراك (`up` و`down` و`delete` والبدء التلقائي). أما الأعمال الجديدة (الإنشاء والتفريع وتغيير الحجم وتجديد الترخيص) ونقل النسخ الاحتياطية (`push` و`pull` و`sync`) فتخضع لاستحقاق نشط.
