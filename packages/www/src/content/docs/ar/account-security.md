...
**ميزات الامان:**
- ربط IP: يعمل الرمز من عنوان IP الذي جاء منه طلبه الاول فقط؛ ويتطلب العنوان الجديد تحققا عبر TOTP او تسجيل دخول جديدا (انظر ادناه)
...
يُرفض الرمز المرتبط بعنوان IP من اي عنوان آخر، مثلا عندما يمنح مزود الانترنت عنوانا جديدا. تتولى CLI عملية النقل:

- **طرفية تفاعلية مع تفعيل 2FA**: تطلب CLI الرمز المكوّن من 6 ارقام من تطبيق المصادقة، وتنقل الرمز الى العنوان الجديد، ثم تعيد تشغيل الامر. لا تُقبل الرموز الاحتياطية في النقل.
- **السكربتات وأنظمة CI (بلا طرفية)**: يفشل الامر ويذكر الحلين: تشغيل اي امر `rdc` مرة واحدة في طرفية تفاعلية (مثل `rdc subscription status`) وادخال الرمز، او تشغيل `rdc subscription login`.
- **2FA معطّل**: لا يمكن نقل الرمز. يصدر `rdc subscription login` رمزا جديدا، ومع تفعيل 2FA يكفي رمز واحد في النقل التالي.
- **رموز خاطئة**: خمسة رموز خاطئة خلال 15 دقيقة تقفل النقل، اولا لمدة 5 دقائق ثم ضعف المدة في كل مرة، حتى ساعة واحدة. بعد 4 اقفال يُعطَّل النقل لهذا الرمز حتى تشغيل `rdc subscription login` التالي.
- لا تتأثر **رموز executor** ذات ربط IP من نوع `unbound` او `cloudflare`.

يظهر كل نقل في سجل النشاط في البوابة، مع العنوان القديم والجديد.
