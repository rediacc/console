...
ثبّت CLI مباشرةً من خادمك المحلي. يكتشف سكريبت التثبيت قناة التحديث تلقائياً ويُوجِّه CLI للتحقق من خادمك بحثاً عن التحديثات.
...
إذا كنت تتصل بخوادم متعددة (محلي، إنتاج، edge)، تُبقي التكوينات المسماة كل بيئة معزولة:
...
- `ENVIRONMENT=production` (الافتراضي): حدود موارد قياسية؛ تستخدم CLIs المتصلة بهذا الخادم قناة التحديث **stable** افتراضياً. اسم القيمة `production` هو معرف نشر قديم. كلاً من `production` و`edge` بجودة الإنتاج.
- `ENVIRONMENT=edge`: ضعف حدود Community؛ تستخدم CLIs قناة التحديث **edge** افتراضياً

راجع [قنوات الإصدار](/ar/docs/release-channels) للاطلاع على تفاصيل ما توفره كل بيئة.

## ما يخبرك به الخادم عن CLI
...
- **البيئة**: ملف تعريف النشر الذي يشغله الخادم (حدود قياسية مقابل edge بحدود 2X)

يعني هذا التكوين التلقائي أنك تحتاج فقط إلى عنوان URL للخادم. كل شيء آخر يُكتشف تلقائياً.

## الترخيص لعمليات النشر المعزولة عن الإنترنت

تُصدر الخوادم المحلية والمستضاتة ذاتياً التراخيص محلياً باستخدام **شهادة تفويض** موقّعة من المفتاح الرئيسي الأعلى. تُقيّد الشهادة الخادم المحلي بحدود خطته وتُنشئ سلسلة مقاومة للتلاعب. راجع [سلسلة التراخيص والتفويض](/ar/docs/license-chain) للتصميم التشفيري (سلامة السلسلة، اكتشاف الشوكة، إثباتات التدقيق).
...
احفظ المفتاح الخاص إلى جانب أسرارك الأخرى (مثل Docker secret أو Kubernetes Secret). لا يغادر الخادم المحلي أبداً.

### 2. طلب شهادة تفويض من الخادم الأعلى

يمكنك طلب الشهادة من بوابة الحساب الأعلى بثلاث طرق:

**الخيار أ: الخدمة الذاتية للعميل (موصى به).** سجّل الدخول إلى البوابة الأعلى بوصفك مالك أو admin للمؤسسة وانتقل إلى **/account/delegation-certs**. انقر على **Create New**، والصق المفتاح العام للخادم المحلي (base64 SPKI)، واختر فترة الصلاحية (أو اقبل الافتراضي لكل خطة)، ثم نزّل ملف `.json` الناتج.

**الخيار ب: Admin (عبر العملاء).** يمكن لدعم Rediacc أو مسؤول النظام الأعلى استخدام `POST /admin/delegation-certs` بنفس المعاملات.

**الخيار ج: CLI `rdc` (مخطط).** سيُغلِّف أمر CLI مستقبلي تدفق البوابة.
...
تحكمها سياسة الصلاحية (إعدادات افتراضية وحدود قصوى لكل خطة، تجاوز لكل اشتراك، محدودة بنهاية الاشتراك بالإضافة إلى 3 أيام سماح). تتضمن الاستجابة أيضاً `effectiveDays` و`reason` لترى سبب اختياره لتلك القيمة. راجع [سلسلة التراخيص - سياسة الصلاحية](/ar/docs/license-chain) للاطلاع على القواعس الكاملة.
