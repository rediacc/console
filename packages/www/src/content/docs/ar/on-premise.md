...
- `ENVIRONMENT=production` (الافتراضي): حدود موارد قياسية؛ تستخدم CLIs المتصلة بهذا الخادم قناة التحديث **stable** افتراضيًا. اسم القيمة `production` هو معرف نشر قديم. كلاً من وضعي `production` و`edge` هما بجودة الإنتاج.
- `ENVIRONMENT=edge`: ضعف حدود Community؛ تستخدم CLIs قناة التحديث **edge** افتراضيًا
