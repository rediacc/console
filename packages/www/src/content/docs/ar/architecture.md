...
تُخزَّن جميع حالات CLI في ملفات إعداد JSON مسطّحة تحت `~/.rediacc/`. تتيح لك المخازن مزامنة هذه الإعدادات إلى خلفيات خارجية للنسخ الاحتياطي أو المشاركة أو الوصول من أجهزة متعددة. تُحفظ بيانات اعتماد المخزن بشكل منفصل في `~/.rediacc/.credentials.json`.

![الإعداد والمخازن](/img/arch-operating-modes.svg)

### محوّل المحلي (الافتراضي)

الافتراضي للاستضافة الذاتية. تعيش جميع الحالات في ملف إعداد على محطة عملك (مثلاً `~/.rediacc/rediacc.json`).
...
كل إعداد هو ملف JSON مسطّح مخزّن في `~/.rediacc/`. الإعداد الافتراضي هو `rediacc.json`؛ تستخدم الإعدادات المسمّاة الاسم كاسم للملف (مثلاً `production.json`). فيما يلي مثال مشروح:
