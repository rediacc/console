...
تعيش جميع الحالات في ملف إعداد على محطة عملك (مثلاً `~/.config/rediacc/rediacc.json`).
...
- مزامنة تكوين مشفرة اختيارية تخزن الملف ذاته في مخزن التكوين، محدودة النطاق لكل فريق
...
## طبقات تخزين مخزن البيانات

مخزن البيانات هو تجمع تخزين لكل جهاز يحتفظ بصور المستودعات. له طبقتان يُختار بينهما عند `datastore init`:

- **محلية (افتراضية)**: نظام ملفات BTRFS مدعوم بجهاز loop على قرص الجهاز نفسه. صور المستودعات هي ملفات مشفرة بـ LUKS داخله؛ التفريع هو أمر واحد `cp --reflink=always`. هذه هي الطبقة التي يستخدمها أي نشر بجهاز واحد، ولا تحتاج إلى شيء غير قرص الخادم.
- **Ceph RBD**: يعيش مخزن البيانات على صورة RBD مُعيَّنة من عنقود Ceph خارجي، مع BTRFS بسيط فوقها (لا توجد طبقة LUKS في هذا المستوى، لأن عُقد Ceph لا تفتح LUKS أبداً). يستخدم التفريع وبنية العملاء المتعددين للقراءة فقط أوليات النسخ عند الكتابة الخاصة بـ RBD (اللقطة، الحماية، الاستنساخ) وفضاءات أسماء RADOS للعزل لكل مستأجر.

تعرض كلتا الطبقتين نفس نموذج المستودع لكل ما يعلوهما، بحيث تعمل أوامر `repo` والنسخ الاحتياطية والتفريعات بشكل متطابق. الفرق يكمن في مكان وجود البايتات وآلية النسخ عند الكتابة التي يستخدمها التفريع (reflink في BTRFS مقابل استنساخ RBD). راجع [إعداد الجهاز](/ar/docs/setup) لمعرفة كيفية تهيئة كل طبقة و[مرجع الخادم](/ar/docs/server-reference) لأوامر مخزن البيانات على مستوى renet.

## مستودعات Kubernetes

إلى جانب مستودعات Docker، يمكن للجهاز استضافة **عناقيد**. يحافظ Rediacc على عقلية المستودع بقلب نموذج الكائنات المعتاد: العنقود هو الحاوية، ومستودع Kubernetes هو فضاء أسماء بداخله.

- تعيش حالة العنقود (مجلد بيانات k3s لكل عقدة) في ملفات صور نسخ عند الكتابة مدعومة بمخزن البيانات، واحدة لكل عقدة، بحيث يتفرع العنقود ويهاجر كمجموعة من الصور.
- مستودع Kubernetes هو فضاء الأسماء `<repo>` بالإضافة إلى أحجامه. الأحجام الثابتة هي وحدات نسخ عند الكتابة **منفصلة** (صور RBD على Ceph، أو ملفات صور صغيرة لمخزن البيانات عبر موفر PV محلي)، وليست أبداً مجلدات داخل صورة عنقود معتمة. هذا الفصل هو ما يجعل التفريعات لكل مستودع نسخ عند كتابة مستقلة.
- يُحقن `KUBECONFIG` كنظير لـ `DOCKER_HOST`، وتُطبِّق غلاف `renet kube` البيانات من `up()` بنفس الطريقة التي يُشغِّل بها `renet compose` عمليات Docker.

يعيش استنساخ العنقود الكامل ونقله في `rdc cluster fork` و `rdc cluster migrate`. هذه هي القدرة المميزة: تفريع أو نقل عنقود قيد التشغيل، بما في ذلك بياناته، إلى جهاز أو مركز بيانات آخر بفترة انتقال قصيرة. راجع دليل [Kubernetes](/ar/docs/kubernetes) للاطلاع على النموذج الكامل والأوامر وأرقام فترة الانتقال المقيسة.
