...
يعرض جميع المستودعات على الجهاز مع حجمها وحالة التوصيل وحالة التشفير، بالإضافة إلى عمود `Type` الذي يصنّف كل مستودع على أنه `grand` (مستوى أعلى) أو `fork` (نسخة CoW من مستودع آخر) أو `unknown` (لا يمكن تحديد النسب). راجع [المستودعات. عمود Type ومرآة الحالة](/ar/docs/repositories#type-column-and-the-state-mirror) للاطلاع على قواعد التصنيف الكاملة.
