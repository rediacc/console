...
> يجب أن يكون المستودع غير محمل قبل تغيير الحجم. `repo expand` يعمل دون توقف. يؤدي تغيير الحجم إلى تعديل الحجم الأقصى للمستودع؛ لإعادة الكتل المحررة إلى التجمع دون تغيير الحد الأقصى، استخدم [`repo trim`](#reclaim-space-trim) بدلاً من ذلك.
...
ضمانات: يرفض auto-grow عندما تكون المساحة الحرة في التجمع أقل من احتياطي (10 جيجابايت أو 5% من التجمع، أيهما أكبر)، وينتظر 30 دقيقة على الأقل بين كل توسعتين للمستودع ذاته، ولا يتجاوز `--max-quota` أبداً. لا يوجد تقليص تلقائي: تقليل الحجم الأقصى للمستودع يبقى عملية يدوية وغير متصلة عبر [`repo resize`](#resize).
