...
لمعرفة كيف تحصل الخدمات على عناوين IP الاسترجاعية ونظام الفتحات `.rediacc.json`، راجع [الخدمات](/ar/docs/services#service-networking-rediaccjson).
...
1. البنية التحتية مُكوَّنة على الجهاز ([إعداد الجهاز، تكوين البنية التحتية](/ar/docs/setup#infrastructure-configuration)):
...
2. سجلات DNS تشير إلى نطاقك على عنوان IP العام للخادم (راجع [تكوين DNS](#dns-configuration) أدناه).
...
3. **ذاكرة التخزين المؤقت من جانب العميل (`rediacc.json`).** يُخزّن CLI نسخة مضغوطة من `acme.json` تحت `acmeCertCache` في ملف تكوينك، مفهرسة بـ `baseDomain`. يتيح ذلك لأجهزة متعددة مشاركة الشهادات (عبر `rdc machine infra cert push <machine>`) ويعمل كسجل غير متصل بالشبكة.
...
| منفذ TCP غير قابل للوصول | المنفذ غير مسجل في البنية التحتية | شغّل `rdc machine infra set <machine> --tcp-ports ...` و`push-infra` |
