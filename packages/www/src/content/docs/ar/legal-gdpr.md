...
| [المادة 17](https://gdpr-info.eu/art-17-gdpr/)، حق المحو | حذف البيانات الشخصية عند الطلب | يمحو `rdc repo delete` مجلد LUKS تشفيرياً. حذف النسخة المستنسخة يزيل النسخة المستنسخة بالكامل. |
...
| [المادة 30](https://gdpr-info.eu/art-30-gdpr/)، سجلات المعالجة | الحفاظ على سجلات أنشطة المعالجة | يتتبع تسجيل التدقيق أكثر من 40 نوع حدث على مستوى الحساب: المصادقة، رموز API، عمليات مخزن التكوين، والترخيص. التصدير عبر لوحة تحكم المسؤول أو صفحة نشاط البوابة (يتوفر تصدير JSON). |
...
بالنسبة للعمليات عبر الأجهزة، ينقل `rdc repo push/pull` البيانات عبر SSH. يستقبل وجهة النسخ الاحتياطي وحدات تخزين مشفرة بـ LUKS لا يمكن قراءتها بدون بيانات اعتماد المشغل.
