...
- **مستودع Kubernetes هو فضاء أسماء.** ينشئ `rdc repo create --cluster <name>` مستودعاً موطنه وقت التشغيل هو فضاء أسماء Kubernetes `<repo>` داخل ذلك العنقود.
...
تتجسد عُقد العنقود أيضاً في `resources.machines`، بحيث يمكنك الاتصال عبر SSH بعقدة محددة باستخدام `rdc term connect -m <cluster>-<pool>-<n>` المعتاد.
