...
تغطي هذه الصفحة نشر وإدارة الخدمات المحتوية، بما في ذلك ملفات Rediaccfile وشبكات الخدمات والتشغيل/الإيقاف والعمليات المجمّعة والتشغيل التلقائي.

## ملف Rediaccfile

**Rediaccfile** هو سكريبت Bash يحدد كيف يتم تشغيل الخدمات وإيقافها. يتم **استدعاؤه** (وليس تنفيذه كعملية منفصلة)، لذا تشاركه الدوال نفس السياق الحد وتملك الوصول إلى جميع متغيرات البيئة المُصدّرة. يجب أن يكون اسمه `Rediaccfile` أو `rediaccfile` (غير حساس لحالة الأحرف) ويوضع داخل نظام ملفات المستودع المحمّل.
...
يحتوي Rediaccfile على دالتين كحد أقصى:

| الدالة | وقت التشغيل | الغرض | سلوك الخطأ |
|--------|------------|-------|------------|
| `up()` | عند التشغيل | بدء الخدمات (مثل `renet compose -- up -d`) | فشل الجذر **حرج** (يوقف كل شيء). فشل المجلدات الفرعية **غير حرج** (يُسجّل ويستمر) |
| `down()` | عند الإيقاف | إيقاف الخدمات (مثل `renet compose -- down`) | **أفضل جهد** -- الأخطاء تُسجّل لكن تُنفّذ جميع ملفات Rediaccfile دائماً |

كلتا الدالتين اختيارية. إذا لم تُعرّف دالة، يتم تخطيها بصمت.
...
| `REDIACC_REPOSITORY` | معرّف GUID للمستودع | `a1b2c3d4-e5f6-...` |
| `REDIACC_NETWORK_ID` | معرّف الشبكة (عدد صحيح) | `2816` |
| `DOCKER_HOST` | مقبس Docker للعملية المعزولة لهذا المستودع | `unix:///var/run/rediacc/docker-2816.sock` |
| `{SERVICE}_IP` | عنوان IP الحلقي لكل خدمة معرّفة في `.rediacc.json` | `POSTGRES_IP=127.0.11.2` |

يتم إنشاء متغيرات `{SERVICE}_IP}` تلقائياً من خلال تعيينات الفتحات في `.rediacc.json` وتُصدّر قبل تنفيذ دوال Rediaccfile. تحوّل قاعدة التسمية اسم الخدمة إلى أحرف كبيرة مع استبدال الشرطات بشرطات سفلية، ثم تضيف `_IP`. على سبيل المثال، الخدمة باسم `listmonk-app` مع فتحة `0` تصبح `LISTMONK_APP_IP=127.0.11.2`.

> **تحذير: لا تستخدم `sudo docker` في ملفات Rediaccfile.** أمر `sudo` يعيد تعيين متغيرات البيئة، مما يعني فقدان `DOCKER_HOST` واستهداف أوامر Docker لعملية النظام بدلاً من العملية المعزولة للمستودع. هذا يكسر عزل الحاويات وقد يسبب تعارضات في المنافذ. سيمنع Rediacc التنفيذ إذا اكتشف `sudo docker` بدون `-E`.
>
> استخدم `renet compose` في ملفات Rediaccfile، فهو يتعامل تلقائياً مع `DOCKER_HOST` ويحقن تسميات الشبكة لاكتشاف المسارات ويهيئ شبكات الخدمات. راجع [الشبكات](/ar/docs/networking) لتفاصيل كيفية كشف الخدمات عبر الوكيل العكسي. إذا استدعيت Docker مباشرة، استخدم `docker` بدون `sudo`، دوال Rediaccfile تعمل بالفعل بصلاحيات كافية. إذا كان يجب عليك استخدام sudo، استخدم `sudo -E docker` للحفاظ على متغيرات البيئة.
>
> `renet` هي أداة منخفضة المستوى بعيدة. لسير العمل الطبيعي للمستخدم من محطة العمل، فضّل أوامر `rdc` مثل `rdc repo up` و `rdc repo down`. راجع [rdc vs renet](/ar/docs/rdc-vs-renet).
...
للمشاريع ذات مجموعات خدمات مستقلة متعددة، استخدم المجلدات الفرعية:
...
3. تعيين الفتحة التالية المتاحة للخدمات الجديدة
4. حفظ النتيجة في `{repository}/.rediacc.json`

### حساب عنوان IP

يُحسب عنوان IP للخدمة من معرّف شبكة المستودع وفتحة الخدمة. يتم توزيع معرّف الشبكة على الثُمانية الثانية والثالثة والرابعة من عنوان الاسترجاع `127.x.y.z`. تحصل الخدمات على إزاحة تبدأ من 2:
...
يختلف renet و Docker عن قصد في كيفية التعامل مع إعادة تشغيل الحاويات. فهم هذا التقسيم مهم عند تشخيص سبب عودة حاوية إلى العمل أو عدمها.
...
1. يقرأ `.rediacc.json` لكل مستودع ويجد الخدمات ذات `restart_policy` قابلة للاسترداد.
2. يسرد جميع الحاويات لـ daemon ذلك المستودع، ويحدد الموقوفة منها، ويعيد تشغيلها وفقاً للسياسة المحفوظة. فترة سماح مدتها 30 ثانية تمنع التعارض مع مشغّل نفّذ `docker stop` للتو.
3. تعالج نفس الحلقة أيضاً `/var/run/rediacc/cold-backup-<guid>.running.json` (راجع [دلالات النسخ الاحتياطية الباردة](backup-restore.md#cold-backup-semantics)). تُعاد تشغيل الحاويات المدرجة بغض النظر عن السياسة المحفوظة، لأن الـ sidecar يعني "أوقف renet هذه الحاويات عن قصد وعليه إعادة تشغيلها للمشغّل."
...
> **ملاحظة:** مستودعات fork تحصل على مسارات تلقائية تحت نطاق فرعي الأصل: `{service}-fork-{tag}.{repo}.{machine}.{baseDomain}`. يتم تخطي النطاقات المخصصة لمستودعات fork.
...
| الخيار | الوصف |
|--------|-------|
| `--no-wait` | العودة فور بدء تشغيل الحاويات؛ تستمر فحوصات الصحة في الخلفية |
...
2. بدء عملية Docker المعزولة
3. إنشاء `.rediacc.json` تلقائياً من ملفات compose
4. تشغيل `up()` في جميع ملفات Rediaccfile (بترتيب A-Z)

بعد النشر، يُظهر الإخراج قسم **PROXY ROUTES** مع عناوين URL الفعلية لكل خدمة. الخدمات ذات تسميات Traefik المخصصة (مثل `traefik.http.routers.myapp.rule=Host(...)`) تُظهر نطاقاتها المخصصة كعناوين URL رئيسية:
...
الخدمات بدون تسميات Traefik المخصصة تُظهر فقط المسار المُنتج تلقائياً. استخدم هذه العناوين (وليس النمط العام المطبوع بواسطة CLI) لولوج المتصفح وندءات API والإعدادات بين الخدمات.

### التشغيل المنفصل

مع `--no-wait`، يعود الأمر فور انطلاق الحاويات دون انتظار اكتمال فحوصات الجاهزية. يُنهي التشغيل عمله في الخلفية: يواصل الوكيل إعادة المحاولة حتى ترتبط كل خدمة بمنفذها، فتستعيد المسارات عملها تلقائياً. تابع التقدم بـ `rdc machine status <machine> --containers`. مثالي للنسخ المؤقتة والحلقات البرمجية التي لا تحتاج إلى جاهزية الخدمات قبل الخطوة التالية.

### اختبار الجاهزية

بعد `up()`، يفحص renet كل خدمة HTTP ويتحقق من قبولها اتصالات TCP قبل إعلانها جاهزة، حتى لا يصطدم أول طلب من المتصفح بخطأ 502 من الوكيل. تُعامَل الحاويات التي تُعرّف فحص صحة Docker معاملةً مميزة: الحاوية السليمة تتجاوز الفحص مباشرةً، وتلك التي لا تزال ضمن `start_period` تُسجَّل كملاحظة إعلامية لا تحذيراً. يتخلى renet عن الانتظار بعد 15 ثانية (عدّلها بمتغير البيئة `REDIACC_READINESS_TIMEOUT`، بالثواني، على الجهاز)؛ ويتجاوز التشغيل المنفصل هذا الفحص كلياً.
...
| `--no-start` | التثبيت والتحضير فقط، دون تشغيل خطوات `up()` الخاصة بالمستودع |
...
افتراضياً، يجب تحميل المستودعات وتشغيلها يدوياً بعد إعادة تشغيل الخادم. يقوم **التشغيل التلقائي** بتهيئة المستودعات للتحميل تلقائياً وبدء Docker وتنفيذ `up()` في Rediaccfile عند إقلاع الخادم.
...
العنصر النائب `POSTGRES_PASSWORD: changeme` أعلاه بخير للبرنامج التعليمي، لكن التطبيقات الحقيقية تحتاج بيانات اعتماد حقيقية، والتزام تخزينها في ملف compose (أو ملف `.env` داخل المستودع) يعني أن fork يرثها أيضاً. للبيانات الاعتمادية على النشر، استخدم `rdc repo secret`. تعيش القيم خارج صورة المستودع المشفرة، لذا تبدأ forks بخريطة أسرار فارغة.

يعمل نمطا تسليم في compose:

**نمط `env`.** قم بالاستيفاء عبر `${REDIACC_SECRET_<KEY>}` في أي قيمة `environment:`. يمرر غلاف renet القيمة إلى بيئة الحاوية عند النشر.

**نمط `file`.** تنزل القيمة إلى ملف tmpfs على جانب المضيف في `/var/run/rediacc/secrets/<networkID>/<KEY>`، وتحمّله في الحاوية عبر كتلة `secrets:` القياسية لـ Docker compose. تقرأ الحاوية `/run/secrets/<key>`. فضّل هذا النمط لأي شيء حساس. القيم لا تظهر أبداً في `docker inspect` أو `/proc/<pid>/environ`.
