...
الأوامر التي تُجري تغييرات على الجهاز، مثل عمليات النشر والنسخ الاحتياطي و`rdc machine setup`، توفّر تلقائيًا ملف `renet` الثنائي المطابق. إذا تم تحديث الملف الثنائي، يُعاد تشغيل خادم المسارات (`rediacc-router`) تلقائيًا لاعتماد الإصدار الجديد. أما أوامر القراءة فقط، مثل `rdc machine status` و`rdc repo list`، فلا تستبدل الملف الثنائي أبدًا: بل تستخدم الإصدار الموجود على الجهاز وتعرض تحذيرًا إذا كان مختلفًا عن إصدار واجهة سطر الأوامر.
...
لتخطي إعادة التشغيل التلقائية، مرّر `--skip-router-restart` إلى أمر يوفّر renet، أو عيّن متغير البيئة `REDIACC_SKIP_ROUTER_RESTART=1`.
