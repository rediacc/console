...
- شغّل `rdc config scan-keys server-1` لتحديث مفاتيح المضيف
- تأكد من تطابق منفذ SSH: `--port 22`
- اختبر بأمر بسيط: `rdc term server-1 -c "hostname"`
...
- شغّل مع `--debug` للحصول على مخرجات مفصّلة: `rdc config setup-machine server-1 --debug`
...
- تحقق من إمكانية الوصول إلى صور Docker (فكّر في تشغيل `renet compose -- pull` في `prep()`)
