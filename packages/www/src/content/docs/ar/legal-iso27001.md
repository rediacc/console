...
حسناً. ISO/IEC 27001:2022 هو المعيار الدولي لأنظمة إدارة أمن المعلومات. منشور من قبل ISO/IEC، وهو وثيقة طويلة تسرد ضوابط للتشفير وإدارة الوصول والاستجابة للحوادث وعشرات مجالات الأمن. أنت بالتأكيد تعرف ما هذا. لذا اسمح لي أن أكون صريحاً: Rediacc لا يعالج كل ضابط في المعيار، ولن نتظاهر بأنه يفعل. ما يلي هو خريطة صادقة لحيث يندرج Rediacc. الإصدار الحالي هو ISO/IEC 27001:2022.
...
|-------------|--------|--------------|
| **A.8**، إدارة الأصول | A.8.1 جرد الأصول | كل مستودع هو أصل منفصل قابل للتحديد بمعرف GUID فريد. `rdc machine status <machine> --repositories` يسرد جميع المستودعات مع الحجم وحالة التركيب وعدد الحاويات. |
...
| **A.12**، أمن العمليات | A.12.3 النسخ الاحتياطي | `rdc repo push/pull` مع تخزين مشفر خارج الموقع لوجهات متعددة (SSH، S3، B2، Azure، GDrive). لقطات CoW للاسترداد في نقطة زمنية محددة. `rdc repo admin validate` يتحقق من صحة النسخ الاحتياطية وسلامة المستودع. |
| **A.12**، أمن العمليات | A.12.4 التسجيل والمراقبة | 70+ نوع حدث (المصادقة، رموز API، التكوين، الترخيص، عمليات الجهاز). مراقبة صحة الجهاز عبر `rdc machine status`. مراقبة حالة الحاويات والموارد. |
...
- المستودعات قابلة للتعداد لكل جهاز (`rdc machine status --repositories`)
...
3. **الترقية**: استخدام `rdc repo promote` لتبديل النسخة إلى الإنتاج
4. **التدقيق**: جميع العمليات مسجلة بطوابع زمنية وتحديد هوية الفاعل

## التحسين المستمر

- تصدير سجلات التدقيق يدعم المراجعات الأمنية الدورية
- فحوصات صحة الجهاز (`rdc machine status --system`) تدعم المراقبة التشغيلية
- `rdc repo admin validate` يتحقق من صحة النسخ الاحتياطية بعد كل عملية
