...
كل جهاز يشغّل Renet يخزن حالة السلسلة الأخيرة المعروفة لديه في `{licenseDir}/chain-state.json` (أي `/var/lib/rediacc/license/chain-state.json`، وهو مسار شقيق لمجلد `repos/` الخاص بكل مستودع). حالة السلسلة محددة النطاق لكل مفتاح توقيع واشتراك، بمفتاح على شكل `"<keyId>:<subscriptionId>"`، بحيث تتعقب الأكوان الموقَّعة بمفاتيح مختلفة تسلسلاتها بشكل مستقل. عند كل تحقق من الترخيص، يتحقق Renet من:
