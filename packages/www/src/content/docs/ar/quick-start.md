...
rdc config machine setup my-server        # يجهّز renet + ينشئ مخزن البيانات
```
...
rdc repo template apply app-postgres -m my-server -r my-app   # نشر docker-compose.yml + Rediaccfile
```
...
rdc repo status my-app -m my-server                  # حالة التوصيل، Docker، الحجم، التشفير
```
...
```bash
rdc vscode my-server my-app              # يفتح VS Code عبر SSH، داخل صندوق حماية المستودع
```
...
**سير العمل التجريبي:** `rdc repo template apply` ثم `rdc vscode my-server my-app` ثم عدّل `docker-compose.yml` ثم `renet dev up` ثم شاهد التطبيق يعمل ثم كرّر.
...
**الطرفية:**
```bash
rdc term my-server my-app                            # SSH إلى صندوق حماية المستودع
rdc term my-server my-app -c "curl localhost:3000"   # تنفيذ أمر والخروج
rdc term my-server                                   # SSH إلى الجهاز (بدون صندوق حماية)
```
...
```bash
rdc repo tunnel my-server my-app                     # كشف تلقائي للحاوية والمنفذ
rdc repo tunnel my-server my-app --port 5432         # نفق Postgres
rdc repo tunnel my-server my-app --port 5432 --local 15432  # منفذ محلي مخصص
```
...
```bash
rdc repo fork my-app -m my-server --tag experiment --up     # استنساخ فوري بتقنية CoW + تشغيل
rdc repo list -m my-server                                  # يعرض: my-app (أصلي) + my-app:experiment (متفرع)
rdc repo delete my-app:experiment -m my-server              # حذف المتفرع، الأصلي لا يتأثر
```
...
```bash
rdc config infra set my-server           # إعداد: النطاق الأساسي، عناوين IP العامة، نطاقات المنافذ
rdc config infra show my-server          # مراجعة الإعدادات
rdc config infra push my-server          # دفع إعدادات الوكيل إلى الخادم البعيد
```
...
```bash
rdc repo template apply proxy -m my-server -r infra     # نشر الوكيل في مستودع
