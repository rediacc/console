...
rdc machine setup my-server  # Aprovisiona renet + crea almacén de datos
```
...
```bash
rdc repo admin template list                                        # Mostrar plantillas integradas
rdc repo admin template apply my-app --template app-postgres  # Despliega docker-compose.yml + Rediaccfile
```
...
```bash
rdc repo up my-app  # Ejecutar Rediaccfile up()
rdc repo list -m my-server                           # Ver todos los repos en la máquina
rdc repo status my-app  # Estado de montaje, Docker, tamaño, cifrado
```
...
**Flujo de demostración:** `rdc repo admin template apply` → `rdc vscode connect my-app` → editar `docker-compose.yml` → `renet dev up` → ver la app ejecutándose → iterar.
...
**Terminal:**
```bash
rdc term connect my-app                            # SSH al sandbox del repo
rdc term connect my-app -c "curl localhost:3000"   # Ejecutar comando y salir
rdc term connect my-server                                   # SSH a la máquina (sin sandbox)
```

**Sincronización de archivos (rsync sobre SSH):**
```bash
rdc repo sync upload my-app --local ./src                                   # Subir un directorio
rdc repo sync upload my-app --local ./config.yml --remote conf              # Subir un archivo
rdc repo sync download my-app --local ./backup                              # Descargar un directorio
rdc repo sync download my-app --remote-file conf/config.yml --local ./dl    # Descargar un archivo
rdc repo sync download my-app --local ./backup --dry-run                    # Previsualizar primero
```

**Túnel (reenvío de puertos SSH al contenedor):**
```bash
rdc repo tunnel my-app -c app  # Auto-detectar puerto para el contenedor app
rdc repo tunnel my-app -c db --port 5432  # Túnel a Postgres
rdc repo tunnel my-app -c db --port 5432 --local 15432  # Puerto local personalizado
```
...
```bash
rdc repo fork my-app --tag experiment --up  # Clon CoW instantáneo + iniciar
rdc repo list -m my-server                                  # Muestra: my-app (grand) + my-app:experiment (fork)
rdc repo delete my-app:experiment  # Eliminar fork, grand intacto
```
...
### 3. Respaldar en el almacenamiento fragmentado

```bash
# Tomar un snapshot puntual
rdc backup snapshot my-app

# Ver qué snapshots existen
rdc backup manifests my-app

# Comprobar los bytes almacenados frente a su cuota
rdc backup usage
```

Los snapshots se suben al almacenamiento fragmentado, que necesita un servidor de cuenta y un repositorio con licencia. Solo se almacenan los datos que cambiaron, así que un segundo snapshot del mismo repositorio cuesta la diferencia en vez de la imagen completa.

Subir un repositorio a almacenamiento en la nube con `--to <storage>` está retirado y ahora se rechaza. Para leer un archivo escrito antes de ese cambio, `rdc storage browse` sigue funcionando y necesita `rclone` en su PATH.
...
```bash
rdc machine infra set my-server  # Configurar: dominio base, IPs públicas, rangos de puertos
rdc machine infra show my-server  # Revisar configuración
rdc machine infra push my-server  # Enviar configuración de proxy al remoto
```
...
```bash
rdc repo admin template apply infra --template proxy  # Desplegar proxy en un repo
