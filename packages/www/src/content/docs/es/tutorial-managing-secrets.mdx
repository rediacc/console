...
Si quieres secrets en el fork para pruebas, configúralos explícitamente en el fork con valores de sandbox, por ejemplo `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`. Ahora el fork habla con el sandbox de Stripe y arranca limpiamente. Las credenciales de producción nunca salieron de producción.
