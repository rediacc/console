...
| [Art. 17](https://gdpr-info.eu/art-17-gdpr/), Diritto alla cancellazione | Cancellare i dati personali su richiesta | `rdc repo destroy` cancella crittograficamente il volume LUKS. L'eliminazione di un fork rimuove completamente la copia clonata. |
...
Per le operazioni tra macchine diverse, `rdc repo backup push/pull` trasferisce i dati tramite SSH. La destinazione del backup riceve volumi cifrati con LUKS che non possono essere letti senza le credenziali dell'operatore.
