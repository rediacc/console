...
Sostituisci `<machine>` e `<repo>` con i nomi effettivi della tua macchina e del tuo repository. Esegui `rdc repo list` per elencare i repository disponibili con la loro mappatura nome-GUID. È più semplice di quanto sembri: la configurazione è già completa.
