...
Se vuoi segreti nel fork per i test, impostali esplicitamente nel fork con valori sandbox, ad esempio `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`. Ora il fork comunica con la sandbox di Stripe e si avvia correttamente. Le credenziali di produzione non hanno mai lasciato la produzione.
