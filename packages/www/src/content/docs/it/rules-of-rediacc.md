...
- Il datastore è un file pool BTRFS a dimensione fissa sul disco di sistema. Usa `rdc machine status <name> --system` per vedere lo spazio libero effettivo. Espandi con `rdc datastore resize`.
...
- **`repo fork --checkpoint`** cattura lo stato dei processi dal padre in esecuzione e clona il checkpoint con il fork via CoW. Il `repo up` del fork ripristina i processi mentre il padre continua a girare, sulla stessa macchina. I processi ripristinati che fanno riferimento agli indirizzi loopback del padre (socket associati, IP di servizio in memoria) vengono reindirizzati in modo trasparente agli indirizzi propri del fork: parlano quindi con la copia dei dati del fork, mai con quella del padre.
...
- **CRIU viene installato** durante `rdc machine setup`.
...
Su tutti e cinque gli OS server ufficialmente supportati (vedi [Requisiti](/it/docs/requirements)), il daemon docker per singolo repository e i container che esegue usano **etichette container predefinite**. `rdc machine setup` non installa una policy SELinux personalizzata né un profilo AppArmor. Questa è una scelta intenzionale: il compromesso è che i processi dei container vengono eseguiti con la policy di etichettatura predefinita del sistema operativo host, non con un profilo di confinamento specifico di Rediacc. Se il tuo modello di minaccia richiede controlli di accesso obbligatori a livello di container, configurali a livello di host prima di distribuire.
...
In sintesi: `rdc` e `renet` rilevano automaticamente il sistema operativo in esecuzione e producono lo stesso isolamento per singolo repository su tutte le cinque distribuzioni supportate. Non è richiesto alcun flag di postura di sicurezza per OS.
...
- **Takeover** (`rdc repo promote <fork>`) sostituisce i dati del repository grand con quelli del fork. Il grand mantiene la propria identità (GUID, networkId, domini, autostart, catena di backup). I vecchi dati di produzione vengono preservati come fork di backup. Usa per: testa l'aggiornamento sul fork, verifica, poi esegui takeover in produzione. Ripristina con `rdc repo promote <backup-fork>`.
...
- `repo delete` esegue una pulizia completa inclusi IP loopback e unità systemd. Esegui `rdc machine prune <name>` per ripulire i residui di eliminazioni legacy.
