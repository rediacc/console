...
- **Cross-repo connections are kernel-blocked**: The kernel blocks connections to loopback IPs outside the repository's `/26` subnet automatically. A service in one repo cannot reach services in another repo. One exception: a fork's connections into its parent's subnet are transparently redirected to the fork's own services (same service, fork's copy of the data), never to the parent. Unrelated repositories stay blocked.
- **Inter-service communication**: Use **service names** (e.g. `db`, `redis`) - renet automatically injects every service name as a hostname that resolves to the correct IP. Docker DNS names do NOT work in host mode, but service names via `/etc/hosts` do. Avoid embedding `${DB_IP}` or similar in persistent config files (e.g. connection strings stored in a database) - a fork redirects its immediate parent's carried-over IPs to its own services, but the redirect covers only one lineage step: a fork of a fork still holds grandparent IPs that dead-end. Service names always resolve correctly per repo.
...
- The datastore is a fixed-size BTRFS pool file on the system disk. Use `rdc machine status <name> --system` to see effective free space. Expand with `rdc datastore resize`.
...
- **`repo fork --checkpoint`** captures process state from the running parent and CoW-clones the checkpoint with the fork. The fork's `repo up` restores the processes while the parent keeps running, on the same machine. Restored processes that reference the parent's loopback addresses (bound sockets, in-memory service IPs) are transparently redirected to the fork's own addresses, so they talk to the fork's copy of the data, never the parent's.
...
- **Takeover** (`rdc repo promote <fork>`) replaces the grand repo's data with a fork's data. The grand keeps its identity (GUID, networkId, domains, autostart, backup chain). Old production data is preserved as a backup fork. Use for: test upgrade on fork, verify, then takeover to production. Revert with `rdc repo promote <backup-fork>`.
