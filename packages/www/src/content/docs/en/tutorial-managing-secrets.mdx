...
If you want secrets in the fork for testing, set them on the fork explicitly with sandbox values, for example `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`. Now the fork talks to the Stripe sandbox and starts cleanly. Production credentials never left production.
