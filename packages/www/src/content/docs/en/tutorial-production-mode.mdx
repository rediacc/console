...
If you have a `renet dev up` session still running inside the repo, bring it down first with `rdc vscode connect my-app` followed by `renet dev down`. From there, the rest is driven entirely from your laptop.
