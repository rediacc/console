...
This creates a config named `my-infra` and stores it in `~/.rediacc/my-infra.json`. The default config (when no name is given) is stored as `~/.rediacc/rediacc.json`.
