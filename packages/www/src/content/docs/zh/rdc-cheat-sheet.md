...
| `rdc config machine setup <machine>` | 运行机器初始配置 |
...
| `rdc repo sync upload -m <machine> -r <repo> --local <path>` | 将本地文件上传到仓库 |
| `rdc repo sync download -m <machine> -r <repo> --local <path>` | 将仓库文件下载到本地 |
