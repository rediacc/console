...
Rediacc 部署限制。三个限制是硬性的，无法通过添加硬件更改：每个仓库的 61 个服务上限（网络地址空间分配）、Linux 6.1 最低内核版本要求（CRIU 需求），以及 Let's Encrypt 每个注册域名每周 50 个通配符证书的签发上限。其他所有限制都是软性的：增加硬件时这些限制会改变。在规划拓扑前，请了解两者的区别。
...
注意：在单个仓库中达到 61 个服务通常表示架构设计问题，而不是 Rediacc 的限制。解决方案是将辅助容器和监控代理迁移到具有独立隔离边界的单独仓库中，或减少应用程序本身中独立运行的进程数量。
