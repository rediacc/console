...
- Rediaccfile 由 renet 加载（source），它可以访问 shell 变量，而不仅仅是环境变量。
...
- **跨仓库连接被内核阻止**：内核会自动阻止对仓库 `/26` 子网之外的回环 IP 的连接。一个仓库中的服务无法访问另一个仓库中的服务。唯一的例外：fork 指向其父仓库子网的连接会被透明地重定向到 fork 自己的服务（同一服务、fork 的数据副本），绝不会到达父仓库。无关联的仓库之间仍然会被阻止。
- **服务间通信**：请使用**服务名**（如 `db`、`redis`）：renet 会自动将每个服务名注入为解析到正确 IP 的主机名。Docker DNS 名称在 host 模式下不可用，但通过 `/etc/hosts` 注入的服务名可用。避免把 `${DB_IP}` 之类的值写进持久化配置文件（例如存储在数据库中的连接字符串）：fork 会把从直接父仓库继承的 IP 重定向到自己的服务，但这种重定向只覆盖一代，fork 的 fork 仍然带着指向祖父仓库、最终无处可达的 IP。而服务名在每个仓库中始终能正确解析。
...
- 数据存储是系统磁盘上固定大小的 BTRFS 池文件。使用 `rdc machine status <name> --system` 查看有效可用空间。使用 `rdc datastore resize` 扩容。
...
- **`repo fork --checkpoint`** 从运行中的父仓库捕获进程状态，并将 checkpoint 随 fork 一起 CoW 克隆。fork 的 `repo up` 会在父仓库于同一台机器上继续运行的同时恢复这些进程。恢复出的进程若引用父仓库的 loopback 地址（已绑定的套接字、内存中的服务 IP），会被透明地重定向到 fork 自己的地址，因此它们访问的始终是 fork 的数据副本，绝不会是父仓库的。
...
总之：`rdc` 和 `renet` 会自动检测运行环境，在所有五个支持的发行版上提供相同的仓库级隔离。无需指定特定于操作系统的安全姿态标志。
...
- **仓库专用 daemon 禁用了 Docker bridge 网络。** 每个仓库的 `daemon.json`（`FlavorRediacc`）都带有 `"bridge": "none"` 和 `"iptables": false`，因此一个简单的 `docker run <image>` 创建的容器只有 loopback 接口，没有出站连接。这不是 bug，而是跨仓库隔离的强制方式：阻止一个仓库访问另一个仓库 loopback IP 的内核级 eBPF 钩子，只对位于主机网络命名空间中的容器生效。生产服务请使用 `renet compose`，它会自动注入 `network_mode: host`。在 shell 中运行临时一次性容器时，请显式传递 `--network host`。（每用户 Hub daemon（`FlavorHub`，开发环境）是例外：它们启用 `bridge="docker0"` 和 `iptables=true`，使用户运行的容器可获得正常的出站网络连接。）
...
- **接管**（`rdc repo promote <fork>`）将 grand 仓库的数据替换为 fork 的数据。Grand 保留其身份（GUID、networkId、域名、自动启动、备份链）。旧的生产数据作为备份 fork 保留。用途：在 fork 上测试升级，验证后接管到生产。使用 `rdc repo promote <backup-fork>` 回滚。
