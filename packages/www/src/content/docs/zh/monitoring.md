...
碎片化程度高且共享块为零的仓库可以使用 `btrfs filesystem defragment` 安全进行碎片整理。含有共享块的仓库不应进行碎片整理，因为碎片整理会将共享块替换为唯一副本，从而增加磁盘使用量。
