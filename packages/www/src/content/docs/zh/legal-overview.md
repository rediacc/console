...
| 框架 | 范围 | Rediacc 关键能力 |
|------|------|------------------|
| [GDPR](/zh/docs/legal-gdpr) | 欧盟数据保护和隐私 | 同一机器上的 CoW 克隆、LUKS2 加密、零知识配置存储、审计日志、通过 `rdc repo destroy` 实现删除权 |
...
- **加密备份**：`rdc repo backup push/pull` 通过 SSH 传输数据。备份目标接收 LUKS 加密的卷。
