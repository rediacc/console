...
- **审计日志**：70 多种事件类型，涵盖身份验证（登录、2FA、密码更改、会话撤销）、API 令牌生命周期、配置存储操作、订阅/许可活动以及 CLI 机器操作（仓库生命周期、备份、同步、终端会话）。可通过管理仪表板、门户活动页面（带组织范围筛选）和 `rdc audit` CLI 访问。机器操作也记录在您的系统日志中，以实现纵深防御。
