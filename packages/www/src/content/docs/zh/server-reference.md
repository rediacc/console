...
# 在 Landlock 沙箱中运行命令（renet 内部使用）
renet sandbox-exec --allow-rw /path --allow-ro /usr --allow-exec /bin -- command
```

`sandbox-exec` 会将 Landlock LSM 文件系统限制应用到命令上。被沙箱隔离的进程只能访问显式允许的路径，其他所有文件系统访问都会被内核阻止。renet 在内部使用它，将 Rediaccfile 执行、compose 操作和 SSH 命令限制在各自仓库的挂载路径内。
