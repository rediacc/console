...
VS Code 会通过 SSH 直接在仓库内部打开。工作区一开始是空的：这就是你的隔离环境。你在这里创建的一切都存储在加密卷内，对同一服务器上的其他任何仓库都是不可见的。

---

下一篇：[部署第一个应用](/zh/docs/tutorial-deploy-app)。
