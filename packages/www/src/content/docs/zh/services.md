...
| `up()` | 所有 `prep()` 完成后 | 启动服务（例如 `renet compose -- up -d`） | 根 Rediaccfile 失败是**关键性的**（停止一切）。子目录失败是**非关键性的**（记录日志，继续下一个）。 |
...
> **重要：** 请始终使用 `renet compose --` 而非 `docker compose`。`renet compose` 封装器强制实施主机网络、CRIU 检查点/恢复功能、IP 分配以及 renet-proxy 所需的服务发现标签。直接使用 `docker compose` 会被 Rediaccfile 验证拒绝。详情请参阅[网络](/zh/docs/networking)。
...
> **注意：** 不要手动添加 `network_mode: host` — `renet compose` 会自动注入。不要使用 `restart: always` 或 `restart: unless-stopped` — 这些会导致 Docker 在 CRIU 检查点恢复运行之前自动启动容器。如果需要，请使用 `restart: on-failure`，或省略该配置（Rediaccfile `up()`/`down()` 管理生命周期）。
