...
| 函数 | 运行时机 | 用途 | 错误行为 |
|------|----------|------|----------|
| `up()` | 启动时 | 启动服务（例如 `renet compose -- up -d`） | 根 Rediaccfile 失败是**关键性的**（停止一切）。子目录失败是**非关键性的**（记录日志，继续下一个）。 |
...
> **重要：** 请始终使用 `renet compose --` 而非 `docker compose`。`renet compose` 封装器强制实施主机网络、IP 分配以及 renet-proxy 所需的服务发现标签。CRIU 检查点/恢复功能会添加到带有 `rediacc.checkpoint=true` 标签的容器。直接使用 `docker compose` 会被 Rediaccfile 验证拒绝。详情请参阅[网络](/zh/docs/networking)。
...
**重启策略转换。** 当你在 compose 文件中写入 `restart: always`（或 `unless-stopped`，或 `on-failure`）时，renet 在合成实际 compose 部署时会**删除**它，并用 `restart: no` 替换。原始值保存到仓库的 `.rediacc.json` 中 `services.<name>.restart_policy` 下。这可以防止 Docker 守护进程级别的自动重启干扰 CRIU 检查点/恢复（守护进程驱动的重启会从过时的检查点前状态恢复）。
