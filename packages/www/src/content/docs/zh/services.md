...
由于每个仓库运行隔离的 Docker 守护进程，`renet compose` 会自动为所有服务配置 `network_mode: host`。请将服务绑定到其分配的回环 IP：
...
> **注意：** Fork 仓库获得扁平化的自动路由：`{service}-{tag}.{machine}.{baseDomain}`。自定义域名在 fork 中被跳过。
