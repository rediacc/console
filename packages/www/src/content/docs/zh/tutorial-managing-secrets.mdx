...
正确的地方是 `rdc repo secret`。两种交付模式，设计上只写不读，fork 启动时什么都没有。
...
注意两点：`--mode env` 使值作为环境变量落入容器，`--current ""` 声明这是一个全新密钥，没有先前的值。
...
如果你确实记得旧值，可以用 `--current <old-value>` 来证明。这是更安全的路径，它能捕捉到"我在错误的终端里"这类失误。
...
如果你想在 fork 中使用密钥进行测试，用沙盒值在 fork 上单独设置，例如 `rdc repo secret set --name my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file --current ""`。现在 fork 与 Stripe 沙盒通信。生产凭据从未离开生产环境。
