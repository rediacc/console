...
如果你想在 fork 中使用密钥进行测试，用沙盒值在 fork 上单独设置，例如 `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`。这样 fork 与 Stripe 沙盒通信，可以正常启动。生产凭据从未离开生产环境。
