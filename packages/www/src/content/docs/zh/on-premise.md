...
用户可以直接从本地部署服务器安装 CLI。安装脚本会自动检测更新通道，并将 CLI 配置为从您的服务器检查更新。
...
- `ENVIRONMENT=production`（默认）：标准限制，向客户端推荐 stable 更新通道
- `ENVIRONMENT=edge`：Community 计划限制翻倍，向客户端推荐 edge 更新通道
...
如果您需要独立的环境（生产、预发布、灾备、多地区），请为每个实例单独购买订阅。单一有效性约束将此契约编码化：尝试创建第二个有效证书将返回 `409 DELEGATION_CERT_ALREADY_ACTIVE`，同时附带现有证书 ID 以及续期（推荐，保留证明链）或撤销后重建（重置证明链）的操作指引。
...
**方式 A - 客户自助服务（推荐）。** 以组织所有者或管理员身份登录上游门户，导航至 **/account/delegation-certs**。点击 **Create New**，粘贴本地部署公钥（base64 SPKI），选择有效期（或接受计划默认值），然后下载生成的 `.json` 文件。
