...
> **Hot 与 cold 布局。** 当策略使用 `--mode hot` 时，备份会落在存储的 `<bucket>/<folder>/hot/<guid>` 中；`--mode cold` 策略写入 `<bucket>/<folder>/cold/<guid>`。规范模式是一个每小时 hot 策略加一个每周 cold 策略。完整细节参见[备份与恢复. 存储布局](/zh/docs/backup-restore#storage-layout)。
...
三个字段全部必填：`--public-ipv4`、`--base-domain` 和 `--cert-email`。运行 `rdc config infra show -m <machine>` 检查缺少哪些字段。
