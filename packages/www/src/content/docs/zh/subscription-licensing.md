...
机器槽位追踪在服务器端实施。当 CLI 发放仓库许可证时，账户服务器检查订阅的机器槽位配额（例如，Community 有 2 台机器，Professional 有 5 台）。槽位从该机器上最后一次仓库许可证发放起保持 1 小时，不活跃后自动释放。因此，一个 5 槽位的计划可以随时间覆盖数十台机器，因为槽位仅在您积极配置时才被保持。
