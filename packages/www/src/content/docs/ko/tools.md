...
Rediacc는 원격 머신과 저장소에서 일상 업무를 위한 네 가지 도구를 제공합니다: SSH를 통한 파일 동기화, SSH 터미널, VS Code 통합, CLI 자동 업데이트입니다. 네 가지 도구 모두 SSH를 통해 실행됩니다. 원격 쪽에 에이전트나 데몬이 필요하지 않습니다. GUI가 필요하다면 이 페이지는 당신을 위한 페이지가 아닙니다.
...
디렉터리에는 `--remote`를 (기본값), 단일 파일에는 `--remote-file`을 사용합니다. 두 플래그는 함께 사용할 수 없습니다.
...
이 명령은:
1. 머신에 오픈소스 에디터 서버를 한 번 설치합니다(읽기 전용 공유 경로, 체크섬 검증)
2. 저장소 샌드박스 내부에서 실행하여 파일 트리, 통합 터미널, 모든 하위 프로세스가 저장소와 동일한 환경을 봅니다
3. 로컬 포트로 SSH 터널을 열고 세션별 토큰 URL로 브라우저를 시작합니다
...
| 옵션 | 설명 |
|--------|-------------|
| `--no-open` | 브라우저 실행 대신 URL 출력 |
| `--url-only` | 표준 출력에 URL 한 줄만 출력하고 터널 유지 (스크립팅용) |
| `--local <port>` | 로컬 터널 포트 지정 |
| `--server-provider <id>` | 에디터 서버 구현체: `openvscode` (기본값) 또는 `code-server` |
| `--server-archive <file>` | 머신의 미리 준비된 tar볼에서 설치 (인터넷 없이 가능) |

Linux, macOS, Windows, 태블릿 어디서든 사용할 수 있습니다. 로컬에 필요한 것은 브라우저뿐입니다.
