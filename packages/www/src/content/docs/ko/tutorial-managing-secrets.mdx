...
테스트를 위해 포크에 시크릿이 필요하다면 포크에 명시적으로 샌드박스 값을 설정합니다. 예: `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`. 이제 포크는 Stripe 샌드박스와 통신하며 정상적으로 시작됩니다. 프로덕션 자격 증명은 프로덕션을 떠나지 않았습니다.
