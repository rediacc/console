...
별도의 환경(프로덕션, 스테이징, DR, 멀티 리전)이 필요한 경우, 설치당 하나의 구독을 구매하세요. 단일 활성 적용은 이 계약을 성문화합니다: 두 번째 활성 인증서를 생성하려는 시도는 기존 인증서 id와 함께 `409 DELEGATION_CERT_ALREADY_ACTIVE`를 반환하고, 갱신(권장 - 체인 보존) 또는 취소 후 재생성(체인 초기화) 지침을 제공합니다.
...
**옵션 B - 관리자 (교차 고객).** Rediacc 지원 또는 업스트림 시스템 관리자가 동일한 매개변수로 `POST /admin/delegation-certs`를 사용할 수 있습니다.
