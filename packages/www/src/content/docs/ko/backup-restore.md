...
출력은 해당 레포지토리에 대해 청크 스토리지가 보관 중인 스냅샷을 나열합니다.
...
`--mode hot`과 `--mode cold`는 데이터가 어디에 저장되는지가 아니라, 백업을 뜨는 동안 레포지토리를 어떻게 다루는지를 나타냅니다.

**핫**은 실행 중인 레포지토리의 스냅샷을 찍습니다. 컨테이너는 계속 요청을 처리하며 쓰기 도중에 이미지가 캡처되므로, 백업은 충돌 일관성을 가집니다. 즉 그 순간 머신의 전원이 나갔을 때와 정확히 같은 상태입니다. 이는 자체 저널에서 복구되는 것, 즉 대부분의 데이터베이스에는 문제가 없습니다.

**콜드**는 먼저 컨테이너를 멈추고, 디스크에 내려쓰고, 실제로 멈췄는지 확인한 다음 이미지를 동결하고 그제야 컨테이너를 다시 시작합니다. 실질적인 다운타임이 발생하지만, 그 다운타임은 전송이 아니라 일정한 시간이 걸리는 동결이며, 결과는 애플리케이션 일관성을 가집니다.

두 방식 모두 같은 청크 스토리지에 씁니다. 셀은 콘텐츠로 주소 지정되므로, 매시간 핫 일정과 매주 콜드 일정 양쪽으로 백업되는 레포지토리는 공유 블록을 두 번이 아니라 한 번만 저장하며, 포크 계열도 이를 공유합니다. 사용량은 `rdc backup usage`로 쿼터에 반영되어 계량됩니다.
...
표준 기본값은 두 전략 분할입니다. 모든 레포를 캡처하는 빠른 시간별 hot 스트림과, 애플리케이션 일관성 스냅샷을 위해 컨테이너를 정지시키는 느린 주별 cold 스트림입니다. 둘 다 같은 청크 스토리지에 쓰며, 공유 블록은 스트림마다가 아니라 한 번만 저장됩니다.
...
두 전략 어느 쪽에서도 제외되지 않은 레포는 둘 다에 의해 캡처되므로, 시간별 충돌 일관성 스냅샷과 주별 애플리케이션 일관성 스냅샷을 함께 갖게 됩니다. `rdc backup snapshot list <repo>`는 이들을 함께 보여주며, 공유되는 블록은 한 번만 저장됩니다.
