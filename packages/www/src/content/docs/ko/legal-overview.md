...
Rediacc는 전적으로 귀하의 인프라에서 실행됩니다. 클로닝, 백업 및 배포 작업 중에도 데이터가 귀하의 머신을 벗어나지 않습니다. 귀하는 데이터 컨트롤러이자 처리자로 유지됩니다. 어떠한 제3자 SaaS도 귀하의 데이터를 처리하지 않습니다.

Rediacc의 기술적 기능을 주요 컴플라이언스 프레임워크의 요건에 매핑합니다. 각 페이지는 공식 법률 문서에 대한 참조와 함께 특정 규정을 다룹니다.

## 컴플라이언스 매트릭스

| 프레임워크 | 적용 범위 | Rediacc 주요 기능 |
|-----------|-------|--------------------------|
| [GDPR](/ko/docs/legal-gdpr) | EU 데이터 보호 및 개인정보 보호 | 동일 머신에서 CoW 클로닝, LUKS2 암호화, 제로 지식 설정 저장소, 감사 로깅, `rdc repo delete`를 통한 삭제권 |
...
- **감사 로깅**: 인증(로그인, 2FA, 비밀번호 변경, 세션 취소), API 토큰 수명 주기, 설정 저장소 작업, 구독/라이선싱 활동 및 CLI 머신 작업(리포지토리 수명 주기, 백업, 동기화, 터미널 세션)을 포함하는 70가지 이상의 이벤트 유형. 관리자 대시보드 및 포털 활동 페이지(조직 범위 필터링 및 JSON 내보내기 포함)를 통해 접근할 수 있습니다. 머신 작업은 심층 방어를 위해 시스템 로그에도 기록됩니다.
- **암호화 백업**: `rdc repo push/pull`이 SSH를 통해 데이터를 이전합니다. 백업 목적지는 LUKS 암호화 볼륨을 수신합니다.
...
컴플라이언스 위반은 막대한 비용을 초래합니다. 정말 막대한 비용입니다. 다음 사례들은 Rediacc의 아키텍처가 구조적으로 방지하는 문제들을 보여줍니다.
...
이 페이지들은 Rediacc의 아키텍처가 컴플라이언스 요건과 어떻게 일치하는지 설명합니다. 하지만 현실은 이렇습니다. 컴플라이언스는 소프트웨어를 넘어서는 것입니다. 정책, 절차, 교육, 그리고 아마도 제3자 감사가 필요합니다. Rediacc는 인프라 부분을 담당합니다. 나머지는 법률 및 컴플라이언스 팀과 협력하십시오.
