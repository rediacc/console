...
- **저장소 간 연결은 커널에 의해 차단됩니다.** 커널은 저장소의 `/26` 서브넷 외부의 루프백 IP에 대한 연결을 자동으로 차단합니다. 한 저장소의 서비스는 다른 저장소의 서비스에 접근할 수 없습니다. 단 한 가지 예외: 포크가 부모의 서브넷으로 보내는 연결은 포크 자신의 서비스(같은 서비스, 포크의 데이터 사본)로 투명하게 리디렉션되며, 부모에게는 절대 도달하지 않습니다. 관련 없는 리포지토리는 여전히 차단됩니다.
- **서비스 간 통신**: **서비스 이름**(예: `db`, `redis`)을 사용하세요. renet은 각 서비스 이름을 올바른 IP로 해석되는 호스트 이름으로 자동 주입합니다. Docker DNS 이름은 host 모드에서 동작하지 않지만, `/etc/hosts`를 통한 서비스 이름은 동작합니다. `${DB_IP}` 같은 값을 영구 설정 파일(예: 데이터베이스에 저장된 연결 문자열)에 넣지 마세요. 포크는 직계 부모에게서 물려받은 IP를 자신의 서비스로 리디렉션하지만, 이 리디렉션은 한 세대까지만 적용됩니다. 포크의 포크에는 여전히 어디에도 연결되지 않는 조부모의 IP가 남습니다. 서비스 이름은 모든 리포지토리에서 항상 올바르게 해석됩니다.
