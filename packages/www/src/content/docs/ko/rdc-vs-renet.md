...
워크스테이션에서 `rdc`를 실행하면, `rdc`가 서버로 SSH 연결을 열고 거기서 해당 `renet` 명령을 대신 실행합니다. 한 곳에서 한 명령만 입력하면 됩니다.
...
`Rediaccfile` 내부에서 `renet compose -- ...`를 볼 수 있습니다. 걱정하지 않아도 됩니다. Rediaccfile 함수는 서버에서 실행되며, 서버에는 `renet`이 이미 설치되어 있습니다.
