...
Stable은 기본 채널입니다. 릴리스는 보고된 문제 없이 Edge에서 7일간 머문 후에만 해당 채널에 도달합니다.
...
Edge는 main에 병합되는 순간 모든 변경 사항을 받습니다. 지속적으로 배포되는 소프트웨어의 라이브 버전입니다.
...
Edge와 Stable은 별도의 데이터베이스를 사용하는 별도의 인프라에서 실행됩니다. 한쪽의 계정은 다른 쪽에 존재하지 않으며, 채널 간 마이그레이션 경로도 없습니다. Edge에서 시작하고 나중에 유료 플랜을 원한다면 Stable에서 처음부터 새 계정을 만들어야 합니다.
...
따라서 Stable은 Edge보다 최대 7일 뒤처집니다. 소크 기간은 Edge의 회귀가 Stable에 도달하기 전에 이를 포착합니다.
...
각 채널의 설치 명령, 패키지 관리자 구성, Docker 태그는 [설치](/ko/docs/installation)를 참조하세요.

## CLI 채널 관리

CLI는 설치 또는 로그인 시 구성한 채널을 사용합니다. 전환하려면:
...
`rdc subscription login`을 실행하고 Edge 리전을 선택하면 CLI가 Edge 업데이트 채널을 자동으로 설정합니다. `--channel` 플래그가 필요하지 않습니다.
