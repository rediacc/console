---
title: 웹 애플리케이션
description: "Rediacc 웹 콘솔을 사용하여 머신, 저장소, 백업을 관리하는 방법"
category: Reference
order: 1
language: ko
...
**Rediacc**는 자체 서버에 직접 설치하는 소프트웨어입니다. 전체 라이브 시스템을 통째로 백업하고, 포크하고, 복원합니다. 앱, 데이터베이스, 설정을 함께 다루며 파일 단위로 처리하지 않습니다. 기존 백업 도구 대부분이 파일만 복사하고 파일 간 관계를 잃어버리는 문제를 해결한 것이 핵심입니다.

이 가이드는 [https://www.rediacc.com/](https://www.rediacc.com/) 웹 콘솔 사용법을 안내합니다.
...
사용자 관리는 조직 내 누가 접근 권한을 갖는지 제어하는 곳입니다.
...
팀은 사용자를 묶어 일괄로 접근 권한을 부여할 수 있게 합니다.
...
접근 제어는 그룹을 통해 권한을 중앙에서 관리하며, 사용자별 개별 설정이 필요 없습니다.
...
머신 섹션은 서버와 그 위에 있는 저장소를 관리하는 곳입니다.
...
저장소는 격리된 애플리케이션 배포 단위입니다. 앱, 데이터, 설정, 전용 Docker 데몬이 모두 포함됩니다. 따라서 이 페이지의 거의 모든 버튼은 머신이 아니라 저장소에 작동합니다. **삭제**를 누르기 전에 이 점을 반드시 확인하세요.
...
저장소에 연결하는 방법은 두 가지입니다. 데스크톱 앱은 실제 터미널과 SSH를 제공합니다. 브라우저는 확인 및 클릭 관리에는 충분하지만 긴 셸 세션에는 적합하지 않습니다. 실제 작업을 할 때는 데스크톱 앱을 사용하세요.
...
스토리지 섹션은 백업 데이터가 저장될 물리적 위치를 정의하는 곳입니다.
...
자격 증명 섹션은 저장소가 외부에 접근하는 데 사용하는 시크릿을 관리하는 곳입니다.
...
대기열 섹션은 시스템 전반의 대기 중인 작업과 완료된 작업을 추적합니다.
...
감사 섹션은 시스템에서 실행된 모든 작업의 기록을 보관합니다.
