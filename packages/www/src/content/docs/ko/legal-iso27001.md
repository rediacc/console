...
| **A.12**, 운영 보안 | A.12.3 백업 | `rdc repo push/pull`을 사용한 여러 목적지(SSH, S3, B2, Azure, GDrive)로의 암호화 오프사이트 저장. 시점 복구를 위한 CoW 스냅샷. `rdc repo admin validate`로 백업 상태 및 리포지토리 무결성 검증. |
| **A.12**, 운영 보안 | A.12.4 로깅 및 모니터링 | 70가지 이상의 이벤트 유형(인증, API 토큰, 설정, 라이선싱, 머신 작업). `rdc machine status`를 통한 머신 상태 모니터링. 컨테이너 상태 및 리소스 모니터링. |
...
3. **승격**: `rdc repo promote`를 사용하여 포크를 프로덕션으로 교체
...
- `rdc repo admin validate`가 각 작업 후 백업 상태를 검증합니다
