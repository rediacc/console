...
- **교차 저장소 연결 차단**: 서비스가 저장소의 `/26` 서브넷 외부의 루프백 IP에 연결하려 하면 커널이 차단합니다. 저장소 A의 프로세스는 저장소 B의 서비스에 접근할 수 없습니다. 유일한 예외는 포크입니다. 부모의 서브넷으로 향하는 연결은 포크 자신의 서비스로 리디렉션됩니다([제한](/ko/docs/limits) 참조). 부모 자체는 여전히 접근 불가 상태로 남습니다.
