...
이렇게 정리됩니다. 워크스테이션에는 rdc, 서버에는 renet, SSH로 통신합니다. Rediacc의 전체 아키텍처는 이 분할에 기반합니다. 이 페이지에서는 두 도구가 책임을 어떻게 나누는지, 어댑터 감지가 상태를 어떻게 라우팅하는지, 보안 모델이 어떻게 구성되는지, 그리고 설정이 어떻게 구조화되는지를 다룹니다.
...
`rdc machine setup`을 실행하면 renet이 원격 서버에 `rediacc`라는 시스템 사용자를 생성합니다.
