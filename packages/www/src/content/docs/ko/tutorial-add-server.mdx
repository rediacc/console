...
`rdc config init`은 필요하지 않습니다. 구성 파일은 첫 사용 시 자동으로 생성되며, `rdc`는 표준 `~/.ssh` 키를 기본으로 사용합니다. 키 선택은 의도적으로 최소화되었습니다. 에이전트 포워딩이나 점프 호스트가 필요하면 `~/.ssh/config`에서 일반적인 방식으로 설정하세요. `rdc`가 자동으로 인식합니다.
