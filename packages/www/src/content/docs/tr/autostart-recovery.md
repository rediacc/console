...
Anahtar dosyası slotu hızlı PBKDF2 KDF'yi kullanır: 256 baytlık rastgele bir anahtar dosyası zaten kendi güvenlik marjınıdır; bu nedenle bellek açısından yoğun bir KDF, ek koruma sağlamadan yalnızca bağlama gecikmesini artırırdı. Bağlamalar bir saniyenin çok altında açılır. Bu optimizasyon öncesinde oluşturulan depolar, bağlama başına birkaç saniyelik Argon2id türetme maliyeti öder; bunları makinede (depo bağlı değilken) `renet repository kdf-migrate --name <guid>` operatör komutuyla yerinde dönüştürebilirsiniz. Slot 0 Argon2id kullanmaya devam eder; bu, insan parolası için doğru tercih.
