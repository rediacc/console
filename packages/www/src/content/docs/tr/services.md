...
title: Servisler
description: >-
  Rediaccfile'lar, servis ağı ve otomatik başlatma kullanarak konteynerleştirilmiş
  servisleri dağıtın ve yönetin.
category: Guides
order: 5
language: tr
...
Bu sayfa neleri kapsar: konteynerleştirilmiş servislerin dağıtılması ve yönetilmesi, Rediaccfile'lar, servis ağı, başlatma/durdurma, toplu işlemler ve otomatik başlatma.

## Rediaccfile

**Rediaccfile**, servislerinizin nasıl başlatıldığını ve durdurulduğunu tanımlayan bir Bash betiğidir. Dosya **sourced** (ayrı bir işlem olarak çalıştırılmaz) olduğundan, işlevleri aynı shell bağlamını paylaşır ve dışa aktarılan tüm ortam değişkenlerine erişebilir. Dosya adı `Rediaccfile` veya `rediaccfile` (büyük/küçük harf duyarsız) olmalı ve deponun bağlı dosya sistemi içine yerleştirilmelidir.
...
`{SERVICE}_IP` değişkenleri `.rediacc.json` dosyasından otomatik olarak oluşturulur ve Rediaccfile işlevleriniz çalışmadan önce dışa aktarılır. Adlandırma kuralı, servis adını büyük harfe çevirir, tireleri alt çizgi ile değiştirir ve sonuna `_IP` ekler. Örneğin, slot `0` ile `listmonk-app` servisi `LISTMONK_APP_IP=127.0.11.2` olur.
...
Yukarıdaki `POSTGRES_PASSWORD: changeme` yer tutucu bir öğretici için uygundur, ancak gerçek uygulamalarda gerçek kimlik bilgileri gereklidir; bunları compose dosyasına (veya depodaki `.env` dosyasına) işlemek, bir fork'ün bunları da miras alması anlamına gelir. Dağıtım zamanı kimlik bilgileri için `rdc repo secret` kullanın. Değerler şifrelenmiş depo görüntüsünün dışında yaşadığından, fork'lar boş bir sırlar haritasıyla başlar.

Compose'ta iki teslimat modu çalışır:

**`env` modu**. Herhangi bir `environment:` değerinde `${REDIACC_SECRET_<KEY>}` aracılığıyla enterpolasyon yapın. renet sarmalayıcısı değeri dağıtım zamanında konteynerin ortamına iletir.

**`file` modu**. Değer, `/var/run/rediacc/secrets/<networkID>/<KEY>` konumunda host tarafı tmpfs dosyası haline gelir ve Docker compose'un standart `secrets:` bloğu aracılığıyla konteynere bağlanır. Konteyner `/run/secrets/<key>` dosyasını okur. Hassas her şey için bu modu tercih edin. Değerler `docker inspect` veya `/proc/<pid>/environ` dosyasında asla görünmez.
