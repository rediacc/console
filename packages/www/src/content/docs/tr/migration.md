...
Mevcut bir projeyi, dosyaları, Docker servislerini ve veritabanlarını geleneksel bir sunucudan veya yerel geliştirme ortamından şifrelenmiş bir Rediacc deposuna taşıyın.
...
Yüklenen dosyalar yerel kullanıcınızın UID'si ile gelir (örneğin 1000). Rediacc, VS Code, terminal oturumları ve araçların tutarlı erişime sahip olması için evrensel bir kullanıcı (UID 7111) kullanır. Dönüştürmek için sahiplik komutunu çalıştırın:
...
Docker konteynerleri çalışıyorsa (veya çalıştıysa), sahiplik komutu otomatik olarak yazılabilir veri dizinlerini algılar ve **atlar**. Bu, dosyalarını farklı UID'lerle yöneten konteynerlerin bozulmasını önler (örneğin MariaDB UID 999, Nextcloud UID 33 kullanır).
...
4. **Servisler arası bağlantılar için servis adlarını kullanın** (örneğin `postgres`, `redis`) - renet her servis adını çözümlenebilir bir ana bilgisayar adı olarak enjekte eder. Veritabanlarında veya yapılandırma dosyalarında saklanan bağlantı dizelerine ham IP'ler yerleştirmeyin; fork izolasyonunu korumak için servis adını kullanın
5. **Bağlama otomatiktir** - kernel `bind()` çağrılarını otomatik olarak doğru geri döngü IP'sine yeniden yazar. Servisler `0.0.0.0` veya `localhost` kullanabilir
...
Her depo benzersiz geri döngü IP'leri alır ve kernel `bind()` çağrılarını otomatik olarak doğru IP'ye yeniden yazar. Depolar arasında port çakışmaları yaşanmaz. Beklenmedik davranış görürseniz, servislerin `docker compose` yerine `renet compose` aracılığıyla başlatıldığını doğrulayın. Diğer servislere bağlanırken ham IP'ler yerine servis adını (örneğin `postgres`) kullanın. Servis adları her fork'ta doğru şekilde çözümlenir.

### Sahiplik Düzeltmesi Konteynerleri Bozuyor

`rdc repo admin ownership` çalıştırdıysanız ve bir konteyner çalışmayı durdurduysa, konteynerin veri dosyaları değiştirilmiştir. Konteyneri durdurun, veri dizinini silin ve yeniden başlatın. Konteyner onu yeniden oluşturacaktır:

```bash
rdc repo down my-project
# Konteynerin veri dizinini silin (örneğin database/data)
rdc repo up my-project
```
