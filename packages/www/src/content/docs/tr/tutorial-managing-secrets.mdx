...
Test için fork'ta gizli bilgi isterseniz, bunları sandbox değerleriyle açıkça fork üzerinde ayarlayın, örneğin `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`. Artık fork Stripe sandbox'ına bağlanır ve temiz başlar. Üretim kimlik bilgileri hiçbir zaman üretimi terk etmedi.
