...
Si vous voulez des secrets dans le fork pour les tests, définissez-les explicitement sur le fork avec des valeurs sandbox, par exemple `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`. Le fork parle alors au sandbox Stripe et démarre proprement. Les identifiants de production n'ont jamais quitté la production.
