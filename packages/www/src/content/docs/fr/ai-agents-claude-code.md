...
description: >-
  Guide étape par étape pour configurer Claude Code pour la gestion autonome de
  l'infrastructure Rediacc.
category: Guides
tags:
  - ai-agents
  - cli
...
- Autoriser `rdc machine status *`, vérifications d'état en lecture seule
- Autoriser `rdc machine status * --containers`, listage des conteneurs
- Autoriser `rdc machine health *`, vérifications de santé
- Autoriser `rdc repo list`, listage des dépôts
...
- Exécutez `rdc --help-all` pour que Claude Code découvre toutes les commandes disponibles
