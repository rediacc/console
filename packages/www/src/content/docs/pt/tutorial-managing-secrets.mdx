...
Se quiser segredos no fork para testes, defina-os explicitamente no fork com valores de sandbox, por exemplo `rdc repo secret set my-app:test --key STRIPE_KEY --value sk_sandbox_yyy --mode file`. Agora o fork fala com o sandbox do Stripe e arranca sem problemas. As credenciais de produção nunca saíram da produção.
