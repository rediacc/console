...
| Controlos de integridade | [45 CFR 164.312(c)](https://www.govinfo.gov/content/pkg/PLAW-104publ191/html/PLAW-104publ191.htm) | Os snapshots CoW preservam os dados originais antes das modificações. `rdc repo admin validate` verifica a integridade do repositório e o estado do backup (contentor LUKS, consistência do sistema de ficheiros, configuração). |
...
| Planeamento de contingência | `rdc repo push/pull` suporta backup encriptado para múltiplos destinos. Os snapshots CoW permitem recuperação instantânea. |
