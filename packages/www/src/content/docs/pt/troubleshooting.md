...
description: "Correções para problemas comuns com SSH, configuração, repositórios, serviços e Docker."
category: "Guides"
order: 10
language: pt
...
Problemas comuns e como resolvê-los. Em caso de dúvida, comece com `rdc doctor` para executar um diagnóstico completo.

## Falha na Ligação SSH

- Verifique se consegue ligar manualmente: `ssh -i ~/.ssh/id_ed25519 deploy@203.0.113.50`
- Execute `rdc machine scan-keys server-1` para atualizar as chaves do servidor
- Confirme que a porta SSH está correta: `--port 22`
- Teste com um comando simples: `rdc term connect server-1 -c "hostname"`
...
- Execute com `--debug` para saída detalhada: `rdc machine setup server-1 --debug`
...
...o servidor está a executar o kernel padrão do RHEL 10, que não inclui o módulo btrfs integrado. Isto não é um problema do Rediacc; o RHEL 10 removeu o btrfs intencionalmente. A correção é utilizar **Oracle Linux 10**. O Oracle 10 usa por predefinição o Unbreakable Enterprise Kernel (UEK), que mantém o btrfs. Consulte [Requisitos -> Por que UEK?](/pt/docs/requirements) para a história completa.
...
Isto é intencional. O modelo de rede do Rediacc é **host networking para todos os serviços**, imposto pelo `renet compose`. Uma bridge Docker padrão com NAT contornaria o isolamento de loopback ao nível do kernel que impede um repositório de alcançar os serviços de outro, pelo que o daemon Docker por repositório (`FlavorRediacc`) é configurado com `"bridge": "none"` e `"iptables": false`. Não existe bridge encaminhável a que um contentor `docker run` simples se possa ligar. (Os daemons Hub por utilizador (`FlavorHub`) utilizados em ambientes de desenvolvimento são a exceção: ativam bridge e iptables para que os contentores do utilizador tenham conectividade de rede de saída.)
...
Ao ligar com `rdc vscode connect <repo>` após uma sessão anterior do VS Code, versões mais antigas do renet produziam erros como `scp: .../.vscode-server/vscode-cli-*.tar.gz: Permission denied`. A causa: propriedade de ficheiros mista dentro do diretório sandbox, onde tanto o utilizador SSH como o utilizador interno `rediacc` tinham escrito ficheiros.
...
Isto verifica o seu ambiente, a instalação do renet, a configuração e o estado de autenticação. Cada verificação reporta OK, Aviso ou Erro com uma explicação breve.
