...
const docsCollection = defineCollection({
  ...
  schema: z
    .object({
      ...
      // translated label lives under documentation.categories.* in the locale catalogs. A translated value here once shipped a seventh docs tab whose label appeared twice in Arabic.
      category: z.enum(['Tutorials', 'Guides', 'Concepts', 'Reference', 'Use Cases', 'Legal']),
      // The English TAG vocabulary, in EVERY locale, for the same reason `category` is: a tag is an identifier the browse filter matches on, not display text. The single
