...
export function remarkVideoEmbed() {
  return function transformer(tree: Root) {
    visit(tree, 'paragraph', (node: Paragraph, index, parent) => {
      ...
      // Always render <video> elements rather than probing for the file: the sources are populated after this plugin runs, so an existence check here would be wrong even when the video is present.
      //
      // Status of the user-guide set, checked 2026-07-28 so the next reader does not have to: NOTHING currently references it. Zero .md/.mdx files under packages/www/src embed an /assets/videos/user-guide/ path, there is no such directory locally (only videos/solutions/), nothing is tracked in git for it, and media.rediacc.com 404s for one. So this branch is not reached for
      // user-guide today and no empty player is served.
      //
      // It used to be fed by a CI step that downloaded an `e2e-videos-*` artifact. That artifact has never been produced by any workflow, so the step and its script were removed. Solution and tutorial videos are a DIFFERENT set: those are produced by the local pipelines and served from media.rediacc.com, and they are unaffected by that removal.
