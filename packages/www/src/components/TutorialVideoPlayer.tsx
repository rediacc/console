...
function findActiveWordIndex(words: WordEntry[], t: number): number {
  ...
  // If we are past the last word's end but still inside the cue, keep the last word highlighted so the trailing punctuation doesn't visually "release" the highlight.
...
function renderCueIntoOverlay(el: HTMLDivElement, cue: CueEntry, activeWordIdx: number): void {
  el.innerHTML = '';
  // Defensive fallback: a malformed sidecar (or a single-word phrase) lands here with one word entry covering the whole cue. Render as plain text so we don't paint the entire line in the active-word style.
...
function alignPlyrCaptionLanguage(lang: string): void {
  try {
    ...
    // localStorage can be unavailable (private mode, blocked cookies). Plyr degrades to config.captions.language in that case, which is already the value we want.
  }
}
...
const TutorialVideoPlayer: FC<TutorialVideoPlayerProps> = ({
  ...
  // The picker offers what the manifest published for THIS cast, ordered by the site's own locale list rather than by JSON key order, and disappears below two entries.
  ...
  // ONE named condition, used by the src, the remount key and the class. It was spelled out three times and each repeat is a branch the complexity limit counts.
  ...
  useEffect(() => {
    const video = videoRef.current;
    // THE OVERLAY IS OPTIONAL, THE PLAYER IS NOT. This guard used to demand the chapter overlay node as well, so a video with no chapters -- every solution video -- would
    ...
    // Relocate the word-by-word caption overlay INTO Plyr's video wrapper. The Fullscreen API only renders descendants of the fullscreened element (the .plyr container), so a caption left in .tvp-root vanishes in fullscreen. Moving it inside .plyr__video-wrapper keeps it visible in both modes and lets the `.plyr--fullscreen-active .tvp-caption` CSS match.
    ...
    //
    // Located by DOM id rather than through `player.elements.settings`, which Plyr's own types do not declare. Every lookup is guarded and a miss is a no-op: if a Plyr upgrade changes the id convention, the row simply never appears and the in-frame overlay below stays visible, so a viewer is never left with no picker at all.
    player.on('ready', () => {
      detachChapterOverlay = mountChapterOverlay();
      mountCaptionOverlay();
      // The overlay is the FALLBACK, not the default: it shows only when the pane could not be built, so there is exactly one picker on screen either way.
      setMenuMounted(
        mountLanguagePane({
          video,
          controlClass:
    ...
    return () => {
      ...
      // Return the caption to .tvp-root before Plyr.destroy() tears down the .plyr wrapper, so React still finds the node to unmount cleanly.
    ...
    pickerLangs,
    t,
    // `usePortrait` now feeds Plyr's `ratio`. It moves in lockstep with `activeSrc` today, but exhaustive-deps is a failing build here (`--max-warnings 0`), and a value read inside the effect belongs in the list regardless of what it correlates
    // with.
    usePortrait,
  ]);
  ...
  useEffect(() => {
    ...
    // A single tick after scrubbing so the overlay reflects the new position even while paused.
    const stepOnce = () => {
      cancelAnimationFrame(raf);
      tick();
  ...
  useEffect(() => {
    ...
    // activeLang, because the <video> below is keyed on it: after a switch this effect's captured node is a detached one, and the CC button would stop reaching the overlay.
  }, [activeLang]);
  ...
  // getLanguageName, not a local table: the one this component used to carry had twelve entries and no `et`, so Estonian rendered as a raw "ET" next to twelve real names.
