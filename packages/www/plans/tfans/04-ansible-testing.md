...
Unit tests verify module internals without calling `rdc`. They mock `module.run_command()` and test argument validation, command building, and output parsing.
...
Integration tests run against real infrastructure provisioned by `rdc ops`. They execute actual `rdc` commands through the Ansible modules.
