...
const ILLUSTRATIONS_DIR = path.join(__dirname, '../src/assets/images/illustrations');
const TARGET_W = 800;
const TARGET_H = 450;
...
function scaleCss(css, scale) {
  return css
    .replace(/font-size:\s*([\d.]+)px/g, (_, v) => `font-size: ${fmt(parseFloat(v) * scale)}px`)
    .replace(/stroke-width:\s*([\d.]+)/g, (_, v) => `stroke-width: ${fmt(parseFloat(v) * scale)}`);
}
...
function shouldScaleAttr(el, attr) {
  ...
  if (
    ...
    if (['x1', 'y1', 'x2', 'y2', 'cx', 'cy', 'r', 'fx', 'fy'].includes(attr)) {
      return false;
    }
  }
...
// Process all files
console.log(`Scaling ${Object.keys(FILES).length} SVG files to ${TARGET_W}x${TARGET_H}\n`);
