...
export function createPermissionsService(client: ApiClient) {
  return {
    listGroups: async (): Promise<PermissionGroup[]> => {
      const response = await client.get<PermissionGroup>(endpoints.company.getCompanyPermissionGroups)
    ...
    getGroupDetails: async (groupName: string): Promise<Permission[]> => {
      const response = await client.get<Permission>(endpoints.permissions.getPermissionGroupDetails, { groupName })
      return parseResponse(response, {
