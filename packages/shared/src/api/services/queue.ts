...
export function createQueueService(client: ApiClient) {
  return {
    list: async (teamName?: string | string[], filters: QueueFilters = {}): Promise<QueueListResult> => {
    ...
    getNext: async (machineName: string, itemCount = 5): Promise<QueueItem[]> => {
      const response = await client.get<QueueItem>(endpoints.queue.getQueueItemsNext, { machineName, itemCount })
...
function normalizeQueuePosition(entry?: Partial<QueuePositionEntry> | null): QueuePositionEntry | null {
