...
export function createCompanyService(client: ApiClient) {
  return {
    ...
    getVault: async (): Promise<CompanyVaultDetails> => {
      ...
      const row = getRowByIndex<{ vaultContent?: string; vaultVersion?: number; companyCredential?: string | null }>(response, 1)
      return {
    ...
    getAllVaults: async (filters?: Record<string, unknown>): Promise<CompanyVaultCollections> => {
      ...
      const bridgesWithRequestToken = getRowsByIndex<Record<string, unknown>>(response, 2).map((vault) =>
        normalizeRecord(vault)
      )
      return {
        vaults,
    ...
    updateAllVaults: async (updatesPayload: unknown): Promise<CompanyVaultUpdateResult> => {
      ...
      const row = getRowByIndex<{ result?: { totalUpdated?: number; failedCount?: number } }>(response, 0)
    ...
    updateBlockUserRequests: async (blockRequests: boolean): Promise<CompanyBlockUserRequestsResult> => {
      const response = await client.post(endpoints.company.updateCompanyBlockUserRequests, { blockRequests })
    ...
    getLookupData: async (context?: string): Promise<CompanyDropdownData> => {
      const response = await client.get(endpoints.company.getLookupData, context ? { context } : {})
      const row =
      ...
      const dropdownValuesRaw = typeof row.dropdownValues === 'string' ? safeJsonParse(row.dropdownValues, {}) : row.dropdownValues
      ...
      if (Array.isArray(parsedValues.bridgesByRegion)) {
        parsedValues.bridgesByRegion = (parsedValues.bridgesByRegion as Array<Record<string, unknown>>).map((region) => ({
          ...region,
...
function parseDashboard(response: ApiResponse): CompanyDashboardData {
  ...
  const payload = row?.subscriptionAndResourcesJson ? safeJsonParse<Record<string, unknown>>(row.subscriptionAndResourcesJson, {}) : null
  ...
  const distributedStorageStats = parseObject<CompanyDistributedStorageStats>(normalized.distributedStorageStats)
  const allActiveSubscriptions = parseObjectArray<CompanySubscriptionDetail>(normalized.allActiveSubscriptions)

  if (distributedStorageStats && distributedStorageStats.team_breakdown) {
    distributedStorageStats.team_breakdown = parseObjectArray<DistributedStorageTeamBreakdown>(distributedStorageStats.team_breakdown)
  }
...
function parseCompanyDataGraph(response: ApiResponse): CompanyDataGraph {
  ...
  const graph = parseDoubleEncodedJson<CompanyDataGraph>(graphJson, ['metadata', 'nodes', 'relationships', 'summary'])
