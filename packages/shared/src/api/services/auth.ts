...
const extractAuthStatus = (response: ApiResponse<AuthStatusRow>): AuthRequestStatus => {
  const row =
    parseFirst<AuthStatusRow>(response, { extractor: responseExtractors.byIndex<AuthStatusRow>(1) }) ??
    parseFirst<AuthStatusRow>(response, { extractor: responseExtractors.byIndex<AuthStatusRow>(0) }) ??
    (typeof (response as AuthStatusRow).isTFAEnabled !== 'undefined' ? (response as AuthStatusRow) : null)
...
export function createAuthService(client: ApiClient) {
  return {
    ...
    forkSession: async (sessionName: string, options: ForkSessionOptions = {}): Promise<ForkSessionCredentials> => {
      ...
      const row = credentialsSet?.data?.[0] as Partial<ForkSessionCredentials> & {
    ...
    getRequestStatus: async (): Promise<AuthRequestStatus> => {
      const response = await client.get<AuthStatusRow>(endpoints.auth.getRequestAuthenticationStatus)
    ...
    getTfaStatus: async (): Promise<AuthRequestStatus> => {
      const response = await client.post<AuthStatusRow>(endpoints.users.updateUserTfa, { action: 'status' })
      return extractAuthStatus(response)
    },

    enableTfa: async (passwordHash?: string, options: EnableTfaOptions = {}): Promise<EnableTfaResponse | null> => {
      // Legacy mode for CLI - no password/options provided
      if (!passwordHash && Object.keys(options).length === 0) {
        const response = await client.post<EnableTfaResponse>(endpoints.users.updateUserTfa, { action: 'enable' })
    ...
    verifyTfa: async (code: string, sessionName?: string): Promise<VerifyTfaResult> => {
      const response = await client.post<VerifyTfaResult>(endpoints.auth.privilegeAuthenticationRequest, {
        currentCode: code,
        sessionName,
      })
    ...
    checkRegistration: async (email: string): Promise<{ isRegistered: boolean }> => {
      ...
      const response = await client.get<RegistrationRow>(endpoints.auth.isRegistered, { userEmail: email })
