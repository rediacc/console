...
import { isFork, type RepositoryWithRelations } from './core.js';
 ...
 */
export function canBackupToStorage(repository: RepositoryWithRelations): BackupValidationResult {
  // Only grand repositories can backup to storage
  if (isFork(repository)) {
    return {
      canBackup: false,
      reason:
        'Forks cannot backup to storage. Only grand repositories (credentials) can perform storage backups.',
    };
  }
 ...
 * @param _repo - Repository to check (unused, all repositories can backup to machines)
 * @returns Validation result
 */
export function canBackupToMachine(_repo: RepositoryWithRelations): BackupValidationResult {
 ...
 */
export function getBackupOptions(repository: RepositoryWithRelations): {
