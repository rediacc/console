// AUTO-GENERATED by renet functions generate-types - DO NOT EDIT Run: ./build.sh deploy prep Source: renet/pkg/functions/commands/
...
export interface FunctionDefinition {
  ...
  showInMenu: boolean;
  requirements: Partial<Record<'machine' | 'team' | 'repository' | 'storage' | 'organization' | 'network_id', boolean>>;
...
export type TypedFunctionPayload<F extends RenetFunctionName> = {
  functionName: F;
  params: FunctionParamsMap[F];
};

export function createFunctionPayload<F extends RenetFunctionName>(
  functionName: F,
  params: FunctionParamsMap[F]
...
export function getTypedParams<F extends RenetFunctionName>(
  _functionName: F,
  params: FunctionParamsMap[F]
): FunctionParamsMap[F] {
  return params;
...
export function isRenetFunction(name: string): name is RenetFunctionName {
  return (RENET_FUNCTIONS as readonly string[]).includes(name);
}

// All functions in this file are public. Visibility validation is handled server-side by renet.
export function isPublicRenetFunction(name: string): boolean {
  return isRenetFunction(name);
}

export function getFunctionsByCategory(category: FunctionCategory): RenetFunctionName[] {
  return RENET_FUNCTIONS.filter((name) => FUNCTION_DEFINITIONS[name].category === category);
}

export function getMenuFunctions(): RenetFunctionName[] {
  return RENET_FUNCTIONS.filter((name) => FUNCTION_DEFINITIONS[name].showInMenu);
}
