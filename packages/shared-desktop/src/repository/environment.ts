...
export function buildRepositoryEnvironment(
  ...
  const datastorePath = machineVault.datastorePath ?? '/mnt/datastore';
  const universalUser = machineVault.universalUser ?? 'rediacc';
  const universalUserId = machineVault.universalUserId ?? '1000';
  const repoPath = repositoryVault.path ?? `/home/${repositoryName}`;
  ...
  const dockerPluginDir = `${datastorePath}/docker/plugins`;
