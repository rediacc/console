 ...
 * @param hostEntry - Known hosts entry from vault (REQUIRED)
...
export async function setupSSHAgentConnection(
  ...
  try {
    ...
    const knownHostsPath = await createTempKnownHostsFile(hostEntry);
...
export class SSHConnection {
   ...
   *
   * @param sshKey - SSH private key content
   * @param hostEntry - Host key from vault (REQUIRED for security)
   * @param options - Connection options (port, preferAgent, forceTTY) or port number for backward compatibility
   */
  constructor(sshKey: string, hostEntry: string, options?: SSHConnectionCtorOptions | number) {
    if (!hostEntry || hostEntry.trim().length === 0) {
      throw new Error(
        'Security Error: hostEntry is required for SSH connections. ' +
