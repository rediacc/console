...
import { FILE_DEFAULTS, TIMEOUT_DEFAULTS } from '../constants';
...
export class ScreenshotManager {
  ...
  private readonly testCaseDir: string;
  ...
  async captureElementScreenshot(
    ...
    try {
      await element.screenshot({
        path: filePath,
        type: options.type ?? FILE_DEFAULTS.PNG,
        timeout: options.timeout ?? TIMEOUT_DEFAULTS.SCREENSHOT,
      });
  ...
  async capturePageFlow(steps: { name: string; action: () => Promise<void> }[]): Promise<string[]> {
