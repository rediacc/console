...
export function createTestLocalContext(
  ...
  const args = ['config', 'init', '--name', name, '--ssh-key', sshKey];
...
export function addTestMachine(
  ...
  const args = ['config', 'machine', 'add', '--name', name, '--ip', ip, '--user', user];
...
export async function setupE2EEnvironment(
  ...
  return async () => {
    const runner = new CliTestRunner();
    await runner.run(['config', 'delete', '--name', contextName]);
  };
}
...
export async function cleanupContexts(...contextNames: string[]): Promise<void> {
  ...
  for (const name of contextNames) {
    try {
      await runner.run(['config', 'delete', '--name', name]);
    } catch {
