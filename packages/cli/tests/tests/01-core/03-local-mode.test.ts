...
test.describe("Local Context Commands @cli @core", () => {
  ...
  test.describe("context create-local", () => {
    ...
    test("should accept custom renet path", async () => {
      ...
      // Cleanup
      await runner.run(["context", "delete", customPathContext, "--force"], { skipJsonParse: true });
    });
  ...
  test.describe("context machines", () => {
    ...
    test("should show machine details in table format", async () => {
      const result = await runner.run(
        ["--context", testLocalContext, "context", "machines"],
        { outputFormat: "table" }
      );
  ...
  test.describe("context remove-machine", () => {
    test("should remove a machine", async () => {
      ...
      await runner.run(
        ["--context", testLocalContext, "context", "add-machine", "to-remove", "--ip", "1.1.1.1", "--user", "test"],
        { skipJsonParse: true }
      );
      ...
      // Verify it's gone
      const machinesResult = await runner.run(["--context", testLocalContext, "context", "machines"]);
  ...
  test.describe("run command (local mode detection)", () => {
    ...
    test("should require machine in local mode", async () => {
      ...
      await runner.run(["--context", testLocalContext, "context", "clear", "machine"], { skipJsonParse: true });

      const result = await runner.run(
        ["--context", testLocalContext, "run", "machine_ping"],
        { skipJsonParse: true }
      );
...
test.describe("Switching Between Local and Cloud Contexts @cli @core", () => {
  ...
  test("should use correct mode for each context with -C flag", async () => {
    ...
    await runner.run(
      ["context", "create-local", localContext, "--ssh-key", "~/.ssh/id_rsa"],
      { skipJsonParse: true }
    );
    await runner.run(
