...
test.describe("E2E Local Execution @cli @e2e", () => {
  ...
  test.describe("Error Handling", () => {
    ...
    test("should handle SSH connection timeout", async () => {
      ...
      const result = await timeoutRunner.run(
        ["run", "machine_ping", "--machine", "timeout-vm"],
        { timeout: 30000 }
      );
  ...
  test.describe("Multiple Machines", () => {
    test("should switch between machines", async () => {
      test.skip(!config.enabled || !getE2EConfig().vm2Ip, "E2E not configured or VM2 not available");
      test.setTimeout(600000);

      // Execute on vm1
      const result1 = await runLocalFunction("machine_ping", "vm1", { contextName, timeout: 300000 });
      console.log("VM1 result:", result1.success);

      // Execute on vm2
      const result2 = await runLocalFunction("machine_ping", "vm2", { contextName, timeout: 300000 });
...
test.describe("E2E Context Switching @cli @e2e", () => {
  ...
  test.beforeAll(async () => {
    ...
    await runner.run(["context", "create", cloudContext, "--api-url", "https://test.example.com/api"]);
  ...
  test("should execute with context override", async () => {
    ...
    const result = await localRunner.run(
      ["run", "machine_ping", "--machine", "e2e-vm"],
      { timeout: 300000 }
    );
