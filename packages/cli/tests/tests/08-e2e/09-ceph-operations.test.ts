 ...
 */
test.describe.serial('Phase 8: Ceph Operations @e2e', () => {
  ...
  test.afterAll(async () => {
    ...
    if (config.enabled && cephConfigured) {
      const cleanupOps = [
        ...
        { fn: 'ceph_clone_unmount', params: { clone: CLONE_NAME, mountPoint: CLONE_MOUNT_POINT, pool: POOL } },
        { fn: 'ceph_clone_delete', params: { clone: CLONE_NAME, pool: POOL } },
        { fn: 'ceph_snapshot_unprotect', params: { image: IMAGE_NAME, snapshot: SNAPSHOT_NAME, pool: POOL } },
        { fn: 'ceph_snapshot_delete', params: { image: IMAGE_NAME, snapshot: SNAPSHOT_NAME, pool: POOL } },
