 ...
 */
test.describe.serial('Ceph Clone Lifecycle (E2E) @cli @e2e', () => {
  let ctx: TestContext;
  ...
  test.beforeAll(async () => {
    ...
    const imgResult = await ctx.runner.run(
      ['ceph', 'image', 'create', imageName, '--pool', poolName, '--size', '1G', '--machine', ctx.machineName],
    ...
    const snapResult = await ctx.runner.run(
      ['ceph', 'snapshot', 'create', snapshotName, '--image', imageName, '--pool', poolName, '--machine', ctx.machineName],
      { timeout: ctx.defaultTimeout }
    );
    ...
    const protectResult = await ctx.runner.run(
      ['ceph', 'snapshot', 'protect', snapshotName, '--image', imageName, '--pool', poolName, '--machine', ctx.machineName],
      { timeout: ctx.defaultTimeout }
    );
  ...
  test.afterAll(async () => {
    if (ctx) {
      ...
      await ctx.runner
        .run(['ceph', 'snapshot', 'unprotect', snapshotName, '--image', imageName, '--pool', poolName, '--machine', ctx.machineName], { timeout: 120_000 })
        .catch(() => {});
      await ctx.runner
        .run(['ceph', 'snapshot', 'delete', snapshotName, '--image', imageName, '--pool', poolName, '--machine', ctx.machineName], { timeout: 120_000 })
        .catch(() => {});
      // Delete image
      await ctx.runner
        .run(['ceph', 'image', 'delete', imageName, '--pool', poolName, '--machine', ctx.machineName], { timeout: 120_000 })
        .catch(() => {});
      await ctx.cleanup();
    }
  });
