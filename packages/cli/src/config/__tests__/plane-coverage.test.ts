...
describe('command plane coverage', () => {
  ...
  it('plane counts match the recorded distribution', () => {
    ...
    // Post-P4-w2a totals (the config exodus consolidated ~20 leaves: config machine/provider/infra/cert-cache/storage/repository/cluster/backup-strategy all moved onto their resource nouns, and the new `backup` noun + `config reconcile` were added), minus the hidden `_refprobe run` P4 task-zero probe retired by w2b (its `repo cat` acceptance vehicle is now a real leaf), minus
    // `repo mount`/`repo unmount` folded into `repo up --no-start` / `repo down --unmount` by w2b (both machine-plane, so 93 -> 91). Still includes the +5 `rdc job` verbs.
    //
    // 160 -> 162 (machine 91 -> 93) in the w2b cluster batch: `repo canary`'s actionable parent became the real leaf `repo canary create` (net 0), and the two NEW `cluster snapshot create|list` leaves landed (R2-F13). Both reach a machine (the control node's group-snapshot bridge verbs), so both are machine-plane.
    //
    ...
    //
    //
    // The `repo admin` subtree move (§5.4) relocated validate/fsck/ownership/ autostart/template. The counts are UNCHANGED by it, and that is the point: the COMMAND_PLANES exception moved WITH the command (`repo template list` -> `repo admin template list`, the one `other`-plane leaf, which prints the compiled-in catalog). Had the plane entry been left behind, the leaf would have
    // silently inherited repo's `machine` default and claimed to reach a machine it never touches, the §4.10 staleness hazard, made real.
    //
  ...
  it('records the interactive commands', () => {
    ...
    // Commands that need a TTY or never return: an interactive shell, $EDITOR, a tunnel or stream held until Ctrl+C, a browser callback, or a daemon that listens until SIGINT.
  ...
  it('records the machine-plane commands a remote executor could run', () => {
    ...
    // The `rdc job` verbs are proxyable too: even `job logs --follow` streams to stdout and ends on its own when the job finishes, so none of them needs a TTY and a headless executor can drive all five.
    expect(machineNonInteractive).toContain('job logs');
    // The hidden `_refprobe run` P4 task-zero probe was retired by w2b (90 -> 89), then `repo mount`/`repo unmount` folded into `repo up`/`repo down` (89 -> 87), then the w2b cluster batch added `cluster snapshot create|list` (87 -> 89), then the subscription flatten collapsed 5 machine-plane leaves into 2 (89 -> 85), then the datastore family went from 5 facade leaves to 10 real
    // ones (85 -> 90), then `repo logs` + `repo exec` landed (90 -> 92). The `repo admin` subtree move changes no count: the plane exceptions moved with their commands.
    //
    // 92 -> 89: `repo admin archive {list,restore,purge}` are config-plane, so a remote
    // executor can no longer be handed them. They were never machine commands, the relocation out of the `config` noun silently gave them repo's machine default, and this list is exactly the set the proxy will run remotely, which is why an archive verb appearing in it was a wrong-target data-loss bug (§4.9). 89 -> 90: `backup verify` is a machine-plane, non-interactive read (the
    // backup_verify verb runs on the machine that holds the anchor). 90 -> 91: `backup snapshot` is machine-plane and non-interactive, so a remote executor can run it, exactly like its `backup verify` sibling. 91 -> 92: `backup browse` reads the repository IMAGE, which only exists on the machine holding it, so it is machine-plane for the same reason `backup verify` is. Note it is
    // NOT `other` like `usage`/`manifests`: those ask the account server, this one cannot be answered without the image.
