 ...
 */
function getVSCodeInsidersSettingsPath(): string {
  const home = process.env.HOME ?? process.env.USERPROFILE ?? '';
  const platform = getPlatform();
 ...
 * @param settingsPath - Path to settings.json
 */
function writeVSCodeSettings(settings: Record<string, unknown>, settingsPath?: string): void {
  const path = settingsPath ?? getVSCodeSettingsPath();
 ...
 */
function getServerInstallPath(datastorePath?: string): string {
 ...
 */
function getRequiredRemoteSSHSettings(datastorePath?: string): VSCodeRemoteSettings {
