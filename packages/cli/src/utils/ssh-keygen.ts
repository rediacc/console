...
export function generateSSHKeyPair(comment = 'rediacc-repo-key'): {
  ...
  // ssh2's ed25519 OpenSSH encoder mis-pads the private section on ~0.6% of generations (private blob 3 bytes short) -- OpenSSH then rejects the key
  ...
  // the CI tutorial sequence on repo-ref connects). ssh2's own parseKey rejects exactly the malformed outputs, so validate and retry: at 0.6% per draw, five draws make a bad pair astronomically unlikely.
