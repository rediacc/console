...
export function discoverRegions(): Promise<RegionInfo[]> {
  // Returns a Promise WITHOUT being `async`, deliberately. Every caller already awaits this, so the signature must stay thenable -- but an `async` function that never awaits anything is what `require-await` exists to flag, and it would be true here now that the fetch is gone.
