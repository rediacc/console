...
import {
  ...
  type RemoteFile,
} from '@rediacc/shared/storage-browser';
...
class StorageBrowserService {
   ...
   * Calls `rclone lsjson` directly with credentials from vault content.
   */
  async browse(vaultContent: Record<string, unknown>, subPath?: string): Promise<RemoteFile[]> {
    ...
    const parser = new FileListParserFactory(subPath ?? '', {
      detectGuids: true,
    });
