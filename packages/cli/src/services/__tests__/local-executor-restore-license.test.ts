...
const restoreOptions = {
  ...
  params: {
    repository: 'rtcli',
    // The command computes this as `source.grandGuid ?? source.repositoryGuid` and hands the SAME value to the chunk store as the lineage.
    lineage: SOURCE_GUID,
    at: SNAPSHOT_ID,
    dry_run: false,
  },
...
describe('backup restore: licensing the target machine', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // Without this the whole pre-flight early-returns, and every assertion below would pass for a reason that has nothing to do with the fix.
  ...
  it('licenses the SOURCE guid, in the TARGET datastore scope', async () => {
    ...
    expect(mockIssueRepoLicense.mock.calls[0][2]).toMatchObject({
      // Not a carrier, not a fresh guid, not the target NAME: the source guid, which is the licence `repository_up` will look for after `--up`, guid-specifically and with no any-repo fallback.
      repositoryGuid: SOURCE_GUID,
      grandGuid: SOURCE_GUID,
      kind: 'grand',
      // The blob has to land where `datastore.IdentityAt` looks for it. Unscoped here is a licence installed somewhere nothing reads.
  ...
  it('sizes the licence from the snapshot manifest, rounded up', async () => {
    ...
    // The lookup is exact, not "newest": the 999 GB decoy manifest in the same lineage must not be the one that was read.
  ...
  // The bypass must stay honoured: nolicense/CI builds have no subscription server to mint against.
