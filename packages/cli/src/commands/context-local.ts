...
async function scanSingleMachine(machineName: string): Promise<void> {
  ...
  if (keyscan) {
    ...
    outputService.success(
      t("commands.context.scanKeys.keysScanned", { name: machineName }),
    );
  } else {
    outputService.warn(
      t("commands.context.scanKeys.noKeys", { name: machineName }),
    );
  }
}
...
async function scanAllMachines(): Promise<void> {
  ...
  for (const m of machines) {
    try {
      const keyscan = scanHostKeys(
        m.config.ip,
        m.config.port ?? DEFAULTS.SSH.PORT,
      );
      if (keyscan) {
        ...
        outputService.info(
          t("commands.context.scanKeys.keysScanned", { name: m.name }),
        );
        scanned++;
      }
    } catch {
      outputService.warn(
        t("commands.context.scanKeys.noKeys", { name: m.name }),
      );
    }
  }
  outputService.success(
...
export function registerLocalCommands(
      ...
      try {
        ...
        try {
          ...
          throw new ValidationError(
            t("errors.sshKeyNotFound", { path: sshKeyPath }),
          );
        }
        ...
        outputService.success(
          t("commands.context.createLocal.success", { name }),
        );
  ...
  context
    ...
    .requiredOption(
      "--endpoint <url>",
      t("commands.context.createS3.optionEndpoint"),
    )
    .requiredOption(
      "--bucket <name>",
      t("commands.context.createS3.optionBucket"),
    )
    .requiredOption(
      "--access-key-id <key>",
      t("commands.context.createS3.optionAccessKeyId"),
    )
    .requiredOption(
      "--ssh-key <path>",
      t("commands.context.createS3.optionSshKey"),
    )
    .option(
      "--secret-access-key <key>",
      t("commands.context.createS3.optionSecretAccessKey"),
    )
    .option(
      "--region <region>",
      t("commands.context.createS3.optionRegion"),
      "auto",
    )
    .option("--prefix <prefix>", t("commands.context.createS3.optionPrefix"))
    .option(
      "--renet-path <path>",
      t("commands.context.createS3.optionRenetPath"),
    )
    .option(
      "--master-password <password>",
      t("commands.context.createS3.optionMasterPassword"),
    )
    .action(async (name, options) => {
      try {
        const sshKeyPath = options.sshKey.startsWith("~")
        ...
        const secretAccessKey =
          ...
          (await askPassword(
            t("commands.context.createS3.promptSecretAccessKey"),
          ));

        if (!secretAccessKey) {
          throw new ValidationError(
            t("commands.context.createS3.errorSecretAccessKeyRequired"),
          );
        }
        ...
        const stateService = await S3StateService.load(
          testClient,
          masterPassword ?? null,
        );
      ...
      try {
        ...
        try {
          ...
          if (keyscan) {
            ...
            outputService.info(
              t("commands.context.scanKeys.keysScanned", { name }),
            );
          }
        } catch {
          /* non-fatal */
  ...
  context
    ...
    .action(async (name) => {
      try {
        ...
        outputService.success(
          t("commands.context.removeMachine.success", { name }),
        );
      } catch (error) {
        handleError(error);
      }
  ...
  context
    ...
    .action(async (renetPath) => {
      try {
        ...
        outputService.success(
          t("commands.context.setRenet.success", { path: renetPath }),
        );
      } catch (error) {
        handleError(error);
      }
