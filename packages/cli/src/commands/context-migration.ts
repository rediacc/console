...
export function registerMigrationCommands(context: Command): void {
  ...
  context
    ...
    .requiredOption(
      "--endpoint <url>",
      t("commands.context.toS3.optionEndpoint"),
    )
    .requiredOption("--bucket <name>", t("commands.context.toS3.optionBucket"))
    .requiredOption(
      "--access-key-id <key>",
      t("commands.context.toS3.optionAccessKeyId"),
    )
    .option(
      "--secret-access-key <key>",
      t("commands.context.toS3.optionSecretAccessKey"),
    )
    .option(
      "--region <region>",
      t("commands.context.toS3.optionRegion"),
      "auto",
    )
    .option("--prefix <prefix>", t("commands.context.toS3.optionPrefix"))
    .option(
      "--master-password <password>",
      t("commands.context.toS3.optionMasterPassword"),
    )
    .action(async (options) => {
      try {
        ...
        if (!secretAccessKey) {
          throw new ValidationError(
            t("commands.context.toS3.errorSecretRequired"),
          );
        }
        ...
        if (ctx.ssh?.privateKeyPath) {
          const sshPrivateKey = await fs.readFile(
            ctx.ssh.privateKeyPath,
            "utf-8",
          );
          let sshPublicKey = "";
          try {
            const pubPath =
              ctx.ssh.publicKeyPath ?? `${ctx.ssh.privateKeyPath}.pub`;
  ...
  context
    ...
    .option(
      "--ssh-key-path <path>",
      t("commands.context.toLocal.optionSshKeyPath"),
    )
    .action(async (options) => {
      try {
        const ctx = await contextService.getCurrent();
        ...
        if (sshContent?.privateKey) {
          ...
          if (sshContent.publicKey) {
            await fs.writeFile(
              `${sshKeyPath}.pub`,
              `${sshContent.publicKey}\n`,
              { mode: 0o644 },
            );
          }
          outputService.info(
            t("commands.context.toLocal.sshWritten", { path: sshKeyPath }),
          );
        }
