  ...
  program
    ...
    .action(async (options) => {
      try {
        ...
        if (options.checkOnly) {
          ...
          if (result.updateAvailable) {
            ...
            if (result.releaseNotesUrl) {
              outputService.info(
                t('commands.update.releaseNotes', { url: result.releaseNotesUrl })
              );
            }
          } else {
        ...
        if (!options.force && !checkResult.updateAvailable) {
          outputService.success(t('commands.update.upToDate', { version: VERSION }));
          return;
        }
          ...
          onProgress: (downloaded, total) => {
            if (total > 0) {
              ...
              process.stderr.write(
                `\r${t('commands.update.progress', { percent: percent.toString() })}`
              );
            }
          },
        });
        ...
        if (result.success && result.fromVersion !== result.toVersion) {
          ...
          outputService.error(t('commands.update.failed', { error: result.error ?? 'unknown' }));
          process.exit(1);
        }
      } catch (error) {
        handleError(error);
