...
function getDescriptionFromName(functionName: string): string {
  const nameMap: Record<string, string> = {
    ...
    // Daemon lifecycle
    daemon_setup: 'Set up network daemon',
    daemon_start: 'Start network daemon',
    daemon_stop: 'Stop network daemon',
    daemon_status: 'Get daemon status',
    daemon_teardown: 'Tear down network daemon',
    daemon_restart: 'Restart network daemon',
    daemon_logs: 'Get daemon logs',
    daemon_wait_docker: 'Wait for Docker daemon readiness',
    daemon_nop: 'No operation (test command)',
    // Plugin management
    plugin_start: 'Start network plugins',
    plugin_stop: 'Stop network plugins',
    plugin_status: 'Get plugin status',
    // Network operations
    network_ensure_ips: 'Ensure IP addresses are allocated',
    network_cleanup_ips: 'Clean up unused IP addresses',
    network_prune: 'Prune unused network resources',
    network_ps_status: 'Get network process status',
    // Datastore management
    datastore_init: 'Initialize datastore on disk',
    datastore_mount: 'Mount datastore',
    datastore_unmount: 'Unmount datastore',
    datastore_resize: 'Resize datastore',
    datastore_expand: 'Expand datastore size',
    datastore_status: 'Check datastore status',
    datastore_validate: 'Validate datastore integrity',
    // Checkpoint operations
    checkpoint_create: 'Create process checkpoint',
    checkpoint_restore: 'Restore from checkpoint',
    checkpoint_validate: 'Validate checkpoint integrity',
    checkpoint_cleanup: 'Clean up checkpoint files',
    checkpoint_check_compat: 'Check CRIU checkpoint compatibility',
    // Ceph infrastructure
    ceph_bootstrap_cluster: 'Bootstrap Ceph cluster',
    ceph_cluster_create: 'Create Ceph cluster',
    ceph_cluster_destroy: 'Destroy Ceph cluster',
    ceph_cluster_status: 'Get Ceph cluster status',
    ceph_cluster_dashboard: 'Get Ceph dashboard URL',
    ceph_health: 'Check Ceph cluster health',
    ceph_install_prerequisites: 'Install Ceph prerequisites',
    ceph_pool_create: 'Create Ceph storage pool',
    ceph_pool_delete: 'Delete Ceph storage pool',
    ceph_pool_info: 'Get Ceph pool info',
    ceph_pool_list: 'List Ceph storage pools',
    ceph_pool_stats: 'Get Ceph pool statistics',
    ceph_clone_cleanup: 'Clean up orphan clone resources',
    // Machine diagnostics
    machine_check_btrfs: 'Check BTRFS filesystem support',
    machine_check_cli: 'Check renet CLI installation',
    machine_check_criu: 'Check CRIU installation',
    machine_check_drivers: 'Check GPU drivers',
    machine_check_kernel: 'Check kernel compatibility',
    machine_check_memory: 'Check system memory',
    machine_check_renet: 'Check renet installation',
    machine_check_setup: 'Check machine setup status',
    machine_check_sudo: 'Check sudo access',
    machine_check_system: 'Run system checks',
    machine_check_tools: 'Check required tools',
    machine_check_users: 'Check user configuration',
    machine_fix_groups: 'Fix user group memberships',
  };
