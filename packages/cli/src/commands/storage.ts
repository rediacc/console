...
export function registerStorageCommands(program: Command): void {
    ...
    .action(async (name: string, options: { path: string }) => {
      try {
        ...
        let files = await withSpinner(
          ...
          () =>
            storageBrowserService.browse(
              storageConfig.vaultContent,
  ...
  storage
    ...
    .requiredOption(
      "-r, --repository <name>",
      t("commands.storage.pull.repositoryOption"),
    )
    .requiredOption(
      "-m, --machine <name>",
      t("commands.storage.pull.machineOption"),
    )
    ...
    .action(
      async (
        ...
        try {
          ...
          const repo = await contextService.getLocalRepository(
            options.repository,
          );
          if (!repo) {
            throw new Error(
              `Repository "${options.repository}" not found in context`,
            );
          }
          if (!repo.credential) {
            outputService.warn(
