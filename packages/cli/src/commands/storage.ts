...
import { formatSizeBytes, resolveGuidFileNames } from '@rediacc/shared/queue-vault';
  ...
  storage
    ...
    .action(async (name: string, options: { path: string }) => {
      try {
        ...
        let files = await withSpinner(
          t('commands.storage.browse.listing', { name }),
        ...
        const tableData = files.map((f) => ({
          name: f.isDirectory
            ? `${f.name}/`
            : f.isGuid
              ? f.originalGuid && f.name !== f.originalGuid
                ? f.name
                : `[backup] ${f.name}`
              : f.name,
          type: f.isDirectory ? 'dir' : f.isGuid ? 'backup' : 'file',
    ...
    .action(
      async (
        ...
        try {
          ...
          if (!repo.credential) {
            outputService.warn(
              t('commands.storage.pull.noCredential', { name: options.repository })
            );
          }
          ...
          if (result.success) {
            outputService.success(
              t('commands.storage.pull.completed', { duration: result.durationMs })
            );
          } else {
            outputService.error(
