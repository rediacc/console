    ...
    .action(async (oldName, newName) => {
      try {
        ...
        outputService.success(
          t("commands.context.rename.success", { oldName, newName }),
        );
      } catch (error) {
        handleError(error);
      }
      ...
      try {
        ...
        if (!validKeys.includes(key)) {
          throw new ValidationError(
            t("errors.invalidKey", { keys: validKeys.join(", ") }),
          );
        }

        await contextService.set(
          key as "team" | "region" | "bridge" | "machine",
          value,
        );
        outputService.success(
          t("commands.context.set.success", { key, value }),
        );
      } catch (error) {
        handleError(error);
      }
    ...
    .action(async (key) => {
      try {
        if (key) {
          ...
          await contextService.remove(
            key as "team" | "region" | "bridge" | "machine",
          );
          outputService.success(
            t("commands.context.clear.keyCleared", { key }),
          );
        } else {
