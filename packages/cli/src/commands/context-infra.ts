  ...
  context
    ...
    .action(async (machineName, options) => {
      try {
        ...
        // Validate IP addresses (empty is acceptable)
        if (infra.publicIPv4 && isIP(infra.publicIPv4) !== 4) {
          outputService.error(t('commands.context.setInfra.invalidIPv4', { ip: infra.publicIPv4 }));
          return;
        }
        if (infra.publicIPv6 && isIP(infra.publicIPv6) !== 6) {
          outputService.error(t('commands.context.setInfra.invalidIPv6', { ip: infra.publicIPv6 }));
          return;
        }
