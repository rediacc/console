...
export class SqlHelpers {
     ...
     */
    async executeSql(containerName, sql, networkId) {
        // Use base64 encoding to safely pass SQL through multiple SSH hops
        const base64Sql = Buffer.from(sql).toString('base64');
        const result = await this.executeViaBridge(`echo "${base64Sql}" | base64 -d | sudo docker -H unix:///var/run/rediacc/docker-${networkId}.sock exec -i ${containerName} psql -U postgres -d testdb -t`);
     ...
     * @param networkId Network ID for the docker socket
     */
    async insertUserRecord(containerName, username, origin, networkId) {
        await this.executeSql(containerName, `INSERT INTO users (username, email, repo_origin) VALUES ('${username}', '${username}@test.com', '${origin}')`, networkId);
    }
    /**
     * Check if a record with the given origin exists in users table.
     * @returns true if at least one record exists with this origin
     */
    async recordExistsByOrigin(containerName, origin, networkId) {
        const count = await this.executeSql(containerName, `SELECT COUNT(*) FROM users WHERE repo_origin = '${origin}'`, networkId);
        return Number.parseInt(count.trim()) > 0;
    }
    /**
     * Get total record count in users table.
     */
    async getUserRecordCount(containerName, networkId) {
     ...
     */
    async getUsersDataHash(containerName, networkId) {
        const hash = await this.executeSql(containerName, `SELECT MD5(string_agg(id::text || username || email || COALESCE(repo_origin, ''), '' ORDER BY id)) FROM users`, networkId);
        return hash.trim();
    }
    /**
     ...
     */
    async insertBulkUserRecords(containerName, count, origin, networkId) {
        ...
        for (let i = 0; i < values.length; i += batchSize) {
            ...
            await this.executeSql(containerName, `INSERT INTO users (username, email, repo_origin) VALUES ${batch.join(', ')}`, networkId);
        }
    }
}
