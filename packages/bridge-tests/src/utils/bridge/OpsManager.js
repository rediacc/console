import { OpsManager as BaseOpsManager, loadConfigFromEnv, } from '@rediacc/provisioning';
...
export class OpsManager extends BaseOpsManager {
     ...
     */
    constructor(provisioningConfig) {
        super(provisioningConfig);
        // Initialize domain-specific managers
        this.rustfsManager = new OpsRustFSManager(this.getVMExecutor(), this.getBridgeVMIp(), this.runOpsCommand.bind(this));
        this.cephManager = new OpsCephManager(this.getCephVMIps(), this.runOpsCommandWithEnv.bind(this));
    }
    /**
     ...
     */
    async initializeAllDatastores(size = '10G', datastorePath = DEFAULT_DATASTORE_PATH) {
        ...
        for (const ip of workerIPs) {
            ...
            const result = await this.executeOnVM(ip, `renet bridge once --test-mode --function datastore_init --datastore-path ${datastorePath} --size ${size} --force`, 120000 // 2 minute timeout for datastore initialization
            );
            if (result.code !== 0) {
...
let opsManagerInstance = null;
/**
 * Get the singleton OpsManager instance using environment configuration.
 *
 * @throws Error if required environment variables are missing
 */
export function getOpsManager() {
    opsManagerInstance ?? (opsManagerInstance = new OpsManager(loadConfigFromEnv()));
    return opsManagerInstance;
}
//# sourceMappingURL=OpsManager.js.map
