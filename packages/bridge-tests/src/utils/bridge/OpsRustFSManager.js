...
export class OpsRustFSManager {
    constructor(vmExecutor, bridgeIp, runOpsCommand) {
        this.vmExecutor = vmExecutor;
        this.bridgeIp = bridgeIp;
        this.runOpsCommand = runOpsCommand;
    }
    /**
    ...
    async start() {
        ...
        const checkResult = await this.vmExecutor.executeOnVM(this.bridgeIp, 'docker ps --filter name=rustfs --format "{{.Names}}"');
        ...
        const verifyResult = await this.vmExecutor.executeOnVM(this.bridgeIp, "curl -s -o /dev/null -w '%{http_code}' http://localhost:9000/");
    ...
    async isRunning() {
        const RUSTFS_CONTAINER_NAME = 'rustfs';
        const result = await this.vmExecutor.executeOnVM(this.bridgeIp, 'docker ps --filter name=rustfs --format "{{.Names}}"');
     ...
     */
    async configureWorker(vmId) {
        console.warn(`[OpsRustFSManager] Configuring RustFS access on worker VM ${vmId}...`);
        const result = await this.runOpsCommand(['rustfs', 'configure-worker'], [vmId.toString()], 60000);
        if (result.code !== 0) {
     ...
     */
    async verifyAccessFromWorker(vmId, calculateVMIp, bucket) {
        ...
        const result = await this.vmExecutor.executeOnVM(ip, `rclone ls rustfs:${bucketName} 2>&1 || echo "(empty or bucket not found)"`, 30000);
        if (result.code !== 0) {
...
//# sourceMappingURL=OpsRustFSManager.js.map