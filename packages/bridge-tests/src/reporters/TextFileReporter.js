 ...
 *   └── summary.txt
 */
export default class TextFileReporter {
    constructor(options = {}) {
     ...
     */
    unescapeLogOutput(text) {
        return text.replaceAll('\\n', '\n').replaceAll('\\t', '\t');
    }
    addTestHeader(lines, test, result) {
    ...
    addOutputSection(lines, outputs, title) {
        if (outputs.length > 0) {
    ...
    addAttachments(lines, attachments) {
    ...
    addSummaryHeader(lines, result, durationSeconds) {
        lines.push('='.repeat(80));
    ...
    addFailedTests(lines) {
        const failedTests = Array.from(this.testResults.values()).filter(({ result }) => result.status === 'failed' || result.status === 'timedOut');
    ...
    buildSummary(result, durationSeconds) {
        const lines = [];
...
//# sourceMappingURL=TextFileReporter.js.map