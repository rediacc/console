...
import { CEPH_HEALTH_RETRY_MS, CEPH_HEALTH_TIMEOUT_MS, DEFAULT_DATASTORE_PATH, RENET_SETUP_TIMEOUT_MS, } from '../constants';
 ...
 */
async function waitForCephHealth(opsManager) {
    // eslint-disable-next-line no-console
    console.log('');
    // eslint-disable-next-line no-console
    ...
    while (Date.now() - startedAt < healthTimeoutMs) {
        ...
        // eslint-disable-next-line no-console
        console.log(`  ! Ceph not healthy yet (attempt ${attempt}). Retrying in ${Math.round(retryIntervalMs / 1000)}s...`);
 ...
 */
async function setupAllVMs(opsManager) {
    console.warn('');
    ...
    for (const ip of allVmIps) {
        ...
        if (result.code === 0) {
            ...
            throw new Error(`Setup failed on ${ip} (${vmType}): exit code ${result.code}\nstderr: ${result.stderr}\nstdout: ${result.stdout}`);
        }
    }
}
/**
 ...
 */
async function verifyVMsWithRetry(opsManager, infra) {
    console.warn('');
    ...
    try {
        ...
        if (message.includes('(renet not installed)')) {
            // eslint-disable-next-line no-console
            console.log('Renet missing on one or more VMs; re-deploying renet and retrying verification...');
 ...
 */
async function initializeDatastoresIfNeeded(opsManager, workerIps) {
 ...
 */
async function deployCRIUIfNeeded(infra, workerIps) {
    if (workerIps.length > 0) {
        console.warn('');
 ...
 */
async function startRustFSStorage(opsManager) {
    console.warn('');
 ...
 */
async function configureRustFSWorkersIfNeeded(opsManager, workerIps) {
    if (workerIps.length === 0) {
        return;
    }
    console.warn('');
 ...
 */
function writeSetupErrorLog(error) {
    try {
        const e2eDir = path.resolve(__dirname, '..', '..');
 ...
 */
async function bridgeGlobalSetup(_config) {
    ensureEnvFile();
...
//# sourceMappingURL=bridge-global-setup.js.map