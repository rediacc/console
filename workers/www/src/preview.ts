/**
 * Combined preview worker entry point.
 *
 * Embeds the account server directly instead of using a Service Binding,
 * so only a single Worker (`pr-N`) is needed for preview deployments.
 * Production uses index.ts with a Service Binding to the separate account-server worker.
 */
