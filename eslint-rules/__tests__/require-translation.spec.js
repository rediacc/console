 ...
 *
 * A key is looked up only when the call is a `t` bound by useTranslation(). Before 2026-09-26 a
 * string with a colon was split as `ns:key` and checked whatever the callee was, so the Tailwind
 * class string in `cn('h-11 md:text-lg')` was reported as a missing translation key.
 */
