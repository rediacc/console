...
//
// The i18nLocaleConfigs() generator and everything downstream of it: the CLI
// and account-web source-side enforcement, the three per-package locale trees
// it expands into nine blocks, the JSON rule disables that keep JS/TS rules off
// locale files, the account email-copy restriction, and the generated-file
// exemption that sits between them in the original.
//
// The last two blocks are here because they are here in the original. Order is
// the contract (see below), so a block does not move across a module boundary
// to make a filename read better.
// ---------------------------------------------------------------------------
//
