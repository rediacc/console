#!/bin/bash
# The private/growth side of the media pipelines: one wrapper that cannot get the
# working directory or the venv wrong.
#
# WHERE THIS CAME FROM. It is media.sh, moved. media.sh is a top-level script rather
# than a library, so it could only ever be run, never called; putting the same three
# operations here as functions is what lets media-entry.sh expose them alongside the
# tutorial verbs, and what lets a gate test drive them with python3 and ffmpeg absent.
# The header below, and every comment inside the three functions, is media.sh's own
...
#
# SOURCED, NEVER EXECUTED. GROWTH is resolved from ROOT_DIR at source time and can be
# overridden by the caller's environment, which is the seam the gate test uses to point
# it at a fixture tree instead of a real checkout.

GROWTH="${GROWTH:-$ROOT_DIR/private/growth}"
...
pass_owns() {
    ...
    for log in /tmp/claude-*/*/*/scratchpad/full-pass.log; do
        ...
        #
        # THE MTIME GOES THROUGH THE SEAM, and this line is the reason portable.sh exists.
        # It used to be a bare `stat -c %Y`, which is GNU-only and, worse, prints nothing
        # when the file has vanished between the glob and the stat -- one log rotated away
        # mid-loop and the arithmetic evaluated `$(( now -  ))`. A guard that dies is
        # better than one that decides a live pass finished in 1970, but neither is what
        # was wanted: an unreadable log is skipped, out loud.
        if ! mtime="$(media_mtime "$log")"; then
            continue
        fi
        [ $(($(date +%s) - mtime)) -lt 600 ] || continue
        # POSIX ERE plus a field, NOT `grep -oP '\K'`. -P is a GNU extension compiled in
        # only when PCRE is available; where it is not, grep exits 2 and this pipeline's
        # `tail -1` turns that into an empty `current`, which matches no slug and hands
        # the in-flight slug to a second pass.
        current=$(grep -oE '^######## [AB] [^ ]+' "$log" | tail -1 | awk '{print $3}')
...
# `./run.sh media run <pipeline> [args...]` -- a normal pipeline invocation.
growth_run() {
    pipeline="${1:-}"
    shift || true
    ...
    # `|| exit 1` is the one line here that media.sh does not carry, and it is not a
    # style edit. media.sh is a SCRIPT with `set -euo pipefail` at the top, so a failed
    # cd already aborts it and shellcheck says nothing. This is a sourced LIBRARY, and a
    # library cannot assume its caller set -e -- without the guard, a cd that failed would
    # fall through to the exec and run the pipeline from whatever directory happened to be
    # current, which is precisely the failure this wrapper exists to make impossible.
    cd "$GROWTH" || exit 1 # THE point of this wrapper
    exec python3 "$GROWTH/$pipeline/main.py" "$@"
}

# `./run.sh media teaser <slug> [lang...]` -- drop the sentinel AND rebuild, as ONE
# operation that cannot half-finish.
growth_teaser() {
    slug="${1:-}"
    shift || true
    ...
    export PATH="$vbin:$PATH"
    # See growth_run: a sourced library cannot assume its caller set -e.
    cd "$GROWTH" || exit 1
    langs="$*"
...
# `./run.sh media luma <mp4>` -- measure a rendered file (light is meanY ~210).
growth_luma() {
    mp4="${1:-}"
    [ -f "$mp4" ] || die "usage: ./media.sh luma <mp4>"
...
# The usage text media.sh prints for an unknown verb, carried across verbatim so the
# two entry points cannot describe the same three operations differently.
growth_usage() {
    cat >&2 <<'USAGE'
