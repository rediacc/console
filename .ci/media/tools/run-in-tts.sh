...
#
# THIS FILE MOVED FROM .ci/docker/run-in-tts.sh (W10 phase 3), together with the image
# context it builds, which is now .ci/media/tts/. The narration wrapper is media, not
# generic docker plumbing: it exists to hold ONE GPU lease across the host/container
# boundary for the tutorial and solution pipelines, and it was the only thing under
# .ci/docker that the media pipeline owned. Its two siblings, run-in-render.sh and
# run-in-web.sh, are not media and stayed put.
#
# THE OLD PATH IS STILL A LIVE ENTRY POINT. .ci/docker/run-in-tts.sh is now a three-line
# exec shim, because the callers that matter -- step4000_voiceover.py in
# private/generative and tts_bridge.py in private/growth -- live in gitignored
# repositories this checkout cannot open, so their spelling of this path cannot be
# updated in the same change and cannot even be read to confirm what it is. The shim is
...
docker image inspect "$IMAGE" >/dev/null 2>&1 || {
    ...
    docker build -t "$IMAGE" "$ROOT/.ci/media/tts"
}
