...
if [[ -z "${ACCOUNT_ED25519_PRIVATE_KEY:-}" ]]; then
    ...
    ")
    ACCOUNT_ED25519_PRIVATE_KEY=$(echo "$KEYS" | node -e "process.stdout.write(JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')).private)")
    ACCOUNT_ED25519_PUBLIC_KEY=$(echo "$KEYS" | node -e "process.stdout.write(JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')).public)")
fi
