...
- A downloaded release binary failing to EXECUTE with "is not a valid application for this OS platform" (Windows), "cannot execute binary file", or "Exec format error" in an install-validation job — the runner received a truncated or stale download from releases.rediacc.com/CDN; the staged artifact itself verifies as a valid executable. Retry re-downloads and passes. (The watchdog enforces the safety condition itself: if EVERY install-validation job in the run failed this way, it overrides this verdict to code-change, so classify on the log in front of you.)
