...
# Demonstrates rdc machine infra set / push and the proxy template apply. The
...
run_cmd "rdc machine infra set $M --public-ipv4 $TUTORIAL_MACHINE_IP --base-domain $BASE_DOMAIN --cert-email $CERT_EMAIL --cf-dns-token $CF_TOKEN"

pause 2

run_cmd "rdc machine infra show $M"
...
type_only_cmd "rdc machine infra push $M"

pause 2

section "Step 4: Deploy the proxy"
run_cmd "rdc repo create infra --machine $M --size 1G"
...
run_cmd "rdc repo admin template apply infra --template single-service"

pause 1

run_cmd "rdc repo up infra"
