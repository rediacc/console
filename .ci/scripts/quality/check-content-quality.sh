...
# check:ci-content-quality is now registered to the Python port's entry point,
# .ci/scripts/quality/check_content_quality.py, so a header here would declare a
# registration that has moved and gate-bind refuses that by name:
#   package.json runs ".ci/scripts/quality/check_content_quality.py" but its header derives ".ci/scripts/quality/check-content-quality.sh"
