...
and `python3 -m rediacc_ci.quality.dead_case_arms` works but is the wrong registration
...
`.ci/scripts/quality/check-dead-case-arms.sh` by an awk range over its
...
`kind: test` IS THE LOAD-BEARING ONE. This gate has NO workflow step of its
own: it reaches CI through `.ci/scripts/test/gates/test-dead-case-arms.sh`
inside the shared "Quality-gate unit tests" step. Grepped every file under
`.github/workflows/` for `check-dead-case-arms.sh` and found ZERO hits, which
is what `kind: test` predicts and what makes the `blocker:` below the whole of
this gate's CI story. It is carried byte for byte, because a suppression reason
that goes missing in a file move is a quiet exemption.

NO `selftest:` IS CORRECT, carried exactly as found: the twin did not declare
one. The port does implement `--selftest`, but declaring a field the twin never
had would be a NEW claim rather than a moved one, and `selftest:` is inert for
a `.py` gate anyway (`gate-bind.ts:598`).

NO `id:` IS CORRECT HERE: `derivedId` (`gate-header.ts:260`) maps this basename
to `check:ci-dead-case-arms`, which is the manifest id.
...
PINNED BY PATH IN FOUR HARNESS ROWS, ALL OF THEM RUN-IN-PLACE, so all four take
the ENTRY POINT arm if they are ever repointed:
...
BECAUSE THIS GATE IS `kind: test`, REPOINTING IS NOT COSMETIC HERE, and it is
the driver's call, not this writer's: while `test-dead-case-arms.sh:25` still
names the `.sh`, CI executes the TWIN and only the local `npm run` executes the
port. That is the one-sided shape this programme exists to prevent, so it is
called out in the report rather than left in a diff.
...
DRIVEN RED AS WELL, through the twin's own three root overrides
(`DEAD_CASE_TEST_DIRS`, `DEAD_CASE_MEDIA_DIRS`, `DEAD_CASE_CODE_DIRS`), which the
port reads under the same names. The fixture is three scratch directories: a test
...
GATE. The media root was left EMPTY, so both sides went red on the anti-vacuity
refusal `VACUOUS: the media scan root holds no shell files, so scanning it proves
nothing` and never reached the detector. Both agreeing on the wrong red is
exactly the shape a writer must not accept as evidence. With a shell file added
to the media root the fixture goes GREEN first, which is the control that matters:
...
The key was also confirmed absent from the whole real tree before the run, so
nothing in the repository could have vouched for it. THE REAL TREE WAS NEVER
WRITTEN TO for this gate.
