...
# check:ci-renet-types is now registered to the Python port's entry point,
# .ci/scripts/quality/check_renet_types.py, so a header here would declare a
# registration that has moved and gate-bind refuses that by name:
#   package.json runs "...check_renet_types.py" but its header derives "...check-renet-types.sh"
