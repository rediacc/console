...
and `python3 -m rediacc_ci.quality.trap_registry` works but is the wrong registration
...
`.ci/scripts/quality/check-trap-registry.sh` by an awk range over its
`---- gate ----` block, de-commented, and diffed as an ordered list of whole
lines against the block in this docstring. The twin carried SEVEN fields in this
order: `step`, `emit`, `blocker`, `needs`, `selftest`, `lane`, `why`. THE `why:`
IS ELEVEN CONTINUATION LINES and it is the largest header in this batch; every
one of those lines is carried, indentation included, because a `why:` truncated
to its first line still parses and still binds and would lose the argument for
the deliberate absence of `paths`.

`emit: false` PLUS ITS `blocker:` ARE THE PAIR THE PILOT LOST. This gate's step
is HAND-WRITTEN at ci-quality.yml:676, which is ABOVE the `quality-code`
`# >>> gate-bind` region (739-916) and therefore above that lane's
`- id: setup` guard. Emitting it into the region would move it below the guard
and skip it whenever setup fails. Confirmed by line arithmetic against the
region markers, not by trusting the field: 676 is outside 739-916.

NO `id:` IS CORRECT HERE: `derivedId` (`gate-header.ts:260`) maps this basename
to `check:ci-trap-registry`, which is the manifest id.

`selftest: true` is inert for a `.py` gate (`gate-bind.ts:598`) and is carried
because the twin declared it and because `trap_registry.main(["--selftest"])`
exits 0.
...
`TRAP_FLOOR` IS NOT TOUCHED BY THIS CHANGE. It is 79, it lives in BOTH
`.ci/scripts/quality/check-trap-registry.sh` and
`.ci/rediacc_ci/quality/trap_registry.py`, and
`.ci/rediacc_ci/tests/test_quality_trap_registry.py` asserts the two agree.
...
Unlike this batch's three `kind: test` gates, this one has its own workflow
step, so CI runs whatever the step's `run:` line names. That line is
`npm run check:ci-trap-registry`, an indirection through package.json, so the
driver's package.json edit moves the CI side too and no workflow edit is needed.
...
DRIVEN RED AS WELL, through the twin's own `TRAP_CORPUS` seam, which the port
reads under the same name. The fixture is a `cp` of the real
`docs/agent-reference/TRAPS.md` with ONE pointer re-aimed: the
`check-cannot-fail` entry's first `gate:` becomes
`gate:check:ci-gate-probe-8a-does-not-exist`.
...
the fixture still carries 79 `## ` entries, so the corpus FLOOR is satisfied and
the red below is the pointer check rather than the floor. That last one matters:
a plant that dropped the count would have produced a red that said nothing about
pointers.
...
THE REAL TREE WAS NEVER WRITTEN TO for this gate. `docs/agent-reference/TRAPS.md`
is untouched, and so is `TRAP_FLOOR` in both of the two places it lives.
