...
Contract section 5d puts a gate's entry point where `scripts/gate-bind.ts` can see it; the logic lives in `rediacc_ci.quality.trap_registry`, which pytest and the port's own `--selftest` import directly.
...
THE HEADER BELOW IS THE TWIN'S, FIELD FOR FIELD, extracted from `.ci/scripts/quality/check-trap-registry.sh` by an awk range over its `---- gate ----` block, de-commented, and diffed as an ordered list of whole lines against the block in this docstring. The twin carried SEVEN fields in this order: `step`, `emit`, `blocker`, `needs`, `selftest`, `lane`, `why`. THE `why:` IS ELEVEN
CONTINUATION LINES and it is the largest header in this batch; every one of those lines is carried, indentation included, because a `why:` truncated to its first line still parses and still binds and would lose the argument for the deliberate absence of `paths`.
...
`diff` against the real file shows EXACTLY ONE changed line, so nothing else about the corpus moved; the planted id appears ZERO times in both `scripts/ci-runner/manifest.ts` and `package.json`, so it genuinely dangles; and the fixture still carries 79 `## ` entries, so the corpus FLOOR is satisfied and the red below is the pointer check rather than the floor. That last one
matters: a plant that dropped the count would have produced a red that said nothing about pointers.
