...
and `python3 -m rediacc_ci.quality.host_toolchain_coverage` works but is the wrong
registration because `check:ci-parity`'s tokenizer cannot read `-m` and resolves
the leaves to `[python3]`.

THE HEADER BELOW IS THE TWIN'S, FIELD FOR FIELD, and this is the one of the six
where the diff earns its keep. The twin carried FIVE fields -- `step`, `emit`,
`blocker`, `needs`, `selftest` -- and no `id:`, no `kind:`, no `lane:`, no
`run:`, no `why:`. The two that a retyped header loses:
...
Measured on both files: `inferredNeeds(twin)` is `["node"]`, `inferredNeeds(this
file)` is `[]`, because `inferredNeeds` strips Python docstrings as prose
...
DRIVEN RED AS WELL, by adding `gateprobetool` to `GATED_TOOLS` in
`.ci/scripts/quality/check-toolchain-pins.sh` -- a pinned tool the runtime guard
does not cover, which is precisely the drift this gate exists for. Both sides
exit 1 with byte-identical stdout (122 bytes, both controls still firing) and
stderr that agrees on every word.

THE ONE RESIDUE, stated rather than smoothed over: on the two per-finding stderr
lines the twin's own `fail()` prints `printf '%s✗%s   %s\n'`, three spaces after
the marker, while the port's `log.error` prints one. Everything else on that
stream is byte-identical, including the summary line and the two indented advice
lines, and `diff <(sed 's/^✗  */✗ /' twin.err) <(sed 's/^✗  */✗ /' port.err)` is
empty. Named exactly: the ONLY normalisation applied is collapsing the run of
spaces between the `✗` marker and the message to one. Not "fixed" in the port,
because the alternative is printing the primary findings raw and abandoning the
house logger's tty and NO_COLOR handling for a two-space cosmetic.

The plant was reverted by its exact inverse and `check-toolchain-pins.sh`
verified byte-identical to its pre-plant state, with both sides back at exit 0.
