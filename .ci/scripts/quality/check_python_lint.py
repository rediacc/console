...
and `python3 -m rediacc_ci.quality.python_lint` works but is the wrong registration
...
`.ci/scripts/quality/check-python-lint.sh` by an awk range over its
...
NO `lane:` IS CORRECT, and it is not an omission. This gate's step sits INSIDE
the `# >>> gate-bind` region of `quality-static` (ci-quality.yml:199-346), so
the lane is read off the region the step already lives in rather than declared.
The twin ran that way; nothing about the lane moves here.

NO `id:` IS CORRECT HERE: `derivedId` (`gate-header.ts:260`) strips the `check_`
prefix and maps `_` to `-`, so this basename derives `check:ci-python-lint`,
which is the manifest id. Verified by calling `bind()` rather than by reading
the rule.
...
DRIVEN RED AS WELL, and NOT by mutating the real tree. The plant lives in a
`cp -r` fixture repository at a scratch path: a `git init` tree carrying twelve
clean `.py` files (the twin's `MIN_PY_FILES` floor is 10, so eleven would have
been a vacuity refusal wearing a lint failure's exit code), this repo's
`pyproject.toml`, `.devcontainer/toolchain.env`, `.ci/scripts/lib/`, and a copy
of the TWIN at the same relative path so its `$BASH_SOURCE`-derived root lands
on the fixture. The port is aimed at the same tree with `REDIACC_CI_ROOT`.
`cp -r`, never a link, for the reason `mutate-check.sh:122` records: `realpath
--relative-to` resolves symlinks, and a fixture linked at the real tree makes
both sides agree by reading the same corrupted tree.

THE CONTROL WAS PROVED BEFORE EITHER SIDE RAN, twice over: bare `ruff check` on
the planted file reports F821 and exits non-zero, and `git ls-files` in the
fixture still enumerates twelve `.py` files, so the plant changes the VERDICT
and not the corpus size.
...
A PORT GAP WAS FOUND DOING THIS AND FIXED IN `python_lint.py`. The first
fixture had no `.devcontainer/`, and the two sides split: the twin exited 1
printing `toolchain: pins file missing or unreadable: ...` and nothing else,
...
refusal silently dropped in a port. The refusal is now in the module, in the
twin's position (after the two VACUOUS INPUT checks, before the resolver), with
`toolchain.sh:28`'s `REDIACC_TOOLCHAIN_LOADED` early return, and both sides now
print the same 91 bytes and exit 1 on that input.

THE REAL TREE WAS NEVER WRITTEN TO for this gate. `git status --porcelain`
carries no probe file, and neither this twin nor any other was edited here.
