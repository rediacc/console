...
The logic lives in `rediacc_ci.quality.worklist_env_registry`; this file exists so
the registry can invoke it BY PATH, for the parity-tokenizer reason recorded in
`gate-header.ts`'s `derivedRun` and in `_cipath`'s docstring. The
`---- gate ----` header is on the MODULE.
...
lane: quality-static
selftest: true
why: 133 WORKLIST_* names are read at 183 sites with no registry and no schema.
