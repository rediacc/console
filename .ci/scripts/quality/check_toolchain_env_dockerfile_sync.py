...
on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.toolchain_env_dockerfile_sync`
...
`.ci/scripts/quality/check-toolchain-env-dockerfile-sync.sh` PROGRAMMATICALLY, de-commented, and diffed
as an ordered list of WHOLE LINES against the block in this docstring; the diff
is empty over 12 line(s). The twin carried exactly these fields, in this
order: `step`, `emit`, `blocker`, `needs`, `selftest`, `why`. NO `id:`: the basename derives `check:ci-toolchain-env-dockerfile-sync`, the
manifest id. The `why:` field is FIVE lines of continuation and all five moved
...
untouched, which is the exact divergence this gate exists to catch. THE CONTROL
WAS PROVED BEFORE EITHER SIDE RAN: the planted value was read back out of the
env file and the Dockerfile's ARG line was printed to confirm it still said
1.26.6, so the two files genuinely disagreed. The file was restored from a `cp`
and its sha256 compared with the pre-plant value
(99fa72aa0760f6b6...), identical.
Both sides -> exit 1, stdout 724 bytes, sha256 977922f5a3bc2e7a..., stderr 453 bytes, sha256 ca485efa31cb8714....

INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-toolchain-env-dockerfile-sync.sh` is NOT deleted
by this change. It stays on disk as the differential twin that
`.ci/rediacc_ci/tests/test_quality_toolchain_env_dockerfile_sync.py` compares this port against, and
