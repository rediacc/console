...
The logic lives in `rediacc_ci.quality.resolved_threads`; this file exists so the registry can invoke the port BY PATH. `python3 -m rediacc_ci.quality.resolved_threads` also works and is still wrong here: `check-ci-parity`'s tokenizer cannot read `-m` and resolves the leaves to `[python3]`, so the gate fails parity. `gate-header.ts`'s `derivedRun` records that trap for every `.py`.
...
THE TWIN IS GONE. W7 P5 batch G1 deleted `.ci/scripts/quality/check-resolved-threads.sh` after `.ci/shadow/w7p2-resolved-threads.observations.jsonl` asserted equivalence over six distinct trees; its recorded output lives in `.ci/rediacc_ci/tests/goldens/`, headed with the blob sha it was captured from.
