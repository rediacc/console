...
and `python3 -m rediacc_ci.quality.review_turn_capacity` works but is the wrong registration
...
`.ci/scripts/quality/check-review-turn-capacity.sh` by an awk range over its
...
THIS TWIN IS ANOTHER GATE'S CONTROL SOURCE, which is the reason invariant 5
matters more here than usual. `check-control-vacuity.sh:159` pins
`check-review-turn-capacity.sh` BY PATH, strips its
...
disk, so the control keeps firing. This is stated here so that W7 P5's deletion
pass finds the dependency written down at BOTH ends rather than only at the
consumer.
...
the substitution find nothing, and the gate would have aborted with "the control
could not plant its defect" -- a red that proves the gate refuses a broken
control, and nothing about the property under test. So a different constant on
