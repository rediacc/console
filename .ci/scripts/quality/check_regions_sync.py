...
THE `blocker:` BELOW NAMES A HARNESS THAT MUST BE RETARGETED IN THE SAME CHANGE. `.ci/scripts/test/gates/test-regions-sync.sh:38` hard-codes
`SUT=...check-regions-sync.sh`. That harness IS this gate's whole CI coverage --
no lane invokes it directly -- so if the registration moves here while the harness still drives the twin, CI stops running the registered gate and the blocker below becomes false. The harness line is part of this cutover's patch set, not a follow-up.
