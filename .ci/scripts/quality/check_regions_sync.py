...
CUT OVER FROM BASH 2026-09-07 (W7 P4). The condition the previous version of
this docstring was waiting for is met. Measured that day:
...
RED as well, through the `REGIONS_ROOT_FILE` / `REGIONS_BAKED_FILE` seams with
one extra key planted in the baked copy: both sides exit 1 and print the same
3,834 bytes, diff included.

`python3 -m rediacc_ci.quality.regions_sync` also works and is still the wrong
registration: `check:ci-parity`'s tokenizer cannot read `-m` and resolves the
...
`.ci/scripts/test/gates/test-regions-sync.sh:38` hard-codes
`SUT=...check-regions-sync.sh`. That harness IS this gate's whole CI coverage --
no lane invokes it directly -- so if the registration moves here while the
harness still drives the twin, CI stops running the registered gate and the
blocker below becomes false. The harness line is part of this cutover's patch
set, not a follow-up.

INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-regions-sync.sh` is NOT
deleted here. Registering this file is not a second gate for the same subject:
what moved is which of the two the registry invokes. Deletion is W7 P5.
