...
THE HEADER BELOW IS THE TWIN'S, FIELD FOR FIELD. It was extracted from `.ci/scripts/quality/check-setup-idempotency.sh` PROGRAMMATICALLY, de-commented, and diffed
as an ordered list of WHOLE LINES against the block in this docstring; the diff
is empty over 8 line(s). The twin carried exactly these fields, in this order: `step`, `emit`, `blocker`, `needs`, `selftest`, `lane`. NO `id:`: the basename derives `check:ci-setup-idempotency`, the manifest id. The pilot lost `emit: false` plus a blocker off a header and SEVEN GATES PASSED ANYWAY, because `emit: false` suppresses only the three workflow-region checks
...
nothing beyond what it declares from its body and this two-import entry point infers nothing, because `inferredNeeds` strips Python docstrings as prose (`gate-header.ts:301`). Both sides bind to `needs: ['submodules']`.
...
DRIVEN RED AS WELL, which is the half that matters. `needs: submodules` IS THE FIELD THIS GATE WAS MOST LIKELY TO LOSE, and it is the reason the `bind()` comparison above is a measurement and not a reading. A
bash twin can infer a need from its body; a two-import Python entry point infers
nothing at all, so a `needs:` line dropped in the move would not have been replaced by inference. It is declared here, and both sides bind to `['submodules']`.
