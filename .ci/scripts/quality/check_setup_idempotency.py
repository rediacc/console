...
see it; the logic lives in `rediacc_ci.quality.setup_idempotency`, which pytest and the port's
...
on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.setup_idempotency`
...
`.ci/scripts/quality/check-setup-idempotency.sh` PROGRAMMATICALLY, de-commented, and diffed
...
order: `step`, `emit`, `blocker`, `needs`, `selftest`, `lane`. NO `id:`: the basename derives `check:ci-setup-idempotency`, the manifest id.
...
nothing beyond what it declares from its body and this two-import entry point infers nothing, because
`inferredNeeds` strips Python docstrings as prose (`gate-header.ts:301`).
Both sides bind to `needs: ['submodules']`.
...
`needs: submodules` IS THE FIELD THIS GATE WAS MOST LIKELY TO LOSE, and it is
the reason the `bind()` comparison above is a measurement and not a reading. A
bash twin can infer a need from its body; a two-import Python entry point infers
nothing at all, so a `needs:` line dropped in the move would not have been
replaced by inference. It is declared here, and both sides bind to
`['submodules']`.

The plant replaces `if devbox_image_present && ...` with `if false && ...`
inside `devbox_ensure_image` in the TRACKED `.ci/lib/devbox.sh`, so the function
still pulls and no longer carries its guard, which is rule A's shape. THE
CONTROL WAS PROVED BEFORE EITHER SIDE RAN by re-extracting the function body
...
INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-setup-idempotency.sh` is NOT deleted
by this change. It stays on disk as the differential twin that
`.ci/rediacc_ci/tests/test_quality_setup_idempotency.py` compares this port against, and
