...
infers nothing either, because `inferredNeeds` strips Python docstrings as prose (`gate-header.ts:301`). Both sides resolve to the empty set `needs: none` declares.
...
THE ONE RESIDUE, stated rather than smoothed over. The twin assigns its GREEN to a literal ANSI escape UNCONDITIONALLY (`check-devbox-exec.sh:53`, an ESC byte
...
the twin's clean stdout is 661 bytes and the port's is 650, and the eleven-byte difference is exactly one seven-byte set-green sequence plus one four-byte reset. Named exactly: the ONLY normalisation applied is one sed deleting every
CSI SGR sequence, `s/<ESC>[[0-9;]*m//g` with the ESC byte written literally,
applied to BOTH sides. Not "fixed" in the port, because the alternative is hard-coding escapes into a redirected stream and abandoning the tty and NO_COLOR handling `rediacc_ci.log` exists to centralise.
...
---- gate ---- step: Devbox exec invocation emit: false blocker: BLOCKER: runs before this lane's `- id: setup` step, so its hand-written step carries no `steps.setup.outcome` guard. Emitting it into the region would move it below that guard and skip it whenever setup fails. needs: none selftest: true lane: quality-code ---- end gate ----
"""
