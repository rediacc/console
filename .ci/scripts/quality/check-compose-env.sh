...
# check:ci-compose-env is now registered to the Python port's entry point,
# .ci/scripts/quality/check_compose_env.py, so a header here would declare a
# registration that has moved and gate-bind refuses that by name:
#   package.json runs "...check_compose_env.py" but its header derives "...check-compose-env.sh"
