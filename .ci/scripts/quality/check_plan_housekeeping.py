...
THE HEADER BELOW IS THE TWIN'S, FIELD FOR FIELD. It was extracted from `.ci/scripts/quality/check-plan-housekeeping.sh` PROGRAMMATICALLY, de-commented, and diffed
...
rest. The step sits inside a `# >>> gate-bind` region in that job, so it is emitted rather than hand-written. The pilot lost `emit: false` plus a blocker off a header and SEVEN GATES PASSED ANYWAY, because `emit: false` suppresses only the three workflow-region checks (`gate-bind.ts:1724`) while the registration assertions above them still ran.
...
builder: commit the plans with a BACKDATED committer date and both sides exit 1 naming all four as unchanged for
more than 33 days; commit the identical tree with TODAY's date and both sides
exit 0 with byte-identical streams. The red is therefore the backdate's and not the fixture's. Both sides -> exit 1, stdout 253 bytes, sha256 a3864c24435671ba..., stderr 2049 bytes, sha256 35b802ff561bad2f....
