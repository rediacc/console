...
see it; the logic lives in `rediacc_ci.quality.plan_housekeeping`, which pytest and the port's
...
on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.plan_housekeeping`
...
`.ci/scripts/quality/check-plan-housekeeping.sh` PROGRAMMATICALLY, de-commented, and diffed
as an ordered list of WHOLE LINES against the block in this docstring; the diff
is empty over 6 line(s). The twin carried exactly these fields, in this
order: `step`, `needs`, `selftest`, `lane`. NO `id:` and NO `emit:`; `lane: quality-i18n` IS present and moved with the
rest. The step sits inside a `# >>> gate-bind` region in that job, so it is
emitted rather than hand-written.
...
NO REAL-TREE PLANT WAS NEEDED. Both implementations honour the same four
seams -- `PLAN_HK_ROOT`, `PLAN_HK_CONFIG`, `PLAN_HK_ALLOWLIST` and
`PLAN_HK_MIN_FILES` -- so the fixture is a throwaway git repository in /tmp
holding four plan files, built with `cp`-free literal writes and never a
symlink. BOTH DIRECTIONS on the same builder: commit the plans with a
BACKDATED committer date and both sides exit 1 naming all four as unchanged for
more than 33 days; commit the identical tree with TODAY's date and both sides
exit 0 with byte-identical streams. The red is therefore the backdate's and not
the fixture's.
Both sides -> exit 1, stdout 253 bytes, sha256 a3864c24435671ba..., stderr 2049 bytes, sha256 35b802ff561bad2f....

INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-plan-housekeeping.sh` is NOT deleted
by this change. It stays on disk as the differential twin that
`.ci/rediacc_ci/tests/test_quality_plan_housekeeping.py` compares this port against, and
