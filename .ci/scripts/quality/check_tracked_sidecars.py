...
and driven again on this tree, both streams captured SEPARATELY: the twin and this port exit 0 with byte-identical stdout and byte-identical stderr.

DRIVEN RED AS WELL. In a fixture repository holding a copy of `.ci` and the real `.claude/hooks/stop/wl_store.py` (the gate's single source for the pattern list), `.claude/hooks/stop/.epics` was force-added to the index. Both sides exit 1, name that one path, and print byte-identical stdout and stderr. The pattern count in the clean line -- "21 pattern(s) derived from wl_store.py"
-- is the shape a reader can watch for: if it ever collapses, the list stopped parsing, and this gate answers that with a refusal rather than a pass.

WHY AN ENTRY POINT AT ALL: `check_npmrc.py` states both measured reasons. A port cannot be run by path (nothing puts `.ci` on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.tracked_sidecars` works but is the wrong registration because `check:ci-parity`'s tokenizer cannot read `-m` and resolves the leaves to `[python3]`.
