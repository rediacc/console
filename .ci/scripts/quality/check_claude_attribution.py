...
registry can invoke the port BY PATH. `python3 -m rediacc_ci.quality.claude_attribution` also works and is still wrong here: `check-ci-parity`'s tokenizer cannot read `-m` and resolves the leaves to `[python3]`, so the gate fails parity. `gate-header.ts`'s `derivedRun` records that trap for every `.py`.
