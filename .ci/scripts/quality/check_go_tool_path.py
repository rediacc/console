...
see it; the logic lives in `rediacc_ci.quality.go_tool_path`, which pytest and
...
this port exit 0 with byte-identical stdout and byte-identical stderr (373 bytes
of stdout, 0 of stderr).

DRIVEN RED AS WELL. In a fixture repository holding a copy of `.ci`,
`scripts/dev/plant.sh` was committed with `go install .../goimports@latest`
followed by a bare `goimports -w .`, with no GOBIN and no GOPATH/bin on PATH.
Both sides exit 1, name `scripts/dev/plant.sh`, and print byte-identical stdout
and stderr. Note the port keeps the twin's anti-vacuity floor of 50 tracked
shell files, so the fixture is a real corpus rather than one planted file.
...
and `python3 -m rediacc_ci.quality.go_tool_path` works but is the wrong
...
INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-go-tool-path.sh` is NOT
