...
`check:ci-autopilot-bp-align`, so without the line below the header would re-derive a gate that does not exist.
...
---- gate ---- kind: test test: .ci/scripts/test/gates/test-autopilot-breakpoint-alignment.sh
...
id: check:ci-autopilot-bp-align needs: none ---- end gate ----
"""
