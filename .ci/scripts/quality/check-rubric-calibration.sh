...
# check:ci-rubric-calibration is now registered to the Python port's entry point,
# .ci/scripts/quality/check_rubric_calibration.py, so a header here would declare a
# registration that has moved, and gate-bind refuses that by name:
#   package.json runs "...check_rubric_calibration.py" but its header derives "...check-rubric-calibration.sh"
