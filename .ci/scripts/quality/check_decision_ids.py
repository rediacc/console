...
`agent/PLAN-tooling-transformation.md` ended up pointing at OPPOSITE FILES.
`W12 P2.7` cited "A5 in `docs/ci-overhaul/04-decisions.md`"; `grep` for that
token in that file returns 0, because it meant the GATE RULE at
`.ci/scripts/quality/check_plan_boxes.py:41`. Box X0.1 fixed the source on
2026-09-08 by PREFIXING: gate rules became `G-A<n>`, operator decisions `D-A<n>`.
A prefix stops the collision recurring by transcription. It does not, on its own,
stop a `D-` id from naming nothing at all, which is the same failure one step
...
WHAT "ENFORCING A LICENCE" CAN MEAN, stated here so nobody builds the impossible.
`D-A6` is a standing operator licence to substitute a better design, provided the
substitution is stated out loud. NO GATE CAN TELL A GOOD SUBSTITUTION FROM A BAD
ONE. What a gate CAN assert is the licence's PRECONDITION: that the substitution
was stated, at a pointer that resolves, naming the licence it was taken under.
...
WHAT R5 CANNOT SEE, stated so a green is not read as more than it is. It compares
two COMMITTED versions of the register, so a supersession sitting uncommitted in
a working tree is not judged until it is committed. That is not a hedge, it is
the rule's own definition: the requirement is a COMMIT citing the id, and an
uncommitted edit has no commit to cite it. In CI on a pull request the working
tree is the head commit, so the same code path answers the same question with
nothing outstanding. When there is no usable base, R5 is SKIPPED and the summary
says so, because a skip must never read as a clean result.

ANTI-VACUITY, three refusals, because "no findings" and "read nothing" look
...
The success line prints the SHAPE, not just a verdict: rows, source keys, cited
ids, files scanned, and how many rows are superseded. A reader can then notice
when a number collapses.
...
def parse_rows(text):
    ...
    THE TABLE IS THE GRAMMAR. A row is a markdown table line whose first cell is
    an exact id, which is why the separator row, the source-key table above it
    and every sentence of prose in the file are invisible here without a single
...
def corpus_files(root):
    ...
    `git ls-files`, so an untracked scratch file cannot red the tree and a
    deleted one cannot survive in it. THE REGISTER IS ADDED UNCONDITIONALLY when
    it exists on disk, because on the change that introduces it the register is
    still untracked and `ls-files` does not list it -- which would have left the
    citation count at 1 on the very run that added twenty-one rows, a number one
    step from the vacuity floor for a reason that has nothing to do with the
    tree.
    """
    out = []
...
def added_supersessions(root, base):
    ...
    Reads two COMMITTED versions of the register through `git show`, which no
    working tree can rewrite. Returns None when there is no usable base, so the
    caller can SKIP loudly rather than report a clean R5.
    """
    if not base:
        return None
...
def resolve_base(root):
    ...
    The same ladder `check_plan_citations.resolve_base` walks, and for the same
    reason: `GITHUB_BASE_REF` is a bare branch name in CI, the fork point is what
    R5 must compare against rather than the base tip, and a checkout with no
    remote still has to answer. `DECISION_IDS_BASE` is the test seam and is taken
    LITERALLY, so a control can pin an exact commit.
    """
