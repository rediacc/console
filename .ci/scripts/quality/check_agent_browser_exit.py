...
and `python3 -m rediacc_ci.quality.agent_browser_exit` works but is the wrong registration
...
`.ci/scripts/quality/check-agent-browser-exit.sh` by an awk range over its
...
the twin's shape and is carried as an absence: the manifest already places this
step in `quality-static`, so a `lane:` here would be a new claim.

NO `id:` IS CORRECT HERE, checked rather than assumed: `derivedId`
(`gate-header.ts:260`) maps this basename to `check:ci-agent-browser-exit`,
...
because `agent_browser_exit.main(["--selftest"])` exits 0.
...
THIS ENTRY POINT IS INVISIBLE TO THE GATE IT REGISTERS, and that was checked
rather than assumed, because a new file landing in the swept directory is
exactly how a cutover changes a verdict by accident. The twin's sweep is
`grep -rl --include='*.sh'` (`check-agent-browser-exit.sh:55`), so a `.py` file
in that directory is not enumerated at all. The twin's self-exclusion
`grep -vF 'check-agent-browser-exit.sh'` and the port's `SELF_NAME` constant
therefore still name only the bash twin, correctly, and neither needs to grow a
second entry.
...
DRIVEN RED AS WELL, AND THE FIRST PLANT DID NOT FIRE. That is worth writing down
because the CONTROL was wrong, not the gate, which is the same shape that cost
batch 6 two plants. The first probe was a shell script under `.ci/scripts/`
containing a bare `agent-browser open "$url"`, and BOTH sides stayed green. The
cause is `check-agent-browser-exit.sh:31`: the sweep skips any file that does
not match `^[[:space:]]*set[[:space:]]+-[a-z]*e`, deliberately, because only a
script that would DIE on a non-zero status has a load-bearing exit code. The
probe had no `set -e`, so it was never a subject. Suspecting the gate at that
point would have been wrong in both directions: the gate was right, and the
probe was not in its corpus.

The corrected probe carries `set -euo pipefail`, and the control was then proved
BEFORE either side ran, on both preconditions separately: the file appears in
