...
# `scripts/gen-gates-lock.ts` and kept faithful by `check:ci-gates-lock`, which fails
...
[[ -f "$LOCK" ]] || fail "check-gate-id-convention: $LOCK not found; refusing to pass while measuring nothing. It is generated by scripts/gen-gates-lock.ts and guarded by check:ci-gates-lock."
...
try:
    ...
    print(f"PARSE: {lock_path} is not readable JSON ({exc}); cannot check anything. "
          f"It is generated by scripts/gen-gates-lock.ts; run check:ci-gates-lock.")
    raise SystemExit(0)
...
if [[ -n "$REAL_OUT" ]]; then
    ...
    echo "  manifest.ts by scripts/gen-gates-lock.ts; if it is stale, check:ci-gates-lock" >&2
