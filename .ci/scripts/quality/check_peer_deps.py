...
`selftest: true` IS INERT HERE and is carried anyway. `headerLines` only emits that field for `.ts` (`gate-bind.ts:598`, `selftestIsReal`), so for a `.py` gate
it decides nothing; the registered command stays the bare path. It is copied
because the twin declared it and because it happens to be TRUE of this port: `peer_deps.main(["--selftest"])` runs a real control battery.
