...
see it; the logic lives in `rediacc_ci.quality.probe_parity`, which pytest and
...
this port exit 0 with byte-identical stdout (EMPTY, both of them) and
byte-identical stderr (286 bytes). THE EMPTY STDOUT IS THE POINT OF CAPTURING
THEM SEPARATELY: this gate writes its whole verdict, pass line included, to
stderr, so a comparison that merged the streams would have compared the same
bytes twice and a comparison that only read stdout would have compared nothing.

DRIVEN RED AS WELL. In a fixture root holding a copy of `.ci` and copies of the
two real subjects, one call to a verb the preflight never exercises was planted
in the consumer:

    execFileSync('keyctl', ['link', '@u', '@s'])

Both sides exit 1, print "The keyring preflight does NOT exercise: link", and
their stdout and stderr are byte-identical.
...
and `python3 -m rediacc_ci.quality.probe_parity` works but is the wrong
...
`emit:`, no `blocker:`, no `lane:` -- which is the twin's shape, not an
omission. This gate IS emitted, so `gate:bind --write` owns its step in
`quality-static` and rewrites that step's `run:` from the twin's path to this
file's path in the same pass.

INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-probe-parity.sh` is NOT
