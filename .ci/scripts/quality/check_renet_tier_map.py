...
and `python3 -m rediacc_ci.quality.renet_tier_map` works but is the wrong
...
retyped from memory. THIS IS THE ODD ONE OF THE SIX and the one where a
field-for-field diff earns its keep: the twin carried `kind`, `id`, `blocker`
and `needs`, and carried NO `step:`, NO `selftest:` and NO `lane:`. Every one of
those four presences and three absences is deliberate.
...
`needs: go, submodules` IS LOAD-BEARING AFTER THE MOVE. `bind()` unions the
declared needs with `inferredNeeds(source)`, and the twin got both for free from
its own body (`go test` matches the go pattern, `private/renet` the submodule
one). This entry point contains neither string outside its docstring, and
`inferredNeeds` strips Python docstrings as prose (`gate-header.ts:300`), so it
