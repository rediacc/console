...
# check:ci-audit-coverage is now registered to the Python port's entry point,
# .ci/scripts/quality/check_audit_coverage.py, so a header here would declare a
# registration that has moved and gate-bind refuses that by name:
#   package.json runs "...check_audit_coverage.py" but its header derives "...check-audit-coverage.sh"
