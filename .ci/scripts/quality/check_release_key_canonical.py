...
and `python3 -m rediacc_ci.quality.release_key_canonical` works but is the wrong
...
`.ci/scripts/quality/check-release-key-canonical.sh` with an awk range over its
`---- gate ----` block and diffed line by line against this one. The twin
carried exactly four fields -- `step`, `needs`, `selftest`, `lane` -- and no
`emit:`, no `blocker:`, no `id:`, no `run:`, no `kind:`, no `why:`. The step
sits INSIDE the `# >>> gate-bind` region of `quality-security`
(ci-quality.yml:1828), which is why inventing an `emit: false` here would be
wrong rather than merely extra.

NO `id:` IS CORRECT HERE, checked rather than assumed: `derivedId`
(`gate-header.ts:260`) maps this basename to `check:ci-release-key-canonical`,
the manifest id.
...
because `release_key_canonical.main(["--selftest"])` exits 0.
...
entry point infers nothing, because `inferredNeeds` strips Python docstrings as
prose (`gate-header.ts:301`). Both resolve to the empty set `needs: none`
declares. GPG IS NOT A DECLARABLE NEED and its absence is not an oversight: the
module probes for the binary and refuses loudly with the install command in the
message ("gpg is not installed, so nothing here was verified ... sudo apt-get
install -y gnupg"), rather than folding a missing tool into a pass.
...
Byte-identical despite twelve controls that each generate a throwaway RSA key,
weld it, repair it and sign with it: the key material never reaches either
stream, and both sides print the same twelve control lines plus the same
"(throwaway key; the real one is a secret and is deliberately out of scope)".
Nothing was normalised.

DRIVEN RED AS WELL, against a HARD-COPIED FIXTURE rather than the live tree,
because both implementations honour `RELEASE_KEY_ROOT`. The fixture was built
with `cp -r` and checked with `find -type l` for zero symlinks, for the reason
batch 4 paid for: `mutate-check.sh:122` resolves symlinks with
`realpath --relative-to`, so a linked fixture makes a runner write into the real
tree and then shows twin and port AGREEING because both read the same corrupted
file.

The plant drops the `|| canon_rc=$?` guard from the canonicaliser call site in
`build-linux-pkg.sh`, which under `set -e` is exactly the shape that turns the
canonicaliser's REPAIRED signal (exit 10) into an aborted build. Both sides exit
1 with BYTE-IDENTICAL 638-byte stdout and BYTE-IDENTICAL 132-byte stderr:
