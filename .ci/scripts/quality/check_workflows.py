...
THE HEADER BELOW IS THE TWIN'S, FIELD FOR FIELD. It was extracted from `.ci/scripts/quality/check-workflows.sh` PROGRAMMATICALLY, de-commented, and diffed
as an ordered list of WHOLE LINES against the block in this docstring; the diff
is empty over 5 line(s). The twin carried exactly these fields, in this order: `step`, `needs`, `selftest`. NO `id:`, NO `emit:`, NO `lane:`, all three absent in the twin. The step sits INSIDE a `# >>> gate-bind` region in `ci-quality.yml` (job quality-code), so it is emitted and needs no hand-written-step suppression. The pilot lost `emit: false` plus a blocker off a header and
SEVEN GATES PASSED ANYWAY, because `emit: false` suppresses only the three workflow-region checks (`gate-bind.ts:1724`) while the registration assertions above them still ran.
...
`node`, from its own body from its body and this two-import entry point infers nothing, because `inferredNeeds` strips Python docstrings as prose (`gate-header.ts:301`). Both sides bind to `needs: ['node']`.
...
fixture is one `probe.yml` whose `run:` block carries 16 logic lines against a cap of 8. BOTH DIRECTIONS on the same seam: the fat block reds both sides identically, and a second fixture whose step runs one thin command greens both sides identically, so the red is the plant's and not the fixture's. Both sides -> exit 1, stdout 276 bytes, sha256 cd8229d78325ce47..., stderr 219
bytes, sha256 369083d9543b466a....
