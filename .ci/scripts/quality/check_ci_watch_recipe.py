...
THE HEADER BELOW IS THE TWIN'S, FIELD FOR FIELD. It was extracted from `.ci/scripts/quality/check-ci-watch-recipe.sh` PROGRAMMATICALLY, de-commented, and diffed
as an ordered list of WHOLE LINES against the block in this docstring; the diff
is empty over 9 line(s). The twin carried exactly these fields, in this order: `step`, `emit`, `blocker`, `needs`, `id`, `selftest`, `lane`. THE `id:` LINE IS LOAD-BEARING AND IS NOT DERIVABLE. This basename is `check_ci_watch_recipe.py`, so `derivedId` (`gate-header.ts:260`) yields `check:ci-ci-watch-recipe`, with `ci-` twice. The registered id is `check:ci-watch-recipe`. The twin
carried the same override for the same reason
