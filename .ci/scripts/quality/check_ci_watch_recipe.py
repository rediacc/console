...
see it; the logic lives in `rediacc_ci.quality.ci_watch_recipe`, which pytest and the port's
...
on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.ci_watch_recipe`
...
`.ci/scripts/quality/check-ci-watch-recipe.sh` PROGRAMMATICALLY, de-commented, and diffed
as an ordered list of WHOLE LINES against the block in this docstring; the diff
is empty over 9 line(s). The twin carried exactly these fields, in this
order: `step`, `emit`, `blocker`, `needs`, `id`, `selftest`, `lane`. THE `id:` LINE IS LOAD-BEARING AND IS NOT DERIVABLE. This basename is
`check_ci_watch_recipe.py`, so `derivedId` (`gate-header.ts:260`) yields
`check:ci-ci-watch-recipe`, with `ci-` twice. The registered id is
`check:ci-watch-recipe`. The twin carried the same override for the same reason
and it is copied verbatim; dropping it would have renamed the gate.
...
The plant is rule C's banned invocation (`gh run watch ... --exit-status
--interval`) appended to the TRACKED file `.claude/commands/ask.md`. Tracked is
required, not incidental: `scan_files` enumerates with `git ls-files`, so an
untracked probe would never have been read and the plant would have looked like
a gate that cannot fail. The file was restored from a `cp` taken before the
plant and its sha256 compared byte for byte with the pre-plant value
(472a1877bc9e57da...), identical.
Both sides -> exit 1, stdout 716 bytes, sha256 cf9603c7f3f927f3..., stderr 84 bytes, sha256 8631886f416f29fd....

INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-ci-watch-recipe.sh` is NOT deleted
by this change. It stays on disk as the differential twin that
`.ci/rediacc_ci/tests/test_quality_ci_watch_recipe.py` compares this port against, and
