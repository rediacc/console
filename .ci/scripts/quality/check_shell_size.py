...
WHY AN ENTRY POINT AT ALL: `check_npmrc.py` states both measured reasons. A port cannot be run by path (nothing puts `.ci` on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.shell_size` works but is the wrong registration because `check:ci-parity`'s tokenizer cannot read `-m` and resolves the leaves to `[python3]`.

THE HEADER BELOW IS THE TWIN'S, FIELD FOR FIELD, extracted from `.ci/scripts/quality/check-shell-size.sh` by an awk range over its `---- gate ----` block, de-commented, and diffed as an ordered list of whole lines against the block in this docstring. The twin carried exactly SIX fields in this order: `step`, `emit`, `blocker`, `needs`, `selftest`, `lane`. No `id:`, no `run:`, no
`kind:`, no `why:`. All six are here and the `blocker:` is byte for byte the twin's, because it is a live suppression reason under the BLOCKER convention and a suppression whose reason went missing in a file move is a quiet exemption. The pilot lost exactly `emit: false` plus a blocker off a header and SEVEN GATES PASSED ANYWAY: `emit: false` suppresses only the three
workflow-region checks (`gate-bind.ts:1724`) while the registration assertions above them still ran and still agreed.

NO `id:` IS CORRECT HERE, checked rather than assumed. `derivedId` (`gate-header.ts:260`) strips the `check_` prefix and maps `_` to `-`, so this basename derives `check:ci-shell-size`, which is the manifest id.
...
directive. It is deliberately UNTRACKED, which exercises the half of the twin's discovery that `git ls-files` alone would miss: the gate unions `ls-files '*.sh'` with `ls-files --others --exclude-standard '*.sh'` precisely because a file is being grown before it is committed. THE CONTROL WAS PROVED BEFORE EITHER SIDE RAN, by running that same union and confirming the plant appears
in it, because a probe the sweep never walks is how a plant fails to fire for reasons that say nothing about the gate.
