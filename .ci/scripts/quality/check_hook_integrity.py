...
and `python3 -m rediacc_ci.quality.hook_integrity` works but is the wrong registration
...
`.ci/scripts/quality/check-hook-integrity.sh` by an awk range over its
...
no `run:`, no `kind:`, no `why:`. The `blocker:` is byte for byte the twin's and
is NOT the shared boilerplate the other `emit: false` gates carry: this one
names a reason specific to this gate, that its subject IS the setup path, so
gating it on setup succeeding would silence the gate that explains a broken
setup. A blocker replaced by a lookalike is the same loss as a blocker deleted.

`emit: false` IS CONFIRMED BY ARITHMETIC, not by trusting the field: the step is
hand-written at ci-quality.yml:662 and the `quality-code` gate-bind region runs
739-916, so 662 is outside it.

THIS IS THE ONE ROW IN THE BATCH WITH A HAND-WRITTEN STEP THAT NAMES THE TWIN BY
PATH. ci-quality.yml:663 reads `run: .ci/scripts/quality/check-hook-integrity.sh`
rather than going through `npm run`. `gate-bind` matches a step by NAME only
(`gate-bind.ts:1749`) and never reads its `run:` line, so after the registry is
flipped that step stays green while CI keeps executing the TWIN. Every file
under `.github/workflows/` was grepped for this basename and 663 is the only
...
to `check:ci-hook-integrity`, which is the manifest id.

`selftest: true` is inert for a `.py` gate (`gate-bind.ts:598`) and is carried
because the twin declared it and because `hook_integrity.main(["--selftest"])`
exits 0.
...
ITS `paths:` DOES NOT SELECT THIS NEW LEAF, and this is the batch's only
instance of the shape that caught `check:ci-shell-size` on 2026-09-08. The
manifest entry lists `.ci/scripts/quality/check-hook-integrity.sh` literally,
alongside `.claude/hooks/**` and three `scripts/data/*.json` files. A literal is
not a glob, so once the leaf becomes `check_hook_integrity.py` the gate's own
...
PINNED BY PATH IN ONE PLACE, and it is a DIFFERENTIAL.
`.ci/rediacc_ci/tests/test_quality_hook_integrity.py:33` sets
`TWIN = paths.from_root(".ci", "scripts", "quality", "check-hook-integrity.sh")`
and MUST KEEP NAMING THE TWIN. Nothing under `.ci/scripts/test/gates/` names
this gate.
...
an unchanged tree and is byte-stable against itself on both streams. Neither
side emits colour to a redirected stream here, because this twin gates its
escapes on `[ -t 1 ]` (check-hook-integrity.sh:103) rather than assigning them
unconditionally, and the port agrees.

DRIVEN RED AS WELL, in a `cp -r` fixture root rather than on the real tree, since
this gate has no input override. The fixture carries the twin at its own relative
path (so its `$BASH_SOURCE` root is the fixture), the three
`scripts/data/hook-*.json` files, `.claude/hooks/` and `.claude/rediacc_hooks/`;
the port is aimed at it with `REDIACC_CI_ROOT`. `cp -r`, and `find -type l`
confirms ZERO symlinks in the fixture, for the reason `mutate-check.sh:122`
records. The clean fixture was driven FIRST and both sides exited 0 with
identical bytes.

The plant DELETES one guard file from the fixture,
`.claude/hooks/post-bash/cancel-old-ci.sh`.

THE CONTROL WAS PROVED ON BOTH OF ITS PRECONDITIONS BEFORE EITHER SIDE RAN,
because a file that was outside either list would have been deleted with nothing
noticing, which is the exact shape of a plant that fails for reasons that say
nothing about the gate: the victim is present in
`scripts/data/hook-inventory-baseline.json`, and it sits inside a directory
declared in `hook-audit-scope.json`'s `guard_dirs`. The real tree's copy was
also re-confirmed present after the fixture's was removed.
