...
see it; the logic lives in `rediacc_ci.quality.devcontainer_scripts`, which pytest and the port's
...
on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.devcontainer_scripts`
...
`.ci/scripts/quality/check-devcontainer-scripts.sh` PROGRAMMATICALLY, de-commented, and diffed
...
order: `step`, `emit`, `blocker`, `needs`, `selftest`, `lane`. NO `id:`, and that is checked rather than assumed: `derivedId`
(`gate-header.ts:260`) strips the `check_` prefix and maps `_` to `-`, so this
basename derives `check:ci-devcontainer-scripts`, which is the manifest id.
...
The plant is an UNTRACKED `.devcontainer/__gate_probe_devcontainer_scripts.sh`
carrying `git clone ... 2>/dev/null`, i.e. rule A's exact shape: a PRIMARY
operation whose stderr is discarded. Untracked is the right shape here because
the gate's corpus is the glob `"$DC"/*.sh`, not `git ls-files`, so a probe the
sweep never walks would have been a control failure rather than a gate failure.
...
Both sides -> exit 1, stdout 314 bytes, sha256 c83e1b98f1ba9d4a..., stderr 232 bytes, sha256 b47774f79c5278be....

INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-devcontainer-scripts.sh` is NOT deleted
by this change. It stays on disk as the differential twin that
`.ci/rediacc_ci/tests/test_quality_devcontainer_scripts.py` compares this port against, and
