...
WHY AN ENTRY POINT AT ALL: `check_npmrc.py` states both measured reasons. A port cannot be run by path (nothing puts `.ci` on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.content_quality` works but is the wrong registration because `check:ci-parity`'s tokenizer cannot read `-m` and resolves the leaves to `[python3]`.

THE HEADER BELOW IS THE TWIN'S, FIELD FOR FIELD, extracted from `.ci/scripts/quality/check-content-quality.sh` by an awk range over its `---- gate ----` block, de-commented, and diffed as an ordered list of whole lines against the block in this docstring. The twin carried exactly FOUR fields in this order: `step`, `needs`, `selftest`, `lane`. No `emit:`, no `blocker:`, no `id:`, no
`run:`, no `kind:`, no `why:`.

NO `id:` IS CORRECT HERE: `derivedId` (`gate-header.ts:260`) maps this basename to `check:ci-content-quality`, which is the manifest id.

`selftest: true` is inert for a `.py` gate (`gate-bind.ts:598`) and is carried because the twin declared it and because `content_quality.main(["--selftest"])` exits 0.
...
THIS ENTRY POINT COULD HAVE ENTERED THE GATE'S OWN CORPUS, and that was the first thing checked, because this gate sweeps prose for AI-slop patterns and a docstring this long is prose. It does not: the corpus is content under the documentation and marketing trees, not `.ci/scripts/quality/`. Verified behaviourally rather than by reading the glob, by the file count in the gate's
shape line being identical on both sides of the cutover with all nine of this batch's entry points already on disk. That matters more here than elsewhere: a corpus that silently grew or shrank across the cutover would make the differential meaningless.

PINNED BY PATH NOWHERE THAT RUNS IT. The only reference outside the workflow is `.ci/rediacc_ci/tests/test_quality_content_quality.py:38`, a comment recording that the exemption program is copied verbatim from `check-content-quality.sh` lines 111-118, and that module is the DIFFERENTIAL, which must keep naming the twin regardless. Nothing under `.ci/scripts/test/gates/` names this
gate.

DRIVEN, on this tree, both streams captured SEPARATELY, `CI=true` on both.
THIS IS THE ONE PAIR IN THE BATCH THAT IS NOT BYTE-IDENTICAL, and the difference is ORDER ONLY. Stated plainly rather than folded into a pass:
...
So the VERDICT, the corpus, the finding set and the exit code all agree, and the print order does not. A reader deciding whether to flip the registry on this row should know that "byte-identical stdout" is NOT true here, which is why it is the first thing this section says.

DRIVEN RED AS WELL, and the red side IS byte-identical, because the twin takes FILE ARGUMENTS (`check-content-quality.sh:283`) and so does the port (content_quality.py:476), so naming one fixture file removes enumeration from the comparison entirely. The plant is a two-line addition to a copy of a clean fixture document.

THE CONTROL WAS PROVED BEFORE EITHER SIDE RAN: the clean fixture was driven first and exited 0, and the planted line was confirmed to sit OUTSIDE the frontmatter and OUTSIDE any fence, which are the two exemptions that would have made the plant invisible while looking like a gate that could not fail.
