...
and `python3 -m rediacc_ci.quality.label_inventory` works but is the wrong registration
...
`.ci/scripts/quality/check-label-inventory.sh` by an awk range over its
...
`kind: test` IS THE LOAD-BEARING ONE, and the `blocker:` under it is 559 bytes
of live suppression reason carried byte for byte. Grepped every file under
`.github/workflows/` for `check-label-inventory.sh`: ZERO hits, which is what
`kind: test` predicts. This gate reaches CI only through
`.ci/scripts/test/gates/test-label-inventory.sh` inside the shared
"Quality-gate unit tests" step, so that blocker is the whole of its CI story and
losing a clause of it would be a quiet exemption.

NO `selftest:` IS CORRECT, carried exactly as found: the twin did not declare
one, and declaring one here would be a new claim rather than a moved one.

NO `id:` IS CORRECT HERE: `derivedId` (`gate-header.ts:260`) maps this basename
to `check:ci-label-inventory`, which is the manifest id.
...
BECAUSE THIS GATE IS `kind: test`, the first bullet is not cosmetic: while
`test-label-inventory.sh:34` still names the `.sh`, CI executes the TWIN and
only the local `npm run` executes the port. Repointing is the driver's call and
is called out in the report rather than left in a diff.
...
THAT RUN READS THE NETWORK, which is worth saying because a network read is the
classic source of an unstable stream. It was byte-stable across two runs of the
twin on both streams, and both sides reached the same live list. No
normalisation was applied.

DRIVEN RED AS WELL, and the red run is deliberately OFFLINE, through the twin's
own two injection seams which the port reads under the same names:
`LABEL_INVENTORY_LABELS_FILE` for the declaration file and
`LABEL_INVENTORY_LIVE_FILE` for the live list. Removing the network from the
comparison is the point: a red that depended on a live read would be a claim
about GitHub as much as about the gate.

THE CONTROL WAS PROVED IN BOTH DIRECTIONS BEFORE THE PLANT RAN. A clean copy of
`.github/labels.yml` paired with a 29-name live list exits 0 on the twin, which
proves the two seams are not themselves what reds; the planted copy is the same
file with exactly ONE label block deleted (`wontfix`), so the declared side
carries 28 against a live 29.
