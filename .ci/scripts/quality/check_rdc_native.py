...
entry point where `scripts/gate-bind.ts` can see it. There is no bash twin: this gate is
NEW, written in the same change that moved the SEA build out of `rdc.sh`, so invariant 5
has nothing to keep.
