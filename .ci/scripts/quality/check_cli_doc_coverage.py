...
CUT OVER FROM BASH 2026-09-07 (W7 P4), on the condition this docstring used to
be waiting for. Measured that day:
...
this port exit 0 with byte-identical stdout and byte-identical stderr. The
failure direction was driven too, since two greens prove nothing about a gate:
a fixture copy of the tree with every mention of `--until-final` stripped out
of `.claude/skills/ci-watch/SKILL.md` turns both sides red, exit 1, again byte
for byte.

WHY AN ENTRY POINT AT ALL, rather than registering the module. Both reasons are
measured, and `check_npmrc.py` records them for the pilot:
