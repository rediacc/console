...
# THE ONE EDIT, per the driver contract: all three pytest roots land together rather
# than one per workstream. Six test files under .ci/rediacc_ci/tests and the shellscan
# differential under .claude/rediacc_hooks/tests were invisible to this gate until now,
# which is the shape it exists to catch -- a test file nothing runs.
SEARCH_DIRS = (
    ".claude/hooks",
...
#
# `scripts/ci-runner` USED TO BE IN THIS TUPLE and was removed on 2026-09-06
# (W2.4a). It was here for exactly one reason -- to text-match a basename inside
# the 5,700-line TypeScript literal in manifest.ts -- and registration is not a
# text fact, it is a structured one. See manifest_reached() below, which reads
# the committed JSON projection instead. The two were run side by side over the
...
# The manifest's machine-readable projection. Generated by scripts/gen-gates-lock.ts,
# kept faithful to scripts/ci-runner/manifest.ts by check:ci-gates-lock.
    ...
    for entry in parsed:
        ...
        for value in values:
            if isinstance(value, str):
                # A `run` is a command line, not always a bare path: split it so
                # `bash x/y.sh --flag` still contributes `x/y.sh`.
                toks.add(value)
...
def main():
    ...
    for f in files:
        ...
        if f.name in EXEMPT:
            exempted += 1
            continue
        # Reached by COLLECTION, not by mention. Not an exemption: the file must
        # still sit under a root pytest is configured to collect, and
        # check:ci-pytest runs that collection in CI and floors it.
        if rel.endswith(".py") and any(rel.startswith(r) for r in roots):
            collected += 1
            continue
        # Reached by REGISTRATION. `npm run ci` dispatches every manifest entry
        # through scripts/ci-runner, so an entry naming this file IS a path to CI,
        # and it is the path 115 of the 164 subjects take.
