...
`needs: none` IS LOAD-BEARING AND IS NOT A NO-OP HERE. `bind()` unions the declared needs with `inferredNeeds(source)`, and the twin's own body carries
`go build`-shaped strings; this two-import entry point carries none outside its
docstring, and `inferredNeeds` strips Python docstrings as prose (`gate-header.ts:300`), so it infers NOTHING. Declaring `none` verbatim keeps
the declared side identical; what the twin inferred for free is discussed under
DRIVEN, because the gate itself probes for `go` and answers 2 when it is absent rather than pretending to have checked.
