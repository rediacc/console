...
on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.toolchain_pins`
...
`.ci/scripts/quality/check-toolchain-pins.sh` PROGRAMMATICALLY, de-commented, and diffed
as an ordered list of WHOLE LINES against the block in this docstring; the diff
is empty over 8 line(s). The twin carried exactly these fields, in this
order: `step`, `emit`, `blocker`, `needs`, `selftest`, `lane`. NO `id:`: the basename derives `check:ci-toolchain-pins`, the manifest id. The
`blocker:` is 233 characters and moved byte for byte, because it is a live
suppression reason under the BLOCKER convention and a reason lost in a file move
is a quiet exemption.
...
The plant is an UNTRACKED `.ci/scripts/quality/__gate_probe_toolchain_pins.sh`
that runs `shellcheck` without ever resolving it at the pin, which is A6's
shape. Untracked is deliberate and is what the plant is FOR: A6 unions
`git ls-files` with `ls-files --others --exclude-standard` precisely because a
gate not yet committed was once invisible to it. THE CONTROL WAS PROVED BEFORE
EITHER SIDE RAN, by running that same `--others` enumeration and confirming the
probe appears in it. The plant was removed with `rm` and `git status
--porcelain` diffed against its pre-plant capture with no difference.

THE PORT NEEDED ONE FIX TO REACH PARITY, and it is recorded here rather than
left implicit. The first differential run produced the SAME 21 lines and the
SAME 1468 bytes on both sides in a DIFFERENT ORDER: the port ran every control
from one tail block, while the twin prints the two A8 controls between A8 and A6
(twin lines 181-197) and the A9 control right after A9 (twin line 286). Same
byte count, different bytes, and comparing lengths would have called it equal.
`run_controls` was therefore split into `run_a8_controls`, `run_a9_control` and
`run_controls`, called at the twin's positions, with the shared fixture-dir
`mkdir` moved into the A8 function so the twin's own "mkdir first" property
survives the split.
Both sides -> exit 1, stdout 1239 bytes, sha256 43d6c2425a570014..., stderr 109 bytes, sha256 e22bbde06b531536....

INVARIANT 5 IS INTACT: `.ci/scripts/quality/check-toolchain-pins.sh` is NOT deleted
by this change. It stays on disk as the differential twin that
`.ci/rediacc_ci/tests/test_quality_toolchain_pins.py` compares this port against, and
