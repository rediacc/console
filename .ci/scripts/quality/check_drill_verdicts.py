...
and `python3 -m rediacc_ci.quality.drill_verdicts` works but is the wrong
...
`[]` (it drives bash only) and `inferredNeeds(this file)` is `[]` too, so both
resolve to the empty set the declaration names.
...
its whole verdict, five lines of it, on stderr. A merged comparison would have
compared the log lines and a stdout-only comparison of merged output would have
compared nothing.

DRIVEN RED AS WELL, which for this pair is the load-bearing half. Planted the
2026-08-05 defect itself into `scripts/drills/lib.sh`: the zero-assertion branch
made to print `PASSED` where it must print `SKIPPED`. Both sides exit 1, both
print an EMPTY stdout, and their stderr is byte-identical at 568 bytes -- the
first of the four verdict assertions reds, the other three still pass, so the
gate is shown discriminating rather than collapsing. Reverted by its exact
inverse and the file verified byte-identical to its pre-plant state, with the
twin back at exit 0.
