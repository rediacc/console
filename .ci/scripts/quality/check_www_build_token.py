...
and driven again on this tree, both streams captured SEPARATELY, the twin and this port exit 0 with byte-identical stdout and byte-identical stderr. Driven RED as well, against a copy of `.github/workflows/` carrying one extra job that runs `npm run build:www` with no `GITHUB_TOKEN`: both sides exit 1 and print the same MISSING line.

THE FIRST ATTEMPT AT THAT CONTROL WAS THE UNFAIR KIND, and it is worth keeping because it found a real defect in the twin. Pointed at a directory holding exactly ONE `.yml`, `check-www-build-token.sh` dies with `line 52: run: unbound variable`: `find_sites()` runs GNU grep, which prints no filename column for a single file operand even under `-r`, so `file` becomes the line number
...
INVARIANT 5 IS DISCHARGED: `.ci/scripts/quality/check-www-build-token.sh` was the differential twin, and W7 P5 retired it; the shadow ledger under `.ci/shadow/` is the licence record.
