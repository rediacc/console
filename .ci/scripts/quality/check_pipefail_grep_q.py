...
pathspec is `scripts/**/*.sh`, and git without `:(glob)` magic reads that as `scripts/` + anything + `/` + anything + `.sh`, so a file at depth one under `scripts/` matches NOTHING. Listing the tracked files under that pathspec printed nothing for it, which is the proof rather than the guess. (The pathspec is quoted here WITHOUT the enumeration command on the same line, because
`check:ci-pathspec-scope` reads a quoted pathspec beside a `ls-files` call as a USE and would flag this prose. That gate is right about the shape and cannot see
either of the two live instances; see the report accompanying this cutover.)
Moved to `scripts/dev/plant.sh` (pipefail set, a local `emit()` producer, and `emit | grep -q needle`) and both sides then exit 1, naming `scripts/dev/plant.sh:6`, with byte-identical stdout and stderr.

THAT PATHSPEC HOLE IS REAL AND IS NOT THIS PORT'S: the twin has it too, which is why the differential stays equivalent. Three tracked shell files sit at depth one under `.claude/hooks/` -- `require-jq.sh`, `require-python.sh`, `test-hooks.sh` -- and none of the three is in this gate's corpus. None of them carries the racing shape today (checked: two do not set pipefail at all, and
