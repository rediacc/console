...
Contract section 5d puts a gate's entry point where `scripts/gate-bind.ts` can see it; the logic lives in `rediacc_ci.quality.command_tree`, which pytest and the port's own `--selftest` import directly.
