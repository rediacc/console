...
WHY AN ENTRY POINT AT ALL: `check_npmrc.py` states both measured reasons. A port cannot be run by path (nothing puts `.ci` on `sys.path`, hence the insert below), and `python3 -m rediacc_ci.quality.battery_clean_tree` works but is the wrong registration because `check:ci-parity`'s tokenizer cannot read `-m` and resolves the leaves to `[python3]`.
...
`needs: none` IS THE MEASURED ANSWER ON BOTH SIDES. `inferredNeeds(twin)` is `[]` and `inferredNeeds(this file)` is `[]`, so the union does not move.
