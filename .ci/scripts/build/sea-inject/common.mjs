...
export function findSentinelFuse(fd, searchLimit, fuse) {
    ...
    if (hits.length === 0) {
        throw new Error(`sentinel fuse ${fuse}:0 not found (not a SEA-capable node, or already injected)`);
    }
    if (hits.length > 1) {
        throw new Error(`sentinel fuse ${fuse}:0 found ${hits.length} times; cannot tell which is the real fuse`);
    }
