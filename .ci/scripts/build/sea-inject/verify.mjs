...
function findPe(fd, resourceName) {
  ...
  for (let i = 0; i < numSections; i++) {
    ...
    const va = secTable.readUInt32LE(o + 12);
    if (va === secRva)
      secRaw = readAt(fd, secTable.readUInt32LE(o + 16), secTable.readUInt32LE(o + 20));
  }
...
function main() {
  ...
  try {
    ...
    if (embeddedHash !== sourceHash)
      fail(`sha256 mismatch: embedded ${embeddedHash} != source ${sourceHash}`);
    ...
    if (onCount !== 1 || offCount !== 0)
      fail(`sentinel fuse state wrong: ${fuse}:1 x${onCount}, ${fuse}:0 x${offCount}`);
