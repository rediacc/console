...
export function injectMacho({ binaryPath, resourceName, blobPath, fuse }) {
  ...
  try {
    ...
    if (magic !== MH_MAGIC_64)
      throw new Error(`not a 64-bit little-endian Mach-O (magic 0x${magic.toString(16)})`);
    ...
    if (segVmAddr < lastMappedSegEnd)
      throw new Error('NODE_SEA vmaddr would overlap a mapped segment');
