...
# synth_tsv <out> <tier> <cpu_ceil> <mem_ceil> <label> <src> <hint> <samples> <cpu_milli> <mem_base>
#
...
# readings identical) does not fire on a synthetic capture.
synth_tsv() {
    local out="$1" tier="$2" cceil="$3" mceil="$4" label="$5" src="$6" hint="$7"
    local n="$8" cpu="$9" mem="${10}"
