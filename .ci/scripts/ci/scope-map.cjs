...
// --------------------------------------------------------------------------- Modules. A module is a subtree with a known job surface. Submodules are modules too: the gitlink path (`private/renet`) and any expanded content path under it (`private/renet/pkg/x.go`) classify identically, so the pointer bump and the later compare-API expansion land in the same bucket.
// ---------------------------------------------------------------------------
const KNOWN_MODULES = [
  ...
  //
  // It gets its OWN module rather than reusing `docs` for the reason the gates entry above states: a future edit that gives docs a job surface must not silently drag an unrelated tree in with it.
  //
  // Without a rule here this tree would be `unclassified` = FULL CI, and
  // STATE.md is rewritten many times in a single session -- exactly the shape that makes a false full run expensive rather than merely conservative.
  'agent',
...
// Root files that shape every job's environment: the npm tree (lockfile is the setup-workspace cache key for EVERY job, VM/E2E legs included), the build
...
// Tutorial docs order and populate the ops tutorial-sequence job (.ci/tutorials/run-sequence.sh derives its list from these files), so they carry the tutorials module ON TOP of being www content.
...
const RULES = [
  ...
  // The tracked agent working-notes root. matchPrefix('agent/') is a PREFIX, not a substring: `agents/x.md` or `agentic/x.md` still falls through to
  ...
  // Runs INSIDE the executable build (.ci/scripts/build/prepare-cli-assets.sh:190, reached from build-cli-executables.sh:123) and its output ships in the CLI binary. A throw reds the ungated build job, but a silently-WRONG credits file is only caught by install_methods/package_tests, which ARE gated.
  {
...
//
// The 8 VM/E2E jobs check out with `submodules: true` and run setup-workspace, so their surface includes packages/shared, packages/provisioning and every submodule pointer (verified against ct-tests.yml). package-lock.json is in
...
const JOB_SURFACES = {
  ...
  renet: ['renet'],
  // The licensing enforcement leg builds renet from source and drives cmd/renet, so renet is its whole surface. Its own harness (.ci/scripts/private/ license-e2e.sh and license-mint/) lives under .ci/, which already forces full CI, so it needs no separate module.
  license_enforcement: ['renet'],
  account_e2e: ['account', 'shared'],
  // The drills leg runs `./run.sh drill universe` + `drill transfer` + `drill backup`: the CLI's own config resolution (`packages/cli`), the shared
  ...
  // triggered by a change to them. A drill that cannot be reached by an edit to the thing it tests is a gate that only fires by coincidence.
...
// A surface naming a module the table cannot produce is rot that silently never matches, i.e. a job that can never re-enter scope. Throw at load.
...
// deleted path is just a path: deletion is a change like any other, cases 19/20 live in the caller's parser). ctx.workflowClosure: Set of '.github/workflows/x.yml' paths reachable from
...
function classify(paths, ctx = {}) {
  ...
  // Case 22: an empty delta means the computation upstream failed, never that nothing changed. Full, not reduced.
