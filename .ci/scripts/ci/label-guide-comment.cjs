...
//
// WHY THIS EXISTS. The repo carries 26 labels, and several of them are kill switches whose effect is invisible unless you already know it exists: `full-ci` forces a full round past the scope engine's skips, `no-cancel-push` disarms the watchdog, `autopilot-blocked` latches the babysit loop off, `rollback` blocks stable promotion. Nobody remembers those, and there is nowhere on a
// PR that says what they do. So each PR gets one comment explaining them, rendered from .github/labels.yml.
//
// NOT ALL 26. An entry may carry `guide: false`, and 14 do: GitHub's stock defaults, bot-applied labels, and the three with zero consumers in code. The complaint that produced this comment was that the roster is too long to remember, so listing `duplicate` and `good first issue` would recreate the problem it solves. Those still have to be DECLARED -- the inventory gate reconciles
// the whole file against the live repo -- they are just not listed. Absent means true, so a new label is visible unless someone opts it out.
//
// WHY IT IS RENDERED, NEVER HAND-WRITTEN. .github/labels.yml is already the single source of truth (check:ci-label-refs enforces code -> declaration, check:ci-label-inventory enforces declaration <-> the live repo). A guide typed by hand becomes a fourth, rotting copy the moment a label changes. This module makes the comment a projection of that file, so it cannot disagree
...
// The comment's identity. It must be the FIRST bytes of the body so a body prefix test is enough to find it -- a marker buried mid-body would also match a human quoting this comment in a reply.
const MARKER = '<!-- rediacc:label-guide -->';

const DEFAULT_LABELS_FILE = path.join(__dirname, '..', '..', '..', '.github', 'labels.yml');

// ANTI-VACUITY FLOORS. Strict parsing (below) rejects malformed lines, but it cannot notice a file that was TRUNCATED to something still well-formed. A guide listing one label is not a guide, so refuse rather than render it.
//
// TWO floors, because there are two ways to end up with an empty guide. The second one appeared with `guide: false`: a parser change that mis-read the field would mark every entry hidden, and MIN_LABELS would still be satisfied by the full declaration set while the rendered table came out empty. So the VISIBLE set carries its own floor, checked at render time.
const MIN_LABELS = 2;
const MIN_GUIDE_LABELS = 2;

// Parse .github/labels.yml without a YAML dependency.
//
// DELIBERATELY STRICT. The tempting version is a `grep`-shaped reader that picks out the lines it recognises and ignores everything else -- and that version degrades SILENTLY: a reindent, a quoting change, or a `- name` on a continuation line simply yields fewer labels, and a short table looks like a perfectly good table. So every non-blank, non-comment line must match one of the
// two shapes below or this throws. The file is ours and its shape is
...
const parseLabels = (text, source) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (entryStart) {
      ...
      // `guide` defaults to TRUE, and the default direction is deliberate: a label that quietly vanishes from the guide is invisible, while one that appears when it need not is merely a row too many. Fail toward showing.
    ...
    if (field) {
      ...
      if (key === 'guide') {
        ...
        // false. Anything else (`no`, `0`, `False`, a typo) is a loud parse failure rather than a coerced truthy string, because coercion here silently flips a label's visibility in the direction nobody notices.
...
//
// `guide: false` entries are declared but not listed. The guide exists because the roster got too long to remember, so padding it with GitHub's stock defaults and bot-applied labels would recreate the problem it solves. They still have to be DECLARED -- check-label-inventory.sh reconciles the whole file against the live repo -- which is why the two sets differ.
...
const renderBody = (labels) => {
  ...
  return [
    ...
    ...rows,
    '',
    // A BLOCKQUOTE, not <sub>. The trailer is the only place that tells a reader their hand-edits will be overwritten and where to change the wording instead, so it has to be readable: <sub> renders it as tiny subscript text, which is where a reader's eye skips. A blockquote reads as a footer at normal size. Every line carries its own '>' rather than relying on markdown's lazy
    // continuation, which is easy to break by editing one line.
...
//
// Otherwise anyone able to comment on a PR could suppress the guide forever by posting an empty comment carrying the marker. GitHub reports app and Actions
...
const postLabelGuide = async ({ github, context, core }) => {
  ...
  // Read and parse BEFORE any API call: a malformed labels file must fail the job loudly, never half-post a truncated guide.
  ...
  // alone rather than deleted, because deleting a comment is not reversible and this module has no business doing it.
