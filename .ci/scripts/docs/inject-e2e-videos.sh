#!/bin/bash
# Download and inject E2E test videos into a pages directory
#
# Downloads the e2e-videos artifact and extracts .webm files into the target
# directory so the documentation site shows actual video recordings instead
# of placeholders.
#
# Modes:
...
#     --artifact-name "e2e-videos-abc123" \
...
#
# Exit codes:
#   0 - Videos injected successfully
#   1 - No videos found or download failed
...
if [[ -z "$OUTPUT_DIR" ]]; then
    log_error "Usage: inject-e2e-videos.sh --output <dir> [--run-id <id> --artifact-name <name>] [--search-latest]"
    exit 1
fi
...
if [[ -n "$RUN_ID" && -n "$ARTIFACT_NAME" ]]; then
    ...
    # Mode 2: Cross-workflow — find the most recent e2e-videos artifact
    log_step "Searching for latest e2e-videos artifact..."
    ARTIFACT_ID=$(gh api "repos/${GITHUB_REPOSITORY}/actions/artifacts?per_page=30" \
        --jq '[.artifacts[] | select(.name | startswith("e2e-videos-")) | select(.expired == false)] | sort_by(.created_at) | last | .id // empty' 2>/dev/null || echo "")
else
...
# Count .webm files
VIDEO_COUNT=$(find "$DOWNLOAD_DIR/extracted" -name "*.webm" -type f 2>/dev/null | wc -l)

if [[ "$VIDEO_COUNT" -eq 0 ]]; then
    log_warn "Artifact contained no .webm files"
    exit 1
fi

# Copy to output directory
mkdir -p "$OUTPUT_DIR"
find "$DOWNLOAD_DIR/extracted" -name "*.webm" -type f -exec cp {} "$OUTPUT_DIR/" \;

log_info "Injected $VIDEO_COUNT video(s) into $OUTPUT_DIR"
