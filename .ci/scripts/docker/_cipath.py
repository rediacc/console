...
to be the only other one before it was written. A shared import cannot work here, and the reason is the same one that makes the hop necessary at all: what a path invocation puts on `sys.path[0]` is THE SCRIPT'S OWN DIRECTORY. For an entry point in `.ci/scripts/docker/` that is `.ci/scripts/docker`, so `import _cipath` can only ever resolve to a file sitting beside it. A module that
could be imported from the quality directory would need `.ci` on the path already, which is exactly what has not happened yet.
...
WHY THE HOP IS NEEDED. Nothing puts `.ci` on `sys.path` for a path invocation. Run `python3 .ci/rediacc_ci/docker/retag_image.py` and it dies at `from rediacc_ci import log, proc` before doing any work. That `sys.path[0]`
behaviour is switched off by `-P`, `-I` and `PYTHONSAFEPATH`; none of the three
is used anywhere in this tree, and a script started under any of them would fail loudly at this import rather than quietly skip work.
