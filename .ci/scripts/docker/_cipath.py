...
`scripts/ci-runner/manifest.ts` entry, and no `gates.lock.json` row. The reason a file at a path is still needed is the call sites themselves -- a workflow step runs `.ci/scripts/docker/create_manifest.py --image renet` today, and the flip replaced one path string with another, not with a `python3 -m` invocation that would need `PYTHONPATH` set in every job that uses it.
