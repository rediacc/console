...
THE SECOND HALF OF THE TWIN. `.ci/scripts/lib/common.sh` is two libraries in one file: lines 1-514 are the refuse-early base library every script sources (ported next door in `core.common`), and lines 516-772 are this -- the review budget, thirteen functions and four constants that decide whether the review workflow may spend money on another pass.
...
==========================================================================
`review_spend_total` -- the CAP NUMERATOR -- used to RETURN 0 WITH EXIT 0 WHEN `gh` FAILED. Driven against the live twin with a `gh` on PATH that writes "HTTP 403: API rate limit exceeded" to stderr and exits 1, BEFORE the fix:
...
Each was `gh ... 2>/dev/null | ... || true`. `pipefail` is on (common.sh:11), so the pipeline did fail -- and `|| true` threw that away AFTER `wc -l` had already printed `0`.

WHY THIS WAS NOT MERELY A BUG BUT A CONTRADICTION. One hundred and sixty lines above, in the same file, `_gh_probe`'s own header (common.sh:424-429) condemns exactly this spelling:
...
BLAST RADIUS, MEASURED RATHER THAN ESTIMATED. Four live call sites, all behind a workflow:
...
Two consequences the bug used to have, and the second is the one that would have cost a human:
...
THE SWALLOW WAS NOT ONLY ABOUT `gh`. Driven the same day, with `gh` working but `GITHUB_REPOSITORY` unset:
...
four failing-`gh` drives above now exit nonzero with the retry/error text `_gh_probe` already produces, and a genuinely empty (not failed) result still answers `0`/empty correctly -- checked separately, because a naive fix piping through a here-string (`wc -l <<<"$out"`) would count an empty result as ONE line, not zero, the same trap `blocker-validator.sh`'s anti-vacuity check
fell into elsewhere in this file family. `report_count`, `attempt_ states`, `spent_attempt_count` and `spend_total` here already raised `core.ghx.GhError` rather than answering 0 -- the PORT was ahead of the twin, not behind it -- so this fix makes the two sides AGREE rather than requiring any change here. `test_core_review_budget.py` pins the agreement on both sides now, in place
of the divergence it used to pin.

`pr_diff_loc` is UNCHANGED, DELIBERATELY, because common.sh:764-765 states its 0 as a decision: "Failing to 0 puts an unreadable PR in the SMALLEST bucket, which is the conservative direction: it spends fewer review passes, never more." That reasoning holds on its own terms once the numerator (the three functions above) is trustworthy again, which it now is.
...
==========================================================================
`review_chargeable_attempts` computes `$((n - REVIEW_FREE_REATTEMPTS_PER_HEAD))` (common.sh:714). Under `nounset`, bash arithmetic treats a non-numeric word as a VARIABLE NAME, so a states row of `a<TAB>zz<TAB>unknown` dies:

    common.sh: line 717: zz: unbound variable

LATENT, and the reason it is latent is worth recording so nobody "fixes" the awk by accident: the producer is `review_attempt_states`, whose awk guards the
...
hand-built states string reaches the crash. Reproduced as a `ValueError` here rather than as an abort.
...
def cap_for(changed_lines: str | int | None) -> int:
    ...
    becomes 0 and lands in the SMALLEST tier. `abc`, `""`, `-5` and `1e3` all answer 3 (driven). That is the conservative direction for this number and it is the twin's, so it is reproduced without comment beyond this one.

    The bands are inclusive at the top: 10000 -> 3, 10001 -> 5, 50000 -> 5, 50001 -> 7 (driven at every boundary). common.sh:526-528 records that the 50k-100k band lands in the TOP bucket deliberately, "a 60,000-line diff is not meaningfully easier to review than a 100,000-line one".
    """
...
def class_is_infra(cls: str) -> bool:
    ...
    An INFRA-CLASS failure is not a verdict on the code (common.sh:642-648): the harness ran out of turns or fell over, so the same head earns REVIEW_FREE_REATTEMPTS_PER_HEAD retries. Anything else, INCLUDING an empty
...
def parse_attempt_states(raw: str) -> list[AttemptState]:
    ...
    SEPARATED FROM THE FETCH ON PURPOSE. The awk is pure and the `gh` call is not, and it is the awk that carries every surprising rule below. Every one was driven by piping fixtures through the twin's own awk program on 2026-09-10.
...
def chargeable_attempts(states: list[AttemptState]) -> int:
    ...
    An infra head is charged `attempts - REVIEW_FREE_REATTEMPTS_PER_HEAD`, CLAMPED at 0 (common.sh:714-715). common.sh:650-654 prices that: "Three attempts cost three attempts' money, and only one of them is charged -- a deliberate under-charge that keeps the loop moving. What stops that being unbounded is the per-head ceiling".
...
def attempts_from_tsv(raw: str) -> list[AttemptState]:
    ...
    The seam DEFECT 2 lives on. The twin does `$((n - 2))` on whatever word sat in column two and, under `set -u`, dies with `zz: unbound variable` naming a line in a library the caller never opened. This raises a `ValueError` that names the row.
    """
    states: list[AttemptState] = []
    for line in raw.split("\n"):
        if not line.strip():
...
def head_is_exhausted(states: list[AttemptState], sha: str) -> bool:
    ...
    `review_attempt_class_is_infra "$cls" || return 1` comes FIRST, so a non-infra head is never exhausted no matter how many attempts it has. See the quote at common.sh:733-735 in the module docstring for why.
    """
...
def epic_ids(branch: str, env: dict[str, str] | None = None) -> list[str]:
    ...
    The snapshot is the CONTRACT (common.sh:600-602): `agent/pr/<branch>.md` is in the repo, the worklist store is in TMPDIR and unreadable from CI. No snapshot means no epics, which the caller treats as the flat, pre-epic review rather than as an error -- so a missing file returns `[]` and does not raise.
    ...
    ANCHORED TO THE REPO ROOT, not to the cwd, and common.sh:606-611 records why: "It used to be a bare relative path, so the answer depended on the caller's CWD: a gate invoked from a subdirectory saw no epics and silently took the flat path, which looks exactly like a PR that declares none."
    """
    if not branch:
        return []
...
def report_count(
    ...
    The twin USED TO BE `gh api ... 2>/dev/null | wc -l || true`, which answered 0 on a rate limit (DEFECT 1, fixed 2026-09-10). It now routes through `gh_retry`, matching this port, which has raised `core.ghx.GhError` on failure from the start -- the same decision
    `gh_json` made a hundred lines above it in the same file. `attempts=3`
    matches `_gh_probe`'s retry count exactly.

    Everything else is the twin: the needle is the PRODUCER CONSTANT `**Claude finished`, matched with `startswith`, with `(epic <id>)` appended
...
def diff_loc(
    ...
    *,
    ...
    `on_error` IS THE ONE ARGUMENT IN THIS MODULE, and it exists because this is the one place the twin's swallow is a stated decision rather than an oversight (common.sh:764-765). Default `None` raises. Passing
    `on_error=DIFF_LOC_FAILS_TO_ZERO` reproduces the twin, VISIBLY, at the call
    site -- which is the difference between a caller who chose the smallest bucket and a caller who never learned there was a choice.
...
def _repo_slug(repo: str | None, env: dict | None) -> str:
    ...
    the variable unset it dies with `common.sh: line 592: GITHUB_REPOSITORY: unbound variable` (driven). Reproduced as a refusal with the same cause named in words, because "unbound variable" naming a library line is not a message a caller can act on.
    """
    if repo:
        return repo
    environ = dict(os.environ) if env is None else env
