...
Three copies is exactly the count that made the bash extraction correct, so the Python side has arrived back at the state the bash side left. This module is the one implementation those three can eventually reach. REPOINTING THEM IS NOT DONE HERE and is not this module's licence to grant: they are gates with their own recorded shadow ledgers, and a body swap under a ledger is a
separate change.

WHY NOT `rediacc_ci.controls.Controls`, WHICH IS THE SAME IDEA. It is a different OUTPUT CONTRACT, and the contract is the thing a caller depends on:
...
"battery" versus "file" is not a synonym here: a reader greps for one or the other. `.ci/rediacc_ci/tests/test_quality_release_signing_coverage.py:201` already records the split in as many words. Two vocabularies with two live reader populations is a merge with its own argument, and folding it in here would smuggle that argument past review as tidying.

THE FLOOR IS THE WHOLE POINT, and it is why this file is not a convenience. A battery that did not run is not a green one. `gate_finish` counts the controls that actually executed and refuses under the floor, which is the only thing that catches a gate whose assertions stopped running -- an early `return`, a fixture that produced no rows, a loop over an empty list. Every one of
those otherwise ends with a clean exit and no output that says anything is wrong.

THE GLOBALS ARE PART OF THE PORT, NOT AN ACCIDENT. `GATE_FAILS` and `GATE_N` are bash globals shared by everything sourced into one shell, so `MODULE_TALLY` below is a module-level singleton with `gate_check` / `gate_finish` as free functions bound to it. That is the literal translation and it is what the entry point drives. A caller who wants two independent tallies in one process
constructs `GateTally()` directly, which bash cannot do and is the one capability the port adds.

`gate_finish` IS NOT IDEMPOTENT, IN EITHER LANGUAGE. Under the floor it APPENDS a failure to the tally, so calling it twice on a short battery reports two. The
bash does the same (`GATE_FAILS=$((GATE_FAILS + 1))` inside the `if`), and a port
that quietly fixed it would disagree with the twin on a real input. It is pinned by a case rather than corrected.

THE ENTRY POINT reads a US-separated program on stdin so a differential can hand the same bytes to both sides:
...
read -r a b c` COLLAPSES consecutive tabs into one delimiter and an empty field simply disappears, shifting every later field left. Measured on the first run of this differential: a case with an empty `name` made the bash side read the FIX hint as the name and print a different header, and the comparator reported a STDOUT-DIFF that looked exactly like a port defect. It was a defect
in the DRIVER. `\x1f` is not IFS whitespace, so empty fields survive on both sides, and an empty field is a case this contract has to be able to express.

Zero lines of program is a REFUSAL (exit 2), not an empty green: a driver that fed nothing would otherwise get `✓ ...: 0 control(s) passed` out of a floor of zero, which is the exact vacuity the floor exists to prevent, manufactured one level up.
"""
...
class GateTally:
    ...
    Not a module global on the class: two tallies in one process each get their own counters, which is what lets this be tested by a test suite that is itself counting things. The module-level `MODULE_TALLY` below is the literal translation of the bash globals, for callers that want that.
    """
    ...
    def check(self, label: str, got: object, want: object) -> bool:
        ...
        The comparison is on `str(got)` and `str(want)` rather than on the objects: bash has only strings, so `check("n", 1, "1")` must PASS here or the port disagrees with the twin the first time a caller passes an int. Returns the boolean so a caller can branch without recomputing the comparison, which is the one thing the bash cannot offer.
        """
        self.n += 1
    ...
    def finish(self, minimum: int, subject: str) -> int:
        ...
        Returns an int rather than a bool because the bash returns an exit status and every caller of this port is deciding an exit status. A bool would invert at exactly one call site and be right everywhere else, which is the worst way for it to be wrong.
        """
        if self.n < minimum:
...
def run_program(lines: list[str], tally: GateTally | None = None) -> int:
    ...
    A MALFORMED LINE IS A REFUSAL, NOT A SKIP. A driver that mistypes `chekc` would otherwise contribute nothing to the tally and the floor would report "the battery is not being executed as written" -- true, but naming the wrong cause, and the reader would go looking at the gate instead of at the typo.
    """
