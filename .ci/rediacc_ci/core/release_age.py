...
The TypeScript side won that collapse because it has six consumers to the bash side's two, because it is portable while the bash round-up needed GNU-only `date -u -d`, and because the bash tree is the one scheduled to be ported away. Re-deriving the round-up here would undo all three arguments at once, so this module delegates exactly as the twin does and owns only the transport,
the memo and the two caller policies.

--------------------------------------------------------------------------
THE DUPLICATE THAT ALREADY EXISTS, NAMED RATHER THAN LEFT TO BE FOUND
--------------------------------------------------------------------------
`.ci/rediacc_ci/quality/go_deps.py:300` ALREADY carries a `ReleaseAge` class that is this same shim, inlined into one gate because that gate was ported before the library was. This module is the one implementation it can eventually reach. REPOINTING IT IS NOT DONE HERE and is not this module's licence to grant: `go_deps.py` is a gate with its own recorded shadow ledger, and a body
swap under a ledger is a separate change. The differences between the two are real and listed, so whoever collapses them knows what moves:
...
Measured 2026-09-10 on bash 5.3.9, with a counting wrapper first on PATH and five DISTINCT publish epochs through `is_release_deferred`:
...
WHAT HAS NO COUNTERPART. `release-age.sh:71-78` refuses bash older than 4.2 because `declare -gA` is 4.2 and the cache keys are `"<epoch>:<window>"`, which an INDEXED array evaluates as arithmetic (`syntax error in expression`, once per lookup, while the memo silently misses). A Python dict has no such precondition, so the guard is absent BY CONSTRUCTION rather than dropped.
"""
...
class ReleaseAge:
    ...
    ONE INSTANCE PER RUN. The twin's caches are shell globals that live for the length of the process, so `MODULE_SHIM` below is a module-level singleton and the free functions are bound to it, which is the literal translation. A module-level singleton would also let one test's answer leak into the next, which is the shape of a control that cannot fail, so every test constructs its
    own instance -- the one capability the port adds and bash cannot express.
    """
    ...
    def resolve_runner(self) -> list[str]:
        ...
        THE MEMO IS THE DIVERGENCE, not the ladder. See DEFECT 1 in the module docstring for the measurement of what the twin does instead.
        """
    ...
    def _capture(self, argv: list[str], *, cwd: str | None) -> str | None:
        ...
        `2>/dev/null` on both call sites, so the delegate's own diagnostics never reach the caller and the ONLY thing that distinguishes a failure is the empty answer. Reproduced rather than improved: a port that let the TypeScript's `release-age: publish epoch must be an integer ...` through would print a line the twin swallows, and the differential would call that a mismatch in
        the port.
        """
        try:
            proc = subprocess.run(
    ...
    def eligible_epoch(self, publish_epoch: int, window: int, *, err=None) -> int | None:
        ...
        THE FAILURE IS NOT CACHED, matching `:190`: the write happens only after the regex accepts, so a transient delegate failure is retried rather than frozen into the run.
        """
    ...
    def release_eligible_epoch(
        ...
        NO VALIDATION OF `publish_epoch` HERE, and that is the twin: `:208-216` passes whatever it was given straight to the delegate, which rejects it, which produces the loud stderr refusal and a failure. An empty string therefore prints `treating '' as DEFERRED`, naming nothing useful, and that message is what a caller sees today.
        """
        if window is None:
...
def module_shim() -> ReleaseAge:
    ...
    Cached rather than a module-level constant so importing this module never resolves the repository root: `paths.repo_root()` RAISES when $REDIACC_CI_ROOT names something unusable, and an import that can raise for an environment reason is an import nothing can safely do.

    The cache IS the singleton, which is the literal translation of the twin's shell globals. A caller wanting an independent memo constructs `ReleaseAge()`
