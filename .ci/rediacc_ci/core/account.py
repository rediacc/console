...
# The character `.ci/lib/account.sh:226` really writes into a live .env file, held here rather than inline so the byte survives as a named constant instead of a character a scanner has to special-case.
_HEADER_DASH = "—"  # em-dash-ok: reproduces existing bash output byte for byte, not new prose (comma breaks test_the_port_matches_the_live_twin[fresh-env] against the real bash twin)
...
def fresh_env_text(keys: CryptoKeys, stamp: str, root_email: str) -> str:
    ...
    """
    return """# Auto-generated by ./run.sh %(dash)s %(stamp)s
