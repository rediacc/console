...
COEXISTENCE, NOT REPLACEMENT. Every module here names its `.ci/scripts/test/gates/` twin in a module-level `BASH_TWIN`, and `test_twin_parity.py` drives BOTH on every run and refuses to let them disagree. The bash original is not deleted by the change that ports it; it is deleted by a later change, once the parity test has been green across enough runs to be believed. A twin that
has stopped agreeing is a regression in the port, and the only way to see that is to keep running both.
