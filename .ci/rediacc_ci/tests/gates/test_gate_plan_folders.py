...
def test_the_real_tree_passes_and_says_what_it_counted(gate):
    ...
    gate.assert_exit(0, result, "the real tree must be green with F1 fatal")
...
def test_a_tree_below_the_floor_refuses(gate, tmp_path):
    ...
    _git(root, "commit", "-qm", "base")
    result = _gate(root)
    gate.assert_exit(1, result, "one plan is below the floor and must refuse")
...
def test_a_config_without_the_retention_keys_is_a_setup_error(gate, tmp_path):
    ...
    result = _gate(root)
    gate.assert_exit(2, result, "a missing retention key is a setup error, not a verdict")
...
def test_a_legacy_plan_is_fatal(gate, tmp_path):
    ...
    result = _gate(root)
    gate.assert_exit(1, result, "a plan at the legacy path is a red, not a note")
...
def test_a_stub_at_the_legacy_path_is_not_a_finding(gate, tmp_path):
    ...
    result = _gate(root)
    gate.assert_exit(0, result, "a stub at the old path is the mechanism, not a defect")
...
def test_a_finished_plan_in_the_active_folder_is_fatal(gate, tmp_path):
    ...
    result = _gate(root)
    gate.assert_exit(1, result, "F2 is fatal from the day the gate lands")
...
def test_a_terminal_plan_past_its_retention_is_fatal(gate, tmp_path):
    ...
    result = _gate(root)
    gate.assert_exit(1, result, "a terminal plan past 40 days is a finding")
...
def test_a_stub_pointing_at_nothing_is_fatal(gate, tmp_path):
    ...
    result = _gate(root)
    gate.assert_exit(1, result, "a stub whose target is missing is a dead citation")
...
def test_a_citation_in_the_generated_ledger_must_resolve(gate, tmp_path):
    ...
    result = _gate(root)
    gate.assert_exit(1, result, "a ledger key naming no file is a broken citation")
...
def test_a_clean_fixture_tree_is_green(gate, tmp_path):
    ...
    root = _seed(tmp_path)
    result = _gate(root)
    gate.assert_exit(0, result, "a tree that obeys every rule must be silent")
...
def test_sweep_refuses_to_delete_without_write(gate, tmp_path):
    ...
    gate.assert_exit(0, result, "a dry run is a report, not a verdict")
...
def test_sweep_with_write_deletes_and_leaves_a_tombstone(gate, tmp_path):
    ...
    gate.assert_exit(0, result, "the sweep must succeed")
    gate.assert_eq(
    ...
    gate.assert_exit(0, shown, "the tombstone blob must resolve")
...
def test_move_leaves_a_stub_and_stamps_both_clocks(gate, tmp_path):
    ...
    gate.assert_exit(0, result, "the move must succeed")
...
def test_move_takes_an_untracked_plan_that_carries_its_own_clock(gate, tmp_path):
    ...
    gate.assert_exit(0, result, "an untracked plan with First-Seen must move")
...
def test_move_refuses_a_stub(gate, tmp_path):
    ...
    gate.assert_exit(1, result, "a plan moves exactly once")
...
def test_move_refuses_a_plan_with_no_clock_to_carry(gate, tmp_path):
    ...
    gate.assert_exit(1, result, "a move must not reset a clock it cannot read")
...
def test_move_refuses_a_terminal_plan_with_no_ledger_row(gate, tmp_path):
    ...
    gate.assert_exit(1, result, "a terminal move that starts no clock is refused")
    ...
    gate.assert_exit(0, spared, "an active plan needs no moved_at and is spared")
    gate.assert_eq(
...
def test_status_reports_without_a_verdict(gate, tmp_path):
    ...
    gate.assert_exit(0, result, "--status never reaches a verdict")
