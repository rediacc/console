...
Behavioural test for `.ci/scripts/quality/check-label-references.sh`: every GitHub
label a workflow or a `.ci` script references by name must be declared in
`.github/labels.yml`.

WHAT IT GUARDS. Labels are this repo's kill switches and routing flags, and the
failure is SILENT fail-open: `promote-stable.yml` searched for a label that did
not exist, a search for a nonexistent label returns zero PRs rather than an
error, so the promotion block simply never fired. `full-ci`, `autopilot` and
`autopilot-blocked` were all referenced by merged code for weeks while absent.

ONE DIRECTION ONLY, and that is a decision rather than an omission: a declared
label nothing references is inventory, not an error. `test_declared_but_unreferenced_is_fine`
is the case that pins it, and it is the converse without which every firing case
here would also be satisfied by a gate that reds on anything.

WHY THIS MODULE OPTS IN TO THE REAL-TREE GROUP. Two cases drive the subject
seam-free over the REAL tree: it recursively greps `.github` and `.ci` and
reconciles what it finds against the real `.github/labels.yml`. A battery step
writing under either directory mid-sweep is a divergence that would be blamed on
...
--------------------------------------------------------------------------
THIS IS THE LOAD-BEARING DIFFERENCE FROM THE TWIN, and getting it wrong would
turn the real gate red for every session in the tree.

The subject sweeps `.ci` recursively, and it protects itself from its OWN planted
samples by excluding exactly two basenames: `check-label-references.sh` and
`test-label-references.sh`. The twin is one of those two, so the twin may write
its fixture lines out literally. THIS FILE IS NOT ON THAT LIST, it lives under
`.ci/rediacc_ci/tests/gates/`, and adding it would mean editing the subject,
which this port does not do.

So no complete, matchable reference shape may exist as a literal anywhere in this
source. Every template below carries `%s` where the label goes, and `%` is
outside the `[A-Za-z0-9._:-]` character class all ten extractors use, so each
pattern fails to match the template and matches only the RENDERED fixture, which
lives in a tempdir outside the repository.

`test_this_module_plants_no_label_reference_the_real_sweep_can_see` is the control
on that reasoning: it points the real subject at this very directory with the
floor dropped to zero, so a template that started matching reds HERE, by name,
instead of reddening `check:ci-label-refs` for whoever runs it next.
"""
...
def scaffold(gate, d):
    ...
    ANTI-VACUITY, and it is not decoration. If the template list and the suffix
    list ever fall out of step, the fixture would exercise fewer shapes than the
    assertions claim and every count below would still be internally consistent.
    So the pairing is checked, and an empty template list is a FAILURE rather than
    a fixture that scans nothing.
    """
...
def test_real_tree_is_clean_and_excludes_this_file(gate):
    ...
    one shape broke for all of them and reporting the first is reporting a tenth of
    the finding. And the DISTINCT count is read out of the verdict line and printed,
    so a sweep that collapsed toward the subject's floor is visible rather than
    silent -- "the real tree is clean" says nothing about how much tree was swept.
    """
    bash = require_gate(gate)
...
def test_this_module_plants_no_label_reference_the_real_sweep_can_see(gate):
    ...
    The twin is excluded from the real sweep by basename. This module is NOT, and
    adding it would mean editing the subject. Instead every fixture line here is a
    `%s` template that no extractor can match, which is an argument, and an argument
    is not a control.

    So: point the REAL subject at this directory alone, with the floor dropped to
    zero so the sweep is allowed to legitimately find nothing, and reconcile against
    the REAL labels file. A template that started matching -- because a pattern was
    widened, or because someone wrote a fixture out literally -- shows up as an
    undeclared reference HERE, naming this file, instead of reddening
