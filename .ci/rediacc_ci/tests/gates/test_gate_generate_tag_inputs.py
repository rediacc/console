...
WHAT THE HASH IS FOR. `generate-tag.sh --submodule private/renet` mints the tag
that names ghcr.io/rediacc/renet. `initialize.sh` asks the registry whether that
tag already exists and, if it does, sets `renet_exists=true`, which skips the
45-minute Renet (Full) build AND the image build. The tag is therefore a cache
key, and `BUILD_CONFIG_FILES` is its input list: an input missing from the list
means a changed build resolves to an existing tag and a stale image is reused.
...
the surviving digests concatenate differently, so the tag still CHANGES -- and a
cache miss is indistinguishable from normal behaviour. The list could rot
indefinitely and the script would keep printing a plausible tag and exiting 0.

WHY A FIXTURE TREE. The gate reads real files from the working directory, so the
only way to plant a defect without touching a tracked file is to build a
throwaway repo with the same shape and run the real script inside it. Nothing
about the SUBJECT is reimplemented here: every invocation below is the real
`bash .ci/scripts/ci/generate-tag.sh`.

WHY THIS MODULE OPTS IN TO THE REAL-TREE GROUP, and it is the sharper reason of
the two kinds. Most real-tree twins only READ the working tree. This one WRITES
it: the last two cases overwrite the tracked `.ci/scripts/version/resolve-version.sh`
with a stub resolver and restore it a second later, because `generate-tag.sh`
gives them no fixture seam to do it in (the closure mode invokes the resolver via
`cd "$REPO_ROOT"`). A gate reading that script inside the window sees a
half-written file: on 2026-08-17 that reddened `gate-test:claude-hooks` with a
bash syntax error in a file that parses clean. The twin is in `WRITER_TESTS` in
`run-all.sh` and carries `mutex: ["tree:repo"]` in `gates.lock.json` for exactly
...
WHAT THE PORT ADDS RATHER THAN DROPS. The twin restores the resolver with `cp`
and then infers success from the tag coming back to baseline. This restores in a
`finally` (so a raised assertion cannot strand the stub the way an `exit` from
inside a bash function can) and additionally asserts the restored file is
byte-identical by sha256 and keeps its mode. A tag that matches is good evidence
...
def require_gate(gate) -> str:
    ...
    The twin dies on a bare `command not found` if either the script or bash is
    absent. Probing names which one and what to do, because a missing tool is a
    FAILURE here and never a skip: an unchecked case folded into "fine" is the
    shape this directory refuses.
    """
    if not GATE.is_file():
...
def build_fixture_tree(gate, root: pathlib.Path) -> None:
    ...
    The script under test resolves its lib relative to its own path, so the `.ci`
    layout is mirrored instead of the script being copied somewhere flat.
    """
    require_gate(gate)
    git = require_git()
    if root.exists():
...
def tag(gate, root: pathlib.Path) -> str:
    ...
    tidying: without it every `assert_eq` against a tag would compare a string
    the twin never compares.
    """
...
def declared_array_body(gate) -> str:
    ...
    Anchored on the same two lines the twin anchors on, so a reindentation that
    would hide the array from the twin hides it from the port too, rather than
    the two disagreeing about whether the gate still declares one.
    """
...
def test_declared_inputs_match_the_script(gate):
    ...
    BUILD_CONFIG_FILES, the per-input control proof silently stops covering the
    new entry.
    """
...
def test_infra_build_renet_is_not_an_input(gate):
    ...
    run `.ci/scripts/infra/build-renet.sh` and it is NOT in the list -- correctly,
    because it compiles a dev binary for ct-tests / ci-ops-test, which never pull
    the renet image and are never handed this tag. Pinning it here means the next
    reader gets the answer instead of re-deriving it, and a future decision to
...
def swap_resolver(gate, body: str) -> str:
    ...
    THE WRITE IS THE POINT and there is no seam that avoids it: closure mode
    invokes `.ci/scripts/version/resolve-version.sh` from the repo root, so the
    only way to move the released version is to move that file. It is why this
    module is a real-tree WRITER and why the twin lives in `WRITER_TESTS`.
    """
...
def restore_resolver(gate, backup: pathlib.Path, mode: int, before: str) -> None:
    ...
    A `finally`, not a trap: a bash function cannot clean up after an `exit` from
    inside itself, which is why the twin binds its restore to a `cp` on the happy
    path. Python unwinds, so the stub cannot outlive a failed assertion here.
    """
...
def test_closure_tag_moves_when_the_released_version_moves(gate):
    ...
    Both closure images BAKE a version in, and that version comes from the latest
    git TAG -- not from a path -- so no CLOSURE_PATHS entry can cover it. The
    OID-at-HEAD hashing is even documented as being "immune to the in-job version
    bump", which is right for a dirty working file and exactly wrong for this: the
    key went insensitive to the one input the image is stamped with.

    Release v1.2.12 landed 2026-07-30T10:16:14Z mid-PR. Runs 30534726467 and
    30542942037 both failed `Validate Install Methods / Linux` with "Version
    mismatch: expected '1.2.13', got '1.2.12'", because `Build (Docker) / CLI
    Docker` was SKIPPED while its cached twin succeeded and the mutable pr-546 tag
    kept serving a pre-release image. Deterministic and self-perpetuating, not a
    race: nothing on the branch could move the key.

    Driven by swapping the RESOLVER rather than by cutting a git tag, so the case
    needs no write access to the real tag namespace.
    """
