...
REGISTRY: tuple[tuple[str, str], ...] = (
    ...
    # The type-check half of the same subject, and the same shape one layer up: against an empty tree `git ls-files` enumerates nothing, mypy handed an empty @file list exits 0 saying nothing, and the gate would then compare an EMPTY finding set against a 581-row baseline. That does not read as "no input"; it reads as "581 findings were fixed", which is a drain instruction rather
    # than a refusal. The MIN_PY_FILES floor is answered BEFORE the pin is read and before mypy is resolved, so the empty-tree failure is about VACUITY and not about a missing binary -- an absent mypy is an ENVIRONMENT failure wearing exit 77, and pinning that would assert nothing about the gate.
    (".ci/scripts/quality/check_python_types.py", "VACUOUS INPUT"),
    ...
    #
    # NOT registered here either: .ci/scripts/test/gates/test-scope-gate-outputs.sh, retired in W7 P5 with its cases carried by
    # .ci/rediacc_ci/tests/gates/test_gate_scope_gate_outputs.py, which this harness does not register because it registers bash gate tests only.
    # It was measured the same way and with the same result: all 6 cases pass against the empty tree (exit 0). It BUILDS the tree it needs -- it copies .ci/scripts/ci into a temp dir, `git init`s a repository there with the branch shape a baseline walk requires, and shims `gh` on PATH -- so the only repo input
