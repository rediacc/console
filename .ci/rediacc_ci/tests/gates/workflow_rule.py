...
gets its own gate test driving it against a fixture tree. The incantation is
exact and easy to get subtly wrong: `WORKFLOW_INLINE_ONLY=1` is what empties
GITHUB_YAMLS (check-workflows.sh:38-40) so the banned-pattern scans become
no-ops and the FIXTURE TREE is the only thing judged. Without it a test both
trips on and depends on the real `.github` state.

test-workflow-contracts.sh looks like it belongs here and does NOT: it drives
`.ci/scripts/security/check-workflow-gates.sh` with `WORKFLOWS_DIR` -- a
different script, a different variable, no inline-only switch. The five lines
...
otherwise. Rather than hide that difference behind a default, `ci` is an explicit
argument here and each caller states which side of it it is on, so a reader can
see that inline's fixtures are judged with CI unset exactly as the twin judges
them.

STREAMS ARE MERGED, matching `2>&1` in both bash callers. These rules write their
findings to stdout and their diagnostics to stderr, and every assertion is about
"what the author sees", which is the merged text.
"""
...
def run_check(
    ...
    `gate` is taken so a MISSING subject reds as a named refusal here rather than
    as a `FileNotFoundError` from deep inside subprocess, which would read as
    flake and name no remedy.
    """
...
def write_job(path: pathlib.Path, *lines: str) -> None:
    ...
    Byte-for-byte the twin's `write_job`: four bytes of indent on the caller's
    lines, because job-level keys sit at that depth and the rule under test keys
    on the indentation.
    """
...
def write_step_env(path: pathlib.Path, *lines: str) -> None:
    ...
    Ten bytes of indent, which is what puts them one level inside `env:` at eight.
    The env-shell-var rule's whole dedent behaviour is keyed on that, so the
