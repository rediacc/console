...
def test_the_gate_is_reachable_as_a_program(gate):
    ...
    gate.assert_exit(0, result, "the gate passes on this tree")
...
def test_the_selftest_flag_exits_zero_on_a_green_run(gate):
    ...
    gate.assert_exit(0, result, "a fully green control run exits 0 under --selftest")
...
def test_a_tree_without_the_subject_is_refused_through_the_env_seam(gate):
    ...
    with harness.temp_dir() as root:
        ...
        gate.assert_exit(1, result, "a tree it cannot see exits 1, not 0 and not 2")
        gate.assert_contains(
...
def test_a_deleted_manifest_is_not_a_free_pass(gate):
    ...
    with harness.temp_dir() as root:
        ...
        gate.assert_exit(1, result, "no manifest is a hard refusal")
        gate.assert_contains(
        ...
        gate.assert_exit(0, clean, "CONTROL: the same tree with a manifest passes")
...
def test_the_gate_does_not_write_to_the_vendored_directory(gate):
    ...
    result = harness.run([str(GATE)], cwd=paths.repo_root())
    gate.assert_exit(0, result, "the run under observation actually completed")
    gate.assert_eq(
