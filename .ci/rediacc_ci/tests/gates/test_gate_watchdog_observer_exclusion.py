...
(2026-07-31, run 30660765759), a push deadlocked. The not-yet-re-reviewed head's
...
starts on a GREEN run. The fix is one config line
(`WATCHDOG_EXCLUDE_PATTERNS` in watchdog-monitor.yml), and this gate is what
stops that line from quietly losing an entry and reintroducing the cycle.

CONTROL-FIRST, per the house rule: the checker is proven to FIRE on a planted
copy missing one exclusion BEFORE it is run against the real workflow.

THE TWIN IS A FLAT SCRIPT with no `test_*()` functions, so the port chooses the
split. Its five `PASS:` lines become five tests: the control, the control's own
anti-vacuity clause, and one per required exclusion name -- which is strictly
better than the twin's loop, because a missing name now reds as its own test
rather than aborting the loop at the first one.
"""
...
def excludes(text: str, name: str) -> bool:
    ...
    The comma padding is the whole trick and it survives the port verbatim: a
    bare `in` test would report "Complete" as present because "CI Complete" is,
    and the gate would then pass a list that had lost the entry it names.
    """
...
def test_control_is_not_vacuous(gate):
    ...
    A plant that deleted the line would make the control above fire for a reason
    that has nothing to do with membership, and the gate would look proven while
    testing that an absent list has no members.
    """
...
def test_every_required_name_has_its_own_case(gate):
    ...
    The twin loops over the three names, so adding a fourth to the loop covers it
    automatically. The port has one test per name instead -- better diagnostics,
    and a new hazard: a name added to `REQUIRED` with no test beside it would be
    silently uncovered while the module still reported green. This reads THIS
    FILE'S OWN SOURCE and requires a `require_member` call site per name.
    """
