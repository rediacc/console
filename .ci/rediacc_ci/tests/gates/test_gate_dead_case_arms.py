...
is an assertion that cannot fail, and the gate exists because one of them passed
permanently while proving nothing.

THE SUBJECT IS CONTROL-FIRST, so the FIRST case here is the seam-free one: the real
invocation over the real tree, asserting both that it is clean AND that its own
planted-arm control fired. A gate that reported "clean" without its control firing
would be reporting on a scanner nobody had seen work.

WHY THIS MODULE OPTS IN TO THE REAL-TREE GROUP. `check-dead-case-arms.sh` reads
`.ci/scripts/test`, `.ci/media`, `.ci/scripts`, `scripts` and `packages/www/scripts`
with recursive greps over the working tree, and every case below reaches the real
tree at least through `DEAD_CASE_MEDIA_DIRS`, which the twin never overrides. So it
is a real-tree SCANNER: a battery step rewriting a scanned file mid-grep is the
`grep: ... No such file or directory` flake that would be blamed on this port.
...
`real_tree_admission` in `test_twin_parity.py`, which refuses the combination.

WHAT IS DELIBERATELY NOT RE-IMPLEMENTED. The scanner is never reproduced in Python.
Every case drives the real `bash check-dead-case-arms.sh` with the same two
environment overrides the twin uses, so the code under test is the code that ships.
"""
...
def run_gate(gate, test_dirs, code_dirs: str = DEFAULT_CODE_DIRS) -> harness.RunResult:
    ...
    The twin captures `2>&1` into `$LAST` and asserts on the merged text, so the
    callers below read `.combined` for the same reason.
    """
...
def test_real_tree_is_clean_and_the_control_fired(gate):
    ...
    This is the line the manifest's BLOCKER names, and it is why the subject is
    registered as a real-tree reader in the lock.
    """
    bash = require_gate(gate)
...
def test_the_founding_defect_fires(gate):
    ...
    special precisely because the subject's own header describes the defect in
    prose, and the first implementation grepped comments too -- so the only two
    occurrences of `cores=` in the whole tree were its own comment lines, and it
    ruled the arm live and MISSED the bug it exists to catch. Verified 2026-08-05:
    exit 0 on this exact fixture before the fix, exit 1 after. If `key_is_live` ever
...
def test_the_scanned_media_root_is_not_empty(gate):
    ...
    The subject refuses when `DEAD_CASE_MEDIA_DIRS` holds no shell files, because a
    scan root that has stopped matching reports clean forever and that green is
    indistinguishable from a clean tree. The twin never asserts the refusal exists,
    so a subject that lost it would keep every twin case green. This drives the
    subject at an EMPTY media root and requires the loud refusal.
    """
    bash = require_gate(gate)
    with harness.temp_dir() as d:
        tests = d / "test"
