...
--abbrev-ref` yields the literal string HEAD, which sanitises to the perfectly VALID hostname `head.localhost`, so every detached worktree on the machine converges on one name and one router. Neither is visible to a smoke test; both are visible here.
...
OWN shell; a Python process has no shell to eval into, so this module extracts the same line range with the same two anchors and hands it to a fresh `bash -c` per call. The extractor asserts what `sed` leaves implicit and the twin then re-checks with `declare -F`: a name that did not extract is a FAILURE here and never an empty string, because "the function is not defined" and "the
function returned nothing" are the same observation once the body is missing, and the twin's own comment says every assertion below would then be vacuous.
...
NO `xdist_group`. Every case runs one short-lived `bash -c` with an env overlay and writes only into its own `mktemp -d`; the library is only ever READ, and the one git repository built here lives entirely inside that temp directory.
"""
...
def test_the_hostname_rules_hold(gate):
    ...
    with harness.temp_dir() as tmp:
        ...
        # ----------------------------------------------------------------- 4. Identity and ports must NOT depend on the branch ----------------------------------------------------------------- A branch is renamed; a path is not. If either of these ever consults the slug, a rename orphans the container or shuffles its port block.
