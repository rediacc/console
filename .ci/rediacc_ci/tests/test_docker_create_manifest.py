"""Differential: `rediacc_ci.docker.create_manifest` against its twin `.ci/scripts/docker/create-manifest.sh`.
...
def test_planted_defect_is_caught_only_by_the_call_log(tmp_path) -> None:
    """PROVE THE DIFFERENTIAL CAN FIRE. The plant drops arm64 from the ARGV handed to docker, at the call site, so the printed `Sources:` line still names both architectures. That is exactly the mistake that publishes a silently single-arch manifest under a name every consumer reads as multi-arch: every printed byte and the exit code are unchanged by it.
