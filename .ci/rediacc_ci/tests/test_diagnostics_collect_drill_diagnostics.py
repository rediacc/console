"""Differential: `rediacc_ci.diagnostics.collect_drill_diagnostics` against its twin `.ci/scripts/test/collect-drill-diagnostics.sh`.

Both subjects derive the console root from their own file location (three directories up), so both COPIES live in one shared fixture tree at the right relative depth; the tool is read-only with respect to that tree (it only ever creates a fresh `$RUNNER_TEMP/drill-diagnostics`), so unlike a mutating teardown script the two sides can safely share one source fixture and differ only
in `RUNNER_TEMP`/`TMPDIR`, which route each side's collection to its own scratch directory.
...
def _both(tmp_path: pathlib.Path, *, account_logs: bool, drills: list[str]):
    """Run both subjects and NORMALIZE the one legitimate difference: each side is deliberately given its own `$RUNNER_TEMP` (a mutating collector cannot safely share a destination between two runs), and that path is printed verbatim in `$dest`. Comparing stdout with each side's own scratch prefix stripped is the same idea `differential.py`'s `<repo>` substitution uses for the
    checkout root."""
...
def test_multiple_drill_dirs_are_glob_sorted(tmp_path: pathlib.Path) -> None:
    """The twin's glob sorts lexicographically; `sorted()` in the port must match it rather than an incidental filesystem order."""
    old, _new, old_norm, new_norm = _both(
...
def test_the_fake_is_actually_reached(tmp_path: pathlib.Path) -> None:
    """ANTI-VACUITY. Without a real collected file, every comparison above is two empty collections agreeing trivially."""
