...
WHY A DIFFERENTIAL AND NOT A TABLE OF EXPECTED STRINGS. This gate reaches every one of its verdicts from an external tool's EXIT STATUS, so the only thing a port can get wrong is which command it runs, where it runs it, in what order, and what it does with each status. All four of those are properties of a process, not of a string, and a table of expected strings would be a table
of what the PORT does, asserted against itself.

The committed ledger `.ci/shadow/w7p2-account-portal.observations.jsonl` compares the WHOLE gate over five distinct trees, with `npx` and `npm` stubbed so every phase is deterministic. What it cannot show is the ARGV -- both sides would agree perfectly while both ran `tsc` without `--noEmit`, which would emit JavaScript into the tree and still exit 0. This file records the argv
vectors and the working directories a run actually produces.

THE STUBS BELOW ARE THE MODULE'S OWN (`account_portal.write_stubs`), not a second copy, so a change to them cannot make the test and the selftest disagree about what "running the gate" means.
"""
...
def _trace(tmp_path: pathlib.Path, **codes: str) -> tuple[int, list[tuple[str, str]]]:
    ...
    The tree is rebuilt for every call, so no test can depend on a previous one's artifacts -- which matters here more than usual, because phase 1 WRITES to the tree and phase 7 reads what phase 6 wrote.
    """
    root = tmp_path / "tree"
    bindir = tmp_path / "bin"
...
def test_the_healthy_run_launches_exactly_six_commands(tmp_path: pathlib.Path) -> None:
    ...
    THIS IS THE ASSERTION THE LEDGER CANNOT MAKE. Both implementations would agree perfectly while both ran `tsc` with no `--noEmit`, which EMITS JavaScript into the tree and exits 0 -- a gate that silently became a build step. The exact vectors are therefore written down here, once.

    Six and not seven: `npm ci` is skipped because `node_modules` is present, which is the ordinary case on any machine that has run this before.
    """
...
def test_an_absent_node_modules_adds_the_install_first(tmp_path: pathlib.Path) -> None:
    ...
    Its MIRROR is the test above, where `node_modules` exists and the install is not run at all. Both directions, because a gate that always installed would be slow and a gate that never installed would fail on a fresh checkout.
    """
...
def test_the_lint_phase_is_the_one_that_does_not_stop(tmp_path: pathlib.Path) -> None:
    ...
    Asserted by COUNTING the launches after it, not by reading the exit code: an exit code of 0 would also be produced by a gate that ran phase 4 last.
    """
...
def test_phase_seven_refuses_a_build_that_produced_nothing(tmp_path: pathlib.Path) -> None:
    ...
    `vite build` exits 0 and writes nothing: the gate must still refuse. And the limit, asserted so it is on the record -- the test is EXISTENCE, so a zero-byte file, or a stale one from a previous run, satisfies it.
    """
...
def test_missing_binary_and_missing_cwd_get_bash_s_own_codes(tmp_path: pathlib.Path) -> None:
    ...
    Python raises the SAME exception type for both, distinguished only by the exception's `filename`, so this is the seam where a port most easily collapses two different failures into one code. Bash is the authority and is asked directly.
    """
...
def test_a_nonzero_status_from_any_tool_is_a_failure_not_just_one(
    ...
    Which means a missing `npx` is reported as "Frontend typecheck failed!" by both implementations. That is the twin's behaviour and it is worth an assertion rather than a comment, because it is the shape in which "the toolchain is not installed" reaches a reader as "your types are broken".
    """
