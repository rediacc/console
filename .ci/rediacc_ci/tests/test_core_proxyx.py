"""`rediacc_ci.core.proxyx`, the shared contract every `rediacc_ci.proxies.*` port imports in place of sourcing `.ci/scripts/test/proxies/proxy-lib.sh`.
...
BOTH DIRECTIONS, same shape as the bash `proxy_lib_selftest` it is porting: a proxy that asserts something and has every requirement present must return 0; one that is missing a requirement must return 77, never 0; one that makes zero checks must return 1, never 0; one that declares zero requirements must refuse at `preflight()` with 2, before it can even run.
"""
