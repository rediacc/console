...
THE THING BEING REPLACED is `.ci/scripts/lib/common.sh:18-55`: the colour block
and the five `log_*` functions. It is the canonical bash logger -- 47 files under
`.ci` assign their own `RED=`, but common.sh's is the one every library in
`.ci/lib/` inherits (`local-common.sh:21` and `service.sh:14` both source it, and
`run.sh:20` sources local-common.sh), and it is the only variant that tests the
stream it actually writes to.

HOW EQUIVALENCE IS PROVEN. Not by reading both and agreeing they look alike. A
bash child runs the REAL common.sh, with stdout and stderr captured SEPARATELY
and, where the case needs one, a real pseudo-terminal on stderr. The bytes it
produced are compared against the bytes `rediacc_ci.log` produces under the same
conditions. That is the shape `.ci/scripts/quality/check-python-lint.sh` uses for
its own control: build a specimen, run the instrument, compare.

WHY THE STREAMS ARE NEVER MERGED. The defect these tests exist to prevent is a
stream swap -- `emit-advisory.sh` moved log_info from stderr to stdout on
2026-09-06 and nothing noticed. `test-emit-advisory.sh:100-102` says it plainly:
...
THE TWO DIVERGENCES ARE ASSERTED, NOT AVOIDED. A differential that only tests
where two implementations agree is a differential that will be quietly broken by
the first person who "fixes" a difference nobody wrote down. Both places where
this module deliberately differs from common.sh have a test asserting the
...
def test_off_tty_stdout_stays_empty_on_both_sides():
    ...
    The bash side is the real regression: this is what `emit-advisory.sh` broke.
    The Python side is structural -- `Logger` has no stdout path at all -- but it
    is asserted rather than argued, because "it cannot happen" is what the bash
    library's author believed too.
    """
...
def test_on_a_stderr_tty_bytes_are_identical():
    ...
    disagreement between common.sh and .ci/bootstrap.sh cannot be inherited
    silently -- picking the wrong one turns this red.
    """
...
def test_on_a_stderr_tty_the_output_really_is_coloured():
    ...
    Without this, a pty that silently failed to attach would make both sides
    uncoloured, they would still match, and the comparison would prove nothing.
    Every differential needs the control that says the interesting branch was
    the one taken.
    """
...
def test_stdout_tty_with_a_piped_stderr_stays_uncoloured_on_both_sides():
    ...
    common.sh gates on `-t 2` and writes to fd 2, so a tty on stdout does not
    make it colour. `rediacc_ci.log` tests the stream it writes to, so neither
    does it. The eleven files listed in the module docstring WOULD colour here,
    and the next test plants that variant to show the difference is real.
    """
    _, _, err = diff.bash_streams(
...
def test_planted_the_eleven_file_variant_does_colour_here():
    ...
    is what those eleven files do. With a tty on stdout and a pipe on stderr it
    paints escape sequences into the pipe. If this ever stops reproducing, the
...
def test_ci_true_is_the_one_deliberate_divergence():
    ...
    module has one, because GitHub's log viewer renders escape sequences as
    literal text (`emit-advisory.sh:22` states exactly that), and because a
    devbox with CI=true exported is a real place where a tty and CI coexist.

    BOTH SIDES ARE ASSERTED so the divergence is a recorded decision. If someone
    later adds a CI clause to common.sh, this test fails and the two can be
    reconciled deliberately rather than drifting into agreement unnoticed.
    """
    env = diff.env_for(CI="true", NO_COLOR=None)
    _, _, err = diff.bash_streams(BASH_CALLS, env=env, tty="stderr")
...
def test_bash_interprets_escapes_in_the_message_and_python_does_not():
    ...
    common.sh logs with `echo -e`, which expands backslash escapes in the MESSAGE
    body. A Windows path or a regex printed through it is silently mangled. This
    module formats the message as data.
...
def test_the_differential_harness_can_tell_the_two_streams_apart():
    ...
    Every case above rests on `bash_streams` returning stdout and stderr in the
    right slots. A harness that returned them swapped, or merged, would make the
    stream assertions vacuous while everything still looked green.
    """
...
def test_the_coloured_form_wraps_only_the_glyph():
    ...
    that difference is exactly how the two libraries produced different bytes for
    the same call. This module follows common.sh, and the differential above
...
def test_the_default_logger_is_built_lazily_against_the_current_stderr(capsys):
    ...
    pytest's capsys, `contextlib.redirect_stderr` and a subprocess wrapper all
    REPLACE sys.stderr after import. `default()` therefore builds on first use.
    """
    log._default = None
    log.error("late")
...
def test_the_palette_matches_common_sh_byte_for_byte():
    ...
    This is a SECOND, independent check on the palette: the differential above
    compares rendered output, this compares the source constants. It catches the
...
def test_common_sh_is_actually_there():
    ...
    `source` of a missing file under `set -e` would abort the child, the child
    would print nothing, and a comparison against a Python side that also printed
    nothing... would not arise, because the Python side always prints. But the
    two DEBUG-off cases compare empty to empty, and those would pass on a tree
    where common.sh had been deleted. This is the base case under that.
    """
