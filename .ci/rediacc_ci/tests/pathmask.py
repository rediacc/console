...
WHY THIS EXISTS. Several cases in this suite exercise what a subject does when a tool is MISSING -- `require_cmd aws` refusing, a forward stopping before it can reach R2 -- and they established that condition by ASSUMING the host did not have the tool. `test_release_backfill_write_sentinel.py` even said so out loud, in a test whose only job was to check the assumption:
...
That assumption held on every developer machine here and is false on a GitHub runner, which ships the AWS CLI at `/usr/local/bin/aws`. Measured in CI run 34970782616: three cases failed, and the honest reading is not that CI is wrong but that the tests were never exercising the refusal they claim to -- they were exercising whatever the host happened to make true.
...
(`test_security_shfmt.py`'s scratch-cache case) and it works today only because `aws` happens to live in `/usr/local/bin` on the runner. It is a guess about where a tool is, and the next tool, or the next image, makes it wrong silently -- which is the exact failure mode being fixed here. Masking by construction cannot be wrong about that.

HOW. Every PATH entry that does NOT contain the tool is kept verbatim. An entry that DOES is replaced by a scratch mirror: one symlink per name in that directory, minus the tool. So everything else that directory provided still resolves, and only the named command disappears.
"""
...
def path_without(tool: str, scratch: pathlib.Path, base: str | None = None) -> str:
    ...
    `scratch` is a directory this may create mirrors under -- a pytest `tmp_path` in practice. Returns the new PATH string.
    """
...
def assert_absent(tool: str, path: str) -> None:
    ...
    module exists to retire. Cheap, and it makes the precondition an assertion instead of a hope.
    """
