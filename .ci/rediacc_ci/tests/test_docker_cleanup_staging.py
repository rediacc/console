...
RECORDING FAKES FOR `gh` ON A SCRATCH PATH, AND NOTHING ELSE REAL. No case here
names a credential, and the scratch PATH is built from an explicit list of
symlinks, so a tool that is not listed is genuinely absent rather than inherited
...
machine reached out to `ghcr.io/token` before the twin's first log line. Nothing
in this file can do that.

`jq` IS REAL, NOT FAKED, and deliberately. The port shells out to the same jq
the twin does (see the module docstring for why), so faking it would replace the
thing under comparison with a stub on BOTH sides and prove nothing about the
filters. It is a pure data tool with no remote side.

THE CALL LOG IS THE PRIMARY EVIDENCE. What this script does that matters is
DELETE a GHCR package version, and the fake's stdout is fixed rather than
derived from the argv it was handed, so a port that deleted the wrong version id
or the wrong package would print identical bytes and exit 0.
`test_planted_defect_is_caught_only_by_the_call_log` plants exactly that.

THREE OF THE TWIN'S DEFECTS ARE PINNED HERE RATHER THAN FIXED, because fixing a
twin is a cutover-box decision: a missing `gh` reports success, two versions
sharing one tag build one malformed URL, and a quote in the tag is swallowed.
Each has its own `test_defect_*`.
"""
...
def fixture(tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    BOTH SIDES ARE COPIED IN rather than run from this checkout, because the twin
    resolves common.sh and constants.sh from its OWN location and constants.sh
    then resolves `.devcontainer/toolchain.env` from ITS own location. Running
    the real files would drive them against the real tree.
    """
    root = tmp_path / "repo"
    for rel in (
        ".ci/scripts/docker",
...
def _mask(text: str) -> str:
    ...
    `set -u` message. Masked by the two exact paths, so any OTHER absolute path
    leaking into the output is still compared.
    """
...
def _agree(old, new, label: str, old_calls: str = "", new_calls: str = "") -> None:
    ...
    a message moving between stdout and stderr is invisible once merged.
    """
...
def test_stderr_arriving_before_the_json_is_merged_in_order(tmp_path) -> None:
    ...
    The fake writes a warning to stderr and flushes before writing valid JSON to
    stdout. Under a true merge the warning lands FIRST and jq rejects the whole
...
def test_the_delete_calls_own_stdout_is_inherited_and_interleaves(tmp_path) -> None:
    ...
    two blank lines would arrive AFTER both children's output, byte-identical
    and in the wrong order. This is the only comparison that sees it.
    """
    old, new, old_calls, new_calls = run_both(tmp_path, ["--tag", "staging-abc"])
...
def test_defect_a_missing_gh_reports_two_successes_and_exits_zero(tmp_path) -> None:
    ...
    staging tags survive, and the run prints `Cleanup summary: 2 succeeded` and
    exits 0. The `2>&1` capture turns `command not found` into an unparseable
    response, which the twin files under "package may not exist yet".
    """
...
def test_defect_two_versions_sharing_a_tag_build_one_malformed_url(tmp_path) -> None:
    ...
    splices the whole `"111\\n222"` into a single URL. The port reproduces the
    malformed call rather than picking one id, because picking would change
    WHICH version gets deleted.
    """
    old, new, old_calls, new_calls = run_both(
...
def test_defect_a_quote_in_the_tag_is_swallowed_as_already_deleted(tmp_path) -> None:
    ...
    syntax error, jq's stderr is discarded and the empty result reads as "may
    already be deleted". Nothing warns that the filter never ran.
    """
...
def test_defect_a_non_ghcr_registry_yields_the_host_as_the_org(tmp_path) -> None:
    ...
    leaves `docker.io` and `%%/*` then returns `docker.io` as the ORG. The API
    path becomes `/orgs/docker.io/...`, which is nobody's org.
    """
    old, new, old_calls, new_calls = run_both(
...
def test_planted_defect_is_caught_only_by_the_call_log(tmp_path) -> None:
    ...
    The plant deletes the version under the WRONG package (`renet` for both
    images). Every printed byte and the exit code are unchanged by it, so this
    also demonstrates that a streams-only comparison would have blessed it.
    """
    root = fixture(tmp_path)
...
def test_the_constants_are_still_constants_shs() -> None:
    ...
    sourcing constants.sh (which hard-requires `.devcontainer/toolchain.env`).
    This is the alarm that makes the copy safe.
    """
