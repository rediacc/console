...
WHAT IS ACTUALLY AT RISK HERE, because the obvious assertion is the wrong one.
The derivation returns ZERO groups on this tree today (29 of 29 ported modules
resolve against the lock and none of them is in `mutex | reads`), so a floor
saying "at least one group exists" would be red the day it was written and would
be suppressed the week after. It is deliberately absent.
...
EVERY FIXTURE IS BUILT BY CONSTRUCTION, never by substituting into real source,
so rewording run-all.sh or the lock cannot silently void a control.
"""
...
def test_only_fallback_arrays_are_read_from_the_runner(tmp_path) -> None:
    ...
    THE FIRST SPELLING OF THIS FIXTURE WAS BROKEN, AND IT IS WORTH THE LINE. The
    non-fallback array was named `NOT_A_FALLBACK`, which `endswith("_FALLBACK")`
    is perfectly happy with, so the control fired against the control rather than
    against the reader. The name here must not end in `_FALLBACK` for the same
    reason a probe file must not be named so the glob matches it.
    """
    lock, runner = _write(tmp_path)
...
def test_the_real_join_is_not_empty() -> None:
    ...
    An empty union makes `test_no_ported_twin_is_a_real_tree_writer_or_scanner`
    admit every twin including the four that rewrite tracked files, and makes the
    group derivation unable to serialise anything. Zero GROUPS is the correct
...
def test_the_parity_test_and_the_scheduler_read_the_same_union() -> None:
    ...
    `test_twin_parity.real_tree_tests` is now a call into this module. If someone
    reinstates a local copy there, the two will agree on the day it is written
    and diverge later, which is the failure mode that made moving it worth doing.
    """
...
def test_the_ported_corpus_agrees_with_the_parity_test_about_who_is_unsafe() -> None:
    ...
    are computed here from opposite ends and must be equal -- so this stays true
    the day the first real-tree twin is ported, instead of being a hardcoded 0
    that would have to be edited then.
    """
...
def test_the_port_scan_module_still_declares_its_group() -> None:
    ...
    `find_consecutive_free_ports(7, 20000, 30000)` returns the FIRST free run, so
    every worker picks 20000. No lock entry can express that -- the resource is
    the host's port space, not the tree -- so the declaration lives in the module
    and this is what stops it being tidied away.
    """
...
def _collect(target: str, expr: str) -> subprocess.CompletedProcess:
    ...
    A SUBPROCESS because the question is whether the REPO-ROOT CONFTEST attaches
    the marker during collection, and the collection that is running this test
    already happened. `-m` is pytest's own marker expression, so the answer comes
...
def test_the_root_conftest_marks_exactly_the_declaring_module(target, expr, want_any) -> None:
    ...
    A conftest that marked EVERYTHING would pass cells 1 and 2 and fail 3 and 4.
    A conftest that was never loaded would pass 3 and 4 and fail 1 and 2. Only a
    conftest that marks the declaring module and nothing else passes all four.
    """
