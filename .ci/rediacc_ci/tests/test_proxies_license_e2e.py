"""`rediacc_ci.proxies.license_e2e` against its bash twin `.ci/scripts/test/proxies/proxy-license-e2e.sh`, already wired as the registered gate `check:ci-proxy-license-e2e` (`package.json:387`, `scripts/ci-runner/manifest.ts:7435-7444`, `kind: 'local-only'` since CI runs the identical subject at `.github/workflows/ct-tests.yml:1848`, a workflow outside `paritySurface()`). The bash
proxy is what the gate calls, unchanged by this port -- same as every other proxy in this package, only the shim is ported here, not the call site.
