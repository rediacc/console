...
WHY A DIFFERENTIAL AND NOT A TABLE OF EXPECTED STRINGS. The two decisions this
gate makes are made by git and by the shell, not by the gate: `[[ -e $root/$p ]]`
and `git ls-files --error-unmatch -- "$p"`. The second one is the interesting
one, because its answer for a DIRECTORY is not what a reader guesses -- it
succeeds when the directory contains tracked files, which is why the obvious
"read the index once and test membership" optimisation is wrong. Running the
real command and comparing is the only form of this test that can fail for the
right reason.

The real closure list is 78 paths long and comes out of a CommonJS program, so
this file drives `scan` on hand-built path lists instead. The whole gate over
real closures is what the committed shadow ledger
`.ci/shadow/w7p2-greenlight.observations.jsonl` compares over five distinct
...
def repo(tmp_path: pathlib.Path) -> pathlib.Path:
    ...
    Five states, because the gate distinguishes three of them and a fixture with
    fewer cannot show that the third is distinguished at all.
    """
...
def test_a_symlink_to_a_deleted_target_is_absent_on_both_sides(repo: pathlib.Path) -> None:
    ...
    Stated as a test rather than as prose because it is the one place where "on
    disk" and "the name exists in the directory" disagree, and a port using
    `lstat` would report NOT ON DISK where the twin reports NOT TRACKED.
    """
...
def test_the_tracked_directory_is_why_the_batched_lookup_is_wrong(repo: pathlib.Path) -> None:
    ...
    Eighteen closures name `.github/actions/bws-secrets`, a directory, so this is
    the shape of the very incident this gate exists for. A port that read the
    index into a set would report every one of them as untracked.
    """
...
def test_zero_paths_is_a_refusal_not_a_pass(repo: pathlib.Path) -> None:
    ...
    iteration with `p` empty, which `[[ -n "$p" ]] || continue` skips, leaving
    `n` at zero.
    """
...
def test_the_untracked_case_prints_two_lines_and_the_missing_case_one(repo: pathlib.Path) -> None:
    ...
    `scripts/lib/shadow-gate.ts` folds an indented line into the finding above
    it, so the second explanation line is part of the finding. A port that
    dropped it would change the finding set on every untracked path.
    """
...
def test_a_mixed_list_counts_every_path_including_the_offenders(repo: pathlib.Path) -> None:
    ...
    A count that silently excluded offenders would make the number shrink as the
    tree got worse, which is the opposite of what a shape line is for.
    """
...
def test_the_node_extractor_reads_the_real_closure_file() -> None:
    ...
    A control on the INPUT, not on the verdict: without it every case above could
    be passing while the extractor returned nothing on the real file, which is
