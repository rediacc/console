...
def real_tree_twins(lock: pathlib.Path, runner: pathlib.Path | None = None) -> set[str]:
    ...
    MOVED HERE FROM `test_twin_parity.real_tree_tests`, which now calls this, so the scheduler's idea of what needs isolating and the parity test's idea of what may be driven cannot drift apart. Two copies of this union would be two answers to one question, and the expensive half of that is that both would look right.

    `lock` and `runner` are PARAMETERS rather than module constants so the controls can drive both directions on fixture files. That is the whole reason the signature takes them.

    An unreadable lock contributes NOTHING rather than raising, which is `battery.classify_from_lock`'s documented behaviour and is the caller's decision to interpret: "nothing is declared yet" and "the lock is broken" must not silently become "nothing needs isolating". The union with the runner's arrays is what keeps a broken lock from emptying the answer, and the anti-vacuity
    refusal on the total lives with the callers.
    """
...
def group_for(module: object, unsafe: set[str] | None = None) -> str | None:
    ...
    `unsafe` defaults to the real join, and the conftest passes it in explicitly so the lock and run-all.sh are read once per session rather than once per item across roughly nine thousand of them.
    """
