...
#
# NAMED cleanup_staging.py, NOT cleanup-staging.sh, as of agent/PLAN-w7p4w-docker-cutover.md Stage 5: that plan cut the twin's own call site over to the Python entry point, and this port forwards to whatever the twin forwards to (see the module docstring's "FORWARDED TO RATHER THAN REIMPLEMENTED" -- this constant is the one place that forwarding target is named, and it must track
# the twin's, not stay pinned to the pre-cutover name).
