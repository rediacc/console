...
Version guard: refuses to publish a version whose tag or GitHub Release already exists. Two probes, `git tag -l` (after `git fetch --tags`) and
...
message here is copied byte-for-byte from the twin's own `echo` lines, which were already deliberately written (not a bash diagnostic wrapper).
"""
