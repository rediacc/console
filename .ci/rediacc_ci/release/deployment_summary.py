...
Appends the release job's closing summary to `$GITHUB_STEP_SUMMARY`. Pure
string emission: no network, no subprocess, one file append. Same shape as
`rediacc_ci.deploy.resolve_account_deploy_config`, except this twin has
nothing to decide between, so this port has no branch either -- the whole
comparison surface is "did every line come out identical".
...
carries a bash line number this port does not reproduce. The written summary
text (the actual point of the script) is byte-for-byte identical on both
