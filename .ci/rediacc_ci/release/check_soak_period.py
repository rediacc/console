...
DATE PARSING IS SHELLED OUT, NOT REIMPLEMENTED. The twin tries GNU `date -d`
first, then BSD `date -j -f "%Y-%m-%dT%H:%M:%S"`, so it accepts a wider set of
`EDGE_DATE` spellings than a hand-rolled `datetime.fromisoformat` would (GNU
`date -d` parses far more than ISO-8601). Re-deriving that parser would be a
second implementation of a contract the system `date` binary already owns, and
would drift from it silently on some future EDGE_DATE this port never saw
during review. Running the *exact* twin expression through `bash -c` keeps the
two sides looking at the same parse, byte for byte.
...
assignment, so its exit status is the exit status of the last command
substitution performed -- and `set -e` therefore aborts the *whole script*
right there if both date attempts fail, before any output is produced.
Measured directly: `EDGE_DATE=not-a-date SOAK_DAYS=7 bash check-soak-period.sh`
exits 1 with nothing on stdout, nothing on stderr, and `$GITHUB_OUTPUT`
untouched. This port raises `SystemExit(1)` at the same point with the same
silence, rather than "fixing" it with an error message the twin never printed.
...
two differ only when `EDGE_DATE` is in the future (a negative age), which the
scripts do not defend against either way, so this port uses
`int(delta / 86400)` -- `int()` on a float also truncates toward zero -- to
keep that (mis)behaviour identical rather than accidentally fixing it here.
"""
