...
class Checker:
    ...
    WHAT IT REPLACES. Four `.ci/scripts/quality/check_*.py` gates --
    `check_fetch_retry.py`, `check_git_history_depth.py`,
    `check_judged_rule_wiring.py` and `check_review_prompt_render.py` -- opened
    `selftest()` with the SAME twelve lines, down to the docstring::
    ...
    `check:ci-shape-duplication` reported it as a ten-line span across four files,
    the largest Python finding in the tree, once the quality family entered its
    corpus. Nothing in those twelve lines is per-gate: the LABELS and the
    CONDITIONS are, and they stay at the call site untouched.

    IT IS CALLABLE SO THE CALL SITES DO NOT MOVE. `check = Checker()` then
    `check("label", cond)` is exactly what the four files already write, at roughly
    a hundred call sites between them. `harness.py` makes the same argument for the
    ported bash vocabulary: "rewriting them all is not a consolidation, it is a
    rewrite with its own defect budget".

    WHY NOT `Controls` ABOVE, which is the same idea. Two reasons, and the second
    is the real one. `Controls` is a got/want tally whose failure line is
    `FAIL  <label>: got <got!r>, wanted <want!r>`; these four assert a BOOLEAN, for
    which that message would read `got False, wanted 'something truthy'` -- the
    contradiction `Controls.falsy` exists to avoid, one level up. And these four
    print `  PASS  <label>` on the way past, two spaces indented, which is what a
    reader watching `--selftest` scroll sees; `Controls` prints a count at the end.
    Migrating the output shape is a separate change with its own argument, and
    folding it in here would smuggle it past review as tidying.
    """
...
def controls_first(name: str, selftest) -> int:
    ...
    call site as an argument. `check:ci-shape-duplication` reported the block as two
    overlapping findings, seven copies and three, once the Python quality family
    entered its corpus.

    `selftest` IS A CALLABLE, NOT A RESULT, so the banner is printed before the
    controls run rather than after. That ordering is the point of the idiom: a
    reader watching a gate scroll past sees which gate is talking before it says
    anything, and a control that hangs or crashes has already been attributed.

    RETURN 2, NOT 1, and it is not arbitrary. These gates use 1 for "the tree has a
    finding" and 2 for "the instrument is broken, so there is no verdict". A caller
    that collapsed the two would report a defective gate as a defective tree.
    """
...
class VacuousPlantError(AssertionError):
    ...
    An `AssertionError` and not a bespoke base, because that is what every
    caller's test runner already treats as a failed assertion -- which is
...
def plant(text: str, old: str, new: str, count: int = -1) -> str:
    ...
    `.ci/scripts/quality/check-control-vacuity.sh` exists to catch it -- but only
    on the bash side. Its own comment recorded that no Python gate built a mutant
    by substitution and that "what must not happen is that changing silently".
    It changed, from 21 gates to 50, and the alarm was a count printed inside a
    SUCCESS message, which nothing reads. Measured 2026-09-08: 71 substitution
    sites across 12 Python gate modules, three proofs between them, and one
    already-vacuous mutant in the tree.

    RAISING IS THE POINT, and it is why this is not a checked return. A returned
    flag can be ignored by the caller who most needed it, and many of these
    fixtures are built at MODULE level where no `Controls` instance exists yet to
    record a failure against. An exception needs no receiver and cannot be
    dropped.

    Three separate refusals, deliberately distinguishable, because they are three
    different author mistakes:
...
def plant_re(text: str, pattern: str, repl: str, flags: int = 0) -> str:
    ...
    A SEPARATE FUNCTION rather than a mode flag on `plant`: a single entry point
    taking either would need a boolean at every call site, and a boolean argument
    that changes how the other two are interpreted is how a caller ends up
    passing a regex to the literal path and getting a silent no-op -- the exact
