...
NO `---- gate ----` HEADER ON ANYTHING HERE, INCLUDING `run_renet`. That header is what `scripts/gate-bind.ts` reads to build the gate estate, and `run-renet.sh` already carries it. A second file claiming `id: check:ci-renet` would give one gate id two owners and the parity meta-gates would be right to complain. The bash twin stays the LIVE registered gate; the module beside it is
a verified-equivalent alternative and nothing invokes it yet.
...
CUTOVER IS A SEPARATE, LATER BOX. `package.json:251` still spells `"check:ci-renet": ".ci/scripts/private/run-renet.sh quality"`, and `.github/workflows/ct-tests.yml:1718/1746/1752` still invoke all three bash twins. Repointing any of those, and the `scripts/ci-runner/manifest.ts` `leaves` that follow from it, is a driver-only step gated on the ledgers this box produces. It is not
done here.
