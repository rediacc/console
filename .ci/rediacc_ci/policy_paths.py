...
`RUNNER_ADVICE_ALLOWLIST`, `PLAN_HK_ALLOWLIST` and `PROFILER_COVERAGE_ALLOWLIST` are read by their gates and short-circuit the call; none of them is read here. An override consulted INSIDE the seam would be a second live location, which is rule 3 defeated by the back door, and it would also cost a `os.environ` read on a function whose whole contract is that it reads nothing.
"""
...
# The repository root, derived from the package's own location. `paths.CI_DIR` is `<root>/.ci`, computed once at import of that module; taking its parent here costs no filesystem call, whereas `paths.repo_root()` would `is_dir()` an override on every call and break rule 1.
_STATIC_ROOT = paths.CI_DIR.parent

# The directory, relative to the repository root, that holds the policy files. `.ci/policy` since W4 P2 (commit b80552370). MUST equal POLICY_DIR in scripts/lib/policy-paths.ts; check:ci-policy-inventory compares the two.
...
POLICY_FILES: tuple[str, ...] = (
    ...
    # D0, 2026-09-09. THE ONLY NON-DOTFILE AND THE ONLY .json IN THIS SET, and both are deliberate. It is a pinned MEASUREMENT of what the hook wiring costs, not a list of exempted entries, so a name-per-line dotfile could not hold it; and it is policy by the README's four-part predicate all the same, because every number in it is a claim about the world that a change to
    # .claude/settings.json can stop being true. It is reached through policy_path() from .ci/rediacc_ci/quality/hook_exec_baseline.py and nowhere else.
