...
One Cloudflare `purge_cache` request for the whole `media.rediacc.com` hostname. The twin's header records why it exists: a new CORS policy or a republished file does NOT retroactively fix an already-cached response, and the media objects are
...
NOTHING HERE REACHES CLOUDFLARE IN A TEST. `curl` is the tool that carries the credential, so the differential (`.ci/rediacc_ci/tests/test_deploy_purge_media_cache.py`) puts a RECORDING FAKE `curl` on a scratch PATH that logs its exact argv and answers from a fixture. The zone id and the hostname are HARD-CODED in the twin, not arguments, so the request shape is the whole contract
and the log is compared as well as the two streams.

`jq` IS CALLED, NOT REIMPLEMENTED, and here the reason is sharper than in the sibling ports because the two jq call sites have DIFFERENT `set -e` exposure:
...
THE FAILURE THIS SCRIPT CANNOT REPORT, named because it is the vacuity class and a reader should not have to rediscover it: A TRANSPORT FAILURE IS A SILENT NON-ZERO. `curl -s` (no `-S`) suppresses curl's own error message, so an unreachable API produces the step line, no diagnostic whatsoever, and exit 6. The caller sees a failed step with no reason. Reproduced, not repaired:
...
def auth_headers(env: dict[str, str]) -> list[str] | None:
    ...
    NOTE THE HEADER ORDER DIFFERS FROM `cf-purge-urls.sh`: this script emits `X-Auth-Key` BEFORE `X-Auth-Email`, its sibling emits them the other way round. Neither order matters to Cloudflare and both matter to a recorded argv, so each port carries its own twin's order rather than a shared helper's.
    """
...
def _jq(args: list[str], stdin_text: str) -> str:
    ...
    Both call sites in this twin are substitutions in a position `set -e` ignores (a `[[ ]]` test and an argument to `log_error`), so a failing jq contributes its stderr and an empty string, and the script carries on. jq's stderr is INHERITED for exactly that reason: it is the only trace left.
    """
    proc = subprocess.run(
        ["jq", *args],
        input=stdin_text,
