...
Deletes one PR channel's R2 artifacts, plus the `-promoted` copy the
promotion-simulation test leaves behind. Runs on PR close, which is why every
delete is best-effort: a channel that was never created is not an error.
...
NOTHING HERE REACHES R2 IN A TEST. `aws` is the only external tool involved and
it carries the credential, so the differential
(`.ci/rediacc_ci/tests/test_deploy_delete_r2_channel.py`) puts a RECORDING FAKE
`aws` on a scratch PATH that logs its exact argv. The call log is the main
evidence for this script: two implementations can print identical output while
deleting different prefixes, and the OBSERVABLE EFFECT of this program is
entirely the set of `rm` calls it makes.

`aws` IS SHELLED OUT TO, NOT REPLACED BY `boto3`, and the reason is the same one
that keeps `jq` in the Cloudflare siblings: the twin's contract with the tool
includes the tool's own stdout (`delete: s3://...` lines a workflow log shows),
its exit status, and the `2>/dev/null` that hides its stderr. A boto3 port would
have to re-emit all three.

THE DEFECT THIS PORT REPRODUCES, AND IT IS THE VACUITY CLASS. A REFUSED DELETE
IS REPORTED AS A DELETION. Every call is `2>/dev/null || true`, and the closing
line is unconditional, so an expired key or an `AccessDenied` produces exactly
the output a successful clean-up produces:

    ✓ Cleaning up R2 channel: pr-1...
    ✓ Channel 'pr-1' (+ promoted) deleted from R2

exit 0, with the twelve failures silent. Driven 2026-09-13 with a fake `aws`
exiting 1 on every call. The header justifies `|| true` by "a channel that was
never created is not an error", which is true and does not distinguish that case
from a credential that stopped working. `A_REFUSED_DELETE_READS_AS_DELETED`
names it and the differential pins it in both directions. Reproduced rather than
repaired because the acceptance rule for this wave is agreement with the live
twin.

ONE DIVERGENCE, in refusal text nobody parses. The five `: "${VAR:?msg}"` guards
are bash's own diagnostic, `<path>: line 25: CHANNEL: CHANNEL is required (e.g.
pr-123)`, carrying the bash file name and a bash line number. This port prints
the `VAR: msg` half without the prefix, on the same stream, with the same exit
status 1. Identical ruling to `deploy/wait_for_preview_worker.py`. The ORDER of
the guards is observable and is kept: `require_cmd aws` runs FIRST, so a run
missing both the binary and every variable names the binary.

`:?` IS AN UNSET-OR-EMPTY TEST, not an unset test: `CHANNEL=` refuses exactly as
an absent CHANNEL does. Reproduced, and driven in the differential, because a
port testing `"CHANNEL" in os.environ` would sail past an empty one and then
delete `s3://bucket/cli//`, which is every channel's parent prefix.
...
def prefixes(bucket: str, channel: str) -> list[str]:
    ...
    Exposed as a pure helper so the differential can assert the SET as well as
    compare it against the twin: the observable effect of this program is which
    prefixes it removes, and a port that reordered them would still print the
    same two log lines.
    """
    out: list[str] = []
    for fmt in FORMATS:
...
def _rm(argv: list[str]) -> int:
    ...
    STDOUT IS INHERITED and stderr is DISCARDED, exactly as the twin leaves
    them: the `delete: s3://...` lines reach the workflow log, the error text
