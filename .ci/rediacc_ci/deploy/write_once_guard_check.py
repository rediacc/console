...
WHAT IS PORTED AND WHAT DELIBERATELY IS NOT. The HARNESS moves to Python: the
temp dir, the fake `aws`, the bundle assembly, the four cases, the PASS/FAIL
lines and the exit code. The SUBJECT stays bash and is still driven as bash --
`write_once_guard` is a bash function that is `sed`-extracted out of
`upload-to-r2.sh` and sourced beside the real `release-state-validator.sh`. A
Python reimplementation of the guard would be a second instrument certifying
itself, which is the reason `.ci/scripts/test/lib/git-fixture.sh` and
...
THE OUTPUT IS BYTE-IDENTICAL, ANSI ESCAPES AND `->` ARROWS INCLUDED. This is a
registered gate whose stdout a human reads in a CI log, so `PASS:` is emitted
...
UNSET again in the caller afterwards. Neither is obvious -- POSIX mode makes
such assignments persist -- and a port that set them once on the process would
leak state from one case into the next. Here they are passed per invocation.

`grep -q ... && log_fail ...` DOES NOT ABORT WHEN THE GREP FAILS, even under
`set -e`, because a command that is not the last in an `&&` list is exempt. So a
...
sides make their own temp dir, so any output quoting a temp path would differ.
Nothing in the twin's output quotes one, which is why the differential can
compare bytes.
