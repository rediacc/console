...
Writes the `.released` commit sentinel, which is the ATOMIC COMMIT POINT of the release pipeline: after this, `write_once_guard` in `upload-to-r2.sh` refuses to overwrite the bytes and the drift gate requires a matching git tag. The twin's header carries the contract; this docstring records only what the PORT decided.
...
`aws` AND `jq` ARE SHELLED OUT TO. `aws` because it is a credentialed tool with a real remote side, which is precisely the kind the campaign fakes on `PATH` rather than reimplements; `jq` because the payload IS jq's bytes. `jq -nc`
...
THE ONE THING NEITHER SIDE CAN MAKE IDENTICAL is `released_at`. The twin stamps `date -u +'%Y-%m-%dT%H:%M:%SZ'` and this stamps `datetime.now(UTC).strftime(...)`; two processes started a second apart produce two different payloads. Nothing in either program's OUTPUT quotes the timestamp, so stdout and stderr still compare byte for byte; the differential compares the uploaded
payload with that one field normalised, and asserts separately that both sides emit the same FORMAT.
...
def parse_flags(argv: list[str]) -> tuple[str, str, str]:
    ...
    Returns `(version, channel, commit_sha)`, any of which may be empty; the required-flag checks are the caller's, exactly as in the twin.
...
def main(argv: list[str]) -> int:
    ...
    # Ratchet advance is intentionally NOT done here; cd-v2.yml's tag-and-release job owns `.ci/config/release-contract-floor.txt` and already holds the app-token that can commit it.
    return 0
