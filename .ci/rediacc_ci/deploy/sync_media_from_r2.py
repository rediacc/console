...
The DOWNLOAD direction of the media sync: three `aws s3 sync` calls that pull
`tutorials/video/`, `videos/` and `tutorials/audio/` out of the R2 bucket
`rediacc-www-media` into `packages/www/public/assets/`. Its upload counterpart
is `sync-media-to-r2.sh`, ported beside this file as
`rediacc_ci.deploy.sync_media_to_r2`. The two are twins in the campaign's sense
and in the tree's: same bucket, same three prefixes, same flag vocabulary, and
the differences between them are exactly the differences between reading a
bucket and writing one.

WHY A CHECKOUT NEEDS THIS AT ALL, carried over from the twin's header because it
is the reason the audio prefix is here beside two video prefixes. Most
`npm run dev` browsing does NOT need any of it: the URL builders read the
manifest and point at media.rediacc.com, so the site fetches videos from the CDN
exactly as production does. `tutorials/audio/` is different. It is a build-time
TTS cache read off LOCAL DISK by the tutorial-video pipeline, is not exposed on
the public CDN at all, and is reachable only through this S3 API path, so
restoring it is REQUIRED before re-running `./run.sh www tutorials
generate|video` on a fresh checkout.

-----------------------------------------------------------------------------
NOTHING HERE REACHES R2 IN A TEST
-----------------------------------------------------------------------------
`aws` is the one external tool that carries a credential, so the differential
(`.ci/rediacc_ci/tests/test_deploy_sync_media_from_r2.py`) puts a RECORDING FAKE
...
THE CALL LOG IS THE PRIMARY EVIDENCE FOR THIS SCRIPT. Everything it prints is
three `Restoring ...` lines plus at most one closing line, none of which is
derived from what actually moved; the entire observable effect is the argv of
the `aws s3 sync` calls, in order, with their filter flags. Two implementations
can print identical output while syncing a different prefix, omitting
`--dryrun`, or reordering the three legs.

-----------------------------------------------------------------------------
`mkdir` IS CALLED, NOT REIMPLEMENTED
-----------------------------------------------------------------------------
`mkdir -p` is an external binary here (bash has no `mkdir` builtin), so the twin
genuinely execs `/usr/bin/mkdir` and, under `set -e`, hands its exit status and
its stderr BYTES straight to the caller. Driven 2026-09-13 with a regular file
sitting where `tutorials/video/` belongs: the run printed `mkdir: Already
exists` and exited 1, before the step line. `os.makedirs` would print a Python
traceback for the same condition, with a different status. Calling the same
binary is what keeps those bytes identical, and it is the same ruling
`promote_r2_to_stable.py` makes for `find` and `sed`.
...
-----------------------------------------------------------------------------
When `CLOUDFLARE_R2_MEDIA_SECRET_ACCESS_KEY` or `CLOUDFLARE_R2_MEDIA_ENDPOINT` is
UNSET, bash's own refusal names the bash FILE and a bash LINE NUMBER:
...
The differential asserts BOTH directions so nobody "fixes" either side into the
other.

THE ORDER OF THE THREE REFUSALS IS PART OF THE CONTRACT and is reproduced
exactly: `require_var` on the access key, then `require_cmd aws`, then the
secret, then the endpoint. A machine with no `aws` and no credentials at all
reports the MISSING BINARY, not the missing key.
...
class BashExitError(Exception):
    ...
    Both `mkdir -p` and `aws s3 sync` are unguarded in `restore_dir`, so the
    failing program's own stderr is the only explanation the caller gets and its
...
def parse_args(argv: list[str]) -> Options:
    ...
    the LAST one wins: `--tutorials-only --audio-only` restores audio alone. A
    port that OR-ed them would sync two prefixes where the twin syncs one.
    `--dry-run` is orthogonal and may appear anywhere in the line.
...
def environment() -> dict[str, str | None]:
    ...
    whole of fact 2 in the module docstring: `set -u` fires on the first and not
...
def require_secret_key(env: dict[str, str | None]) -> str:
    ...
    accepted and exported, which is fact 2.
    """
...
def _run(argv: list[str], env: dict[str, str]) -> None:
    ...
    them either, and `aws --no-progress` still writes a `download: ...` line per
...
def restore_dir(remote_prefix: str, local_dir: str, args: list[str], env: dict[str, str]) -> None:
    ...
    `mkdir -p` FIRST AND UNCONDITIONALLY (fact 3), then the step line, then the
    sync. The order matters to the output: a `mkdir` that fails produces its own
    stderr and NO step line at all, so a reader sees the failure without ever
    learning which prefix was being restored.
    """
