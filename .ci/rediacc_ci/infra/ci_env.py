...
-----------------------------------------------------------------------------
`set -e` LEAKS INTO THE CALLER. The twin's line 15 runs in the sourcing shell and stays set after it returns. `ci_start_elite.py` already documents that; a Python caller of `configure()` has no such side effect, which is a difference in the port's favour and is listed here rather than hidden.

THE `.env` HEADER LINE IS COPIED WITH ITS PUNCTUATION AS FOUND. It reads `# Auto-generated by ci-env.sh - do not edit` in the twin with an em dash where the hyphen is here; the byte sequence is reproduced verbatim in `ENV_HEADER` because docker compose reads that file and a differential compares it. It is quoted, not authored.
...
# The seven names masked as a block (:94-100). SYSTEM_ADMIN_PASSWORD is masked separately, later, on its own line (:109), and the order matters because stdout is one of the four observables.
MASKED = (
...
# two that carry one. THE ORDER IS THE FILE'S CONTENT and is asserted against the twin's own heredoc by the tests, because `check-compose-env.sh` parses this same block out of the twin to decide whether a compose variable is persisted -- a name dropped here is a variable that silently arrives empty in a later workflow step.
...
# QUOTED, NOT AUTHORED: the twin's own first line of the .env file, em dash and all. See the module docstring.
...
def strip_and_cut(text: str) -> str:
    ...
    `tr` deletes the three characters anywhere; `cut -c1-64` then takes the first 64 CHARACTERS OF EACH LINE. `openssl rand -base64 48` emits exactly one 64-character line, so the result is at most 64 characters and usually fewer -- the twin's name for this value is a 64-character key and it is not one, which is cosmetic and preserved.
...
def configure(env: dict[str, str] | None = None, root: pathlib.Path | None = None) -> Config:
    ...
    # :51-63 and :68-80. The public key is generated ONLY when the PRIVATE one was absent, which is hazard (a).
    ...
    if src.get("GITHUB_ACTIONS", ""):
        # `echo "::add-mask::$VAR"` with VAR unset prints the bare directive, which is what hazard (a) produces for a missing public key.
...
def apply(env: dict[str, str] | None = None, root: pathlib.Path | None = None) -> Config:
    ...
    # PRINTED BEFORE THE WRITES, because the twin prints them at :94-109 and writes at :165. A failed write must leave the masks already emitted.
    ...
    try:
        ...
        # bash prints `<script>: line 165: <path>: No such file or directory` and `set -e` ends the run with 1. The path and the status are the same here; the wording is this port's own, because inventing bash's line number would be worse than saying which file could not be written.
...
# THE SHELL-PRIVATE NAMES ARE NOT FILTERED HERE, ON PURPOSE. `_`, `SHLVL`, `PWD` and `OLDPWD` describe the SHELL rather than the configuration (`ci_start_elite.py:107` carries the same set), and comparing them would compare bash against python3. But a filter inside the port is a filter the twin has no equivalent of, which is exactly the shape that lets a port drop a name and still
# look equal. So `env` prints everything and BOTH SIDES of the differential are filtered identically, in the harness.
