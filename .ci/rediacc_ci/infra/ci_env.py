...
def apply(env: dict[str, str] | None = None, root: pathlib.Path | None = None) -> Config:
    ...
    try:
        ...
        # bash prints `<script>: line 165: <path>: No such file or directory` and `set -e` ends the run with 1. The path and the status are the
        # same here; the wording is this port's own, because inventing bash's
        # line number would be worse than saying which file could not be written.
