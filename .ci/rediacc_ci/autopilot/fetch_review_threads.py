...
THE NETWORK HALF OF THE REVIEW PAYLOAD (03-v2-autonomy.md section 6, residual 1: "review-response rounds receive a gate-built payload filtered by comment author before the model sees any text"). It runs in the GATE job, with `github.token` and read-only permissions, so the fetch itself holds no write capability. The
FILTERING is `review-payload.sh`, which is pure and therefore testable offline;
keeping the two apart is what makes the security decision -- whose text reaches the model -- reviewable without a network.
...
-----------------------------------------------------------------------------
"No threads" and "could not ask" must not share an output, so `--out` is written ONCE, at the very end, after every page of every target has come back. A port that streamed pages into the file as they arrived would leave a truncated array behind on a mid-pagination failure, and the reader of that file has no way to tell it from a short PR. The `open(out_path, "wb")` is therefore
the LAST thing `main` does, after every return-1 path, and `test_the_out_file_is_absent_after_every_failure` asserts the file does not exist rather than asserting it is empty -- because an empty file is a value a reader could act on, and absence is not.

THE LINKED TARGETS ARE THE EXCEPTION, AND THE TWIN EXPLAINS WHY AT LENGTH: this script runs in the GATE, which by invariant holds no app token, and `github.token` is scoped to console, so a private submodule's PR is simply not readable from here. Killing the whole round over that would take fix rounds down
with it. So each linked target degrades on its own, with a `::warning::` on
STDOUT (an Actions annotation, not a log line), and the console target stays mandatory.
...
-----------------------------------------------------------------------------
`fetch_target` is invoked ONLY from an `||` list and an `if !`, and bash DISABLES `set -e` for the whole body of a function called that way. So a failing
`ALL_NODES="$(jq -n ...)"` inside it does not end the run: the variable is set
to the empty string and the loop carries on, and the next page's `--argjson acc ""` then fails too. The end state is an `--out` file containing one blank line and a summary line reading `fetched review thread(s)`. Every input that can reach that jq has already been validated as JSON by `gh_json`, so it is defence-in-depth failing quietly rather than a live bug -- but it is the one
place in this script where a failure does not fail closed. Preserved exactly (see `_accumulate`), named here, and pinned by `test_a_broken_accumulator_does_not_end_the_run`.
...
def gh_json(what: str, args: list[str], *, sleeper=time.sleep) -> tuple[bool, str]:
    ...
    Three attempts, a `log_warn` between them, a 3-then-6-second backoff, and the captured stderr replayed indented four spaces on final failure. The exit status is ALWAYS checked and a failure is never turned into an empty answer, which is the whole reason `_gh_probe` exists: the shape it replaced
...
def page_args(query: str, owner: str, name: str, number: str, after: str) -> list[str]:
    ...
    string form. Getting that pair backwards makes the server reject the query, so the two spellings are part of the request and are compared as such. `after` is OMITTED on the first page rather than passed empty, because `after: ""` is not the same query as `after: null`.
    """
    argv = [
        "-f",
        "query=%s" % query,
...
class Fetcher:
    ...
    def _accumulate(self, page_json: str, target: str, number: str) -> None:
        ...
        SET -e IS OFF HERE. See the module docstring's hazard: this function is only ever reached from a call site that disables errexit, so a jq failure sets the accumulator to the empty string and the run continues. Reproduced rather than repaired.
        """
        code, out = _jq(
            [
                "-n",
