...
a `---- gate ----` header, and why the call-site cutover is a separate, later
box. `docker/` held exactly 3 bash scripts and zero Python before this box, and
this one ports all three.

NONE OF THE THREE IS A GATE, and two of them are workflow `run:` targets. The
call sites are listed rather than summarised, because a cutover box needs the
list and because an earlier draft of this docstring claimed all three were
called only from other scripts, which the grep disproved:
...
So no `scripts/ci-runner/manifest.ts` entry follows from a module living here
(these are `run:` targets and plumbing, never gates), and every one of those
call sites still names the BASH twin. Flipping them is the cutover box, not this
one.

WHAT "VERIFIED EQUIVALENT" MEANS HERE, and it is narrower than for the quality
gates. Every one of these three talks to a REGISTRY. The differential drives
both sides against recording fakes for `docker` and `gh` on a scratch PATH and
compares stdout, stderr, the exit code and the exact argv of every external
call; the shadow-gate ledger under `.ci/shadow/w7p6-*.observations.jsonl` scopes
its `--finding-re` to those recorded calls, so the compared finding set IS the
set of registry operations each side would have performed. Nothing in this
