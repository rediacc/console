...
Decides, in ONE place, what version number an artifact ships with, and hands it
out under four names: `APP_VERSION`, `VITE_APP_VERSION`, `CLI_VERSION` and the
legacy `TAG`. Resolution order, unchanged from the twin:
...
-----------------------------------------------------------------------------
`inject-env.sh` is on the language-policy allowlist with the reason "sourced by
build-cli-executables.sh to export variables into the CALLER's shell -- a child
process cannot mutate its parent's environment in any language", and that reason
is still true and is not what this port disputes. The twin has TWO callers'
shapes, and only one of them is unportable:
...
So "equivalent" here has FOUR observables, and `test_version_inject_env.py`
compares all four: exit code, stdout, stderr, and the exported set the caller is
left holding. Comparing only the exit code would compare almost nothing -- the
happy path exits 0 in every case this script has.

-----------------------------------------------------------------------------
THE RESOLVER IS SPAWNED, NOT IMPORTED
-----------------------------------------------------------------------------
`resolve-version.sh --current` is run as a subprocess from the path the twin
computes (`<dir of the twin>/resolve-version.sh`), derived here from this
module's own location. `rediacc_ci.version.resolve_version` is deliberately NOT
imported: it is a separately-verified port with its own ledger, and importing it
would make THIS differential's agreement depend on THAT port's correctness
rather than on this one's. Two ports agreeing because they share an
implementation is not evidence.

`2>/dev/null` in the twin is reproduced: a resolver that is missing, that is not
executable, that is not in a git repository, or that prints a diagnostic, all
fall through to the fallback in silence. The twin's own comment records why the
`&& [[ -n ... ]]` half exists -- a resolver that exits 0 printing NOTHING used to
propagate an EMPTY version through `--strict`.
...
sets the version to the literal string "--strict", never enables strict mode,
prints `--strict` on stdout and exits 0. The one guard this file exists to
provide is silently switched off by a caller whose `$NEXT_VERSION` expanded to
nothing, which is the EXACT failure the twin's own `--version was given an empty
value` check was added for -- an unquoted empty expansion drops the argument
entirely rather than passing "". Reproduced byte for byte and pinned by
`test_a_flag_swallowed_as_a_version_is_a_twin_defect`, reported rather than
fixed: a port that rejected it would fail differently from the script it claims
to be equivalent to, and the repair is a cutover-box decision.

-----------------------------------------------------------------------------
ENVIRONMENT IS READ AT THE CALL SITE, ONE NAME AT A TIME
-----------------------------------------------------------------------------
`os.environ.get("VERSION", "")` and nothing else. No `env = dict(os.environ)`
alias: the env-manifest reader parses direct `os.environ` reads, and an alias
makes the name invisible to it.
"""
...
class UsageError(Exception):
    ...
    Carries the message verbatim so `main()` can print it and `inject()` can
    hand it back to a library caller without either re-deriving the text.
    """
...
def resolver_path() -> pathlib.Path:
    ...
    Derived from this module's own path, matching the twin's `BASH_SOURCE[0]`
    derivation rather than cwd or `$REDIACC_CI_ROOT`: a differential that
    pointed one subject at a fixture and the other at the real tree would
    diverge for a reason that has nothing to do with the port.
    """
...
def parse_args(argv: list[str]) -> tuple[str, bool, bool, bool]:
    ...
    `given` is separate from a truthy `override` because the twin distinguishes
    them: `--version ""` is an ERROR, while no `--version` at all falls through
    to `$VERSION`. Folding the two would turn a refusal into a silent fallback,
    which is the very case the twin's comment says let a release-path caller
    build the version it had already published.
    """
    override = ""
    given = False
    strict = False
...
def _resolver_output() -> str:
    ...
    `.strip()`: a resolver that printed leading whitespace would keep it in
    bash, and a port that trimmed it would disagree.
    """
...
def strict_refusal(version: str) -> str | None:
    ...
    Three and not one: the twin's own comment says the original check compared
    only against the literal `0.0.0-dev`, so an EMPTY version sailed through it,
    and anything non-numeric (a curl error page, "none", "latest") reached a
    build define.
    """
    if not version:
...
def inject(argv: list[str]) -> tuple[int, dict[str, str], list[str], list[str]]:
    ...
    This is what a SOURCING caller needs and what `main()` is a thin printer
    over. `exports` is empty on every refusal path, matching the twin: it
    `return`s before reaching its `export` statements, so a caller that sourced
    a failing invocation is left with its previous values, not with new ones.
    """
    try:
