...
WHAT IT CHECKS, per library in `LIBS`:
  ...
  - a `.ci/lib/*.sh` file missing from `LIBS` is a finding, so a new library cannot bypass the gate.
...
# Library -> the Python modules that carry its twin. Every .ci/lib/*.sh must appear here.
LIBS: dict[str, tuple[str, ...]] = {
    "account.sh": ("account.py", "account_lifecycle.py"),
    "devbox.sh": ("devbox.py",),
    "local-common.sh": ("local_common.py",),
    "service.sh": ("service.py",),
}
...
def _prefix(lib: str) -> str:
    return lib.removesuffix(".sh").replace("-", "_") + "_"


def candidates(fn: str, lib: str) -> set[str]:
    bare = fn.removeprefix(_prefix(lib))
    out = {fn, bare, fn.lstrip("_"), bare.lstrip("_")}
    if fn in ALIASES:
        out.add(ALIASES[fn])
    return out
