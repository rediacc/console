...
Ported from `.ci/scripts/quality/check-commit-identity.sh`, retired in W7 P5 once the K=5 shadow ledger licensed the equivalence and `.ci/rediacc_ci/tests/gates/test_gate_commit_identity.py` reached the same verdicts on the same fixtures; see `rediacc_ci.quality.__init__` for the invariant that kept both copies alive until then.
...
The twin's header, carried whole because the measurement, the oracle argument and the rejected alternative are each load-bearing:
    ...
    Usage:
      GITHUB_TOKEN=xxx PR_NUMBER=123 ./check_commit_identity.py
      ./check_commit_identity.py --refresh      # regenerate the local identity cache

-----------------------------------------------------------------------------
PORT NOTES.
-----------------------------------------------------------------------------

THE PAYLOAD IS ONE COMPACT JSON OBJECT PER LINE, and both the count and the verdict depend on that. The twin counts commits with `grep -c .`, which counts LINES, and then feeds the same text to `jq` as a stream of values. Those two readings only agree while gh emits one object per line, which it does; a pretty printed payload would make the completeness refusal fire on every PR.
The port parses line by line for the same reason, so the two implementations disagree in the same way if that ever changes rather than one of them silently coping.
...
# Keep only lines that are actually addresses. The guard against an API error body being mistaken for data; see the note in `refresh_identity`.
...
CACHE_COMMENT = (
    "GENERATED by check_commit_identity.py --refresh. Do not hand-edit: it is a "
...
def email_tally(rows: list[dict]) -> list[tuple[int, str]]:
    ...
    Only `.author`, NOT `.committer`: the summary is about which ADDRESS is unlinked, and a null committer with a resolved author is a different defect. `sort -rn` is descending by count; ties keep `uniq -c`'s order, which is the ascending email order `sort` produced.
    """
    counts: dict[str, int] = {}
    for row in rows:
