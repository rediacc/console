...
Ported from `.ci/scripts/quality/check-commit-identity.sh`, which is NOT deleted; see `rediacc_ci.quality.__init__` for why both copies live.
...
# The `$comment` written into the generated cache. Carried byte for byte; it is the only thing telling the next reader not to hand-edit a derived file.
CACHE_COMMENT = (
