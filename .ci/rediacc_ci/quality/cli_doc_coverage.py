...
against `docs/agent-reference/ci-gates.md`'s flag table, which was ALSO missing three real flags (--heavy-limit, --manifest, --list) the moment this ran against it.

Deliberately NOT folded into every self-improving skill: `testing` (`.claude/skills/testing/SKILL.md`) is a pure router across six docs with no single script whose CLI surface it owns, so there is nothing to flag-diff there. And `rdc`'s CLI docs already have their own, different mechanism (`scripts/gates/check-cli-docs.ts`, generation-based, checks the OPPOSITE direction -- no
stale flag mentioned that doesn't exist) -- not reinvented here.

Extraction is genuinely per-script-family: Python argparse and this repo's own hand-rolled TS switch/case parser look nothing alike on the page, so each PAIRS row names which extractor reads it, rather than one regex pretending to cover both.

Controls are built by CONSTRUCTION (a real doc copy with one real flag's mention stripped), never by pattern-substituting real source, so rewording a target cannot silently void them -- `check-control-vacuity.sh` exists to catch a control that cannot fire.
...
the pin is what makes that true rather than a coincidence of the developer's locale.

THE CONTROL'S `sed` IS A PLAIN GLOBAL SUBSTITUTION and is reproduced as `str.replace`, including its blunt edge: replacing `--changed` also hits `--changed-only` if such a flag existed. That over-reach makes the control STRICTER, never weaker -- more flags go missing from the fixture, and the assertion only asks whether the TARGET went missing -- so it is carried rather than
refined.
"""
...
def extract_python_argparse(script: pathlib.Path) -> list[str]:
    ...
    Scoped to the line AFTER each `add_argument(` call, not every quoted `--flag` in the file: see the port notes for the subprocess-flag reason. Verified against ci-trace.py directly before trusting it.
    """
...
def extract_ts_switch_case(script: pathlib.Path) -> list[str]:
    ...
    CLI parsers. There is no Commander or yargs in this codebase's own tooling scripts, which is why a parser-specific pattern is the right instrument.
    """
...
def selftest() -> int:
    ...
    `REDIACC_CI_ROOT` is what makes this possible without touching a tracked file, and it is the same seam the differential uses. See `rediacc_ci.paths`: one variable for the whole program, rather than the eight per-gate `*_ROOT` names it replaced.
    """
