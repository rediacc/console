...
THE DEFECT THIS COMES FROM, 2026-08-17. run-all.sh fans the gate battery out over
the runner cores in three sets: W (writers, one serial chain, exclusive), S
(scanners, read the real tree, released after W), and T (everything else, isolated
by construction). test-generate-tag-inputs.sh swaps the REAL
.ci/scripts/version/resolve-version.sh for a stub and restores it a second later,
and it was classified T -- in the pool -- on the strength of its own comment, which
claimed it "cannot disturb a shared tree". That was true of the tag namespace it
carefully avoids writing and false of the working tree it overwrites.

WHY THIS CLASS IS WORTH A GATE RATHER THAN A CODE REVIEW. The symptom is not a
clean failure. A concurrent gate read resolve-version.sh mid-restore and reddened
gate-test:claude-hooks with a bash syntax error in a file that parses clean and
passes 884/0 serially. That is a CONCURRENCY ARTIFACT: it does not reproduce on
the serial re-run, so the re-run "clears" it and the next session pays for the
diagnosis again. docs/agent-reference/TRAPS.md carries the entry. The
misclassification is invisible to every other gate in the repo, and it is cheap to
...
decidable in general, and a fuzzy version gets skimmed and then suppressed, which
is the failure mode this repo keeps finding. So the rule is ONE mechanical shape,
chosen because it is what all three real writers actually look like: a write whose
TARGET PATH is rooted at a variable that derives from the repo root, and not at
one that derives from mktemp.
...
file declared W that no longer writes is NOT reported. Over-declaring costs a
...
classifies every variable as REPO-ROOTED or TEMP-ROOTED and propagates that
through assignment; pass 2 reports writes aimed at a repo-rooted target. Two
passes rather than one because a function body can textually precede the
assignment of a global it uses.

The taint SEED is the idiom, never the name: any variable assigned from
`$(cd ... && pwd)`, `get_repo_root`, or `git rev-parse --show-toplevel`. That is
what makes it self-maintaining -- 79 of the 93 gate tests spell it REPO_ROOT and 4
use get_repo_root, but a new file picking a different name is still caught. It
also, correctly, taints SCRIPT_DIR: the gates directory is itself inside the real
tree, so a write aimed there is a real-tree write.
...
THE WRITE TARGET IS THE LAST ARGUMENT, not the whole line. Taking the whole line
instead reads the source path and, for sed, the expression -- which is how an
earlier draft flagged test-installmethods-args.sh:141, where a real-tree dir
appears in the REPLACEMENT TEXT while the file being edited is a temp copy. That
is a false positive of exactly the kind that trains a reader to skim this gate.
`rm`, `mkdir`, `chmod`, `touch` and `truncate` take a LIST of targets rather than
a source/target pair, so for those every non-flag argument is a write target.

`&` AND `<` RULE OUT fd duplication and input redirection, and `-` rules out the
ASCII arrow: test-generate-tag-inputs.sh:309 logs "($before -> $after ->
$restored)", and $restored is repo-rooted, so without this the gate reports a log
line as a write.

CONTROL FIRST. A gate whose green has never been contrasted with a red is a gate
nobody has checked. This one plants both directions and refuses to report on the
real tree unless BOTH land. The NEGATIVE half is not decoration. A scanner that
flagged every file would satisfy the positive control perfectly while being
worthless, and "it fired" is the reassuring half of the evidence. The planted
temp-writer uses the exact mktemp-into-a-root-shaped-name pair that broke the
first draft.

-----------------------------------------------------------------------------
THE ARCHAEOLOGY OF THE THREE STACKED DEFECTS, 2026-09-06. This is why the twin is
the shape it is, and it is carried verbatim because a port that dropped it would
leave the next reader free to make all three again.
-----------------------------------------------------------------------------
...
THE AWK IS TRANSLITERATED, NOT REWRITTEN, and the awk-isms are kept because they
are behaviour. `split(line, a, /[ \t]+/)` with a REGEX field separator produces a
leading EMPTY field for an indented line, so `nonflagargs`, which starts at index
2, skips that empty field and then treats the COMMAND NAME as an argument on an
indented line while skipping it on a flush-left one. That asymmetry is harmless
(a command name carries no `$VAR`, so `reftype` ignores it) and it is preserved
rather than tidied, because tidying it is a change to the corpus of things the
gate looks at.

POSIX ERE IS LEFTMOST-LONGEST AND PYTHON IS LEFTMOST-FIRST. Every pattern here
was checked for a case where the two differ and none does: the assignment pattern
backtracks to the same answer for `local=1` and `exportFOO=1`, the redirect
pattern's `>>?` is greedy in both, and `test-[A-Za-z0-9._-]+\\.sh` lands on the
same token in both engines. Stated because "the regex is the same text" is not
the same claim as "the regex matches the same thing".

THE MULTI-LINE `printf '  %s\\n' "$hits"` IS A TWIN DEFECT, CARRIED. `$hits` is
ONE argument holding several newline-separated lines, so printf applies the format
ONCE and only the FIRST line gets its two-space indent. A port that indented every
line would print different bytes and, worse, would change the finding set:
`scripts/lib/shadow-gate.ts` promotes an unindented `<file>.sh:<line>:` to a
finding by its path-line rule and an indented one by its continuation rule, so
both are compared either way, but the TEXT differs. Reported, not repaired.

THE ONE-DIRECTIONAL RULE MEANS THIS GATE HAS NO SHRINK-ONLY BASELINE and needs
none: it reports only NEW unregistered writers, and a registration that outlived
its write is deliberately not a finding.
...
def _nonflagargs(line: str) -> str:
    ...
    Index 2 in awk is index 1 here. The awk quirk that this skips the leading
    empty field on an indented line, and therefore includes the command name, is
...
def _reftype(text: str, safe: set[str], taint: set[str]) -> str:
    ...
    SAFE WINS over taint, which is the propagation order that took an earlier
    draft's 21 wrong lines down to 6. A path built from a temp root is a temp
    path however repo-shaped the variable's NAME is.
    """
    has_safe = False
    has_taint = False
...
class _Heredoc:
    ...
    A heredoc BODY is skipped in both passes: 40 of the 93 gate tests carry one,
    and a redirect quoted inside it is text rather than a write.
    """
...
def scan_text(text: str, fname: str) -> list[str]:
    ...
    Returns one `<fname>:<line>: <body>` per repo-rooted write site, in file
    order. Empty means clean. Exported so `--selftest` and a pytest case can plant
    both directions without touching the disk.
    """
    lines = text.split("\n")
...
def _log_fail(message: str) -> int:
    ...
    THE HELPER THAT DID NOT EXIST. See archaeology item 1: all three call sites
    are anti-vacuity refusals, every one of them was `command not found`, and the
    gate exited 127 instead of refusing. 127 is not a verdict.
    """
    log.error(message)
    return 1
...
def get_repo_root() -> pathlib.Path:
    ...
    the bash function does, and which is the same answer as
    `rediacc_ci.paths.repo_root()` in every tree either can see. `paths` is used
    so a harness pointing the package at a fixture with $REDIACC_CI_ROOT moves
...
def selftest() -> int:
    ...
    BOTH DIRECTIONS FOR EVERY RULE the scanner has, because a scanner that flagged
    everything would satisfy the positive controls perfectly while being useless,
    and that is the failure the twin's own negative control exists to catch.
    """
