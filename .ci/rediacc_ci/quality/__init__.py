...
WHY A SUBPACKAGE AND NOT `check_*.py` FILES ON THEIR OWN. Contract section 5d
requires a gate's ENTRY POINT to live where `scripts/gate-bind.ts` can see it --
`.ci/scripts/quality/`, or `.ci/rediacc_ci` since the 2026-09-06 amendment -- and
says the entry point may be "three lines of import and dispatch" while the logic
lives in the package. That split is what this directory is the other half of.

The split is not bureaucracy. A gate's logic is the part a PYTEST case wants to
call directly, and a module that is only reachable by running a script is a module
whose error paths are tested by nobody. Every port here exposes `main(argv) -> int`
and a `selftest()`, both importable, so the same code is driven three ways: by the
entry point in `.ci/scripts/quality/`, by its own `--selftest`, and by a test.

THE BASH TWIN IS NOT DELETED, and that is a W7 phase-5 decision rather than a
courtesy. Until a port has a committed differential ledger row over K distinct
trees (`scripts/lib/shadow-gate.ts`, invariant 5), the evidence that it kept the
twin's verdict does not exist, and deleting the twin would destroy the only thing
the port can be compared against. Both files therefore live side by side, the bash
one still registered and still the gate CI runs, until the ledger says otherwise.

WHAT EVERY PORT IN HERE PRESERVES, because these are the properties the tree's
gates are judged on and a port that keeps the happy path and loses these has
kept nothing:
