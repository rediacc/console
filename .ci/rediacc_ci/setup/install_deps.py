...
`npm ci` for the root workspace plus the three `private/account` trees, each
wrapped in `retry_with_backoff 3 10`, with `--ignore-scripts` added on Windows.
Ninety-seven lines of bash, more than half of it comment, and the comments are
the reason this file is worth reading rather than skimming: two of them record
measured incidents that a bare transcription would drop.

-----------------------------------------------------------------------------
THE LOCKFILE WARNING, CARRIED OVER VERBATIM IN SUBSTANCE
-----------------------------------------------------------------------------
`install-deps.sh:10-14` states it and it is not about this script's code at all,
it is about what a reader is tempted to do when this script fails: the lockfile
(`package-lock.json`) must contain resolved entries for ALL platform-specific
optional deps (rollup, lightningcss, esbuild). Never regenerate it by deleting
it on a single platform -- that drops the entries for every other platform.
Update individual packages with `npm install <pkg>` instead, which preserves the
cross-platform entries. The advice is repeated here because the natural reaction
to "npm ci failed three times" is to delete the lockfile, and the twin's authors
clearly met someone who did.

-----------------------------------------------------------------------------
THE `npm cache clean --force` THAT WAS REMOVED, AND WHY IT IS RECORDED
-----------------------------------------------------------------------------
`install-deps.sh:32-42` keeps a NOTE about code that is no longer there, and it
is the single most expensive fact in the file, so it is preserved rather than
summarised. The script used to run `npm cache clean --force` "to avoid
corruption issues on CI runners". It was actively harmful:
...
Callers now cache `node_modules` itself (`.github/actions/setup-workspace`) and
this script only runs on a cache MISS, where a cold npm cache is strictly worse.
`retry_with_backoff` is what actually handles transient corruption. A port that
dropped this paragraph would leave the next person free to "fix flakiness" by
adding the cache clean back.

-----------------------------------------------------------------------------
ALL THREE ACCOUNT TREES, WHICH IS ALSO A RECORDED DECISION
-----------------------------------------------------------------------------
`install-deps.sh:74-80`: `private/account`, `private/account/web` and
`private/account/e2e` used to be installed ad hoc by whichever job happened to
need them (quality-lint installed all three inline so knip could resolve
imports), which meant the precondition "account deps are installed" meant
something different in every job. One list here makes the cached account tree
canonical.

-----------------------------------------------------------------------------
FIVE FACTS ABOUT THE TWIN THAT LOOK LIKE MISTAKES. ALL FIVE ARE REPRODUCED
-----------------------------------------------------------------------------
Reported to the driver rather than repaired here: fixing a twin is a cutover-box
decision, and a port that silently improves on its original stops being evidence
about the original.
...
-----------------------------------------------------------------------------
When `npm` is not on PATH at all, bash writes its OWN diagnostic once per
attempt and gives the failed command status 127:

    .ci/scripts/setup/install-deps.sh: line 60: npm: command not found

`subprocess.run` cannot produce that line -- it raises `FileNotFoundError`, and
an unhandled one would replace a five-line report with a twelve-frame traceback,
which is strictly worse than the twin. So `spawn` below catches it and writes
bash's own wording, `npm: command not found`, WITHOUT the `<script>: line <n>: `
prefix, which names a bash file that does not exist in this implementation and a
line number that would be a lie.

Everything else on that path is identical: three attempts, the 10/20 backoff, the
two closing error lines, exit 1. `test_a_127_from_a_missing_npm_...` asserts the
divergence as exactly that prefix, by stripping it from the twin's bytes and
...
copy would move the caller. `os.chdir` here has the same effect on this process
and the same non-effect on its parent, which is what a caller expects. Every
path below the chdir is relative in both implementations, so the working
directory is part of the contract rather than a detail.
...
`npm $NPM_ARGS` IS UNQUOTED IN THE TWIN, deliberately, so that `ci
--ignore-scripts` word-splits into two arguments. That is a word-splitting
construct that works only because the values are known-safe literals; this port
builds an argv LIST, which cannot split wrongly and cannot be widened by a value
containing a space. Same argv, structurally instead of by luck.
"""
...
class Options:
    ...
    A class rather than an `argparse.Namespace` because the twin's parser is not
    argparse and must not be replaced by one: argparse would REFUSE an unknown
    argument (fact 4 above), print a usage block to stderr and exit 2, where the
    twin ignores it and exits 0. That difference would show up on the first
    differential row as a mismatch in a case the port had "improved".
    """
...
def parse_args(argv: list[str]) -> Options:
    ...
    A `for arg in "$@"` over a three-arm `case` with no default arm. Two
    properties that follow and are both reproduced:
...
def secs(value: float) -> str:
    ...
    floats, and `"%s" % 10.0` is `10.0`, which is one character of divergence in
    a line the differential compares byte for byte. Integral values print as
    integers and anything else keeps its decimal, so a future non-integral delay
    would be visible rather than silently truncated.
    """
...
def retry(action) -> bool:
    ...
    THE TWO ERROR LINES ARE NOT ONE. On exhaustion the helper itself prints
    `Command failed after 3 attempts` and returns 1, and only then does the
    CALLER print its own `Failed to install dependencies...`. Both land on
    stderr, in that order, and a port that emitted only the caller's line would
    lose the half that says how many attempts were made.
    ...
    No exit code is inspected, so a 127 (npm absent) and a 1 (registry refused)
    are the same event -- fact 5 above.
    """
...
def npm_argv(*, ignore_scripts: bool) -> list[str]:
    ...
    Separated from `main` so the differential can assert the argv without running
    anything, and so the one place the flag is added is visible next to fact 1 --
    the account trees do NOT come through here.
    """
    argv = ["npm", "ci"]
    if ignore_scripts:
...
def wants_ignore_scripts(*, requested: bool, os_name: str) -> bool:
    ...
    `CI_OS` is `common.sh:509`, computed at SOURCE time from `uname -s` and
    exported. `core.common.detect_os()` is the ported spelling and answers the
    same four words (`linux`, `macos`, `windows`, `unknown`) with the same
    fail-open arm.
    ...
    lifecycle scripts. It is still passed, because a caller reading the npm
    command line should be able to see the intent without knowing about `.npmrc`.
    """
...
def spawn(argv: list[str], cwd: str | None = None) -> bool:
    ...
    THE `FileNotFoundError` ARM IS THE WHOLE REASON THIS IS A FUNCTION. Bash
    prints `<script>: line <n>: npm: command not found` and moves on to the next
    retry; Python raises, and an unhandled raise here would turn the twin's
    five-line report into a traceback whose first useful frame is
    `subprocess._execute_child`. The message is bash's own wording minus the
    prefix that names a shell script, and it is written once per ATTEMPT because
    that is when bash writes it -- so a run against an absent `npm` still shows
    three of them.

    `PermissionError` and every other `OSError` take the same arm: from the
    caller's point of view "the binary did not run" is one event, and bash's
    `Permission denied` is the same shape of line.
    """
    try:
...
def run_account(directory: str) -> bool:
    ...
    THE SUBSHELL IS THE POINT of the twin's parentheses: the `cd` must not
    outlive the call, because the loop's next iteration and the closing
    `log_info` both assume the repo root. `cwd=` on one child has the same
    property and cannot leak by construction.

    NO `--ignore-scripts`, EVER. Fact 1. Written as a literal list here rather
    than as `npm_argv(...)` with a False argument, so that the absence is a
    visible decision in the code and not an argument someone later "fixes".
    """
