...
#
# ONE ESCAPED LITERAL EACH, ON ONE LINE, and that is deliberate: they were
# generated by slicing the twin rather than retyped, so there is no place a
# hand-wrapped continuation could lose a backslash. Reflowing them into adjacent
# literals would preserve the VALUE and is harmless, but there is nothing to gain
...
# `--limit 60 --budget 90` (:436). Named so a test can assert the twin still
# carries the numbers its own comment argues for at :414-435.
GREENLIGHT_LIMIT = "60"
...
def main(argv: list[str]) -> int:
    ...
    def _dual_write(text: str) -> None:
        ...
        if summary_path is not None:
            ...
            # `SUMMARY` defaults to `/dev/stdout`, so `tee -a` writes the same
            # bytes to the same fd a second time.
            sys.stdout.write(text)
            sys.stdout.flush()
    ...
    if shallow_file:
        ...
        try:
            ...
            raw = b""
        if raw:
            # `wc -l <"$shallow_file"` counts NEWLINES, so a final line with no
            # terminator is not counted -- which is what a graft file never has.
    ...
    if base and head:
        ...
        if not tee_head(classify_json, WIDE_HEAD_BYTES):
            # DEFECT 2: unreachable, because the redirection above created the
            # file whether or not node wrote to it.
            emit("(no output)")
        emit("```", "")
    else:
