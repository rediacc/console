...
it puts `v1.10.0` above `v1.9.0`, which a Python `sorted()` does not without reimplementing git's `versioncmp`. A second implementation of that ordering is the one thing in this script that could pick a different tag from the twin on a real repository, so the port runs the same command with the same arguments. The `git fetch --tags --force --no-recurse-submodules origin` on the
shallow-clone path is likewise the twin's, including its `2>/dev/null || true`, so a fetch that fails for any reason leaves the ladder to fall through to `latest`.
...
`len()` always counts characters. The two can only disagree on a non-ASCII tag, and the regex check at :112 -- `^[a-zA-Z0-9._-]+$` -- runs FIRST and rejects every non-ASCII tag before the length check is reached. So the order of the two validations is what makes the two implementations provably identical, and reordering them would break that.
"""
...
def parse_argv(argv: list[str]) -> tuple[str, bool, bool]:
    ...
    NOT `common.parse_args`. The twin does not use it here, and the differences are observable: this loop REFUSES an unknown option (`*)` -> exit 1) where `parse_args` silently accepts every `--flag`, and it treats `-h` -- a single dash -- as help where `parse_args` would swallow it as the previous flag's value.
...
def git_tags_exist() -> bool:
    ...
    A missing git, or a directory that is not a work tree, produces no stdout and therefore reads as "no tags", exactly as the twin does.
    """
...
def git_fetch_tags() -> None:
    ...
    STDOUT IS INHERITED, because the twin redirects only stderr. Failure is swallowed, which is the twin's `|| true`, and so is a missing binary.
    """
