...
WHY THIS IS THE FIRST MODULE IN THE PACKAGE. Every Python program in this tree begins by answering the same two questions -- "where is the repo root" and "how do I import the module next door" -- and today they are answered 33 times, in five different spellings, none of which agree about what happens when they are wrong. Measured 2026-09-06 across `.ci`, `.claude` and `scripts`:
...
Phase 4 sweeps those call sites onto this module. This phase provides it and proves it.
...
class RootError(RuntimeError):
    ...
    A distinct type rather than a bare RuntimeError so a harness can tell "you pointed me at a path that is not there" from any other failure, and so the gate's own controls can assert on it without matching a message string.
    """
...
def repo_root() -> pathlib.Path:
    ...
    Raises RootError when $REDIACC_CI_ROOT is set to something unusable. That is the whole reason this returns a Path instead of `Path | None`: the two outcomes a caller must distinguish are "here it is" and "your override is wrong", and `None` collapses both into a value that reads as a missing repo.
    """
...
def relative_to_root(path: os.PathLike[str] | str, root: pathlib.Path | None = None) -> str:
    ...
    Gates print paths at humans, and an absolute path in a finding is noise that differs per machine. The fallback matters as much as the happy path: a file genuinely outside the repo -- a tempdir fixture, a submodule reached by symlink -- must still be printable rather than raising ValueError in the middle of a failure report, which is where a crash is least welcome.
    """
...
def find_repo_root(start: os.PathLike[str] | str) -> pathlib.Path | None:
    ...
    RETURNS THE NEAREST, WHICH MAY NOT BE THE ONE YOU WANT. This tree contains repositories inside the repository (`private/renet` is a submodule, `private/growth` a gitignored sibling checkout), so a `start` under one of them answers with that inner repo -- correctly, and not usefully. Prefer `repo_root()`, which is derived from this file's own location and is immune to where the
    caller happens to be standing. Use this only when a path is genuinely all you have.

    `None` rather than a guess when nothing matches, because a guess here produces a root that reads as valid and finds nothing in it.
    """
...
def on_sys_path(directory: os.PathLike[str] | str) -> str:
    ...
    THE REPLACEMENT FOR 33 HAND-WRITTEN HOPS, and it differs from them in one way that matters: it is idempotent. `sys.path.insert(0, d)` run twice -- a module imported once as `__main__` and once by name, which is what happens when a gate imports another gate -- leaves two copies of the directory on the path. Harmless until a later hop tries to shadow it and finds an older copy
    still ahead.

    FRONT, not append, because that is what every existing call site does and the intent is real: these directories hold modules whose names (`paths`, `output`, `worklist`) can collide with site-packages, and first-party must win. Deliberately not sorted or de-duplicated beyond this entry: reordering someone else's sys.path is not this function's business.
    """
...
def walk_tree(
    ...
    *,
    ...
    A DROP-IN REPLACEMENT, yielding the same `(dirpath, dirnames, filenames)` triple, so a call site swaps `os.walk(x)` for `paths.walk_tree(x)` and keeps every downstream behaviour it already had. That is deliberate: the callers are PORTS whose finding text, path spelling and sort order are compared against a bash twin, and a helper that returned a tidy `list[Path]` would have
    forced seventeen behaviour changes to fix one bug.

    THE BUG THIS EXISTS FOR, because it is not the obvious one. `.claude/worktrees/` holds sibling CHECKOUTS OF THIS SAME REPOSITORY, created for isolated sub-agent sessions. It is excluded via `.git/info/exclude`, so `git ls-files` and every CI checkout are blind to it -- but a raw `os.walk` over the filesystem is not. On 2026-09-13 a colleague's stale worktree carrying a copy of
    `.ci/scripts/private/license-mint` turned three gates red (`test_quality_go_module_sync`, `test_security_shfmt`, `test_gate_vacuity_floors::test_shfmt_accepts_the_real_corpus`): the SAME real file, scanned twice, failing only in the copy, whose relative `../../../../private/renet` could not resolve from the nested location. Every local verdict was silently conditional on
    whether a peer happened to have a worktree open, which does not teach people to fix gates, it teaches them to ignore local reds.

    PRUNED IN PLACE, NOT FILTERED AFTERWARDS. `dirnames[:] = ...` is the documented
    `os.walk` idiom and it means the subtree is never ENTERED. Filtering results after the fact would still pay to stat a peer checkout of the whole monorepo, and would still be wrong for any caller that inspects `dirnames` itself.

    THE LIST OBJECT IS MUTATED, NOT REPLACED, so a caller that then does `dirnames.sort()` on the yielded list still steers the walk exactly as it did under `os.walk`. Several call sites rely on that for deterministic output.

    A ROOT THAT IS ITSELF INSIDE A PRUNED DIRECTORY STILL WALKS, and this is load bearing rather than an accident of the implementation. Pruning applies only to DESCENDANTS of `root`, so a session whose own repo root is `<main>/.claude/worktrees/agent-xxxx` -- which is how every isolated sub-agent runs -- scans its own tree normally. A prune written as "is this path under a
    worktrees directory" would instead make every gate in such a session scan nothing and exit 0, replacing a false red with a false green.

    `follow_symlinks` DEFAULTS TO FALSE, matching `grep -r` (as opposed to `-R`) and `find -P`, which is what the twins these callers replace actually do. It is named and passed explicitly rather than left to `os.walk`'s default, because a default that happens to agree is one refactor away from not agreeing.

    NO ANTI-VACUITY FLOOR HERE, ON PURPOSE. A missing or empty `root` yields nothing, because several callers treat that as an empty corpus to match a twin's `2>/dev/null`, and because the meaningful floor is per gate: only the caller knows how many files its corpus must contain before a green means something. Enforcing a count here would either break those callers or install a
    floor too low to catch anything.
    """
...
def ensure_importable(root: pathlib.Path | None = None) -> str:
    ...
    The bootstrap problem, stated plainly: a script that wants this package has to put `.ci` on sys.path BEFORE it can import the module that would do it for it. So a caller outside the package still writes the hop by hand once, and everything after that -- including every other directory it needs -- comes
