...
THE FULL renet BUILD: stage the embedded third-party assets, export the raw
native binaries the runtime image COPYs, cross-compile six `renet` binaries,
checksum them, and report whether each one is stripped.

NOT THE SAME SCRIPT AS `.ci/scripts/infra/build-renet.sh`, which shares this
basename and nothing else. That one is 131 lines, builds ONE binary from Go
source for the local machine, and is ported at
`.ci/rediacc_ci/infra/build_renet.py`. The two live in different packages, so
the module names do not collide either. Anyone editing one of them should check
which twin they are reading.
...
supplies the component list, its per-arch coverage and the image directory each
asset lives in. The twin's comment at `:115-118` records what it replaced: a
hardcoded `<subdir>:<base>` table that had to be kept in step with `build.sh`'s
four lists and `extract-renet-from-image.sh`'s own split, by hand.

-----------------------------------------------------------------------------
WHAT IS SHELLED OUT TO, AND WHAT IS NOT
-----------------------------------------------------------------------------
SHELLED OUT, because each one is the twin's contract with the machine and its
stdout, stderr, argv or exit status is observable:
...
NOT SHELLED OUT: `mkdir -p` (`:66`, `:109`), the `[[ -f ]]` tests, the glob
expansions and `command -v`. A `mkdir -p` that cannot create its target dies
...
-----------------------------------------------------------------------------
Every one of these was DRIVEN against the twin on the differential's fixture,
and each has a test in `tests/test_build_build_renet.py` pinning the twin's
answer next to this port's identical one. None is repaired here: the twin is the
live registered gate and repairing it is a separate cutover decision.

DEFECT 1, A MISSING `file` REPORTS EVERY BINARY AS STRIPPED. `:242` is
`if ! file "$binary" | grep -q "not stripped"`. With no `file` on PATH the
pipeline is `command not found` (127) piped into a `grep` that matches nothing
(1), the negation makes that TRUE, and the script prints
`renet-linux-amd64: stripped (release build)` for all four. The verification
step exists to catch a binary that shipped with debug symbols, and with the tool
absent it reports the answer it was built to disprove. Driven:
...
Both runs exit 0. `file` is never `require_cmd`ed while `jq`, `zstd` and `go`
all are. The same absence makes `:254` print
`renet-windows-amd64.exe: ` with an empty description.

DEFECT 2, `--output` WITH A NON-EXISTENT PARENT DIES SILENTLY, EXIT 1, NO
MESSAGE. `:64` is `OUTPUT_DIR="$(readlink -f "$OUTPUT_DIR")"`, and GNU
`readlink -f` requires every component but the last to exist: given
`--output /tmp/absent/deep` it prints nothing and exits 1, `set -e` takes the
assignment's status, and the script is gone before `mkdir -p` on the very next
...
THE BUILD PASS. `:132-140` feeds `jq` through a PROCESS SUBSTITUTION, whose exit
status no `set -e` can see. On a parse error `jq` writes to stderr, the array
stays empty, and the three loops over `"${_native_assets[@]}"` at `:146`, `:158`
and `:179` each run zero times. The last of those is the "fail fast" check whose
own comment (`:176-178`) says a missing asset "must break HERE, not later as a
cryptic COPY error ... on an artifact that looked complete". Driven with
...
DEFECT 4, NO STAGED ASSETS AT ALL IS A RAW `ls` ERROR, EXIT 2. `:86` is a bare
`ls -la "$RENET_DIR"/pkg/embed/assets/*/*/`, and with nothing staged the
unmatched glob is passed through literally, `ls` exits 2 and `set -e` ends the
run with `ls: cannot access '...*/*/': No such file or directory` as the only
diagnosis. This is the failure mode of an `embed_assets` that returned 0 without
staging anything, and it is reported as a listing problem.

DEFECT 5, `:107` `if [[ -n "$OUTPUT_DIR" ]]` CANNOT BE FALSE. `OUTPUT_DIR` is
non-empty by `:60` and is the output of a `readlink -f` that the script would
not have survived failing (DEFECT 2). The whole export block is therefore
unconditional, and the condition reads as though a caller could opt out of it.

-----------------------------------------------------------------------------
ENVIRONMENT
-----------------------------------------------------------------------------
ONE variable is READ: `ACCOUNT_ED25519_PUBLIC_KEY` (`:200-202`), the account
server public key baked into the binaries through `-ldflags`. It is read with a
literal `os.environ.get` at its single call site, never through a loop or an
alias, because the env-registry gate resolves the NAME and an alias makes the
input invisible to it.

`CGO_ENABLED`, `GOOS` and `GOARCH` (`:214`, `:223`) are SET for the `go build`
children and never read, so they are passed in the child's env dict and are
deliberately not `os.environ.get` call sites.
"""
...
# `:143`, `:145`. The builder image `build.sh embed_assets` leaves behind when
# it really built one. Its ABSENCE selects the cache-hit path, not a failure.
BUILDER_IMAGE = "rediacc/renet:latest"

# `:132-140`, verbatim. One row per (component, arch), as
# `<imageDir>\t<assetBase>\t<arch>`. The field ORDER differs from the filter in
# `extract_renet_from_image.py`, which is the twin's doing and is preserved:
# the two scripts read the same lockfile with two different column orders.
MATRIX_FILTER = """
            .components
            | to_entries[]
            | .value as $c
...
# `:211-226`. The six targets in the twin's order, with the suffix each one's
# output carries. windows is a separate loop in the twin only because of `.exe`.
GO_TARGETS = (
    ("linux", "amd64", ""),
    ("linux", "arm64", ""),
    ("darwin", "amd64", ""),
...
# `:239-248`. Only the ELF targets are asked the stripped question; windows is
# PE and `file(1)` answers differently, which `:250` says out loud.
VERIFY_TARGETS = (
    ("linux", "amd64"),
    ("linux", "arm64"),
    ("darwin", "amd64"),
...
# The lines whose external command bash names when it cannot run it. Used to
# reproduce bash's own `command not found` / `No such file or directory` text,
# which is the only diagnosis the twin emits for these.
...
def console_root() -> pathlib.Path:
    ...
    `get_repo_root` (common.sh:205-210) resolves from common.sh's own directory
    and has no environment override, so `paths.repo_root()` -- which honours
    `$REDIACC_CI_ROOT` -- is deliberately NOT used: it would make the port
    answerable to a variable the twin has never heard of, and the differential
...
def parse_args(argv: list[str]) -> tuple[str, str, bool, str | None, str]:
    ...
    Exercised directly by the differential in both directions, so the arms that
    must NOT fire (a bare `--skip-embed`, an empty argv) are pinned too.
    """
    version = ""
    output_dir = ""
    skip_embed = False
...
def matrix_rows(text: str) -> list[tuple[str, str, str]]:
    ...
    and reads absent ones as empty. Both are reproduced, so a short or long row
    behaves here exactly as it does there rather than raising.
    """
...
def ldflags(version: str, key: str) -> str:
    ...
    so an empty `KEY_LDFLAGS` leaves a trailing space inside it rather than
    disappearing. `go` does not care; the differential compares argv byte for
    byte, and a port that stripped it would diverge on all six calls.
    """
...
def _bash_cannot_run(line: int, name: str, code: int) -> None:
    ...
    Two spellings, and which one bash uses depends on the NAME: a word with a
    slash was a path that did not resolve (`./build.sh: No such file or
    directory`), a bare word was a PATH lookup that found nothing
    (`file: command not found`). Both were driven against the twin.
    """
    if code == 126:
...
def _cd_or_report(target: pathlib.Path, line: int) -> str | None:
    ...
    `(cd "$RENET_DIR" && ...)` (`:83`, `:95`) and `cd "$RENET_DIR"` (`:196`) all
    die this way. Only the two reachable spellings are reproduced; a directory
    that exists but is unreadable prints `Permission denied` in bash and is not
    covered here, which is named rather than silently approximated.
    """
    if target.is_dir():
        return None
...
def _build_sh(renet_dir: pathlib.Path, step: str, line: int) -> int:
    ...
    The `cd` and the exec are separate failures with separate messages, and both
    end the run under `set -e`: the subshell's status is 1 for a failed `cd`,
    127 for a missing `./build.sh`, 126 for a non-executable one.
    """
...
def _export_native(
    ...
    DEFECT 5 lives on the first line of the twin's version of this: the
    `[[ -n "$OUTPUT_DIR" ]]` guard cannot be false, so the block is
    unconditional and is written as such here.
    """
    renet_dir = root / "private" / "renet"

    # `:142-143`. `command -v docker` and then an image lookup whose output and
    # error are both discarded: absence is a route, not a failure.
    have_builder = False
    ...
    # `:176-185`. Vacuous when `rows` is empty; that is DEFECT 3, and it is the
    # whole reason this loop cannot be trusted on its own.
    for _image_dir, base, arch in rows:
        asset = "%s-linux-%s" % (base, arch)
...
def _stage_embedded(root: pathlib.Path, output: pathlib.Path) -> int:
    ...
    # `:128-140`. The status of a PROCESS SUBSTITUTION is unobservable to
    # `set -e`, so a failing `jq` yields an empty matrix and the run continues.
    # That is DEFECT 3, reproduced by ignoring the return code here.
...
def _pipeline(left: list[str], right: list[str], line: int) -> tuple[int, int, bytes]:
    """`left | right`, run as two real processes, with bash's own message for
    either side that cannot be executed.

    THE SECOND HALF IS RUN, NOT REIMPLEMENTED, and that is a decision this port
    got wrong once. `grep -q "not stripped"` was first written as a Python `in`
    test, which is the same answer whenever `grep` exists and a DIFFERENT verdict
    when it does not: bash's pipeline is `command not found` (127) and takes the
    "stripped" arm, while an `in` test still reads the text and can report debug
    symbols. It was caught by a ledger fixture whose `grep` symlink happened to
    be broken, having produced four extra
    `line 242: grep: command not found` lines on the twin's side and none on this
    one. The same argument covers `head -c 80` on the windows arm.

    `left`'s output is buffered rather than streamed. The only observable
    difference is SIGPIPE: `grep -q` exits on its first match and a real pipeline
    could kill `file` with EPIPE. `file`'s output here is one line, written long
    before `grep` can exit, so the case is unreachable rather than ignored.
    """
    _flush()
    left_out = b""
    left_code = 0
...
def _verify(output: pathlib.Path) -> None:
    ...
    DEFECT 1 is the `file`-shaped hole in both loops: an absent tool makes the
    first print `stripped (release build)` for everything and the second print
    an empty description.
    """
    log.step("Verifying release builds...")
    for goos, arch in VERIFY_TARGETS:
        binary = output / ("renet-%s-%s" % (goos, arch))
        # `! file "$binary" | grep -q "not stripped"` under `pipefail`, whose
        # status is 0 only when BOTH sides are 0. A `file` that could not run,
        # a `file` that errored, and a `grep` that found nothing all take the
        # same arm, which is the one that says the binary is clean.
    ...
    # `:250-256`. `$(file -b "$binary" | head -c 80)`: the first 80 BYTES, with
    # trailing newlines stripped by the command substitution.
...
def main(argv: list[str]) -> int:
    ...
    # `:64`. DEFECT 2: a parent that does not exist ends the run right here,
    # exit 1, nothing printed by anybody.
    ...
    # `:199-203`. The ONE environment variable this script reads, at its one
    # call site, spelled as a literal so the env-registry gate can resolve it.
    ...
    # `:229-231`. `cd "$OUTPUT_DIR"` for the checksum, so the recorded names are
    # basenames; the redirect truncates `checksums.sha256` whether or not
    # `sha256sum` then succeeds.
