...
Builds four self-contained package repositories -- APT, RPM, Alpine APK and Arch Linux -- out of an already built directory of `.deb`, `.rpm`, `.apk` and `.pkg.tar.zst` files, and GPG-signs the two that clients verify. "Self contained" is the whole design: the packages sit alongside their metadata, so nothing in a published repository points at an external URL.

Five phases, numbered 1, 4, 5, 6, 7 in the twin's own headings. Phases 2 and 3 were the "download historical versions" steps and are gone; the numbering was left alone, and this port leaves it alone too, because the numbers appear in `log_step` output that a workflow log reader greps for.

-----------------------------------------------------------------------------
WHAT IS SHELLED OUT TO, AND WHAT IS NOT
-----------------------------------------------------------------------------
SHELLED OUT, because the tool IS the behaviour and a Python second opinion would be a second source of truth for what gets published:
...
-----------------------------------------------------------------------------
Every one below was DRIVEN against the real script, not inferred from reading. The line numbers are the live ones.
...
An assignment whose right-hand side is a PIPELINE, under `set -o pipefail`. When docker is absent the branch above it prints `Docker not available, cannot generate APKINDEX for x86_64` and falls through -- but no `APKINDEX.tar.gz` exists, `tar` exits 2, `awk` exits 0, pipefail makes the pipeline 2 and `set -e` kills the script THERE. Measured: exit 2, the last line on stderr is
that warning, no packages are copied, phase 7 never runs and the summary never prints. So the machine every developer would run this on -- one without docker -- gets a repository with an empty `apk/<arch>/` directory and a failure it cannot diagnose. Reproduced, including the status.

DEFECT 2 -- THE VACUITY FLOOR COVERS APT AND NOTHING ELSE. `:212-217` refuses an empty `.deb` pool with a written-out explanation. RPM, APK and archlinux have no equivalent. Measured with a `--local-pkgs` holding ONE `.deb` and nothing
else: exit 0, `rpm/repodata/repomd.xml` generated AND SIGNED over zero packages,
`archlinux/rediacc.conf` written, and the summary reports "metadata for 0 .rpm packages" in the same tick-prefixed green as everything else. A partial build publishes three empty repositories and says it succeeded. Reproduced.

DEFECT 3 -- THE FLOOR IS DISABLED BY A ZERO-PADDED VALUE. `:213` compares with `[[ "$DEB_COUNT" -lt "$MIN_DEBS" ]]`, which is bash ARITHMETIC, which reads a
leading zero as octal. `PKG_REPO_MIN_DEBS=08` is not a number in base 8, so bash
prints `line 213: [[: 08: value too great for base (error token is "08")` and the `[[` returns FALSE -- an error is FALSE, not zero -- so the refusal is
...
disables it, which is at least legible. Same class as this session's `autopilot-gate.sh`, `dispatch-watchdog.sh` and `cleanup-versions.sh` findings. Reproduced by `bash_lt`, which returns False on an arithmetic error for exactly the reason `autopilot_gate.bash_cmp` does.

DEFECT 4 -- `--dry-run` IS NOT A PREVIEW, AND THE TWO CONFIG WRITERS DISAGREE ABOUT WHAT IT MEANS. `:324-331` writes `rpm/rediacc.repo` OUTSIDE the dry-run guard; `:451` writes `archlinux/rediacc.conf` INSIDE it. Driving `--dry-run` produces `rpm/rediacc.repo` and no `archlinux/rediacc.conf`, which is the inconsistency as reported. It is WORSE than that, and this port records the
extra half: `:300`, the `find ... -name "*.rpm" -exec cp {} "$RPM_DIR/"` that
populates the RPM repository, is ALSO outside the guard, so a "preview" run copies the real `.rpm` payload into the output directory. Measured: after `--dry-run` the output tree holds two `.rpm` files plus `rediacc.repo`. The APT phase does not have this problem only because its copy targets a temp pool the EXIT trap deletes. Reproduced exactly, both halves.

DEFECT 5 -- `--max-versions` WITH NO VALUE DIES WITH ZERO BYTES ON BOTH STREAMS. `:51-54` is a deprecated no-op arm that still does `shift 2`. Given one remaining argument, `shift 2` fails, and under `set -e` that ends the script. Measured: exit 1, 0 bytes on stdout, 0 bytes on stderr. Every OTHER flag in the parser dies with a named `<script>: line N: $2: unbound variable` --
`--version` names line 40, `--channel` names line 56. A silent exit 1 is indistinguishable
...
DEFECT 6 -- THE SIGNING-KEY CHECK'S OWN `<unreadable>` FALLBACK IS DEAD CODE. `:161-162` are two more assignments-of-a-pipeline:

    want_fpr=$(gpg --show-keys ... "$PUBLIC_KEY_FILE" 2>/dev/null | awk ...)
    have_fpr=$(gpg --list-keys ... "$GPG_KEY_ID" 2>/dev/null | awk ...)

Same shape as `build-linux-pkg.sh:226-227`, and the same consequence: gpg exits 2 on a file that is not a key, `awk` exits 0, pipefail makes the pipeline 2 and
...
line on stderr is `Using GPG key: DEADBEEF` -- a build that says it found the key and then vanishes. The MISMATCH branch itself is live and was driven separately (two different fingerprints produce the full refusal at exit 1), so this is a dead fallback inside a working check, not a dead check. Reproduced.

DEFECT 7 -- TWO APKS FOR ONE ARCH COLLAPSE INTO ONE FILE, SILENTLY. `:387-395` loops over every `.apk` for an arch and, for EACH ONE, recomputes `apk_name`
from the same `APKINDEX.tar.gz` with an awk program that `exit`s after the FIRST
`V:` line. So every file in the loop is copied to the SAME destination name and each overwrites the last. Measured with two `.apk` files for x86_64: exit 0, no warning, and `apk/x86_64/` holds ONE package. Found by driving, not by reading. Reproduced.

NOT A DEFECT, AND WORTH SAYING SO: the `find ... -exec cp` at `:202` is silent when it matches nothing, and the twin's comment at `:207-211` says so in as many
...
-----------------------------------------------------------------------------
`RELEASE_GPG_PRIVATE_KEY`, `RELEASE_GPG_PASSPHRASE`, `RELEASE_GPG_PUBLIC_KEY_FILE`, `PKG_REPO_MIN_DEBS`, `SOURCE_DATE_EPOCH` and `RELEASES_BASE_URL` are each read with a direct `os.environ.get("NAME", ...)` where they are used. No dict alias and no loop: the env-manifest reader parses direct reads by name, and an alias hides them.
"""
...
# `:212`. The floor's default, as a STRING, because the comparison that consumes it is bash arithmetic over a string and not an integer comparison.
DEFAULT_MIN_DEBS = "1"

# `:225`, `:257`, `:268`. Debian architecture names, in the twin's order. Order is observable: it decides the order of the checksum rows in `Release`.
APT_ARCHES = ("amd64", "arm64")

# `:344`, `:411`. Alpine and Arch name the same two machines differently from nfpm, and the apk phase has to translate while the archlinux phase does not.
APK_ARCHES = (("x86_64", "amd64"), ("aarch64", "arm64"))
ARCHLINUX_ARCHES = ("x86_64", "aarch64")

# `:39-58`. The flags whose value is `$2`, and the LINE the twin dies on when it is missing. `--max-versions` is deliberately absent: it has no `$2` reference at all, only a `shift 2`, which is DEFECT 5.
VALUE_FLAG_LINES = {
    "--version": 40,
    "--local-pkgs": 44,
    "--output": 48,
...
class Refusal(Exception):  # noqa: N818 - named for what the twin does
    ...
    THE EMPTY CASE IS NOT A SHORTCUT. DEFECTS 1, 5 and 6 are all places where the twin genuinely exits non-zero having printed nothing at all, and a port that invented a message there would be describing a better program than the one that runs.

    `raw=True` marks a diagnostic the INTERPRETER emits rather than the script:
    it goes to stderr unadorned, with no `log_error` tick, because bash's `$2: unbound variable` does not wear one either.
    """
...
def console_root() -> pathlib.Path:
    ...
    `rediacc_ci.paths.repo_root()` honours `$REDIACC_CI_ROOT` and is deliberately NOT used: a differential where one side follows an override and the other does not diverges for a reason that says nothing about the port. Same derivation as `build_linux_pkg.py` and `build_linux_packages.py`.
    """
...
def source_common() -> None:
    ...
    Not a function in the twin: SOURCING the library runs it, so this happens before line 28 of the script and every child it later spawns (gpg, dpkg-scanpackages, createrepo_c, docker, tar, gzip, date) inherits `CI_OS`, `CI_ARCH` and `CI_TEMP`. A port that skipped it would hand those children a different environment, and the difference would show up as a divergence
...
def bash_arith(text: str) -> int | None:
    ...
    Measured against bash 5.3.9 for each row above, not inferred. `1+1` and `3.5` are genuinely out of scope -- the first evaluates to 2 in bash and to an error here, the second errors in both with a different message -- and `PKG_REPO_MIN_DEBS` reaching this function as an EXPRESSION rather than a literal is not a case any caller produces. Named so a future reader does not mistake
    the boundary for a bug.

    THE `None` IS THE WHOLE POINT, and it is DEFECT 3. An erroring `[[ ]]` returns FALSE, and false is not "compare against zero": a port that read `08` as `0` would compute `0 -lt 0`, get False, and reach the same answer by luck on this one input while getting `[[ 0 -lt 09 ]]` exactly backwards. Same shape and same reasoning as `autopilot_gate.bash_arith`.
    """
    token = text.strip()
    if token == "":
        return 0
    ...
    try:
        ...
        if token[0].isdigit():
            # bash: `<script>: line 213: [[: 08: value too great for base (error token is "08")`. This copy carries THIS program's name instead of the twin's path; see divergence 1 in the module head.
            print(
...
def bash_lt(left: str, right: str) -> bool:
    ...
    bash evaluates left to right and stops at the first bad token, so only ONE diagnostic is printed even when both operands are malformed. Reproduced, because those diagnostics are on stderr and the differential compares stderr.
    """
    a = bash_arith(left)
    if a is None:
        return False
...
def _bare(argv: list[str], **kw) -> None:
    ...
    An exec failure is bash's 127 ("command not found") or 126 ("cannot execute"), and bash says so on stderr against its own path and line. The STATUS is reproduced exactly; the diagnostic carries this program's name instead, which is divergence 1 in the module head.
    """
    sys.stdout.flush()
    sys.stderr.flush()
    try:
...
def _find(root: str, name: str, *, quiet: bool = False) -> list[str]:
    ...
    `quiet` is the twin's `2>/dev/null || true`: those call sites cannot die, so a failure here yields an empty list. Where the twin has no `|| true` the caller uses `_find_strict` instead, because there the status kills.
    """
    try:
        proc = subprocess.run(
            ["find", root, "-name", name],
...
def _count(root: str, name: str, *, quiet: bool = False) -> str:
    ...
    A string, not an int, because it is interpolated into log lines and into the arithmetic at `:213` without ever being normalised. `wc -l` counts NEWLINES, which is what this counts; a filename containing a newline over-counts on both sides, identically.
    """
...
def _find_exec_cp(root: str, name: str, dest: str, *, quiet: bool = False) -> None:
    ...
    Shelled out whole rather than enumerate-then-`shutil.copy`, so the copy happens in `find`'s DIRECTORY ORDER. That is not cosmetic: when two files would land on the same name, order decides the winner, which is exactly DEFECT 7's mechanism one directory over.

    `quiet=True` is `2>/dev/null || true`, which the apk and archlinux phases
    have and the deb and rpm phases do NOT -- so a `find` failure kills the script in phases 4 and 5 and is swallowed in phases 6 and 7. Preserved.
    """
...
def _first_field(output: str, want: str, index: int) -> str:
    ...
    Four tokens of text processing, not a tool whose behaviour is under test. `index` is 1-based, matching awk.
    """
...
def _fingerprint(argv: list[str]) -> str:
    ...
    DEFECT 6 LIVES HERE AND IS WHY THIS IS A FUNCTION. The twin writes the gpg call as the right-hand side of an assignment, which makes the PIPELINE's status the assignment's status; under `pipefail` a gpg that exits non-zero kills the script with no output at all, so the `<unreadable>` fallback two lines later can never be reached. `awk` always exits 0, so gpg's status is the
    only one that can be non-zero.
    """
    try:
...
def parse_args(argv: list[str]) -> tuple[dict[str, str], bool, bool]:
    ...
    Every refusal in here is the SHELL's, not the script's, apart from the `*)` arm. Two shapes:
    ...
    while rest:
        ...
        if flag in keys:
            ...
            # Deprecated: ignored (historical versions accumulate via s3 sync). DEFECT 5: `shift 2` with one argument left is a silent death.
            if len(rest) < 2:
                raise Refusal(1)
            rest = rest[2:]
        elif flag == "--dry-run":
...
def _phase1_gpg(
    ...
    Returns the key id, which is `""` under `--dry-run` and is then passed to nothing, because every consumer of it sits inside a non-dry-run branch.
    """
    ...
    # `echo "$KEY" | gpg ... --import 2>/dev/null`. A PIPELINE as a command, not as an assignment, so pipefail plus `set -e` kills on a failed import --
    ...
    # `$(gpg --list-keys --with-colons 2>/dev/null | grep '^pub' | head -1 | cut -d: -f5 || true)`. The `|| true` is what stops this one being a DEFECT-6 death, so a missing gpg lands on the refusal below instead.
    try:
    ...
    # THE PUBLISHED PUBLIC KEY MUST BE THE SIGNING KEY. Until 2026-09-04 nothing checked that, and a rotation applied to the secret store and not to the file would have shipped a repository every apt and dnf client rejects
    ...
    if pathlib.Path(public_key_file).is_file():
        ...
        if not want_fpr or not have_fpr or want_fpr != have_fpr:
            ...
            # are written for a case `_fingerprint` has already died on. Kept verbatim so the two files read the same way.
            raise Refusal(
                1,
...
def _release_date() -> str:
    ...
    `$(A 2>/dev/null || B)` -- the fallback catches a `date` that cannot parse the epoch, which is what a non-GNU `date` does.
    """
...
def _checksum_rows(dists_dir: str, algorithm: str) -> list[str]:
    ...
    bash expands `main/binary-*/Packages main/binary-*/Packages.gz` in collation order and leaves an unmatched pattern LITERAL, which the `[[ -f ]]` then skips. Reproduced with a sorted glob and an `is_file` test, because a Release that lists a file it does not have breaks `apt update` on every client.
    """
    rows: list[str] = []
    previous = os.getcwd()
    os.chdir(dists_dir)
...
def _phase4_apt(
    ...
    # VACUITY FLOOR. dpkg-scanpackages over an EMPTY pool succeeds and writes a valid, empty Packages file, so the repository publishes and `apt update` reports no error -- it just offers nothing to install. DEFECT 2 is that this exists for APT and for nothing else; DEFECT 3 is that a zero-padded floor value turns it off.
    ...
    # `cp -r "$APT_WORK_DIR/pool" "$APT_DIR/pool"` -- the packages travel with their metadata, which is what "self-contained" means here.
...
def _phase5_rpm(
    ...
    TWO THINGS HAPPEN OUTSIDE THE DRY-RUN GUARD HERE, and both are DEFECT 4: the `.rpm` payload is copied into the output directory, and `rediacc.repo` is written. archlinux's equivalent config writer is INSIDE its guard.
    """
    ...
    # DEFECT 4, second half: `:324-331` sits after the `fi`, so a preview writes a real `.repo` file. `RELEASES_BASE_URL` is the one constants.sh value the environment can override.
...
def _apkindex_name(index_tarball: str) -> str:
    ...
    DEFECT 1 LIVES HERE. `tar` is a subprocess whose non-zero status the twin propagates through `pipefail` into a bare `set -e` death, and the commonest way to reach that is the docker-less path two branches up, which has ALREADY printed "Docker not available, cannot generate APKINDEX" and believes it is continuing. `awk` always exits 0, so tar's status is the only one that can be
    non-zero.

    The awk program is reimplemented: `-F:`, remember the last `P:` field 2, print `name-<V field 2>.apk` at the first `V:` and stop.
    """
    try:
        proc = subprocess.run(
...
def _phase6_apk(
    ...
    for arch, _nfpm in APK_ARCHES:
        ...
        # Copy packages to output, renamed to match APKINDEX expectations. DEFECT 1 fires on the FIRST iteration of this loop when the branch above took the warning arm. DEFECT 7 fires when it did not and there is more than one file: `apk_name` is recomputed from the SAME index every time and always yields the same string, so each file overwrites the last and the repository ships
        # one package where two were built.
...
def build(argv: list[str]) -> int:
    ...
    Deliberately NOT decomposed further than one function per PHASE: the phases are the twin's own headings and appear in its output, while anything finer would invent a structure a reader diffing the two files cannot follow.
    """
    source_common()
    ...
    # `Path.resolve()` would be `pwd -P` and would print a different path for a symlinked output directory.
    ...
    # `:110-112`. The EXIT trap is armed BEFORE anything is added to the list, so a refusal in phase 1 still runs `rm -rf` over an empty array. Harmless in bash 5 and harmless here; recorded because an empty-array expansion under `set -u` is the kind of thing that used to be fatal.
    cleanup_dirs: list[str] = []
    try:
...
def _phases(
    ...
    `finally` wrapped around half a function body does not. `version` is unused past the banner, exactly as in the twin, and is kept in the signature so a reader does not go looking for where it went.
    """
    ...
    # Historical versions: not downloaded. Each channel is self-contained with only the current version; previous versions accumulate via s3 sync.
