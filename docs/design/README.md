...
| [06-cli-reshape.md](06-cli-reshape.md) | The full command mapping: every current rdc command kept / moved / renamed / deleted / new. **The MAPPING is the contract, not a leaf count** (spec/03 §6; the count is retired, §0). Carries the 2026-07-13 rulings: build the positional ref concept as task zero; jobs are executor-born plus a global `--background`/`-b` (R-P4-2v2, merged from two operator rulings); `--detach` becomes `--no-wait`; canary stays under `repo`. **§1 is now an AS-BUILT transcript of the shipped tree, held there in both directions by `scripts/gates/check-design-tree.ts`; §1.1 records the five differences from the tree it used to draw.** | As-built (P4) |
