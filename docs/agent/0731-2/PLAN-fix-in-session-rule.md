...
## 0. The plan-file convention (docs/agent/<branch>/)

### What it is

Planning agents write durable plans to `docs/agent/<branch>/PLAN-<slug>.md`,
...
- Path: `docs/agent/<branch>/PLAN-<slug>.md`. Slug is kebab-case
...
`  docs/agent/<branch>/PLAN-x.md [draft] (N lines)` plus one count line for
...
- current branch (`C.git_branch`), and whether `docs/agent/<branch>/` exists;
...
- `plan-subagent` -> `CLI_TRIAGE_PLAN`: verdict line, the prefilled plan path
  `docs/agent/<branch>/PLAN-<slug>.md`, then the recipe:
...
{"ev":"triage","id":"<rid>","at":"<stamp>","by":"<me>","v":"plan-subagent",
 "reason":"...","plan":"docs/agent/<branch>/PLAN-<slug>.md"}
```
...
now; big = `worklist.py --triage`, Plan agent to `docs/agent/<branch>/PLAN-*`,
...
`$BASE/proj/docs/agent/agenttest/`.
...
- **165. --triage degraded (judge off)**: with `WORKLIST_JUDGE=off`, prints
  ...
  `docs/agent/`), exit 0; an `add` event landed (finding tracked); NO
...
- **167. --triage judge path via shim**: `shim_judge_out` returns
  ...
  `docs/agent/agenttest/PLAN-fix-x.md`, a `"ev":"triage"` event landed with
...
- **168. plan follow-through in the guide**: plant an open item plus a triage
  event (`"v":"plan-subagent"`, plan path under `docs/agent/agenttest/`),
