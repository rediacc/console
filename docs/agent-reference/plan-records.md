...
The table below is GENERATED from `.claude/hooks/stop/wl_planrec.py` by `npx tsx scripts/gen-docs.ts --write`, and `check:ci-doc-region-parity` fails when the committed bytes and the code disagree. Do not hand-edit between the markers.
