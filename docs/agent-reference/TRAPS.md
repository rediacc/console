...
**In CI the same change is green**, because the PR's tree carries the file. The red exists only in the window between adding a path to a closure and committing the file it points at — which, in a repo whose standing rule is that work stays uncommitted until asked, is a window every session lives inside.
