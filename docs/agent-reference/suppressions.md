...
| Host-toolchain exceptions | `.ci/policy/.host-toolchain-exceptions` | `.claude/rediacc_hooks/guards/block_host_toolchain_run.py`, through `policy_path()`. The only entry in this table read by a HOOK GUARD rather than by a gate. The guard routes a command into the devbox when the host lacks the toolchain it needs, and an entry here says the host is the only place a particular command can run -- which is true of exactly one category, a host-only interactive auth ceremony a headless container cannot complete. It ships EMPTY, and the category it REFUSES is the interesting half: "the devbox does not carry it" is a bug report against `.devcontainer/Dockerfile`, not an exception, and `DEVBOX_GAP_PHRASES` in the guard rejects that reason family directly because the generic BLOCKER rules cannot see it -- "the devbox image has no aws" is specific, over the thirty-character floor, and passes every one of them |
