...
## Outcome
Shipped in [unresolved] with ci_branch_query and allow_branch parameter. Test 1 (test-ci-trace-branch.sh) landed and is wired. Test 2 (check-ci-watch-recipe.sh extension to prove the skill's invocation matches the tool's capability) was NOT done—the visibility gap that let this ship in the first place remains. A third gap found in use: cannot watch a run on a deleted branch,
deliberately left unfixed because run-id keying reintroduces the stale-attempt bug the reader exists to prevent.
