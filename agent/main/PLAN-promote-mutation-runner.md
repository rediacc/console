...
`docs/agent/main/PLAN-trap-enforcement.md`, whose §2.2 fourth failure shape is
...
Recorded in `docs/agent/TRAPS.md:278-308`. A session added two cases to
...
| # | Refusal | Why |
|---|---|---|
| R7 | Re-reading the mutated file from disk does not show `--new` at the expected offset, or still shows `--old` | Never trust a writer's own report. `docs/agent/TRAPS.md:273-276` is explicit: verify the file by reading it back, never from the command's output |
...
`docs/agent/TRAPS.md:254-255`). A design whose correctness depends on which
...
- `scripts/check-dead-bash.ts:9-11` reports a `.sh` whose basename no other
  ...
  documented in `docs/agent/TRAPS.md`'s mutation entry as the command to reach
  for.
...
**Corpus.** `docs/agent/TRAPS.md`'s mutation entry (`:278-308`) gains a
...
3. **`--allow-collateral` is an escape hatch and escape hatches get used.** A
   ...
   `docs/agent/suppressions.md` governs committed escape hatches.
...
per-worktree corpus, not `docs/agent/TRAPS.md`, which is the drift that plan's §9
is about. Verified today: `docs/agent/TRAPS.md:111` is a different entry
...
| 3. Known-fails allowlist | Agreed, reject. It is an allowlist over the exact signal the mutant pass exists to produce, and `docs/agent/suppressions.md` would require a `BLOCKER:` reason for a list whose real reason is "the sandbox is wrong" |
...
belongs in `docs/agent/TRAPS.md` beside the editing-a-running-script entry rather
