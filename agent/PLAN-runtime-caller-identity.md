...
under `docs/agent-reference/suppressions.md`, and would be reachable by a session trying to get
...
- **Run in CI**: the variable is unset, L1 takes its silent-pass path, and **every L1
  ...
  `docs/agent-reference/TRAPS.md` is about, arriving in the very change meant to close a
  can't-fail gap.
...
2. **Should there be a kill switch for L1?** Recommended default: **no separate
   ...
   `docs/agent-reference/suppressions.md` with a `BLOCKER:` reason and a liveness gate.
