...
## Why
The nav server-rendered in English on all 12 non-English locales.
`packages/www/src/hooks/useLanguage.ts:12` returns `'en'` when there is no
`window`, and BaseLayout mounted `<Navigation client:idle />` with no `lang`
prop, so six SSR'd islands emitted English into `dist/<lang>/index.html` while
the Astro-rendered parts of the same page were translated.

## Outcome
SHIPPED. Measured against the tree 2026-09-06, NOT read off the header, which
still says `proposed` and is wrong. `packages/www/src/layouts/BaseLayout.astro:377`
