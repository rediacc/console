...
- `packages/www/src/styles/team-video.css` and `language-switcher-inline.css` are
  gone.
- All three defect fixes are in `scripts/check-dead-css.ts`: defect A as the
  `"cardClass"` harvest over `en.json` (`scripts/check-dead-css.ts:110`), defect B as the class-position
