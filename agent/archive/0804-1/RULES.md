...
**SHARPEN THIS FILE. Do not append to it.** Settled facts and standing constraints for this branch. Wrong rule → edit it here, not below it. Sharpened from `.agent/main/RULES.md` on 2026-08-04 by session d136ac61; the CI-overhaul-specific do-not-relitigate items live on in `.agent/main/`.

## What this branch is

The PR wave landing the ENTIRE licensing campaign tree (fork metering via datastore-scoped license store, auth-by-blob renewal + soft-claim, 4-part chain scope, drills harness, i18n gate rebuild, stop-hook fixes, knowledge agents). Immutable briefing: `~/.claude/projects/-home-muhammed-monorepo-console/reports/pr-babysit-0804-1-briefing.md`. Campaign report:
`agent/REPORT-licensing-bigbang-2026-08-04.md`.
...
deletion, commit→Create tier: campaign decisions, drill-proven 39/39 (full16/full17 logs in the session scratchpad).
...
(another session's live work), `.claude/settings.local.json`, `private/generative`, `private/growth` (non-submodule repos).
