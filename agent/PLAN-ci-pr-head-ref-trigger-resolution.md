...
(`.github/workflows/claude-review.yml:70-88`) admits all three. On the `workflow_dispatch` path both
...
2. **The gate never even reached that step.** Its corpus is READERS, and
   ...
   variable is unset has already solved the problem itself". `.ci/scripts/review/discover-epics.sh:25`
...
- It parses with PyYAML (`import yaml`, `:46`). Its gate header (`:34-40`) declares
  ...
  **Do not reopen that.** `.ci/rediacc_ci/workflows.py` is a dependency-free parser
  ...
  at `:6`), and it already models triggers (`Workflow.triggers`, `.ci/rediacc_ci/workflows.py:705`,
...
`.ci/rediacc_ci/workflows.py:44-52` documents it as the one deliberate divergence from PyYAML.
...
edges: `.github/workflows/ci.yml:506 → ci-quality.yml`, `.github/workflows/claude-review.yml:90 →
...
- `.github/workflows/ci.yml:592` — `needs.initialize.outputs.is_bot != 'true' && github.event_name ==
  ...
  `.github/workflows/ci.yml:673`, which is already correct.**
...
- `.github/workflows/ci-quality.yml:663-668` passes `GITHUB_EVENT_NAME` beside an empty-on-push
  `github.head_ref`; this is what keeps `.github/workflows/ci-quality.yml:665` silent, since its job
...
(`.github/workflows/ci-quality.yml:1114`, `:1121`, `:2200`), zero false positives, and the historical
`.github/workflows/claude-review-reusable.yml:139` would have fired pre-`f1ce6911f` and is silent after.

## The three live findings, and their fixes

`ci-quality.yml` is `workflow_call`-only; its sole caller is `.github/workflows/ci.yml:506`, whose `on:`
(`.github/workflows/ci.yml:3-30`) declares `push` (branches `[main]`), `pull_request`, `schedule`, and
`workflow_dispatch` (documented at `:17-18` as the nightly rehearsal, guarded to main).
Neither the calling job's `if:` (`.github/workflows/ci.yml:495+`) nor `quality-code`'s (`.github/workflows/ci-quality.yml:690`)
...
red. `scripts/gates/check-pr-epic-block.ts:134-147` and `scripts/gates/check-pr-task-trailers.ts:439-451` both fall
through to `git branch --show-current` and then print `skipped: on an unknown branch`.
The workflow's own comment at `.github/workflows/ci-quality.yml:1109-1113` already says so in prose. The
...
origin/${{ github.event.pull_request.base.ref }}` (`scripts/ci-runner/manifest.ts:1031`) evaluates to the
...
(their headers' `blocker:` at `scripts/gates/check-pr-epic-block.ts:30-37` and
`scripts/gates/check-pr-task-trailers.ts:35-38` explain why), so `when:`
(`scripts/lib/gate-header.ts:103`) does not reach them, and a newly-skipping step
interacts with `gate-test:gate-skip-announcer` /
...
says the same. `.ci/rediacc_ci/tests/gates/test_gate_review_status.py:1831` and
...
the value is declared THREE times — the gate header (`scripts/gates/check-pr-epic-block.ts:40`,
`scripts/gates/check-pr-task-trailers.ts:42`), `scripts/ci-runner/manifest.ts:1032`/`:1081`, and the workflow — and
...
it is `scripts/ci-runner/manifest.ts:5048` + the workflow + the lock.
...
- **Four are `concurrency.group`** — `.github/workflows/autopilot.yml:131`, `.github/workflows/claude-mention.yml:24`,
  `.github/workflows/claude-review.yml:63`, `.github/workflows/review-status.yml:57-62`. No script reads them, and each
  either ends in a universal fallback (`github.run_id`, `format('dispatch-{0}', ...)`)
  or already covers every declared trigger. `.github/workflows/claude-mention.yml:24`
...
- **Two are `PR_NUMBER`, and both would be FALSE POSITIVES.**
  `.github/workflows/review-status.yml:114` (`pull_request.number || issue.number || inputs.pr_number`
  against five triggers) is uncovered on `workflow_run` BY DESIGN:
  `.ci/scripts/review/review-status.sh:148-205` takes the `WR_RUN_ID` + `review-target` artifact path on
  that event, and its comment explains at length why the SHA/number cannot be used
  there. `.github/workflows/claude-review-reusable.yml:289` is uncovered on `workflow_run` and
  `pull_request` (`.github/workflows/claude-review.yml:90` passes `pr_number: ${{ inputs.pr_number || ''
  }}`, empty on both), and `.ci/scripts/review/claude-review-gate.sh:782` `case`s on `EVENT_NAME` for the
...
--show-current` (`scripts/gates/check-pr-epic-block.ts:134-141`, `scripts/gates/check-pr-task-trailers.ts:439-446`,
`.ci/scripts/quality/check-review-report-replies.sh:85`, `.ci/scripts/review/discover-epics.sh:23`). That uniformity is exactly
...
1. Step-level/job-level `if:` — `needs.x.outputs.y != 'true' && github.event_name ==
   ...
   workflow. Models `.github/workflows/ci.yml:592`/`:673`. **This one is mandatory: without the narrower,
...
2. `github.event_name` in the step's env (E1) — a `github.head_ref`-only setter beside
   ...
   `if:` does not narrow. Models `.github/workflows/ci-quality.yml:663-668`.
...
so the narrower must refuse to narrow. Models `.github/workflows/ci-quality.yml:634`; without this control
