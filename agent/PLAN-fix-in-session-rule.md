...
the `agent/PLAN-*.md` convention that makes a big finding's design survive a compaction. The plan was itself the first instance of the convention it defined.
...
`.claude/hooks/stop/wl_checks.py` (blob bc44e67c925166eb059b2d58309757f503a9e729): `issue_only_evidence:681`, `plan_records:1154`, `plans_block:1377`, `plan_status_excerpt:1460`, `triage_context:1478`, and `guided_slice:2023` carrying the `root=None` parameter this plan added.
...
split out of `test-worklist-v5.sh` into `.claude/hooks/stop/worklist-cases/`, and this plan's cases are now `16-triage-and-plans.sh` (blob bd06f9dcbc1a44fee3d39479cc5197996b503a23), whose header describes exactly this plan's subject: the `--triage` verb, its refusals and degradation, the judge path and the plan-file demand.
...
have to rediscover: the claim that the guide's "TRIAGED BIG" line reaches the stop judge through `remaining_lines` is WRONG, because `remaining_lines` is built from the classified item lists rather than from `guided_slice`. The intent holds; the mechanism named for it does not.
