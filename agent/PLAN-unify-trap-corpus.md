...
Owner: 99ccf057
Updated: 2026-08-09
Superseded-by: agent/PLAN-trap-enforcement.md
...
One trap corpus, at `docs/agent-reference/TRAPS.md`, read by the Stop hook and pointed at by
...
- `docs/agent-reference/TRAPS.md` is tracked and carries **9** `## ` headings. It is the only
...
6. **The two files have incompatible charters, so the merge is not mechanical.**
   `docs/agent-reference/TRAPS.md:7-10` says in as many words: *"Mechanics of a specific
...
**Make the committed `docs/agent-reference/TRAPS.md` canonical. Repoint the hook at it. Delete
...
1. **Canonical path** — `wl_store.traps_path(root)` returns
   `<root>/docs/agent-reference/TRAPS.md`, with a `WORKLIST_TRAPS_PATH` env override (precedent:
...
- A trap added in a session becomes part of a diff and gets reviewed. Today a trap is
  either invisible to review (`.agent/`) or invisible to tooling (`docs/agent-reference/`).
- `CLAUDE.md:588-596` keeps its shelf of three lookup files under `docs/agent-reference/`
...
- Cost, stated plainly: `docs/agent-reference/TRAPS.md` now appears in PR diffs, and two sessions
...
- **It strands `CLAUDE.md:593`.** Either the operator's `docs/agent-reference/` shelf loses its
...
`docs/agent-reference/TRAPS.md`, which is present because it is tracked; the stale file is inert.
...
"`.agent/TRAPS.md` still exists — the corpus moved to docs/agent-reference/TRAPS.md; run
...
1. **`docs/agent-reference/TRAPS.md`** — perform the merge in §5. Verify 23 headings before
   touching any code.
...
2. **`.claude/hooks/stop/wl_store.py`**
   - Delete `agent_traps_path` (`:231-232`); add `traps_path(root)` returning
     ...
     `pathlib.Path(root) / "docs" / "agent-reference" / "TRAPS.md"`. **Rename, do not keep the
...
5. **`.claude/hooks/stop/worklist_messages.py`**
   - Add `TRAPS_ALARM` and `V_TRAPS_MISSING`. `V_TRAPS_MISSING` must carry the exact
     repair command (`git checkout -- docs/agent-reference/TRAPS.md`) and say why it blocks.
   - Repoint prose: `:265` (`V_AGENT_STATE`), `:1230` (`CTX_POSTCOMPACT_MISSING`),
     ...
     `docs/agent-reference/TRAPS.md`. Assertion D in §6 fails while any remains.
...
14. **`.agent/README.md`** (gitignored, local-only) — the lifetime table's *Forever* row
    points at `docs/agent-reference/TRAPS.md`, and the "Why TRAPS.md is the exception" section says
...
Replace `docs/agent-reference/TRAPS.md:3-10`. The new preamble must:
...
```bash
grep -c '^## ' docs/agent-reference/TRAPS.md      # must print 23
...
old = titles('.agent/archive/TRAPS.pre-unify.md') | titles('docs/agent-reference/TRAPS.md')
new = titles('docs/agent-reference/TRAPS.md')
...
- **A — CANONICAL PRESENT.** `docs/agent-reference/TRAPS.md` is tracked
...
| Mutation | Assertion |
|---|---|
| `cp docs/agent-reference/TRAPS.md .agent/TRAPS.md` | C |
...
| `rm docs/agent-reference/TRAPS.md` | A |
...
`$BASE/proj/.agent/TRAPS.md`. Both become `mkdir -p "$BASE/proj/docs/agent-reference"` +
`$BASE/proj/docs/agent-reference/TRAPS.md`. `setup` (`:82-83`) already `rm -rf "$BASE"`, so no
cleanup changes.
...
- **A missing corpus BLOCKS and names the repair.** No `docs/agent-reference/TRAPS.md`; `run`;
