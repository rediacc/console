...
Written as planning only, with nothing implemented. **Both asks have since shipped**,
which is why this header exists: the file previously carried no `Status:` line at all
and was reported as UNKNOWN.

- **(A) durable sub-agent reports: SHIPPED** as `.claude/hooks/stop/wl_report.py`
  (`--list --unread`, `--show <id>`, `--read <me> <id>`). A report now survives
  compaction and a restart instead of existing only in the lead's conversation.
- **(B) pushed inbox: SHIPPED** as `.claude/hooks/stop/wl_wait.py`, whose own docstring
  states the ask almost verbatim: *"block until something NEW arrives for this session,
  then exit"*, replacing the `*/5` cron poll. Its exit IS the notification.

Verified 2026-08-18 by a session that used both continuously: fourteen sub-agent reports
were delivered through the waiter and read back out of the durable store.

**Residue, not a reason to reopen:** `worklist_messages.py:237` and `:446` still print
`worklist.py --poll <me>` as guidance. The poll verb still works, so this is stale
advice rather than a broken path, but it points a reader at the mechanism this plan
replaced.
...
- `agent/`: tracked, and auto-dumping ~142 reports per session into a
...
- (b) Tracked `agent/`. Reports become reviewable artifacts and travel with
