...
## Decision 4: the gate, `check:ci-actions-vars`
...
- [ ] Build `check:ci-actions-vars` as `.ci/rediacc_ci/quality/actions_vars.py` plus the by-path entry point `.ci/scripts/quality/check_actions_vars.py` carrying the `---- gate ----` header, with all six clauses of Decision 4 and a `selftest()` that proves every reader in both directions on fixtures.
...
- [ ] Prove the two false-positive guards: a `secrets.sh` occurrence in a comment must NOT fire (four exist in the tree today), and a commented `${{ vars.X }}` must NOT fire (`ci.yml:1354` is one).
...
- [ ] Migrate the remaining 18 names at the sites enumerated above, one commit per cluster (Cloudflare, SELLER, SES, git-bot, Turnstile, ROOT_EMAIL, FULL_CI), keeping `cd-v2.yml:668-669` for last since it is the one shell-interpolation site.
...
- [ ] Verify the end state: `check:ci-actions-vars`, `check:ci-bws-map`, `check:ci-secret-supply` and `check:ci-env-manifest` all green, and the printed `vars.*` count is 5 -- the breakpoint pair times two files, plus nothing else.
