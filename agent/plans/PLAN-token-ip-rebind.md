...
Concurrency: parallel. Server (A) and CLI (B) work at the same time once T0 has frozen the wire contract. B's edits to the shared CLI files are serialised behind the relay-only plan (Depends-On).
