...
- [ ] T1 Implement sections 1-7 below in one pass (lead inline or one writer), with tests/test_wl_cap_wait.py and the mutation controls.
...
- **(a) The quiet ladder fires on `worker:queue` items. This is a bug of its own.** `wl_liveness.ladder` skips only workers that have a subagent meta (`wl_liveness.py:685-698`). `queue` has no meta, so a queue item falls through. A lease written by the CLI stores `worker_verified=False`: the id `queue` is never in the harness list (`worklist.py:1170-1178`). The item therefore goes to `teammate_state("queue")`, gets "unverifiable" (`wl_liveness.py:454-456`), and drops into the plain age rungs 45/90/120 (`wl_liveness.py:782-789`, constants at `:46-48`). Those become `ladder-investigate` and `ladder-resolve` blocks at `wl_checks.py:3687-3713`. The roster already treats the queue as "bounded by its own lease expiry" (`wl_roster.py:628`), so the ladder should never see it.
- **(b) The judge blocks.** The lead's block log `/tmp/claude-worklist/home_developer_console.blocklog-d778be9d.jsonl` shows 11 of its last 15 blocks with `key: judge` and `sweep`/`proof` flags. The judge runs whenever `something_remains` (`wl_checks.py:4211`). Its continue exit is at `:4642`, and it prints `R_JUDGE_CONTINUE` ("Do the next action, then stop", `worklist_messages.py:1369-1373`).
- **(c) and (d) are the roster's own keys.** `queue-slot` (`wl_checks.py:2703-2714`) and `roster-dead` (`:2697-2702`).
- **Correction 1: fresh queue leases do not keep `_in_pure_wait` from applying.** `classify_items` puts a fresh lease in `in_flight`, not `open_items` (`wl_store.py:1312-1313`). Only an **expired** queue lease fails closed into an open item (`:1317-1321`, mirrored in `wl_roster.py:518-519`). That happens at the latest 120 minutes after leasing (`wl_core.py:46`). A queue that sits longer than 2 hours breaks the wait; step 2 closes that.
- **Correction 2: the babysitter does count toward the cap.** The lead's own `.lastevent-d778be9d.json` lists it as `type: subagent, agent_type: pr-babysitter`. The roster counts only `type == "subagent"` rows (`wl_roster.py:440-441`), so a teammate-type babysitter would not count; this one does.
...
- It is sealed: no environment reads, per `test_wl_roster.py:614-629` and `.ci/policy/worklist-env-registry.json`.
...
- **Live writers are verified, not claimed.** `verdict["writers"]` holds only agents that the harness lists **and** whose transcript is not proven finished (`wl_roster.py:438-464`). Each is a writer by type or by edit evidence (`:480`, `:487-491`). The verdict is `_roster` at `wl_checks.py:2237-2241`.
...
- **No plain `[ ]` item.** Use `open_items` from `S.classify_items` (`wl_checks.py:2200`). It already contains every state that is not allowed here:
  - plain `[ ]` (`wl_store.py:1290-1291`);
...
- **No harness task the session could do now.** Call `C.actionable_tasks(session_id, event.get("transcript_path"))` (`wl_core.py:614`). Reuse `_bg_actionable` (`wl_checks.py:2492`) when the pure-wait branch already computed it.
...
1. **Fix the ladder at its source.**
   ...
   - This alone removes blocker (a), and also the false `ladder-gone` that a `worker_verified=True` queue lease would trigger (`wl_liveness.py:701`).
2. **Renew queue leases while the cap is full (recommended; the operator may veto it).**
   - Model it on the existing renewal of the covering lead lease (`wl_checks.py:2203-2218`).
...
3. **Hoist `expired`.** Move the list at `wl_checks.py:2878-2886` above the pure-wait block and reuse it at `:2483-2487` and `:2887`. This is a pure refactor.
4. **Compute `_in_cap_wait`** as in section 1. Then pass `supervised=_supervised or _in_cap_wait` to `stuck_rounds` (`:2567-2575`). Without this, the 3x overrun, which counts without firing while saturated, fires on the first stop after saturation ends (`wl_checks.py:116-164`).
...
Put a **keep-list** in `wl_roster.py` beside `ROSTER_SUPPRESSES`: `CAP_WAIT_KEEPS` plus `cap_wait_keeps(key, always, compaction_due)`. A keep-list follows "should not be invoked": a check added later defaults to standing down, which matches `check_tier`'s rule that unknown keys are hygiene (`wl_checks.py:1974`).

### Dropped in the cap-saturated wait

| Check | Key(s) | Where it is added |
|---|---|---|
| 45/90/120 ladder (age-only rungs) | `ladder-investigate`, `ladder-resolve` | `wl_checks.py:3687-3692`, `:3708-3713` |
| The 45-minute ping | outq entry `ladder` (not a block) | `wl_checks.py:2591-2601`; for queue items it is gone after step 1 |
| Judge consult, including "Do the next action", SWEEP THE CLASS, PROOF OBLIGATION, the reggate, defer-audit and shapedup | none (step 7) | `wl_checks.py:4211-4717`; exits at `:4359`, `:4451`, `:4568`, `:4584`, `:4622`, `:4642`, `:4682` |
| Stuck detector | `stuck` | `wl_checks.py:2795-2806` |
| Idle-stall, solo-grind, sweep-moment | `idle-stall`, `solo-grind`, `sweep-moment` | `wl_checks.py:2824-2831`, `:3451-3454`, `:2867-2868` |
| 15-minute background check-in | `bg-report` | `wl_checks.py:2767-2794` |
| Report shape | `no-remaining`, `out-of-sync`, `unstated`, `mislabelled`, `unconfirmed`, `uncited` | `wl_checks.py:3839-3849`, `:3822-3827`, `:3804-3807`, `:3636-3637`, `:3762-3763` |
| STATE.md (except the compaction case below) | `agent-state` (every verdict, not only `stale`), `agent-absent` | `wl_checks.py:3456-3476`, `:3488-3493` |
| Intent, plan and checklist freshness | `intent-expired`, `plan-drift`, `plan-adopted`, `plan-unimplemented`, `plan-fidelity`, `cl-producing`, `cl-flip`, `cl-waves` | `wl_checks.py:3000-3031`, `:3055-3131`, `:3601` |
...
- **Compaction near and STATE.md not current.**
  ...
  - `late_band` is read by a new helper in `wl_checks`, `_ctx_late_band(session_id)`: lazily `import wl_retro`, then `cb = wl_retro.ctx()` (`wl_retro.py:40-49`), then `int(cb.load_state(session_id).get("band", -1)) >=` the index of `"late"` in `cb.BANDS`.
  - The late band is 0.98 of the threshold, i.e. about 2% until auto-compact (`ctx_budget.py:66`). `band-notice.py` writes `st["band"]` at `:265`, re-seats it at `:207`, and resets it after each compaction (`:211-220`).
  - If the context module cannot be loaded (for example in an LKG snapshot, per `wl_retro.py:11`), return False. The PreCompact snapshot covers that case.
...
- **A writer found dead, or a lease on a finished worker.**
  - A writer proven finished drops out of `writers` (`wl_roster.py:448-454`). The cap is then below 4, the predicate is false, and `queue-slot` fires.
  - `roster-dead` (`wl_checks.py:2697`) and `roster-unleased` (`:2691`) are kept, and so are `ladder-gone` and `ladder-idle` (`:3693-3707`).
- **Expired DEFAULT:** `defer-expired` (`:2887-2903`), plus `undefaulted` (`:2871-2876`).
- **A free slot with queued work:** `queue-slot` (`:2703-2714`). While saturated this cannot fire, because `free = 0` (`wl_roster.py:542-546`).
...
- **Hook integrity:**
  ...
  - import failures never reach `run_stop`: they are handled at `worklist.py:2297`, crashes fall back to LKG at `:2340-2360`, and the judge-unavailable exit is skipped along with the judge.
...
- **Secrets and safety.** `run_stop` contains no secret-scanning check: "secret" appears only in comments at `wl_checks.py:407`, `:2463` and `:3035`. Secret checks live in the pre-tool and pre-commit guards, which this change does not touch. The admission detector stays (step 7).
...
**How the harness fakes live writers.** Reuse `mk_sub`, `plant_lease`, `W1`-`W4` and `verdict` from `test_wl_roster.py:47-196`.
...
- It then appends `{"id","type":"subagent","status":"running","agent_type"}` to `fix.bg`, which becomes the event's `background_tasks` (`wlfix.py:483-492`).
...
- The judge is observed with `capturing_judge` and `prompt_of` (`test_wl_judge_fixset_scope.py:39-56`) and `wl.runj()`.
- The band state is planted as `<CTX_BAND_STATE_DIR>/deadbeef.json` containing `{"band": 1, ...}`; set `fix.env["CTX_BAND_STATE_DIR"]`, which `ctx_budget.py:295` honours.
...
| `c5_saturated_expired_default_blocks` | Plant a `[?]` whose add event is 130 min old and carries `DEFAULT:` (pattern from `test_wl_guide_and_deferrals.py:196-209`) | Block with "execute its DEFAULT now". |
...
**Mutation controls** follow the house rule of mutating a copy, never the shared tree (`docs/agent-reference/TRAPS.md:288-291`), and reuse the copy pattern of `test_wl_lkg.py:18-23`:
...
- **Wiring:** nothing new is needed.
  ...
  - No new environment variable, so the env registry and the sealed-module test (`test_wl_roster.py:614`) still pass.
