...
- [ ] Add `.claude/rediacc_hooks/guards/block_uncoordinated_submodule_pointer.py`: `TWIN = None`, `CHAIN = "pre-bash"`, next free `ORDER` slot (verify live, candidate 46), scoped `sys.path` import of `submodule_branches.py` per `block_prose_style_commit.py:164-175`'s pattern, the four-way decision tree in section 5, the `SUBMODULE_POINTER_OK` escape hatch.
