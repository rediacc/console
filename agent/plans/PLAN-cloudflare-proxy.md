...
1. **Client.** `rdc --proxy <url> <cmd>` or `REDIACC_PROXY_URL` (`packages/cli/src/cli.ts:180`, `:221`). The root preAction hook intercepts before the local action runs and calls `runCommandThroughProxy` (`cli.ts:222-243`, `packages/cli/src/services/executor/proxy-command.ts:215-259`). The bearer token is the stored subscription token, or `REDIACC_TOKEN` from the env (`cli.ts:232-240`, `packages/cli/src/services/account/subscription-auth.ts:8,121`). Refusals: 85 of 175 contract commands are `proxyCapable` (`packages/shared/src/cli-contract/data/contract.json`).
...
3. **Container.** `node cli-bundle.cjs serve --mode container --port 8080` (`workers/proxy/Dockerfile:81`), the Hono app in `packages/cli/src/services/serve/server.ts`. It serves these routes (`packages/shared/src/cli-contract/wire.ts:192-205`):
   - `/v1/health`
   - `/v1/server-info`
   - `POST /v1/session`, which mints an X25519 keypair (`server.ts:92-105`)
   ...
   - contract-version check (`server.ts:145-156`)
   - `prepareCommand` hard gate (`:160-166`)
   - `loadConfig` and `authorize` against the policy inside the encrypted config (`:176-193`, `services/serve/policy.ts:97-110`)
   - in-process Commander dispatch (`:206`, `services/serve/command-dispatch.ts:316-360`)
   - an audit event (`server.ts:221-240`)
...
| `ACCOUNT_URL` var | `wrangler.toml:13-14` | live (`https://eu.rediacc.com`) |
| `EXECUTOR_TOKEN` secret: a portal API token, scope exactly `proxy:exec`, in the org that owns hostinger's config | `wrangler.toml:16-17`, `index.ts:32,76` | **missing** |
| Container receives its token | `packages/cli/src/commands/serve.ts:61-67` reads `REDIACC_TOKEN` | **never passed (B1)** |
| Org has a remote config store holding hostinger | `container-config.ts:115-118`, `configs.ts:552-561` | operator must confirm (`rdc config remote status`) |
| Caller token carries `proxy:exec` | `index.ts:89`, `auth.ts:121` | CLI login token does not (B5) |

The design is single-org. Introspection answers `active:false` for a token from another org (`proxy.ts:66-67`), so one `EXECUTOR_TOKEN` means one org.
...
- X25519 session and CEK grant with principal binding (`sessions.ts:216-225`)
...
- **B1: the container never gets its token, so it exits at boot.**
  - The Worker passes only `envVars = { REDIACC_EXECUTOR_MODE: 'container' }` (`index.ts:49-51`). Nothing in `packages/cli/src` reads `REDIACC_EXECUTOR_MODE`, so the variable is dead.
  - `rdc serve` throws when `REDIACC_TOKEN` is absent (`serve.ts:61-67`).
  - `EXECUTOR_TOKEN` exists only in the Worker env. It is never handed to the DO or the container.
  - Fix: in `ExecutorContainer`, build `envVars` from `this.env`: `REDIACC_TOKEN: env.EXECUTOR_TOKEN`, `REDIACC_ACCOUNT_SERVER: env.ACCOUNT_URL` (`serve.ts:69` resolves the URL through `subscription-auth.ts:33-41`). The library forwards `envVars` at start (`node_modules/@cloudflare/containers/dist/lib/container.js:1327,1336`).
...
- **B3: the decrypted config is used for policy only, not for execution.**
  - `server.ts:176` loads the session config and passes it only to `authorize` (`:177-193`).
  - `dispatchCommand` (`:206`) gets no config. The dispatched Commander tree reads `configService` and `configFileStorage` from disk (`command-dispatch.ts:320-322`). `CommandRequestContext` has no config field (`packages/cli/src/services/core/request-context.ts`).
  - In the container that disk is empty, so machine and repo lookups fail. The re-attach route is affected the same way (`server.ts:295`, then `connectForJobs(machine)`).
...
- **B4: the CLI `--proxy` client never opens a session or grants the CEK.**
  ...
  - The container therefore answers every command 404 "This session has no config key yet" (`container-config.ts:100-106`).
  - Only the web console implements the grant (`console.ts:407,446`), and that tier is `mock` in production.
...
- **B5: CLI login tokens lack `proxy:exec`.**
  ...
  - The Worker returns 401 for such a token (`index.ts:89`).
...
- **S1: the grand-repo guard does not apply through the proxy.**
  ...
  - `--proxy` exits before the action (`cli.ts:221-243`). The executor process is not an agent.
  - `authorize` has an `isGrandRepo` field (`policy.ts:85-86`) that `server.ts:177-193` never sets.
...
Smaller defects:
- `deploy-proxy.sh:9-10` claims the dry run does a docker build. It does not (`:38-42`), a gap `.ci/rediacc_ci/quality/container_build_context.py:3` already records.
- The `--region` value is ignored (`deploy_proxy.py:70`).
...
The CLI switch is `--proxy <url>` or `REDIACC_PROXY_URL` (`cli.ts:180,221`). The Cloudflare trial is **blocked on B1-B5**. Phase 1 shows the protocol working against hostinger today, without Cloudflare.
...
2. Terminal A: `REDIACC_TOKEN=<tok> ./rdc.sh serve --mode daemon --host 127.0.0.1 --port 8080`. Proves the server boots and authenticates with the account server. Verify: the listening line (`serve.ts:105-108`). Rollback: Ctrl-C (drain handler at `serve.ts:111-116`).
...
- **Library defaults:** `sleepAfter` defaults to `'10m'` (`@cloudflare/containers` 0.3.7, `dist/lib/container.js:20`). The proxy sets `'4m'` (`index.ts:47`). An open HTTP stream counts as in flight, and the idle timer restarts only when the stream ends (`container.js:887-960`), so long `repo up` streams keep the container awake correctly.
- **Current settings:** `instance_type = "basic"` (1/4 vCPU, 1 GiB, 4 GB disk; `wrangler.toml:28`), `max_instances = 20` (`:29`), `sleepAfter = '4m'`.
...
|---|---|---|
| Today (no EXECUTOR_TOKEN; the Worker 401s before `getContainer`, `index.ts:103-113`) | 0 | **$0 marginal** (Worker requests within included) |
...
3. Keep `instance_type = "basic"` (`wrangler.toml:25-28`) until Phase 2 step 5 measures RSS. `lite` (1/16 vCPU, 256 MiB) would cut memory cost 4× but is probably too slow for ssh2 crypto.
...
3. `npx wrangler delete --name rediacc-proxy-eu` (the runbook's rollback form adds `--force`, `LEAD-RUNBOOK.md:35`). This removes the script, its versions, its secrets (EXECUTOR_TOKEN), the `proxy.eu.rediacc.com` custom domain, and the DO namespace it owns.
