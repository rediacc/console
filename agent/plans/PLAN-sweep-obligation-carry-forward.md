...
Both figures are LOWER bounds, and the reason is structural. A fix stop whose regression gate blocks exits at `.claude/hooks/stop/wl_checks.py:4621-4630` through `C.emit`, which calls `sys.exit(0)` (`.claude/hooks/stop/wl_core.py:386-388`), and that is BEFORE `log_verdict` at `.claude/hooks/stop/wl_checks.py:4691`. Blocked fix stops therefore never appear in the log at all, while
