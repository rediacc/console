...
`.ci/rediacc_ci/tests/gates/test_gate_bws_map.py::test_clean_fixture_passes` fails: `fixture()` (.ci/rediacc_ci/tests/gates/test_gate_bws_map.py:72) never writes `private/account/.env.example` into the synthetic tmp_path tree, but `check_bws_map.py`'s assertion 14 (`.ci/scripts/quality/check_bws_map.py:728`, `ENV_EXAMPLE = ROOT / "private" / "account" / ".env.example"`) reads it unconditionally.
...
Add `private/account/.env.example` to `fixture()`, containing exactly the 3 names `fixture()` already homes in `SECRET_MAP`/`EXEMPTIONS` (`ALPHA_TOKEN`, `ORPHAN_TOKEN`, `PREFIX_EU`) as active assignments, padded with 27 more names as **commented-out** assignments (`# PAD_NNN=x`) of `kind: "opt-in"` in a new `env-local-allowlist.json` fixture file -- `.ci/scripts/quality/check_bws_map.py:833-836` requires an opt-in entry's name to be commented, not active, which sidesteps needing 27 more map/exemption entries.
`MIN_EXAMPLE_NAMES = 30` (`.ci/scripts/quality/check_bws_map.py:740`) counts active-or-commented names together, so 3 active + 27 commented clears the floor.
...
- Diff the fixture addition against `.ci/scripts/quality/check_bws_map.py:740`, `:833-836` by hand to confirm the padding shape matches what those lines actually require.
