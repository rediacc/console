...
Status: done -- verified 2026-09-24: both sub-batches landed (0b4165afd scripts/gen, f59db0679 scripts/ops and scripts/docker), all 10 boxes ticked; .sh paths below record the pre-move layout
...
## 1. `scripts/gen/gen-docs.ts:268`'s "MISSING keys are fatal" mechanism, and `doc-registry-preport.json`, re-measured

The citation has drifted: the line is now `scripts/gen/gen-docs.ts:354` ("MISSING keys are fatal to `--diff-snapshot`; ADDED keys are reported as normal growth."), not `:268`. The mechanism (`diffSnapshot()` at `scripts/gen/gen-docs.ts:410`) compares each provider's recorded pre-port key set against its live set; a `missing` key not present in `retired.<provider>` in
...
- `.ci/scripts/quality/check-gate-id-convention.sh` (retired, ported to `.ci/rediacc_ci/quality/gate_id_convention.py`, e.g. `:63,158,257,334`) -- prose/error-message references to `scripts/gen-gates-lock.ts`; not path-matching logic, but should move in the same commit per invariant 9 ("comments are the asset").
Confirmed landed: the port's own references already read `scripts/gen/gen-gates-lock.ts`.
- `.ci/scripts/test/gates/test-docs-gen.sh` (retired, ported to `.ci/rediacc_ci/tests/gates/test_gate_docs_gen.py:53,178`) -- `GEN = ROOT / "scripts" / "gen" / "gen-docs.ts"` and an error-message/rerun-hint literal. **Functional**: the port's own `GEN` already names the post-move path, confirming this edit landed.
- `.ci/scripts/test/gates/test-doc-region-parity.sh` (retired, ported to `.ci/rediacc_ci/tests/gates/test_gate_doc_region_parity.py:108,111`) -- **functional and the highest-risk item found in this review**: `GEN` / `GEN_ENTRY` (used to run the real generator against a fixture) already name the post-move path in the port.
The port also structurally retires the second risk this citation named: the old twin's `copy_tracked` call hardcoded the fixture-input file list as a literal (`scripts/gen-docs.ts scripts/lib/doc-providers.ts scripts/lib/doc-regions.ts`), which a bare path move could silently drop from; the port's `import_closure()`/`derive_pathspecs()` (`:177,232`) derive that list by walking the generator's own relative imports, so it cannot go stale the same way.
...
- `.ci/scripts/security/shfmt.sh` (retired, ported to `.ci/rediacc_ci/security/shfmt.py:99-100`) -- `for dir in scripts/dev scripts/docker; do if [[ -d "$dir" ]]; ...` -- **silently green**: if the directories no longer exist at the old paths, this loop simply finds nothing to format and exits clean.
Must be repointed to `scripts/ops`; confirmed landed: the port's `OPTIONAL_SCOPES` already reads `("scripts/dev", "scripts/ops")`.
- `.ci/scripts/quality/check-silent-failure-patterns.sh` (retired, ported to `.ci/rediacc_ci/quality/silent_failure_patterns.py:80`) -- `SCAN_DIRS=(".ci/scripts" "scripts/dev")` -- **silently green** for the same reason.
Confirmed landed: the port's `SCAN_DIRS` already reads `(".ci/scripts", "scripts/dev", "scripts/ops")`.
...
- all four gate tests below invoke or reference `scripts/dev/*` by literal path; functional, must move with the files. All are retired bash, ported to pytest:
`.ci/rediacc_ci/tests/gates/test_gate_scrub_sentinel_empty.py:3` (subject renamed `scripts/ops/scrub-sentinel.sh`), `.ci/rediacc_ci/tests/gates/test_gate_worktree_devbox_teardown.py:31` (`SUT` correctly still `scripts/dev/worktree.sh` -- `worktree.sh` did not move with the other 14 files, see the Tasks ledger), `.ci/rediacc_ci/tests/gates/test_gate_bws_map.py:97,121` (`scripts/ops`), `.ci/rediacc_ci/tests/gates/test_gate_vacuity_floors.py`.
...
- [x] Re-check `scripts/data/domains.json:99` (`operator-bash`) and `:156` (`generators`) blocker status live at execution time; do not start either sub-batch until its blocker is confirmed cleared by whoever owns W8/W0/W11/W2's handover.
    (ticked) 2026-09-22T19:53:13Z by d778be9d: scripts/data/domains.json:119 operator-bash and :177 generators both carry blockedOn: 'CLEARED 2026-09-20: ...'
- [x] Sub-batch A (`scripts/gen/`, 2 files): move `scripts/gen-docs.ts` and `scripts/gen-gates-lock.ts` into `scripts/gen/`, with the manifest.ts/package.json/knip.jsonc/test-docs-gen.sh/test-doc-region-parity.sh/check-gate-id-convention.sh edits from section 6a, all in one commit.
    (ticked) 2026-09-22T19:53:13Z by d778be9d: commit 0b4165afd: git show --stat confirms R renames plus manifest.ts/package.json/knip.jsonc edits, files live at scripts/gen/gen-docs.ts, scripts/gen/gen-gates-lock.ts
- [x] Sub-batch A verification: `check:ci-gates-lock`, `check:ci-parity`, `check:ci-doc-region-parity`, `check:ci-domain-partition`, `gen-docs.ts --diff-snapshot` (control), all green or at the known pre-existing red only.
    (ticked) 2026-09-22T19:53:13Z by d778be9d: PLAN-tooling-transformation.md W9P2 box: 'Green: check:ci-parity, check:ci-domain-partition, check:ci-doc-region-parity, check:ci-gate-id-convention, knip, ruff, biome'; re-ran check-domain-partition.ts live: exit 0, 0 unclassified/shadowed/dead
- [x] Flip `domains.json`'s `generators` rule to its finished-state form (`stays: true`, `blockedOn` removed) once sub-batch A lands.
    (ticked) 2026-09-22T19:53:14Z by d778be9d: scripts/data/domains.json:175 stays:true confirmed; blockedOn annotated 'CLEARED...' rather than deleted (minor textual deviation, functionally harmless per check-domain-partition.ts 0 unclassified)
- [x] Sub-batch B (`scripts/ops/`, 15 files): add `"scripts/ops/**"` to `domains.json`'s `operator-bash.paths` in the same commit as the `git mv` of all 15 files (13 under `scripts/dev/`, `scripts/docker/build-server.sh`, `scripts/backup-cutover-preflight.sh`), plus the shfmt.sh / check-silent-failure-patterns.sh / run-legacy.sh / 4 test-*.sh / check-worker-secret-names.ts / check_bws_map.py edits from section 6b.
    (ticked) 2026-09-22T19:53:14Z by d778be9d: commit f59db0679: 14 files moved (not 15, worktree.sh got its own devbox-bash rule instead); operator-bash.paths=['scripts/ops/**'] confirmed at scripts/data/domains.json:115
- [x] Sub-batch B verification: `check:ci-domain-partition` (expect zero UNCLASSIFIED, not a hard-fail), a direct re-run of shfmt.sh and check-silent-failure-patterns.sh confirming their scan counts did not drop, the 4 affected test-*.sh gate tests passing against the new path.
    (ticked) 2026-09-22T19:53:14Z by d778be9d: commit f59db0679 message: silent-failure scan 430->432 files, shfmt 590->591, planted defect in moved file reds; re-ran check-domain-partition.ts live: 0 unclassified
- [x] Flip `domains.json`'s `operator-bash` rule to its finished-state form (`stays: true`, `paths: ["scripts/ops/**"]`, `blockedOn` removed) once sub-batch B lands.
    (ticked) 2026-09-22T19:53:14Z by d778be9d: scripts/data/domains.json:115-117 stays:true and paths:['scripts/ops/**'] confirmed; blockedOn annotated 'CLEARED...' rather than removed (same minor deviation as generators rule)
- [x] Drain `scripts/data/domain-layout-baseline.json` via `--write-baseline` after each sub-batch (expect it to fall from 39 to 37 after A, to 22 after B -- the 22 `incoming-*` entries are out of this plan's scope and belong to W7).
    (ticked) 2026-09-22T19:53:15Z by d778be9d: parent plan text: 'layout baseline drained 39 -> 37' (sub-batch A) and commit f59db0679: 'drains from 37 to 22' (sub-batch B); current file has 19 entries, all unrelated later incoming-* work
- [x] Formally close the `scripts/ci/` leg as N/A in the parent box's next status update: zero `.cjs` files exist under `scripts/` to move.
    (ticked) 2026-09-22T19:53:15Z by d778be9d: parent PLAN-tooling-transformation.md W9P2 box states explicitly: 'scripts/ci/ is dropped: zero .cjs exist under scripts/'
- [x] Re-run `npx tsx scripts/gen-docs.ts --diff-snapshot` after both sub-batches and confirm the only red is the pre-existing, unrelated `bws-unrequested.json` drop (or that it has since been fixed by someone else, in which case this should exit 0).
    (ticked) 2026-09-22T19:53:15Z by d778be9d: ran scripts/gen/gen-docs.ts --diff-snapshot live: DROPPED rows are ci-tree/gates/hook-guards/bws-unrequested.json, none referencing this plan's moved paths (scripts/gen-*, scripts/dev/*, scripts/docker/*, scripts/ops/*)
