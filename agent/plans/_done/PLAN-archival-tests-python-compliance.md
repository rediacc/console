...
1. `--move` really performs `git mv`, the old path is really empty afterwards, and `git status --porcelain` reports it as a rename (`test-agent-session-archival.sh:98-114`).
