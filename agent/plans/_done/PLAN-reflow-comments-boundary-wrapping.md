...
yields two extra trailing fields on every `"para"` item (both `yield` sites, `.ci/rediacc_ci/quality/prose_style.py:1505` and `1532`). Both consumers of that shape -- `reflow_comments` and `underwrap_findings` (`.ci/rediacc_ci/quality/prose_style.py:996-1006`, the R19 sibling `comment_segments`'s own docstring already demands stay on one boundary logic) -- unpack and use the two new fields for their own width-budget math.
