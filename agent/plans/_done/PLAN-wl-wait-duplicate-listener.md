...
- `.claude/hooks/stop/wl_wait.py line 38 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- `DEFAULT_TIMEOUT_MIN = float(os.environ.get("WORKLIST_WAIT_TIMEOUT_MIN", "60"))`, with the comment on line 37 naming "60 minutes, matching the hourly work-loop cadence".
- `.claude/hooks/stop/wl_wait.py line 204 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- `deadline = time.monotonic() + timeout_min * 60.0`. The `* 60.0` is the conversion; the flag value is minutes.
- `.claude/hooks/stop/wl_wait.py line 491-501 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- `main()` parses `--timeout` as a bare float into `timeout_min` with no unit anywhere in the token.
- `.claude/hooks/stop/wl_wait.py line 314 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- the only user-visible statement of the unit is one line inside `HELP`: `--timeout <minutes>   default %d`.
...
- `.claude/hooks/stop/wl_wait.py line 207-220 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- the wait loop. Each pass touches a heartbeat, checks the deadline, then `time.sleep(min(TICK_S, remaining))` with `TICK_S = 2` seconds (`:36`).
- `.claude/hooks/stop/wl_wait.py line 43-49 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- `_stat()` returns `(size, mtime_ns)`. The two watched paths are the requests log (`S.requests_path`, `:197`) and the report index (`RPT.index_path`, `:198`). A change in either triggers a fold; `_new_requests` (`:73-82`) and `RPT.unread` (`:240`) diff against the launch-time baseline from `arm()` (`:52-70`).
...
- **It holds no lock, and that is deliberate and load-bearing.** Module docstring `:15-17`: "IT NEVER TAKES A LOCK, AND THAT IS THE SHARPEST HAZARD IN THE WHOLE DESIGN", because `_append_lines` takes a blocking `LOCK_EX` and the two `LOCK_EX|LOCK_NB` paths (`.claude/hooks/stop/wl_requests.py line 165 (blob 326bae9f3857eefca445a9231d4a779a267a60a9)`, `.claude/hooks/stop/wl_store.py:1981`) give up SILENTLY on contention, so an hour-long holder would turn them into invisible no-ops. Any fix must not violate this.
...
- `.claude/hooks/stop/wl_wait.py line 103 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- `heartbeat_path()` returns ONE path per session: `worklist.with_suffix(".waiter-<me8>")`. It carries a timestamp and no pid.
- `.claude/hooks/stop/wl_wait.py line 208 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- every instance `_touch(hb)` on that same path every 2 seconds.
- `.claude/hooks/stop/wl_wait.py line 218 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` and `:268` -- on exit an instance writes a TOMBSTONE to that same path instead of unlinking it (`tombstone()`, `:150-161`).
...
- `.claude/hooks/stop/wl_wait.py line 418-425 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` -- `nudge()` returns silently when the heartbeat is fresh and not a tombstone. With 13 instances refreshing it every 2 seconds it was permanently fresh, so the nudge said nothing.
...
So the relaunches came from the session acting on the standing instruction that the waiter "FIRES ONCE" and must be relaunched (`.claude/hooks/stop/wl_wait.py line 262-267 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)`, `HELP` `:299-301`, `V_NO_WAITER` `.claude/hooks/stop/worklist_messages.py:627-642`), combined with the belief that a 60-second process had already finished. **Prose cannot fix that.** The script must refuse.
...
- **A non-blocking flock idiom on a dedicated sidecar.** `.claude/hooks/stop/wl_requests.py line 162-166 (blob 326bae9f3857eefca445a9231d4a779a267a60a9)` opens `str(S.requests_path(worklist)) + ".lock"` and takes `S._flock(lock, S.LOCK_EX | S.LOCK_NB)`, returning on `OSError` ("another stop is escalating; it wins, next stop retries").
...
Refuted by the mechanism, not by preference. `.claude/hooks/stop/wl_wait.py line 6 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)` and `:8-9`: "Launched as a BACKGROUND SHELL TASK. Its exit is the ping. [...] the one push channel that exists is the harness notifying the session when a background task finishes".
...
Placed in `wait()` immediately after the path computations (`.claude/hooks/stop/wl_wait.py line 188-191 (blob 20dff1b2804531862015c2cd71bfe5cad3be4151)`) and BEFORE `_safe_scan`/`arm` (`:194-195`), so a duplicate pays for nothing:
