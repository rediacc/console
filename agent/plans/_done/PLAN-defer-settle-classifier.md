...
Its prompt already says almost verbatim what the operator is asking for: *"some of them asking for work that was ALREADY done"* and *"Is the WHY still TRUE? A reason that has expired (the run finished, the file landed, **the answer is in the tree**) is no reason."* A `do_now` verdict reopens the item as `[ ]` (`.claude/hooks/stop/wl_checks.py line 4763-4772 (blob 5a8904da5ad64d44df6a7e6095da16a81a681eff)`, `S.set_state(worklist, "judge", rid, " ", ...)`).
...
**The greppable, permanent marker:** `by="haiku-defer-settle"` on the `state` event (the store already has precedent for a non-human actor in `by` — the audit's reopen uses `by="judge"`, `.claude/hooks/stop/wl_checks.py line 4766-4772 (blob 5a8904da5ad64d44df6a7e6095da16a81a681eff)`). This is visible two ways forever:
