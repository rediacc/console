...
- **`CITE_RE` gained nested dotfiles.** `private/account/.env:12` did not match any branch, so an env-fact settle's evidence could never pass `completion_evidence`. The dotfile branch now takes an optional directory prefix; `.ci/policy/.dead-bash-allowlist:19` was uncitable for the same reason.
