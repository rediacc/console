...
Concrete instance, found and fixed 2026-09-24. The first cut of the Biome half of `.ci/scripts/quality/lint-rule-liveness.mjs` created 7 files in the tree, 2 of them in `private/account`. It also appended to the tracked `private/account/src/services/email.service.ts` and only restored it in a `finally`. A hard kill skips `finally`, so the append stayed behind. The fix is recorded at `lint-rule-liveness.mjs:525` ("WHY NOT THE REAL TREE"): the fixtures now go into an OS temp mirror (`:653`, `:699`). The manifest entry at `scripts/ci-runner/manifest.ts:2443-2455` never had a mutex, and no gate could have noticed. There is a second blind spot: that entry's only leaf is the Python wrapper (`manifest.ts:2448`). The `.mjs` that did the writing is reached through `subprocess.run(["node", .../DRIVER])` (`check_lint_rule_liveness.py:43`, `:69-73`), and the wrapper's docstring (`:24`) forbids listing the `.mjs` as a leaf. A scanner that reads only declared leaves would miss the exact file that did the damage.
...
- Registration:
  ...
  - Manifest entry: `manifest.ts:2692-2703`, CI step in `quality-static` (`.github/workflows/ci-quality.yml:395-397`, inside the gate-bind region `:275-487`).
...
- Resource names are opaque strings (`pool.ts:51-53` "PATH-SCOPED (`tree:<dir>`)"). `tree:repo` does not conflict with `www-src-probe` or `tree:packages/www`.
- **Who holds `tree:` claims today (lock):** only `check:ci-pytest` with `mutex: ['tree:repo']` (`manifest.ts:4820`) and `gate-test:runner-advice` with `reads: ['tree:repo']` (`manifest.ts:4909-4911`).
...
- `mutex` is a HAND-ONLY field. `scripts/gen/gen-manifest.ts:64-72` lists it in `HAND_ONLY`, so an entry that carries it drops out of the generated regions and is written by hand. `gen-gates-lock.ts` serialises the whole spec, so a new GateSpec field reaches the lock without code changes (gate-spec.ts:73-75 says this about `env`).
...
- Parameter propagation is load-bearing, not optional. Without it, 272 sites were PARAM. The real violator `proxy_image_smoke.py:281` (`shutil.copyfile(prebuilt, staged)` inside `_stage_renet(root)`, called with `paths.repo_root()` at `:414-415`) only resolves to TREE through a call site.
- Name heuristics alone are unusable. `actions_vars.py:454`, `ci_job_aggregation.py:603`, `account_portal.py:269`, `audit_coverage.py:399` and `battery_clean_tree.py:432` all name a variable `root` that is bound to `pathlib.Path(tmp)`. This is the same "TEMP beats name" lesson as `pool_writer_safety.py:57-59`.

TS/JS, regex-only over relative-import closures of `.ts/.js/.mjs` leaves: 251 non-temp-named sites. Every one of the 45 "TREE-named" sites opened by hand was either a real tree writer (below), mode-gated, temp in disguise, or unreachable. Three false-positive shapes the real scanner must handle:
- `import type` edges. `check-solution-video-engine.ts:37` imports only a type from `update-video-manifest.ts`, whose `saveManifest` (`:131-134`) writes the tracked `packages/www/src/data/video-manifest.json`.
...
- Callback parameters. In `check-docs-render-parity.control.ts:114-125`, `plant: (root) => …` receives `root` from `c.plant(root)` at `:195`, and that root is a `mkdtempSync` at `:91`.
...
| (fixed) | `check:ci-lint-rule-liveness` (`manifest.ts:2443`) | first cut: `path.join(ROOT, fx.file)` plus append to `email.service.ts` | tracked `private/account` source | tracked | Already refactored to a temp mirror (`lint-rule-liveness.mjs:653`, `:699`). It becomes the **regression fixture pair** (§4). |

Mode-gated baseline/refresh writers that the scanner must classify as not run by the gate. Confirmed by reading the guard:
- `account_env_retired.py:236` via `--write-baseline` (`:250`)
- `plant_proofs.py:1063` (`:1142`)
- `python_env_registry.py:517` (`:549`)
- `check_language_policy.py:463` (`:842`)
- `check_plan_boxes.py:1160` via alias `update = "--update" in argv` (`:1146`)
- `check_resprofile.py:205` via `--seed` (`:334`)
- `prose_style.py:1923` via `if write:`
- `scripts/gate-bind.ts:2139/2216/2445` via `--write`/`--extract-all` (`:2087-2117`)

Prototype TREE hits whose guard the prototype did NOT verify (task 2.3 confirms each):
- `python_types.py:814`
- `check_job_timeout_headroom.py:228`
- `check_runner_advice.py:754`
- `check_secret_reachability.py:222`
- `check_tree_shape.py:449`
- `check_plan_folders.py:456-546`
- `check_plan_record.py:800`
- `check_actions_allowlist.py:106`
- `check_agent_session_archival.py:436`
- `prose_style.py:1299`
- `security/actionlint.py:301`, which is probably a prototype false positive: its comment says the cache lives under a temp root.

Scratch writes (allowed, §3.4): `python_lint.py:429-437` and `:544-553` (`.ci/cache/ruff-control-*`).
...
- That gate's output contract is pinned byte-for-byte by its shadow history (`pool_writer_safety.py:17-32`, `:114-115`).
...
- `.ci/scripts/quality/check_gate_tree_writes.py`: entry point, same 2-import shape as `check_pool_writer_safety.py:41-47`.
...
- **Mutator table** (one shared JSON, `.ci/policy/tree-write-mutators.json`, read by both scanners so they cannot drift):
  ...
  - `cp/mv/install/ln`: last argument, like `pool_writer_safety.py:182` + `:332`.
...
Each expression gets one origin: `TEMP`, `SCRATCH`, `TREE`, `UNRESOLVED`. Combining origins is ordered **TEMP > SCRATCH > TREE > UNRESOLVED**: any TEMP component makes the path TEMP. That is the bash scanner's rule (`pool_writer_safety.py:228-245`), which cut an earlier draft's false positives from 21 lines to 6.
...
- **SCRATCH seeds.** A TREE expression whose literal path segments place it under `.ci/cache/`, which is gitignored (`.gitignore:137`) and pruned by `paths.walk_tree` (`paths.py:175-179`). The allowlist is a one-entry constant with that justification. It is deliberately not "anything gitignored": `packages/*/dist` are ignored and still need `build-artifacts`/`www-dist`, and V3's staging path is ignored and still collides.
- **TREE seeds.**
  - Python: `paths.repo_root/from_root/ci_dir/quality_dir/hooks_stop_dir/find_repo_root`, `get_repo_root()`, any `__file__` chain, `os.getcwd()`, `Path.cwd()`, `Path(".")`, the stdout of `git rev-parse --show-toplevel`, **and any relative string-literal path** used as a target. Relative paths are TREE because run.ts:271 runs every gate from `REPO_ROOT`.
...
- **Propagation (how much taint tracking).** Measured to be the minimum that works:
  1. **Intra-procedural** through `=`, annotated assignment, `with … as`, `for … in`, walrus, `path.join/resolve`, `/`, `joinpath`, `.parent`, `.resolve()`, `str()`, f-strings and template literals, with enclosing-scope lookup for nested functions and closures (the `ci_job_aggregation.py:603-608` shape).
  2. **Module-local interprocedural**, to a fixpoint. A parameter's origin is the combination of its argument origins over every call site in the module. Any TREE argument makes the parameter TREE. All-TEMP makes it TEMP. Arguments omitted from a call take the default's origin, so `root = pathlib.Path(root or paths.repo_root())` (`account_env_retired.py:193`) is TREE when a call site omits it. Callbacks follow property name: a parameter of `k: (p) => …` takes origins from `x.k(args)` in the same module.
...
- **TREE**: the entry must hold an exclusive `tree:` claim in the lock (`mutex`, `reads` rejected, the same rule as `pool_writer_safety.py:150-151`) **and** carry `writesTree: '<reason>'`.
  ...
  - A `writesTree` entry with zero TREE sites is **not** red (one-directional, `pool_writer_safety.py:46`). It is listed as `declared, no site found` so over-declaration stays visible without being pushed.
- **MODE-GATED**: the site is dominated by a truthy test of a flag literal that no registered invocation of that leaf passes (§3.2.2). Recognised guards:
  - Python: `"--x" in argv|sys.argv|args`, `argv[0] == "--x"`, argparse `args.x` where `add_argument("--x", action="store_true")`, and one alias hop (`update = "--update" in argv; if update:`, `check_plan_boxes.py:1146`).
  - TS: `argv.includes('--x')`, `process.argv.includes`, and one alias hop (`const write = argv.includes('--write')`, `gate-bind.ts:2088`).
...
- Total sites seen < floor. Set the floor from the first measured run at roughly 80% of the Python and TS counts, as `Controls(..., floor=…)` does in `pool_writer_safety.py:632`.
...
Controls are planted into a `tempfile.TemporaryDirectory()` and reached through seams `GATE_TREE_WRITES_LOCK`, `GATE_TREE_WRITES_PKG`, `GATE_TREE_WRITES_ROOT`. They are copied, never symlinked, for the reason at `pool_writer_safety.py:28-29`.
...
| C2' mirror | `with tempfile.TemporaryDirectory() as tmp: root = pathlib.Path(tmp); (root/".github").mkdir(...)` (the `actions_vars.py:454` shape, the name that fooled the prototype) | green |
...
## 5. Wiring
- **package.json:** `"check:ci-gate-tree-writes": ".ci/scripts/quality/check_gate_tree_writes.py"`. Bare path, not `python3 …`, because check-ci-parity resolves a `python3` prefix to the leaf `[python3]` (`check_pool_writer_safety.py:8`).
- **Entry header:**
  ```
  ---- gate ----
...
- **Manifest:**
  ...
  - Add `writesTree?: string` to `GateSpec` (`gate-spec.ts`, after `:60`) and to `HAND_ONLY` (`gen-manifest.ts:64-72`).
  - Add `writesTree` to `check:ci-pytest` (`manifest.ts:4820`), plus V3, plus V4 if the seam refactor slips.
...
- [x] 1.1 Export `resolveInvocations(cmd) → {leaf, argv}[]` from `scripts/gates/check-ci-parity.ts` (around `:154`) and make `resolveLeaves` its projection. Prove check-ci-parity's output is byte-identical before and after on the live tree.
- [x] 1.2 Add `.ci/policy/tree-write-mutators.json` (git/sed/cp/rm/…/`--write`/`--fix` table, §3.3), read by both scanners.
- [x] 1.3 Write `scripts/lib/tree-write-sites.ts` on `oxc-parser`: closure (relative imports, `.js→.ts`, skip `import type`, stop at `packages/*/src`), sinks (§3.3), origin lattice (§3.4, steps 1-3 plus property-name callbacks), mode guards (§3.5), pragmas, and `--json`/`--selftest`.
- [x] 1.4 Write `.ci/rediacc_ci/quality/gate_tree_writes.py`: Python closure (imports plus spawn edges, §3.2.3), reachability (§3.2.4), sinks, origin lattice with module-local fixpoint, mode guards, pragmas, `.sh` leaves via `pool_writer_safety.scan_text`, lock policy (§3.5), refusals (§3.6), and controls C1-C10 plus REAL (§4).
- [x] 1.5 Add `writesTree?: string` to `scripts/ci-runner/gate-spec.ts` and `HAND_ONLY` in `scripts/gen/gen-manifest.ts:64-72`. Add `writesTree` to `check:ci-pytest` (`manifest.ts:4820`) naming `test_gate_gate_anti_vacuity.py` and `test_gate_generate_tag_inputs.py`.
- [x] 2.1 Run the gate in report-only mode (`--report`, exit 0, full listing) on the live tree. Record per-bucket counts (SAFE/TREE/MODE-GATED/UNRESOLVED, Python and TS separately) in this plan. Set the §3.6 floors from those counts.
- [x] 2.2 Triage every UNRESOLVED site. Improve the resolver where a shape recurs 3 or more times; otherwise add a `tree-write: safe <reason>` pragma. Record the final pragma count.
- [x] 2.3 Confirm or refute each unverified prototype TREE hit in §2 (python_types.py:814, check_job_timeout_headroom.py:228, check_runner_advice.py:754, check_secret_reachability.py:222, check_tree_shape.py:449, check_plan_folders.py:456-546, check_plan_record.py:800, check_actions_allowlist.py:106, check_agent_session_archival.py:436, prose_style.py:1299, actionlint.py:301). Any that is a real unguarded write becomes a new V-row.
- [x] 3.1 V1: move `python_lint.py:357-365` CONTROL 3 onto a temp `git init` fixture carrying the real `.gitignore`. The control must still red when `enumerate_py` drops untracked files (plant that and show it).
- [x] 3.2 V2: replace the `packages/cli/.config-migrations-check.tmp.ts` write (`config_migrations.py:250-276`) with stdin or `--eval` to `npx tsx` under `cwd=packages/cli`. Measure that `@rediacc/shared/config-schema` resolves. If it does not, fall back to `mutex: ['tree:repo']` + `writesTree` and record why.
- [x] 3.3 V3: add `mutex: ['tree:repo']` + `writesTree: 'stages workers/proxy/renet/renet-linux-amd64 into the docker build context (Dockerfile:54 COPY)'` to `check:ci-proxy-image-smoke` (`manifest.ts:5196`).
- [x] 3.4 V4: add a WWW_SRC root seam to `scripts/gates/check-translation-key-usage.ts` and move `check-translation-key-usage.control.ts:156-212` onto a temp mirror. Then drop `www-src-probe` from both `check:i18n:key-usage` (`manifest.ts:259`) and `lint:unused` (`manifest.ts:169`) if nothing else claims it, and rewrite the "MUST be written into the real tree" comment (`:146-150`). If the seam slips, add `tree:repo` + `writesTree` as the interim fix.
- [x] 3.5 V5: add `# tree-write: mode --write regenerates the vendored manifest; the registered gate runs verify` at `check-breakpoint-drift.sh:207`.
- [x] 4.1 Register: package.json key, entry header, `gate-bind --extract`/`--write` (step emitted into quality-code), `gen:gates-lock`, `check:ci-gates-lock`, `check:ci-gate-bind`, `check:ci-parity`, `check:ci-gate-manifest` green.
- [x] 4.2 Add the anti-vacuity row to `test_gate_gate_anti_vacuity.py` and the pytest module `test_quality_gate_tree_writes.py`. Run `npm run check:ci-pytest` green.
- [x] 4.3 Differential proof on the live tree: re-plant the lint-rule-liveness wave-2 shape (append to a copy of the real `lint-rule-liveness.mjs` in a temp fixture tree reached through the seams, never the real tree) → gate red naming the `.mjs` line. Remove the plant → green with byte-identical other output.
- [x] 4.4 Flip from report-only to enforcing. Confirm `npm run ci` is green and that `check:ci-gate-tree-writes` appears in the pool run.
- [x] 5.1 Docs: `docs/agent-reference/ci-gates.md:69`, `docs/agent-reference/TRAPS.md:631` (Enforced-By/Residue), `pool_writer_safety.py` docstring cross-ref, `pool.ts` header, `gate-spec.ts` `writesTree` doc.
- [x] 5.2 Close worklist #deb0b82d with the measured bucket counts and the V1-V5 dispositions.

## Measured (tasks 2.1-2.3, 2026-09-24)

Report-only first run (`check_gate_tree_writes.py --report`, 335 scheduled entries, 27 bare-tool leaves not scanned, 13 product modules not entered), distinct sites:
- Python: SAFE 976, TREE 38, UNRESOLVED 48, MODE-GATED 3 (1065).
- TS/JS: SAFE 246, TREE 32, UNRESOLVED 19, MODE-GATED 27 (324).
- bash: TREE 1 (V5).

After the resolver work of 2.2 and the V-row fixes (enforcing run, exit 0):
- Python: SAFE 1026, MODE-GATED 32, TREE 11 (1069).
- TS/JS: SAFE 259, MODE-GATED 36, TREE 18, DECLARED 5, COVERED 4 (322).
- bash: MODE-GATED 1.
- Floors: `FLOOR_PY = 855`, `FLOOR_TS = 257` (about 80%).
- Pragmas: 7 across 4 files (generate-cli-contract.ts 4, generate-subscription-schema.ts 1, p7-backlog.js 1 `mode --write-baseline`, wl_ressample.py 1).

Resolver shapes added in 2.2, each seen three or more times:
- A function body runs under the guard at its definition. Function guards are transitive, distributed over every reference, and group-aware: a function another module imports is not guarded by its module-local calls.
- One site per calling context of its innermost function, and TREE-dominant parameters (§3.4 step 2). This is what turns the selftest-plus-`--write-baseline` shape into TEMP plus MODE-GATED.
- Negative flag literals (`!--check`), judged per registered invocation.
- `argv.indexOf('--x') !== -1` tests.
- `self.X` and `obj.X` fields, class `__init__` parameter flow, and tuple unpacking from local returns.
- Callbacks passed as arguments, and lambdas returned from functions.
- `xs.append/push`, rows of `for (const [a, b] of [[...]])`, and dict and object path tables.
- `$HOME` is OUTSIDE. A helper handed a temp root returns a temp path.
- `if (<argv[1] is this file>) main()` runs only for the entry.
- An invoker-controlled ternary (`process.env.X ? ... : default`) is UNRESOLVED, not TREE.
- A bash `if [[ "$MODE" == "write" ]]` guard is traced back to `ARG_WRITE` (`--write`).
- A pragma counts only as a real comment. A `safe` pragma is stale only where its line is safe in every context and every entry.
- COVERED: an UNRESOLVED site inside an entry that already holds `tree:` plus `writesTree` is reported, not red.

2.3, the unverified prototype hits:
- Mode-gated, confirmed: python_types.py:814, check_job_timeout_headroom.py:228 (`--refresh`), check_runner_advice.py:754 (`--refresh`), check_secret_reachability.py:222 (`--refresh`), check_tree_shape.py:449 (`--write-baseline`), check_plan_folders.py:456-546 (`--update`/`--move`), check_actions_allowlist.py:106 (`--refresh`), check_agent_session_archival.py:436 (`--move`), prose_style.py:1299 and :1973 (`--write`).
- actionlint.py:301 is temp.
- check_plan_record.py:800 is a REAL write and becomes V8.

New V-rows. The source refactor of each is outside this plan's file set, so each carries `mutex: ['tree:repo']` plus `writesTree`:
- V6: check:ci-renet-types, `go build -o bin/renet` in private/renet.
- V7: check:ci-security-audit, relative report paths at the repo root.
- V8: check:ci-plan-record, the census append to the tracked agent/ledgers/census-plan-record.jsonl.
- V9: check:ci-search-index. It regenerates tracked public/ files in place and restores them in a `finally`, which is the lint-rule-liveness shape. The generator needs an output-dir seam.
- V10: check:test:tutorial-player, which writes artifacts/ in the repo.
- V11: check:ci-guard-mutations, with a sandbox under packages/cli.

V-row dispositions:
- V1: refactored. CONTROL 3 runs on a temp `git init` fixture carrying the real `.gitignore`, and the selftest plants a tracked-only enumerator and catches it.
- V2: refactored to `tsx -` on stdin with `cwd=packages/cli`. The gate output is byte-identical, the planted failure still reds, and the biome exclusion is gone.
- V3: `mutex` plus `writesTree`.
- V4: the KEY_USAGE_WWW_SRC seam plus a temp mirror. `www-src-probe` is dropped from check:i18n:key-usage and lint:unused.
- V5: no pragma. A pragma line changes the file's hash, and check:ci-breakpoint-drift then reds, so the bash guard is recognised instead and the site is MODE-GATED under `--write`.

4.4 evidence, 2026-09-24:
- The gate enforces by default, and `--report` is the only report-only path. `check:ci-gate-tree-writes` ran `ok 27.7s` inside the `npm run ci` pool: 349 gates, 319 ok, 30 failed.
- None of the 30 is caused by this plan.
- `lint:unused` was fixed by a knip entry for `scripts/lib/tree-write-sites.ts`, and `check:format` by the manifest line wrap.
- Two ledger regenerations remain, both outside this writer's permitted generators: `check:ci-plan-boxes` (`check_plan_boxes.py --update`) and `check:ci-plan-record` (`agent/INDEX.md`, via `--update`).
