...
Note, CORRECTED 2026-08-18: `lint:css` and `lint:css-files` no longer exist. They were
never wired into CI at all (no entry in `scripts/ci-runner/manifest.ts`, no workflow step,
and `check:ci-parity` keys off the `check:ci-*` prefix which they never carried), and they
were deleted as superseded by `check:ci-dead-css`. Measured before deleting them: the
unwired checker found ZERO real classes the wired gate misses, MISSED six it catches, and
INVENTED three it correctly rejects. Run `npm run check:ci-dead-css` after the rename
instead; it also now reports a stylesheet in which every class is dead, which is the
capability `lint:css-files` used to carry.
...
- ~~Whether `lint:css-files` tracks stylesheet filenames in a manifest that step 5's rename
  must also update.~~ ANSWERED 2026-08-18: the script is deleted. `check:ci-dead-css`
  tracks classes and whole sheets by path, so a rename shows up as its old entries going
  stale, which that gate reports because its baseline is shrink-only.
