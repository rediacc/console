...
once (`docs/agent-reference/ci-gates.md:54-60`). Within a lane, gates are serial steps.
...
(`docs/agent-reference/suppressions.md:27-32`), parsed with `parseBlockeredList` /
...
- `.ci/scripts/ci/scope-map.cjs:51` lists `.ci-chain-exempt` in
  ...
  classification.
- `docs/agent-reference/suppressions.md:21` (mechanism table) and `:77` (liveness oracle
  table).
...
- `docs/agent-reference/ci-gates.md` (runner usage, the new failure-output shape, the
  `--changed` caveat)
