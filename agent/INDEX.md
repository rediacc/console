...
A compacted plan keeps its own path, so every citation of it still resolves. Its full text lives in a git BLOB rather than in the file, because this repo merges with `gh pr merge --rebase` and a commit sha recorded on a branch is rewritten at merge -- measured 2026-09-06, 37 of 71 commit-shaped tokens already cited in plans no longer resolve. A blob id is content-addressed and
survives the rebase.
...
Every `agent/PLAN-*.md`, one row, so the SessionStart and PostCompact plans block can be printed from THIS file instead of opening all 96 of them. The hook checks freshness with `stat` alone (path set plus byte size) and falls back to reading the plans, loudly, when the two disagree.
