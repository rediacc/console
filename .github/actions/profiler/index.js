// Runner profiler: start a sampler in `main`, stop it and write the panel in `post`.
//
// WHY THIS IS A JAVASCRIPT ACTION. `runs.post` is JavaScript-actions-only -- composite actions have no equivalent -- and `post-if` defaults to always(), so the panel is still written when the job FAILS, which is exactly the run you want a profile of. There is no `background:` step keyword to lean on: it has been announced but is absent from the syntax reference and has no schema.
//
...
// Synchronous, fork-free pause. The post hook has to give a SIGTERM'd sampler a moment to leave, and there is no blocking sleep in node's stdlib.
function sleepMs(ms) {
  Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms);
}

// "Alive" means alive AND still the sampler. A bare process.kill(pid, 0) says only that SOMETHING holds that pid, and pids are recycled: a job that spawns a few thousand short-lived processes between main and post can hand our number to something else, and the post hook would then SIGTERM an unrelated process in the middle of the job it is supposed to be observing.
function alive(pid) {
  try {
    process.kill(pid, 0);
  } catch {
...
function runMain() {
  if (process.platform !== 'linux') {
    // The five non-Linux install jobs use neither this action nor slim, and can never move there (ubuntu-slim is x64 Linux only), so a cross-platform sampler would buy nothing but surface area.
...
// What the main phase recorded, with the defaults a run that never got there would leave behind.
...
// A sampler that is already gone is the failure worth naming: the panel must say so rather than render whatever partial file it left behind as if it were the whole job.
...
if (process.env.STATE_isPost === 'true') {
  ...
  // Saved BEFORE the work: an early return from runMain (unsupported platform, missing sampler) must still leave a post phase that can explain itself.
