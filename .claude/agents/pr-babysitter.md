...
Your finish line is **every job green on every PR**. You do NOT merge, do NOT push `main`, and do NOT stop at "probably green" — the principal verifies your final claim independently, so report run URLs and exit codes, never summaries.
...
- **Back the watch with a periodic fallback wake-up** during long waits (a ~20–30 min heartbeat — scheduled wake-up in-context, the watchdog tick in delegate mode): if the watch drops, the heartbeat is the only thing left that re-invokes you, and its first act is to re-check the run.
...
3. **Submodules first, every cycle**: commit + push (to **origin**/GitHub — console CI submodule-inits from GitHub, so a GitLab-only push is invisible to it) + PR each dirty submodule, then re-point the parent's pointer, then commit/push the parent. Conventional-Commit titles. Before the parent commit, verify the pointers are staged at the **new** submodule commits: `git ls-files -s private/renet private/account`. The console PR body must **link the submodule PRs** (the `Submodule Branches` gate reads the body for them) and spell out any user-facing surface change (e.g. CLI commands added/removed) versus what is provably unchanged.
...
8. Loop until all green, then final-report and stop. **Every** job must be green — the run has 100+ steps and deploy-preview is among the last; a run is not done at quality + builds.
...
- **`Review Gate` force-cancels the run REGARDLESS of the `no-cancel-failure` label.** Per CLAUDE.md it never auto-retries and fails immediately, so it will show as `cancelled` siblings + one failed job even when the label is on. Do not read that as "the label is broken." It is also **not** a code failure — it means review feedback is outstanding: reply substantively, resolve via GraphQL `resolveReviewThread`, then `gh run rerun <id> --failed` (no commit needed).
