...
- **The first commits drain the residue.** CLAUDE.md rule 1 commits verified work as it lands on the one branch, so the tree should hold little when the loop starts. Whatever is there is committed by path, one epic per commit with its `PR-TASK:` trailer (`git commit -F <msg> -- <paths>`), taking ALL of it - staged and unstaged - across as many commits as there are epics. The **snapshot**, one `git add -A -- .` per repo into a single commit, is the FALLBACK for residue that cannot be split by epic, not the default. Unstage + gitignore only clear accidental artifacts (binaries >5MB, build output, tsc-emit `.js` shadowing `.ts`) and flag them; never delete work. Leave `.claude/settings.local.json` uncommitted when it is local-permission noise.
- **ALL means all, and an ownership note does not carve files out of it.** In-context mode, every uncommitted change in the tree is the principal's, including work from earlier sessions and from before a compaction. A `RULES.md` "these files belong to session X, do not stage without asking" note is a *hygiene* convention for live parallel editing - it is **not** a licence to leave work behind at drain time, and treating it as one orphans exactly the changes nobody will come back for. (Real case, 2026-08-15: a babysitter excluded five finished CI-gate hardenings on the strength of such a note; the operator's correction was that every change in the tree, the pre-compact ones included, belonged to the babysitter.) If a claimed file genuinely looks like someone's live half-finished edit, ask the principal **once** and name the paths - but the default is INCLUDE, in its own labelled commit so it stays reviewable as a distinct thing.
...
- **After the drain, `git add -A` is banned.** Every fix commit is surgically staged: name the files, and before committing, diff the staged set against the round log's "files touched this round" list. Anything staged that was not touched this round is another party's work leaking in - unstage it and flag it to the principal. (History: a round-7 `git add -A` once swept an npm-pruned lockfile into a fix commit.)
...
the one that EXECUTES, so the fix has to be here and not only in the command docs. On a fresh branch, after the console drain commits: `git fetch origin && git rebase origin/main` (commit first - the tree is dirty). Conflicts: **never resolve lockfiles wholesale** - targeted resolution, then reconcile per the npm-10 gotcha below; regenerate search indexes (`cd packages/www &&
...
- **Worker contract**: a worker edits only its named file scope in the primary tree; it never commits, pushes, or touches git state; it reports back the exact files it touched. **Under focus mode the spawn declares its fix**: the fix is a worklist item of this session carrying the PR token (`--add <me> "<what> pr:<n>/fix"`), and the spawn's description or prompt names it with `focus-fix:#<id>`; `block_focus_spawn` refuses any other writer spawn while focus is on. The babysitter verifies by running the acceptance gate itself ("ask the thing that decides"), then stages surgically per the snapshot-boundary rule.
...
- **npm 11 at the exact `NPM_VERSION` pin is the only npm.** Before every commit in `private/account`, check `git status` for `package-lock.json`; rewrite it with `npx -y npm@<NPM_VERSION> install --package-lock-only --ignore-scripts` and confirm `npm run check:ci-lockfile` is green; validate lockfile changes with a REAL cold-cache `npm ci --ignore-scripts` in a clean-room copy - `--dry-run` does NOT run the reify peer check.
