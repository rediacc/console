...
- **worth** — yes / no, and one sentence of why
...
- an existing gate already covers the class — **name the real key**, do not guess
  one; a wrong key is worse than no answer because it reads as coverage
- the surface that could catch it does not exist and building it is a program,
  not a gate — say that plainly rather than proposing a weaker gate elsewhere
...
the work has an incentive to skip being checked, so you may overrule it — and
when you do, say which claim you are rejecting and why.
