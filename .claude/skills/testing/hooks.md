...
Next to the hook, named `test-*.py` or `test-*.sh`, and **called from `.claude/hooks/test-hooks.sh`**. That call is the wiring: without it the file is an orphan that reports its own green to whoever runs it by hand.
