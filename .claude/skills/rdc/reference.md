...
Provision the pool members of a cluster defined with 'rdc config cluster add', bootstrap renet on each, and install components (ceph first). Cloud providers provision via OpenTofu; KVM lands in a later wave.
...
- `--name <name>` — Show a single cluster
- `--output <format>` — Output format (json)
...
Install cluster components (ceph first, then k8s) on already-provisioned members.

**Options:**

- `--name <name>` — Cluster name

> agent: BLOCKED | MCP excluded: Installs cluster components — not an agent operation
...
- `--name <name>` — Resource name
- `-m, --machine <name>` — Target machine name
- `--cluster <name>` — Target Kubernetes cluster (mutually exclusive with -m); routes to the cluster's control node with KUBECONFIG injected
...
- `--to-cluster <name>` — Fork into another existing Kubernetes cluster (cross-cluster namespace fork)
- `--provider <name>` — Provision a new cluster (mirroring the source's pool shape) and fork into it
...
### rdc config cluster add

Declare a new cluster with one or more pools
...
commands.config.rotateCek.description

**Options:**

- `--api-url <url>` — commands.config.rotateCek.optionApiUrl
