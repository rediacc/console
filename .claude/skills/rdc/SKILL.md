...
- **Cluster/datastore verbs are blocked for agents**: `cluster create/destroy/scale/install/fork/migrate` and `datastore create/fork/...` need `REDIACC_ALLOW_CLUSTER_OPS=<cluster>`, a comma-separated list, or `*` (`*` is required for `cluster create` and datastore creation, which have no existing cluster yet). Note `repo create`/`repo migrate` sit behind `REDIACC_ALLOW_GRAND_REPO`, a SEPARATE variable, so infra work usually needs BOTH.
- **The overrides are ANCESTRY-VERIFIED**: they must be exported in the terminal that LAUNCHES the agent session. An export from inside the session (or handed to a sub-agent) is rejected by design; do not retry or work around it. When a task needs a blocked verb, fail early with a message asking the operator to export the variable(s) and restart the session, or to run the command themselves. `rdc run` stays blocked in agent mode regardless of any override.
