...
`trapguard` blocks `git filter-repo --message-callback` without a baseline. The recorded incident is exactly this shape: 93 commits rewritten, 96 trailers lost. Tag commits when you make them; a missed one is fixed by an amend on a commit you have not pushed, or by a follow-up commit, never by rewriting history.
