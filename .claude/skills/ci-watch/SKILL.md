...
`--wait` goes in a background task (`run_in_background: true`). It owns its polling interval (`--timeout` overrides the 5400s default), so you never write a loop; the process exit is the wake-up. Exits: **0** green, **1** red *or superseded*, **2** no verdict (in flight, no open PR, unreadable), **3** head moved by a push (`--json` for machine-readable output).
...
**`cancelled` is never a pass.** Cancelled *with* a failed job means the watchdog killed the run for that failure; with *zero* failures it was superseded, so trace the new head.
