...
repeating a rule already written down. `.claude/rediacc_hooks/guards/block_settled_questions.py`
(chain `pre-ask`) refuses such a question outright. If a decision genuinely belongs to the
...
unread code is a hypothesis. Do not trust a report that has not been
spot-checked, including a subagent's and this session's own from earlier.
Name the gates that ran and the ones that were skipped, and never call a failure
pre-existing without showing that none of its findings are in files this session
touched.

## 5. There is no safety net

The tree usually holds uncommitted work from other sessions. Never
`git checkout`, `restore`, `stash` or `clean` to undo a mistake of this session's
own making; repair forward instead.
