...
# (it matches U+00A0 and friends), which would widen every anchor in this file
# against a command containing non-breaking space -- a silent behaviour change
# in the direction of matching MORE, which for a guard is the safe direction
# but is still not what the bash does.
SPACE = r" \t\n\v\f\r"
# The same class with the newline dropped, for the places where the tool works
# on one record at a time and a newline can therefore never be in the subject.
# Kept separate rather than reused so that a future edit to one is not silently
...
def _wrapper_payload(text):
    ...
    for record in records:
        ...
        # default " " separator it does NOT trim: a leading blank yields an
        # empty tok[1], and that empty token is why the join below can emit a
        # leading space. Python's re.split on "" would give one empty field
        # where awk gives zero, hence the guard.
        tok = re.split(r"[ \t]+", record) if record != "" else []
        n = len(tok)
        for i in range(n):
            if tok[i] == "eval":
                out = "".join(tok[j] + (" " if j < n - 1 else "") for j in range(i + 1, n))
                # awk `print` then `exit`: one record, ORS appended, and the
                # whole input abandoned -- later records are never read.
                return out + "\n"
            name = tok[i]
            name = re.sub(r"[\"']", "", name)
            # awk's sub(/.*\//) is greedy and leftmost-longest, so this strips
            # through the LAST slash. Python's greedy `.*` lands in the same
            ...
            if re.search(r"^(sh|bash|dash|zsh|ash|ksh)$", name):
                for j in range(i + 1, n):
                    ...
                    if t == "-c" or re.search(r"^-[^-].*c$", t):
                        ...
                        return out + "\n"
                # NO `exit` HERE, and that is load-bearing: a shell name whose
                # token run carries no -c selector falls back to the OUTER
...
def _strip_heredocs(text):
    ...
    for record in records:
        if skip:
            # awk builds this pattern by CONCATENATION, so `marker` is a
            # regex, not a literal. The match above only ever captures
            # `[A-Za-z_][A-Za-z0-9_]*`, so it carries no metacharacters --
            # which is the only reason a dynamic regex is safe here.
...
def _strip_env_prefix(text):
    ...
    for record in records:
        ...
        # MULTILINE has the identical `^` rule, so the loop ports one-for-one.
        # It terminates because every substitution deletes a non-empty group 3.
        line = record
        while True:
...
#
# Returns (CMD, SCAN), or None when there is no command -- the caller's whole
# preamble in bash was `hook_init || exit 0`, and here it is
# `state = hook_init(payload)` followed by `if state is None: return`.
#
# THE EMPTY-COMMAND CONTRACT IS NOT WHAT IT LOOKS LIKE. `jq -r` prints the
# four characters `null` for a key that is absent or JSON null, so a payload
# with no `.tool_input.command` yields the STRING "null", passes the `-z` test
# and is scanned as if someone had typed `null`. Only a jq FAILURE (malformed
# JSON, so nothing on stdout) or a genuinely empty string command returns 1.
# That asymmetry is behaviour 25 guards depend on, so it is preserved rather
...
def _jq_raw_command(payload):
    ...
    if isinstance(doc, dict):
        ...
        # jq refuses to index a number, string or array with a key name and
        # exits 5 with nothing on stdout.
        return ""
...
def scan_target(cmd):
    ...
    # Extract the wrapper payload (see _wrapper_payload above), then turn
...
def gh_pr_at_command_pos(scan, verb):
    # `verb` is interpolated into the ERE by the original, so it is a PATTERN
    # and not a literal. Every caller passes a bare word (create, merge,
...
# gh_pr_segment <scan-target> <verb>
...
# target_repo <segment> <whole-scan> <cwd>
...
def target_repo(seg, scan, cwd):
    ...
    if repo == "" and cwd != "":
        # The pipeline's exit status is sed's, never git's, so a failed git
        # here is indistinguishable from a repo with no origin: both leave
        # `repo` empty and fall through to the default below.
...
def pr_selector(seg, verb):
    ...
    # segment naming the verb twice this cuts at the LAST one, and `-n ... p`
    # means non-matching lines are dropped rather than passed through.
    ...
    for record in _records(_sed_out(kept, terminated=True))[0]:
        # awk's default FS is space/tab/newline with leading and trailing runs
        # discarded -- NOT Python's str.split(), which also splits on \r and
        # on Unicode separators and would therefore field a `\r` differently.
        fields = [f for f in re.split(r"[ \t]+", record.strip(" \t")) if f != ""]
        for field in fields:
            if not re.search(r"^-", field):
                # awk `print` then `exit`: the FIRST such field on the FIRST
                # line that has one, and no further lines are read.
                return field + "\n"
    return ""


# target_root <whole-scan> <this-root>
# The FILESYSTEM git root a command operates on, or "" when it is this one.
#
# DIFFERENT QUESTION FROM target_repo ABOVE, which resolves a GitHub repo NAME for a
...
# target_repo's convention. Prints "" when the command targets THIS root, when no
...
def target_root(scan, this_root):
    ...
    if hint != "":
        # Five sed expressions, applied in order to the one line. Note the
        # FIRST one: the grep above accepts `-C<tab>path`, but this strip
        # requires a literal space after `-C`, so a tab-separated hint keeps
        # its `-C\t` prefix and fails to resolve. That is the bash's
        # behaviour, carried across deliberately -- a `re.sub` widened to
        # `[[:space:]]` here would be a silent behaviour change, and this
        # file's whole point is that behaviour changes come from findings and
        # not from tidying.
...
def _git_stdout(args, want_rc=False):
    ...
    try:
        ...
        # `git` absent behaves like `git` failing: bash sends the "command not
        # found" message to the 2>/dev/null the caller already wrote.
    ...
    # differ from what the bash `$( )` captured. surrogateescape because a
    # path is bytes, and refusing to decode one is not this function's job.
