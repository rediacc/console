...
WHY THIS FILE EXISTS AT ALL. W5's stated target is "2 processes per Bash tool call" and W5 P6 recorded "456 execs per Bash call to 35". Neither number can be reproduced from the tree: on 2026-09-09,

    grep -rlnP 'strace|GUARD_PASS|HOOKS_ONLY|FORK_COUNT' .ci .claude scripts

returns ZERO files, and `git ls-files | grep -iE 'strace|exec-baseline'` returns zero as well. The fork counter W5 P0 lists as a deliverable is not in the repository. A target measured against a baseline nobody can recompute is not a target, it is a slogan, and the whole W5 spine hangs off it. This module is the baseline, and `.ci/policy/hook-exec-baseline.json` is where its answer
is pinned so that a change to the wiring has to say so.

WHY NOT strace, WHICH IS THE OBVIOUS INSTRUMENT. Three reasons and any one is disqualifying: it does not exist on macOS, it needs ptrace capability that a container commonly withholds, and it measures a RUN rather than the wiring, so its answer depends on which guard happened to short-circuit that day. A baseline CI cannot reproduce on demand is the same class of problem as the
baseline that went missing. So this counts the wiring, statically, out of `.claude/settings.json`, and says exactly what it is counting.
...
An anchored matcher has no ambiguity, which makes the pinned set also a to-do list: it shrinks by anchoring the matcher, not by editing this file.

ANTI-VACUITY. `count_entries` on wiring it cannot see returns nothing, and every caller here treats that as a refusal rather than as a zero. An empty hooks object, an event with no entries, a probe set that matches no matcher: each is a distinct exception with its own sentence. The failure this guards against is the one that already happened once, which is a baseline artifact that
stopped being produced and was noticed a workstream later.
"""
...
def load_settings(path):
    ...
    Deliberately not tolerant. A settings file that does not parse is not a session with no hooks, it is a session whose hooks the harness also could not read, and answering "0 processes" for it would be the best-looking number this module can print.
    """
    p = pathlib.Path(path)
    try:
...
def parse_entries(settings):
    ...
    One triple per `hooks[].command`, which is one process. The shape is flattened here rather than at each call site so that the two counting functions below cannot disagree about what an entry is.
    """
...
def event_cost(entries, event):
    ...
    No matcher applies: `Stop`, `SessionStart` and the compaction events fire every entry they carry. Counting them the same way as a tool call would silently apply a `Bash` matcher to an event that has no tool.
    """
...
def ambiguous_tools(entries, tools):
    ...
    This is the only place the two readings are compared, and it returns the PAIR rather than a boolean so a caller can print what the disagreement is. A tool absent from the result is a tool where the semantics question does not arise, which is the state every row should eventually be in.
    """
    out = {}
    for tool in tools:
...
def unmatched_matchers(entries, tools):
    ...
    The completeness half of the baseline. A matcher nothing probes is a matcher whose cost the baseline does not carry, so adding one is invisible to a table that only lists tools. Wildcards are excluded because every tool selects them by construction and they can never be unmatched.
    """
    out = []
...
def measure(entries, tools):
    ...
    Returns a dict with `tools` (per tool, both readings), `events` (per non-tool event), `ambiguous`, `entryCount` and `patternCount`. Plain dicts and ints so it can be json.dumps'd into the baseline verbatim.
    """
    if not tools:
        raise WiringError(
...
def _literal_tools(matcher):
    ...
    A convenience for `python3 execcount.py` with no `--tools`, so the file is runnable by hand. It is NOT what the gate uses: the gate reads its probe set out of the baseline, because a set derived from the matchers can never notice a matcher that selects a tool nobody listed. Alternation and anchors are the only two shapes in this repository's settings.json today, and a matcher
    this cannot decompose contributes nothing rather than a wrong guess.
    """
    if matcher in WILDCARD_MATCHERS:
        return []
    body = matcher.strip("^$")
