...
# The fixtures spawn `sleep 8` and the case must be asked while it is alive. 0.3s is
...
def test_block_edit_of_running_script(tmp_path):
    ...
    try:
        ...
        # `running-script CONTROL: no file_path names nothing` lives in
        # hookcases.STATIC: its payload is the empty tool_input and its verdict does
        # not depend on this fixture, so asserting it here as well would put the same
        # label in the multiset twice.
        # THE SAME WILDCARD BUG AS THE BASH-SIDE TWIN, found by sweeping the class
        # rather than by being bitten a second time. Both guards built their pattern
        # by interpolating the basename RAW, so a one-letter name plus the shell
        # suffix gave `[x].sh` -- and `.` is a wildcard, so for `b` that matches
        # /bin/bash, every bash process alive.
        block.check(
            "check 0 guards/block_edit_of_running_script.py",
            path_json(tmp_path / "b.sh"),
            "running-script CONTROL: a one-letter name does not match /bin/bash",
        )
        # THE MENTION-VS-EXECUTION BYPASS, review-found on PR #579. A real incident
        # hit mid-session: the stop-hook judge's own `claude -p '<huge prompt>'`
        # process had this guard's OWN filename embedded in its prompt text, and the
        # guard scored that as "executing" it -- blocking an edit with no interpreter
        # anywhere near the file. Two fixtures reproduce both halves: a non-shell
        # process, and a shell process with the name buried in a late free-text
        # argument rather than at an invocation position.
        #
        # `exec -a` RATHER THAN A TRAILING ARGUMENT, and the difference decides
        # whether this control exists. `sleep 8 -- "prose..."` does not survive to the
        # settle window at all: GNU sleep validates every operand as a NUMBER before
        # sleeping, so a non-numeric trailing argument makes it exit immediately and
        # the control then passes VACUOUSLY. `exec -a` renames argv[0] without
        # touching the numeric operand, so the process stays alive for real with
        ...
        # what closes that pipe. Killing the child and leaving the descriptor to the
        # garbage collector raises an unraisable ResourceWarning that pytest turns into
        # a failure of this test, for a reason that has nothing to do with the guard.
...
def test_block_bash_write_to_running_script(tmp_path):
    ...
    try:
        ...
        # off, and one session produced twelve instances of exactly that class.
        block.check(
        ...
        # THE TARGET NAME ITSELF BECAME A WILDCARD, round six and a different
        # mechanism from the five before it. `pat` interpolated the basename RAW, so a
        # one-letter name plus the shell suffix produced `[x].sh` -- and `.` is a regex
        # wildcard, so for the letter `b` that matches /bin/bash, i.e. every bash
        # process alive. Measured 2026-09-01.
        block.check(
            "check 0 guards/block_bash_write_to_running_script.py",
            bash_json("echo x > %s/b.sh" % tmp_path),
            "bash-write CONTROL: a one-letter name does not match /bin/bash",
        )
        # ROUND FIVE OF "A MENTION IS NOT A TARGET", measured 2026-09-01. Writing a
        # plain markdown file was refused because its PROSE contained
        # `check:ci-hook-worklist-suite -> <a live>.sh`: the ASCII arrow scored as a
        # redirect and the path after it as the target. A real redirect's `>` follows
        # whitespace, start-of-string or a digit.
        block.check(
            "check 0 guards/block_bash_write_to_running_script.py",
            bash_json(
        ...
        # The guard header names `p.write_text(...)` as the idiom it exists for, and it
        # caught that spelling while `open(path, "w").write(...)`, the commoner one,
        # walked through its write-detector untouched. Verified missed 2026-08-27.
        block.check(
            "check 2 guards/block_bash_write_to_running_script.py",
            bash_json("python3 - <<PY\nopen('%s', 'w').write('x')\nPY" % fixture),
            "bash-write: open(path,w) in a heredoc is refused, not only write_text",
        )
        # Naming a live script in the CONTENT you write elsewhere is not writing to it.
        # The broad scan got this wrong, and a guard that blocks correct commands is a
        # guard people route around, which costs more than the block saves.
        block.check(
            "check 0 guards/block_bash_write_to_running_script.py",
        ...
        # THE SAME DISTINCTION ON THE HEREDOC PATH, which is a different branch: the
        # redirect above is caught by the precise grep, while a python heredoc falls to
        # the broad scan that takes every .sh token in the command. That branch had no
        # mention-control, so it refused three honest edits in a row on 2026-08-27.
        block.check(
            "check 0 guards/block_bash_write_to_running_script.py",
            bash_json(
                "python3 - <<PY\np = '%s/notes.py'\n"
        ...
        # THE THIRD ROUND OF THE SAME CLASS, 2026-08-28. Not inside a .write() argument
        # this time -- the mention text is ITSELF shaped like an assignment,
        ...
        # identifiable, the broad scan must still fire. Without this the fix would
        # trade a false positive for a silent miss, which is the worse of the two.
        block.check(
            "check 2 guards/block_bash_write_to_running_script.py",
            bash_json(
                "python3 - <<PY\nimport pathlib\n"
        ...
        # And the expensive one: `pgrep -af` matched a PEER SESSION whose long prompt
        # merely contained a filename, so the guard reported a script as executing when
        # no interpreter had it open. Only a SHELL running it counts.
        ...
        # matched a running `wslServer.sh`, and the guard reported VS Code's server as
        # the job about to be corrupted. The pattern now anchors to a path boundary.
        block.check(
            "check 0 guards/block_bash_write_to_running_script.py",
