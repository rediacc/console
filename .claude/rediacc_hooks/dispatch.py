...
WHAT `--chain` IS FOR, AND IT IS LIVE. `--chain` runs every guard of one chain
IN ONE PROCESS, in the ORDER those guards declare, stopping at the first
refusal. That is the fork saving this workstream is measured by, and since the
P7 cutover on 2026-09-06 it is what `.claude/settings.json` actually runs:
...
Positions were preserved rather than renumbered: require-jq.sh and
require-python.sh still lead every chain as their own commands, and the one
pre-bash guard that is still bash (block-pathspecless-git-commit.sh) sits AFTER
the dispatcher entry, so it still runs 40th. `ORDER` in each guard module is
that position, and `tests/test_dispatch.py` derives it from settings.json rather
...
def run_chain(chain, payload, cwd=None, env=None):
    ...
    A CRASHING GUARD DOES NOT TAKE THE CHAIN WITH IT, and that is the one place
    this deliberately does NOT let an exception propagate. Before P7 a guard that
    died mid-run exited non-zero, the harness showed its stderr and did not block,
    and the 20-odd guards registered behind it still ran as their own commands.
    In one process an uncaught exception would end the interpreter, so those 20
    would silently never run -- a bug in one guard quietly disarming the rest,
    which is a strictly larger blast radius than the arrangement it replaced. So a
    ...
    Returns (rc, stdout, stderr). rc is 2 if any guard REFUSED (a refusal always
    wins, since that is a decision and a crash is not), 1 if any guard crashed and
    none refused, 0 otherwise.
    """
