...
# NOT stripped when the heredoc feeds a shell (`| bash`, `| sh`, `bash <<`), because there the body IS the command and stripping it would open a bypass. Remove this test and `bash <<'EOF' ... wl_wait.py & ... EOF` -- a real command, not a document -- walks straight through.
DEFECT = ("if not hookio.grep_q(FEEDS_SHELL, cmd):", "if False:")

FEEDS_SHELL = hookio.rx(r"\|[{S}]*(bash|sh|zsh)\b|\b(bash|sh|zsh)[{S}]+<<")

# The heredoc opener awk looks for, and the prefix/suffix `gsub` strips off it to recover the delimiter NAME.
...
EDGE_CASES = [
    ...
    # `&&` never matches: the regex requires a NON-& character or end-of-line after the single `&`.
    ...
    # The quote stripper, and the fd-redirect stripper that was caught as a live false positive during this hook's own proving run.
    ("prose quoting the rule", "echo 'never launch wl_wait.py & like this'"),
    ("a redirect is not a background", _LAUNCH + " 2>&1"),
    # Measured 2026-09-01: a session writing its recovery document with `worklist.py --state <<'EOF'` was blocked because the DOCUMENT contained the sentence "never with a shell `&`".
    (
...
def run(ev):
    ...
    # The three `-e` expressions are one sed invocation in the original, wrapped in `$( )`. Applying them one after another over the whole text is the same thing because each is line-local, and the substitution's newline-stripping is spelled out rather than left to sed's own missing-newline handling.
