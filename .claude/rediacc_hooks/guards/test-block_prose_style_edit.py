...
the negative ones. A prose linter is the worst case for a one-sided suite,
because "flag everything" and "flag the right things" look identical from the
block side alone.
...
to judge it against. That test now REQUIRES a `test-<stem>.py` beside any guard
carrying the sentinel, precisely so the sentinel cannot become the cheap way out
of having evidence. `check-hook-integrity.sh` reads this file's existence too,
crediting the guard with both directions under section B.

IT DRIVES THE LIVE GUARD THROUGH THE DISPATCHER, not the module's `run` directly
and not a copy: `python3 dispatch.py block_prose_style_edit` is what
`.claude/settings.json` ultimately invokes, and a suite that tested anything else
would keep passing while the thing that actually runs went unchecked.

THE PATHS BELOW ARE RELATIVE AND DERIVED FROM THIS FILE. An absolute path under
one operator's checkout is the exact failure CLAUDE.md's "Worktree Warning"
names: in another worktree it would silently test a stale copy, or nothing.
"""
