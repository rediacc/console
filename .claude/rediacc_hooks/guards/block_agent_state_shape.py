...
WHY THIS EXISTS. The old compact-recovery handover lived at a TMPDIR path nobody would ever open with Write, so a CLI-side refusal was a complete gate. STATE.md lives at a normal repo path, and any agent reaches for Write first: a CLI-only refusal is bypassed by the most natural tool in the box, and the session bypassing it would not know it had.

WHY THE SHAPE CHECK LEFT THIS FILE (2026-08-09). This guard used to DENY Edit and merely shape-check Write: 250-4000 chars and a '## Next action' section, the same rule the Stop check reads with. That was the right gate for a document whose contract was "rewrite the whole thing every time". The contract is now one OWNED SECTION PER SESSION, merged in place, because
rewrite-every-time met three live sessions in one checkout and a session obeying a staleness nag destroyed a peer's entire state document.
...
WHY A REGEX AND NOT A `case` GLOB (2026-08-14, when the tree moved from .agent/<branch>/ to a per-session directory). `case` globs are not path-segment aware -- `*` happily eats `/` -- so `*/agent/*/STATE.md` does match the deeper path, and would ALSO match anything anywhere whose path merely contains a directory called `agent`. That breadth mattered little
while the leading dot made `.agent/` unmistakable; without it, `agent` is an
ordinary word this repo uses in `docs/agent/`, `.claude/agents/` and agent source trees. This hook FAILS OPEN by design (the `exit 0` below), so an over-broad pattern is not a loud mistake, it is a guard that blocks writes nobody was making while looking exactly as green as a correct one.
...
`regexec` over the WHOLE variable, with no record splitting and without REG_NEWLINE, so `^` and `$` mean start and end of the STRING and nothing
else. Python's `$` additionally matches just before a trailing newline, and
Python's `^` would too under `re.MULTILINE`. A path is not supposed to carry a newline, but a hook payload is arbitrary JSON and the degenerate cases in the differential feed exactly that kind of thing, so the anchors are written `\\A` and `\\Z` rather than `^` and `$` and the test is `re.search` rather than either of the two `grep` helpers, which split on newlines by construction.
"""
