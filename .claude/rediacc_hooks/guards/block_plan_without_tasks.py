...
PORT NOTE ON KEEPING THE SUBPROCESS. The obvious port of the embedded
`python3 -c` program is an in-process import of wl_planfid, and it is the wrong
move twice over. First, `.claude/hooks/stop` is deliberately NOT an importable
package (pyproject.toml's INP001 note says so in as many words), so an import
means a `sys.path` mutation inside a guard that a chained dispatcher runs
alongside 37 others. Second, the fail-open contract above is written in terms
of a SUBPROCESS: "a missing python3, a missing module, or unreadable input" all
arrive as an empty stdout from a program that exited 0, and reproducing that
with try/except around an import is a re-derivation of exactly the thing the
header says must never be re-derived. So the same program text is run the same
way, and `PROBE` below is that text, character for character.
...
third behaviour: a non-numeric value makes `test` itself fail, and the `||`
turns that into an allow. All three are spelled out below rather than collapsed
into an `int()` that would raise.
"""
