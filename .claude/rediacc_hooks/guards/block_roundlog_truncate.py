...
EDGE_CASES = [
    ...
    # The cp finding: the log as a SOURCE is a pure read, and backing it up is
    # the single most useful thing a session can do with it.
...
def run(ev):
    ...
    if hookio.grep_q_line(
        ...
        for match in hookio.grep_o(
            ...
            candidates.extend(hookio.grep_o(r"[A-Za-z0-9_./-]+\.md", match))
        # `| sort -u`: sorted AND deduplicated, then joined by newlines, and the
        # command substitution around it drops the trailing newline.
