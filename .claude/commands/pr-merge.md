...
- Arm the canonical watch in `.claude/skills/ci-watch/SKILL.md` (run_in_background: true; do NOT use `gh run watch`) on that run. Do not paste a watch loop from memory here: a loop that waits only for `status == completed` fires on a superseded attempt when the watchdog auto-retries, so the skill's form requires the same `run_attempt` twice. **One wait per command**. Never chain "then wait for the review marker" onto the same watch, because the notification fires on process exit and a red run posts no review, so the CI verdict would never wake you (observed 2026-08-24, ninety minutes lost).
...
- Find the **Console CI** run for the merged commit: `gh run list --repo rediacc/console --branch main --workflow "Console CI" --limit 3` (event `push`, matching the merged SHA), then arm the canonical watch in `.claude/skills/ci-watch/SKILL.md` (run_in_background: true; do NOT use `gh run watch`). On `main` the watchdog auto-retries transient failures, so a watch that exits on the first `completed` hands you a superseded attempt's verdict.
