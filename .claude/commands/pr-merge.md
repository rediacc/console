...
- Arm the standard terminal-state watch on that run per `.claude/skills/ci-watch/SKILL.md` (run_in_background: true; do NOT use `gh run watch`): `R=<run-id>; until [ "$(gh run view $R --repo rediacc/console --json status --jq .status)" = "completed" ]; do sleep 20; done; gh run view $R --repo rediacc/console --json conclusion,jobs`. **One wait per command** — never chain "then wait for the review marker" onto the same watch, because the notification fires on process exit and a red run posts no review, so the CI verdict would never wake you (observed 2026-08-24, ninety minutes lost).
