...
- Find and background-watch the **Console CI** run for the merged commit: `gh run list --repo rediacc/console --branch main --workflow "Console CI" --limit 3` (event `push`, matching the merged SHA), then `gh run watch <id> --repo rediacc/console --exit-status --interval 100` (run_in_background: true; the harness notifies on completion).
