...
WHY IT IS SEPARATE FROM wl_resprofile, and why it imports so little. Importing that module costs +33 ms at interpreter start because it pulls in json, pathlib and
tempfile. That is fine for a hook process, which imports them anyway; it is not fine
on EVERY python3, where it roughly doubles startup. So this file carries its own minimal recorder: os, sys, time, resource, atexit, and `%`-formatted JSON. The record is byte-compatible with wl_resprofile's `v:1` shape, deliberately, so one reader serves both.
...
sys.path, so ours wins and apport would silently vanish. Its five lines are reproduced verbatim at the end.
...
own `install()` is idempotent by `atexit` but would double-count, so it checks the same marker.
"""
