...
def render(root, rows, last_green):
    ...
    # that way, which is the half a `git log` cannot show. Appended here rather
    # than at the wl_checks call site because the suspect paths only exist here.
    #
    # DEGRADES TO SILENCE, matching this module's stated fail semantics: it can
    # only ADD to a block already being emitted, so losing it loses a hint and can
    # never grant an exit that was otherwise refused.
    try:
