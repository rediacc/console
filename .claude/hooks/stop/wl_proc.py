...
WHY THE HOOKS NEEDED ONE. Eight call sites across four `wl_*` modules launch a command
that FORKS -- `npm run <gate>`, `npx tsx`, and six `claude -p` invocations -- and every
one used `subprocess.run(capture_output=True, timeout=N)`. That combination bounds
nothing when the child has children: on timeout `run` kills the direct child and then
blocks in `communicate()` waiting for pipe write ends a GRANDCHILD still holds.
`.ci/rediacc_ci/proc.py` fixes that by putting the child in its own session and
signalling the whole group, and it is already the tree's single implementation of that
...
THE FAILURE IS DEFERRED TO THE CALL, NOT RAISED AT IMPORT, and that is not politeness --
it is a measured requirement. `test-worklist-v5.sh` case 222j copies the hook directory
somewhere WITHOUT `.ci` beside it and asserts the hook still reports exactly the one
module it planted a crash in. An import-time raise made three more modules unimportable
there, `worklist.py` refused with "3 sibling module(s) unusable", and the case went red.
That red is the real behaviour in miniature: a tree without `.ci` would have lost the
WHOLE Stop hook, every check, over a runner that most stops never reach.

So the import always succeeds, and `run()` refuses -- loudly, naming what is missing --
only if something actually tries to launch a forking child without the runner present.
Everything else the hook does keeps working.
"""
...
def run(argv, **kwargs):
    ...
    A refusal rather than a silent fall back to `subprocess.run`: falling back would
    reinstate the unbounded call this module exists to remove, in the one situation
    nobody is watching, and it would do it invisibly.
    """
    if _run is None:
        raise RuntimeError(
