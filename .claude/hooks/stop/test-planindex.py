...
# wl_planindex restates INDEX_REL rather than importing wl_planrec, to keep the
# hook path free of the record machinery's git dependency. A restated constant is
# only safe if something asserts the restatement, which is this.
...
# ANTI-VACUITY: an empty census must render "" and NOT a table with no rows, or
# `render_index(rows) + census` would grow a generated section that says nothing
# and R8's zero-record controls would start failing for an unrelated reason.
...
# THE CONTROL FOR 3a: regenerating clears it. Without this the STALE verdict
# could be a permanent state the fresh path can never reach, which reports
# "always broken" just as loudly as it reports a real drift.
t.write_index()
eq(
...
# 3c. AN EDIT THAT CHANGES THE BYTE COUNT. This is the signal the hook actually
# has: it cannot hash the plans without reading them, which is the cost it
# exists to avoid, so size is what it compares. The blind spot -- a same-length
# edit -- is asserted below rather than hidden, because a documented gap that a
# CI gate closes is honest and an undocumented one is a trap.
...
# 3d. THE NAMED BLIND SPOT, asserted so it cannot quietly widen. A same-length
# edit is invisible to `stat` -- and `Status: draft` -> `Status: ready` is exactly
# that, which is why the gap is worth an assertion rather than a footnote. R8's
# byte-equality in check_plan_record.py is what catches it. If this control ever
# starts FAILING, the hook grew a stronger signal and this comment is the thing
...
# AND THE CONSEQUENCE, stated as an assertion rather than left to the reader: the
# listing keeps printing the OLD status until the index is regenerated. It is a
# stale field, never a short or empty list, which is the distinction that makes
# the gap tolerable at all.
...
# An index that exists but carries only the record table is ABSENT for this
# reader's purposes, and must say ABSENT rather than parse to zero rows and be
# mistaken for "there are no plans".
...
# section that is present but yields zero rows must never read as "there are no
# plans", because the directory says otherwise. It is STALE: every plan on disk
# is missing from the census, which is exactly what the diff says.
...
# THE FAILURE THIS SUITE EXISTS FOR. A hook that printed a short or empty list
# because its index was missing would be worse than the cost the index saves.
#
# The baseline is RE-TAKEN here rather than reused from section 2: section 3
# perturbed the fixture on purpose, so section 2's listing describes a tree that
# no longer exists. Comparing against it would fail for a reason that has nothing
# to do with the fallback, which is a test that cries wolf about its own setup.
# Section 4 left a junk index behind on purpose, so regenerate before baselining.
t.write_index()
fresh_listing, fresh_live = t.block()
eq(
...
# plan_status_excerpt takes live[0] as "the newest live plan", so an index that
# lost mtime order would silently change which plan a compacted session is handed.
...
# The box arithmetic is what `plan_box_census` used to own, and the two must not
# drift: a plan with NO boxes contributes no row to the totals.
arith = Tree(
    [
...
# ANTI-VACUITY. A suite that plants nothing and asserts nothing exits 0 too, so
# the floor is asserted rather than assumed: if a future edit breaks the fixture
# setup, this fails loudly instead of reporting a clean run over no work.
if Tally.count < 40:
