...
mig_peer() { # mig_peer <prefix> <text> [minutes-old]
    ...
    WORKLIST_SESSION_ID="$pfx" python3 "$HOOK" --add "$pfx" "$text" >/dev/null 2>&1
...
mig() { WORKLIST_SESSION_ID=deadbeef python3 "$HOOK" --migrate deadbeef "$@" 2>&1; }

echo "== 190. the store is a DIRECTORY of per-writer files, not one file =="
setup
say "done for now"
brief_now
python3 "$HOOK" --add deadbeef "an item of my own" >/dev/null 2>&1
...
WORKLIST_SESSION_ID=cafe1234 python3 "$HOOK" --add cafe1234 "a peer item" >/dev/null 2>&1
...
PYEOF
if python3 "$HOOK" --list 2>/dev/null | grep -q '\[x\].*ordered by time'; then
...
if python3 "$HOOK" --list --open deadbeef 2>/dev/null | grep -q 'the stranded work'; then
...
if python3 "$HOOK" --list 2>/dev/null | grep -q 'migrated to #'; then
...
ITEM="$(python3 "$HOOK" --list 2>/dev/null | grep -o '#[0-9a-f]\{8,\}' | head -1 | tr -d '#')"
WORKLIST_SESSION_ID=leasesess python3 "$HOOK" --lease leasesess "$ITEM" +60 worker:ghost1 "held" >/dev/null 2>&1
mig leasesess >/dev/null
if python3 "$HOOK" --list --open deadbeef 2>/dev/null | grep -q '\[ \].*leased to a worker'; then
...
ITEM="$(python3 "$HOOK" --list 2>/dev/null | grep -o '#[0-9a-f]\{8,\}' | head -1 | tr -d '#')"
WORKLIST_SESSION_ID=defersess python3 "$HOOK" --defer defersess "$ITEM" \
...
if python3 "$HOOK" --list --open deadbeef 2>/dev/null | grep -q '\[?\]'; then
