...
#   2. --auto is always allowed: GitHub itself defers the merge until the
#      required check (CI Complete) is green.
#   3. An immediate merge (no --auto) must prove green + zero unresolved
#      review threads NOW. Console gets both checks; other rediacc repos get
#      the thread check (their thread state feeds console's Submodule Branches
#      gate). Network paths are NOT covered by test-hooks.sh; verification
...
echo "$STRIPPED" | grep -qE -- '--auto([[:space:]]|$)' && exit 0
