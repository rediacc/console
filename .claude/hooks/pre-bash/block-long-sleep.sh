...
if [[ -n "$SLEEP_VAL" && "$SLEEP_VAL" -gt "$LIMIT" ]]; then
    if [ "$BG" = "true" ]; then
        ...
        echo "❌ BLOCKED: Do not use sleep > ${FG_MAX}s in the foreground -- it stalls the session, and it is half of the sleep+gh-run-view polling pattern. To wait on CI, arm the canonical terminal-state watch from .claude/skills/ci-watch/SKILL.md with run_in_background:true, where sleeps up to ${BG_MAX}s are allowed. That form waits for the same run_attempt to be complete TWICE, because the CI watchdog auto-retries transient failures and attempt 2 lands on the SAME Console CI run -- always watch the Console CI run, not the Watchdog Monitor chain runs. Do NOT rely on gh run watch: it has dropped silently on terminal runs (observed 4/4); a process exit on terminal state is the reliable notification. Also: a PostToolUse hook auto-cancels old CI runs on every git push, so you never need to cancel manually." >&2
    fi
    exit 2
fi
exit 0
