...
echo "$CMD" | grep -qE '[^>&2]>[[:space:]]*[^>|&[:space:]]*pr-babysit-[A-Za-z0-9._-]+\.md' && TRUNCATING=1
echo "$CMD" | grep -qE '(sed[[:space:]]+-i|truncate|dd[[:space:]]+.*of=|(^|[[:space:];|&])(cp|mv)[[:space:]])' && TRUNCATING=1
echo "$CMD" | grep -qE 'write_text|open\([^)]*["'\'']w' && TRUNCATING=1
echo "$CMD" | grep -qE '(^|[[:space:];|&])tee([[:space:]]+[^-|][^|]*)?($|[[:space:]])' | grep -qv '\-a' && TRUNCATING=1
