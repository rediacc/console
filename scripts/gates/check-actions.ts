 ...
 *
 * Usage:
 *   npx tsx scripts/gates/check-actions.ts           # Check for outdated actions
 *   npx tsx scripts/gates/check-actions.ts --verbose # Show all actions
 *   npx tsx scripts/gates/check-actions.ts --help    # Show help
 *
 * Exit codes:
 ...
 * emit: false
 * blocker: BLOCKER: its `run:` goes through the `.ci/scripts/quality/run-external-gate.sh` WRAPPER, which the derived run does not produce, and its `env:` block carries PROSE comments the `env-<KEY>:` grammar cannot hold.
...
${YELLOW}USAGE${NC}
  npx tsx scripts/gates/check-actions.ts [OPTIONS]
