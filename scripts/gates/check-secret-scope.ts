...
/**
 * A `#` COMMENT IS NOT A READ, and scanning raw text made the two indistinguishable.
 *
 * This gate exists to find `secrets.X` expressions that resolve to the empty string. A
 * comment EXPLAINING that a particular `secrets.X` is dead, or recording why it was
 * removed, is the opposite of a read -- and under a raw-text scan it reported as one. Found
 * 2026-09-09 while removing exactly such a dead reference: the removal comment tripped the
 * gate the removal was satisfying, so the fix looked like the defect.
 *
 * The pressure that creates is the dangerous part: the cheapest way to green is to stop
 * writing the explanation down, which is how a repository loses the reason a credential
 * was retired. `check:ci-env-file-adoption` and `block_host_toolchain_run` both carry the
 * same filter for the same reason.
 *
 * YAML has one comment form and no block form, so a `#` that opens a line is the whole
 * rule. A trailing `#` after code is deliberately NOT treated as a comment start: it can
 * sit inside a `${{ }}` string, and over-scanning is the safe direction here.
 */
...
export function scan(dir: string): string[] {
  ...
  for (const f of readdirSync(dir).filter((n) => n.endsWith('.yml') || n.endsWith('.yaml'))) {
    const code = readFileSync(join(dir, f), 'utf8')
      .split('\n')
      .filter((line) => !YAML_COMMENT_RE.test(line))
      .join('\n');
    for (const m of code.matchAll(SECRET_RE)) {
