 ...
 *   - glob discovery: .ci/rediacc_ci/battery.py globs DEFAULT_PATTERN = "test-*.sh"
 ...
 * scripts/gates/check-suppression-liveness.ts so it cannot rot.
 *
 * Usage:
 *   npx tsx scripts/gates/check-dead-bash.ts
 *   npx tsx scripts/gates/check-dead-bash.ts --json
 *
 * Env:
