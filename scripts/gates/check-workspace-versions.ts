 ...
 *
 * Usage:
 *   npx tsx scripts/gates/check-workspace-versions.ts
 *
 * Exit codes:
 ...
 * needs: node
 * id: check:version
 * run: tsx scripts/gates/check-workspace-versions.ts && syncpack lint
 * ---- end gate ----
 */
...
// Must list EVERY workspace in the root package.json. This is a hand-maintained list, so a new package is invisible to the gate until it is added here — `packages/provisioning` had been missing since it was created, which is exactly the drift this gate exists to catch, so it is added alongside `packages/locales` rather than left for later.
