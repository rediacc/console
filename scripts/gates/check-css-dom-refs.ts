 ...
 *
 * Usage:
 *   npx tsx scripts/gates/check-css-dom-refs.ts [--write-baseline] [--selftest]
 *
 * ---- gate ----
...
function main(): void {
  ...
  if (argv.includes('--write-baseline')) {
    ...
    writeFileSync(
      BASELINE,
      `${JSON.stringify({ note: 'shrink-only; see scripts/gates/check-css-dom-refs.ts', orphans }, null, 2)}\n`
    );
    console.log(
  ...
  if (fixed.length > 0) {
    ...
    console.error('    npx tsx scripts/gates/check-css-dom-refs.ts --write-baseline');
    process.exit(1);
  }
