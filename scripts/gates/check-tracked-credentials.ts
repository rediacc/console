 ...
 * shape -- scripts/gates/check-env-credential-drift.ts -- and only as TEST FIXTURES; it
...
function selftest(): number {
  ...
  check(
    ...
    !scan(ROOT).some((id) => id.startsWith('scripts/gates/check-tracked-credentials.ts:'))
  );
...
function main(): number {
  ...
  if (drained.length > 0) {
    ...
    console.error('  Run: npx tsx scripts/gates/check-tracked-credentials.ts --write-baseline');
    return 1;
  }
  console.log(
