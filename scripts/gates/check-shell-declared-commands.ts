 ...
 * scripts/gates/check-em-dash-surfaces.ts. 55 findings existed when this was written;
...
member with python3, which was already declared.`);
  ...
  if (stale.length) {
    ...
    return refused(
      '\n  Regenerate with: npx tsx scripts/gates/check-shell-declared-commands.ts --write-baseline'
    );
  }
