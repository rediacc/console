 ...
 * list is EMPTY (scripts/gen/gen-docs.ts:571-576). Put those two facts together and a document
 ...
 * that scripts/gen/gen-docs.ts `render()` emits, rather than importing it, for one blunt reason:
 * gen-docs.ts calls `process.exit(main(...))` at module scope with no import guard
 * (scripts/gen/gen-docs.ts:607), so importing anything from it runs the whole generator inside
 ...
 * determinism as a requirement rather than a nicety (scripts/gen/gen-docs.ts:45-50). So a region
...
/** Same escaping as scripts/gen/gen-docs.ts: `|` ends a cell, a newline ends the row. */
const cell = (s: string): string => s.replace(/\|/g, '\\|').replace(/[\r\n]+/g, ' ');

/**
 * The body scripts/gen/gen-docs.ts `render()` writes between the markers.
 *
...
export function renderBody(p: Provider, rows: readonly ProviderRow[]): string[] {
  ...
  out.push('');
  out.push(
    `${rows.length} row(s). Generated by \`npx tsx scripts/gen/gen-docs.ts --write\`; do not hand-edit.`
  );
  out.push('');
  return out;
}
 ...
 * (scripts/gen/gen-docs.ts:142-154). A quotation has text around it, so requiring the line to be
 ...
 * is the artifact the whole gen-docs phase exists to abolish, and scripts/gen/gen-gates-lock.ts
 ...
 *
 * Mirrors scripts/gen/gen-docs.ts:119-131 including the untracked leg: this repo's standing
  ...
  for (const rel of markdownFiles(root)) {
    ...
    if (st.size > MAX_TARGET_BYTES) {
      findings.push({
        ...
        detail: [
          'scripts/gen/gen-docs.ts:141 skips files this large, so its regions are never',
...
function selftest(): number {
  ...
  // --- the emitted SHAPE, pinned line by line. This is what makes the second --- implementation honest: if scripts/gen/gen-docs.ts render() changes, this fails HERE, --- rather than surfacing as every region in the tree drifting at once.
  ck(
    ...
    body[0] === '' &&
      ...
      body[body.length - 3] === '' &&
      body[body.length - 2] ===
        '3 row(s). Generated by `npx tsx scripts/gen/gen-docs.ts --write`; do not hand-edit.' &&
      body[body.length - 1] === '',
    body
  );
  ck(
  ...
  // for this one already (scripts/gen/gen-docs.ts:142-154); both predicates have to agree.
  ck(
...
function main(argv: string[]): number {
  ...
  // Controls before the verdict, same order and same reason as scripts/gen/gen-gates-lock.ts: a report from an instrument that cannot fail is worse than no report.
  if (selftest() !== 0) {
    return refused(
  ...
  for (const { file, region, delta } of a.regions) {
    ...
    fail.push(
      `${at}  region \`${region.provider}\` does not match what gen-docs would emit\n` +
        lines.join('\n') +
        '\n    Fix: npx tsx scripts/gen/gen-docs.ts --write, in the SAME change as the edit that' +
...
// THE IMPORT GUARD. This module exports its helpers so a control can drive them directly. Without the guard, importing one would run the whole gate inside the importer and exit, which is what scripts/gen/gen-docs.ts:607 does today and why this file cannot import it.
