 ...
 * emit: false
 * blocker: BLOCKER: this step has NO `if:` AT ALL -- it runs unconditionally -- so emitting it would ADD the standard guard and change WHEN the gate runs, which is a behaviour change rather than a retirement.
...
/**
 * Epic ids as the LEDGER states them, `agent/worklist/epics.jsonl`.
 *
 * W12 P3.1b. `knownEpicIds` above reads the PUBLISHED SNAPSHOT, which is a copy
 * rendered by `worklist.py --publish`, and a copy drifts: the snapshot is a
 * tracked markdown file a session can hand-edit, and it is only as current as
 * the last `--publish`. The ledger is the source `wl_epic.record_epic` appends
 * to. Both are read below and compared in BOTH directions, because each
 * disagreement means something different and the gate should say which.
 *
 * TOLERANT OF A TORN LINE, deliberately and for the same reason
 * `wl_epic.load_epics` is: the file is appended to under a lock by several
 * sessions, and a crash mid-append must not make every epic unreadable. A line
 * that does not parse is SKIPPED. It is the ZERO case that is fatal, and that is
 * handled by the caller rather than here.
 */
export const ledgerEpicIds = (jsonl: string): string[] => {
  const out: string[] = [];
  for (const line of jsonl.split('\n')) {
    ...
    if (typeof id !== 'string' || !/^[0-9a-f]{6,32}$/.test(id)) continue;
    if (!out.includes(id)) out.push(id);
  }
  return out;
};

/** Set difference in both directions, which is the whole acceptance criterion. */
export const bothWays = (
  ledger: string[],
  snapshot: string[]
): { ledgerOnly: string[]; snapshotOnly: string[] } => ({
  ledgerOnly: ledger.filter((i) => !snapshot.includes(i)),
  snapshotOnly: snapshot.filter((i) => !ledger.includes(i)),
});
...
const selftest = (): number => {
  ...
  check(
    'CONTROL: a ledger of only torn lines yields no ids',
  ...
  check(
    ...
    agree.ledgerOnly.length === 0 && agree.snapshotOnly.length === 0
  );
  check(
    'a ledger-only id is reported as a stale snapshot',
    bothWays(['a1b2c3'], []).ledgerOnly.join(',') === 'a1b2c3'
  );
  check(
    'a snapshot-only id is reported as a document with no event behind it',
    bothWays([], ['a1b2c3']).snapshotOnly.join(',') === 'a1b2c3'
  );
  check(
    'both directions are reported at once, not just the first',
    (() => {
      const d = bothWays(['a1b2c3'], ['d4e5f6']);
      return d.ledgerOnly.join(',') === 'a1b2c3' && d.snapshotOnly.join(',') === 'd4e5f6';
    })()
  );
  ...
  check(
    `the tracked ledger ${fs.existsSync(realLedger) ? 'parses to' : 'is ABSENT, so this reads'} ` +
      `${fs.existsSync(realLedger) ? ledgerEpicIds(fs.readFileSync(realLedger, 'utf8')).length : 0} epic(s), which must be non-zero`,
...
const main = (): number => {
  ...
  if (!fs.existsSync(ledgerPath)) {
    console.error(`✗ no epic ledger at ${ledgerRel}.`);
    console.error('  The valid-id set comes from the ledger, not from the published');
    console.error('  snapshot, so without it this gate has nothing to judge against and');
    console.error('  its green would mean nothing.');
    console.error('  Run: worklist.py --epic <me> new <title>');
    return 1;
  }
  const ledgerIds = ledgerEpicIds(fs.readFileSync(ledgerPath, 'utf8'));
  if (ledgerIds.length === 0) {
    console.error(`✗ ${ledgerRel} declares no epic, so no trailer could ever be valid.`);
    console.error('  An empty or unparseable ledger is UNCHECKED, never clean.');
    return 1;
  }
  ...
  if (bad.length > 0) {
    ...
    for (const v of bad) {
      const why =
        v.problem === 'untagged'
          ? 'no PR-TASK trailer'
          : `PR-TASK: ${v.id} names no epic in the ledger`;
