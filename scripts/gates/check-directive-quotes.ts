 ...
 *
 * Run: tsx scripts/gates/check-directive-quotes.ts
 *
 * The vendored snapshot at scripts/data/nis2-directive-2022-2555-en.txt is the
 * source of truth; regenerate via scripts/gen/fetch-directive-snapshot.ts when the
...
function loadAllSnapshots(): Map<SnapshotLang, LoadedSnapshot> {
  ...
  if (out.size === 0) {
    ...
    console.error(`  Run: tsx scripts/gen/fetch-directive-snapshot.ts --all`);
    process.exit(2);
  }
  if (missing.length > 0) {
    console.warn(
