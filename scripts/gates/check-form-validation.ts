 ...
 *
 * Usage:
 *   tsx scripts/gates/check-form-validation.ts [--root <dir>] [--selftest]
 *
 * ---- gate ----
 * kind: test
 * needs: node
 * test: .ci/rediacc_ci/tests/gates/test_gate_form_validation.py
 * blocker: BLOCKER: no quality lane inspects form submit handlers, and the defect is a MISSING guard rather than a present one, so nothing else can express it; test_gate_form_validation.py:100 runs the gate seam-free against the real components under check:ci-pytest, so the real scan of all six forms runs every CI run, and the mutant case beside it requires the control to go red
 * ---- end gate ----
 */
...
const h = (e) => { e.preventDefault(); const email = r.current?.value; if (!email) { setErrorMsg('x'); return; } };`;
  ...
  check(
    ...
    String(refuseReason('/nope/does/not/exist', false, MIN_FORMS))
  );
  check(
    'REFUSAL CONTROL: a root that DOES exist is not refused, or the gate could never run',
    refuseReason('/tmp', true, MIN_FORMS) === null,
    String(refuseReason('/tmp', true, MIN_FORMS))
  );
  check(
    'REFUSAL: a floor of zero is refused, because no scan can fall below it',
    (refuseReason('/tmp', true, 0) ?? '').includes('no scan can fall below'),
    String(refuseReason('/tmp', true, 0))
  );
...
/**
 * The refusal that keeps a zero-file scan from reading as a clean tree.
 *
 * EXTRACTED SO IT CAN BE CONTROLLED. It worked -- driven against an empty
 * `--root` on 2026-09-08 both gates printed `✗ Refusing to run: ... does not
 * exist.` and exited 1 -- but nothing in either selftest asserted it, so
 * deleting the two lines would have removed the guard silently and left a gate
 * that scans nothing and reports green. That is the anti-vacuity hole these
 * floors exist to close, one level up.
 *
 * Returns the message, or null when the scan may proceed.
 */
export function refuseReason(dir: string, exists: boolean, floor: number): string | null {
  if (!exists) return `✗ Refusing to run: ${dir} does not exist.`;
  if (floor < 1) {
    return (
      `✗ Refusing to run: the floor under ${dir} is ${floor}, which no scan can fall ` +
      'below, so it would license a green over an empty tree.'
    );
  }
  return null;
}
...
function main(): void {
  ...
  const missing = refuseReason(dir, fs.existsSync(dir), MIN_FORMS);
  if (missing) {
