 ...
 *
 * Run: npx tsx scripts/gates/check-backup-bucket-conformance.ts
 *
...
// ── Control: the extractor, both directions, before it is trusted ──────────
{
  ...
  // `.replace()` whose needle has gone returns the fixture UNCHANGED, and the control below
  // then fails with "an absent jurisdiction was reported as present" -- true of the mutant
  // it built, and completely misleading about why. This says the real thing instead.
  const JUR_NEEDLE = 'jurisdiction = "eu"\n\n[vars]';
  const jurHits = fixture.split(JUR_NEEDLE).length - 1;
  if (jurHits !== 1) {
    console.error(
      `✗ instrument control: the fixture contains the jurisdiction needle ${jurHits} time(s), ` +
        `expected exactly 1, so the mutant below would not differ from the clean fixture.`
    );
    process.exit(1);
  }
  const noJur = extractBinding(fixture.replace(JUR_NEEDLE, '\n[vars]'), BINDING);
