 ...
 *
 * Usage:
 *   npx tsx scripts/gates/check-translation-hashes.ts
    ...
    if (!fs.existsSync(config.dir)) {
      errors.push(
        `[${config.name}] Configured locale directory does not exist: ${config.dir}\n` +
          ...
          `scripts/gates/check-translation-hashes.ts (and its writer half, ` +
          `scripts/gen/generate-translation-hashes.ts) to match reality.`
      );
      continue;
    }
    try {
