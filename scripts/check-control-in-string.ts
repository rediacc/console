...
export function controlTextInTemplates(source: string, fileName = 'x.ts'): Finding[] {
  ...
  const found: Finding[] = [];
  const re = /\b(check|ck|control|assertEq)\s*\(\s*['"]/g;
...
function selftest(): boolean {
  ...
  const CLEAN = ["const F = `const C = () => {", "  return <div/>;", "};`;", "check('live', true);"].join(
    '\n'
  );
  ...
  check('a control INSIDE a template literal is reported', spliced.length === 1, JSON.stringify(spliced));
