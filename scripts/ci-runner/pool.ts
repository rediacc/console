...
export async function runPool(specs: readonly GateSpec[], opts: PoolOptions): Promise<GateResult[]> {
    ...
    const expected = (spec: GateSpec): number => opts.durations.get(spec.id) ?? (spec.weight ?? 1) * 5000;
    // Clamped: a gate declaring more weight than the whole budget would never
    // be admissible and would hang the pool at --jobs 1.
    const effWeight = (spec: GateSpec): number => Math.min(Math.max(1, spec.weight ?? 1), Math.max(1, opts.jobs));
    ...
    return specs.map((spec) => {
        ...
        if (result === undefined) throw new Error(`ci-runner: internal error, no result for ${spec.id}`);
        return result;
    });
}
