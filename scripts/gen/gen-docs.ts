 ...
 *
 * USAGE
 *   tsx scripts/gen/gen-docs.ts            verify: exit non-zero if any region has drifted
 *   tsx scripts/gen/gen-docs.ts --write    rewrite every discovered region
 *   tsx scripts/gen/gen-docs.ts --list     provider row counts, nothing written
...
function writeSnapshot(root: string, force: boolean): number {
  ...
  const snap: Snapshot = {
    ...
    why: [
      ...
      'Regenerate ONLY with `tsx scripts/gen/gen-docs.ts --snapshot --force`, deliberately.',
    ],
...
function main(argv: string[]): number {
  ...
  if (drifted.length > 0) {
    console.error(
      `\n${RED}${drifted.length} file(s) drifted.${NC} Run: npx tsx scripts/gen/gen-docs.ts --write`
    );
    return 1;
  }
  return 0;
