 ...
 *
 * Usage: npx tsx scripts/gen/validate-cli-examples.ts
 *
 * ---- gate ----
 * step: CLI examples
 ...
 *
 * PYTHON'S TEST CONVENTION IS HERE TOO, and its absence was the same language gap this
 * repo has now paid for three times in one afternoon: a predicate that enumerated the
 * TypeScript idiom and nothing else. This pattern knew `__tests__`, `.test.ts` and
 * `.spec.ts`; Python names a test `test_*.py` under a `tests/` directory, and the moment
 * the scan reached `.ci/rediacc_ci/**` on 2026-09-08 it reported 17 errors of which 16
 * came from three gate TESTS quoting deliberately-wrong commands -- the exact content
 * this comment already promised to skip. The exclusion was right; it just could not see
 * the files it was written for. */
const COMMAND_PATH_IGNORE =
  /(?:__tests__|\.test\.ts$|\.spec\.ts$|(?:^|\/)tests?\/|(?:^|\/)test_[^/]*\.py$|_test\.py$)/;
...
async function scanForStaleCommandPaths(violations: Violation[]): Promise<void> {
  ...
  for (const relPath of unique) {
    ...
    const python = relPath.endsWith('.py');
    const hits = relPath.endsWith('.sh')
      ? scanShellText(content)
      : scanSourceText(content, { python });
    for (const hit of hits) {
      violations.push({
        file: relPath,
        line: hit.line,
