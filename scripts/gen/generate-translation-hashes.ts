 ...
 *
 * Usage:
 *   npx tsx scripts/gen/generate-translation-hashes.ts
...
function processLocaleDir(config: LocaleConfig): Record<string, string> | null {
  ...
  // VACUITY FLOOR, placed once for BOTH branches below: each reads the same directory and either would hash zero namespaces into an empty hash file, which the next run then compares against happily. A lost corpus must not read as "nothing changed".
