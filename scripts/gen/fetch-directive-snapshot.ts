 ...
 *
 * Run modes:
 *   tsx scripts/gen/fetch-directive-snapshot.ts                       (English, fetch from EUR-Lex)
 *   tsx scripts/gen/fetch-directive-snapshot.ts --use-local           (English, use the vendored CELEX PDF)
 *   tsx scripts/gen/fetch-directive-snapshot.ts --lang de             (German, fetch from EUR-Lex)
 *   tsx scripts/gen/fetch-directive-snapshot.ts --all                 (loop over all 7 EU languages we ship)
 *   tsx scripts/gen/fetch-directive-snapshot.ts --check               (English; compare hash to manifest)
 *   tsx scripts/gen/fetch-directive-snapshot.ts --check --all         (per-language drift check)
 *
 * The snapshots are the source of truth for scripts/gates/check-directive-quotes.ts.
