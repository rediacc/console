...
#
# Other prerequisites:
#   - the Bitwarden bootstrap token (~/.config/rediacc-console/bws-access-token);
...
source "$SCRIPT_DIR/lib/bench-preflight.sh"
log_step "Preflight: installed dependencies match the lockfiles"
bench_preflight_lockfile "$ROOT_DIR" "$ROOT_DIR/private/account" || exit 1
log_step "Preflight: every R2 bucket bound in $CONFIG exists"
(cd "$WORKER_DIR" && bench_preflight_buckets "$CONFIG") || exit 1
...
# NOT in the deploy-bench profile yet: the bench collector credential has no store
# entry until `./run.sh rotation rotate otlp-bench` gains a `bitwarden-sm:` consumer
# (PLAN-account-env-to-bws T13, owned by the rotation work). Until then it must be
# exported by the caller, and this guard is what says so.
