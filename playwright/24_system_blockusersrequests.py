#!/usr/bin/env python3
"""
System Block User Requests Test - Fixed Version
...
def run(playwright: Playwright) -> None:
    ...
    try:
        print("Starting System Block User Requests Test...")
        ...
        screenshot_path = Path(__file__).parent / "artifacts" / "screenshots" / "system_blockusers_before_mode_switch.png"
            ...
            for selector in expert_radio_selectors:
                try:
                    ...
                    if expert_element.is_visible():
                        ...
                        screenshot_path_after = Path(__file__).parent / "artifacts" / "screenshots" / "system_blockusers_after_mode_switch.png"
        ...
        # Click block user requests button
        print("9. Looking for block user requests button...")
        block_button_found = False
        
        try:
            block_button = page.get_by_role("button", name="lock Block User Requests")
            if block_button.is_visible():
                block_button.click()
                block_button_found = True
                print("   Block user requests button clicked")
        except:
            pass
        
        if not block_button_found:
            # Try alternative selectors
            print("   Trying alternative selector for block button...")
            try:
                block_selectors = [
                ...
                for selector in block_selectors:
                    try:
                        block_button = page.locator(selector).first
                        if block_button.is_visible():
                            block_button.click()
                            block_button_found = True
                            print("   Block user requests button clicked using alternative selector")
                            break
                    except:
                        continue
            except Exception as e:
                print(f"   Error finding block button: {str(e)}")
        
        if not block_button_found:
            print("   Warning: Could not find block user requests button")
        else:
            time.sleep(1)  # Wait for confirmation dialog
            
            # Confirm blocking
            print("10. Confirming block user requests...")
            confirm_found = False
            
            try:
                confirm_button = page.get_by_role("button", name="Yes, Block Users")
                    ...
                    for selector in confirm_selectors:
                        try:
                            ...
                            if confirm_button.is_visible():
                                confirm_button.click()
                                confirm_found = True
                                print("   Block user requests confirmed using alternative selector")
                                break
                        except:
                            continue
                except Exception as e:
                    print(f"   Error confirming block: {str(e)}")
            
            if not confirm_found:
                print("   Warning: Could not confirm block user requests")
            else:
                time.sleep(2)  # Wait for blocking to complete
                print("   User requests blocking completed")
        ...
        if 'page' in locals():
            ...
            screenshot_path = Path(__file__).parent / "artifacts" / "screenshots" / "error_screenshot_blockusers.png"
            page.screenshot(path=str(screenshot_path))
