#!/usr/bin/env python3
"""
Container Restart Test - Smart Version
Tests the container restart functionality in Rediacc console with intelligent waits and config-based values
"""
...
class ContainerRestartTest:
    """Smart container restart test with configuration support"""
    ...
    def expand_repository(self):
        ...
        # Wait for repository table to be present
        has_repos = self._wait_for_element_state('[data-testid*="machine-repo-list"], text=Repository, .ant-table', timeout=3000)
        
        if not has_repos:
            print("   No repository table found, skipping...")
            return False
    ...
    def restart_container(self):
        """Find and restart container with intelligent detection"""
            ...
            for i in range(container_buttons.count()):
                try:
                    ...
                    if button.is_visible():
                        print(f"   Found container actions button (index {i})")
                        button.click()
        ...
        # Click container restart option
        print("10. Clicking container_restart...")
        
        # Wait for menu to appear
        self._wait_for_element_state('text=container_restart', timeout=2000)
        
        restart_option = self.page.get_by_text("container_restart")
        if restart_option.is_visible():
            restart_option.click()
            print("    Container restart initiated!")
            
            # Wait for any success notification or state change
            ...
            for selector in success_selectors:
                if self._wait_for_element_state(selector, timeout=3000):
                    success_found = True
                    break
            
            if success_found:
                print("    Container restart confirmed successful!")
            else:
                print("    Container restart initiated (awaiting confirmation)")
            
            return True
        else:
            print("    Could not find container_restart option")
            return False
    ...
    def run(self, playwright: Playwright) -> None:
        """Main test execution"""
        try:
            print("Starting Container Restart Test (Smart Version)...")
            ...
            # Restart container if repository was expanded
            if repo_expanded:
                success = self.restart_container()
            else:
                print("Skipping container restart as no repository was expanded")
                success = False
            
            if success:
                print("\nTest completed successfully!")
                self.take_screenshot("container_restart_success")
            else:
                print("\nTest completed with warnings")
                self.take_screenshot("container_restart_warning")
            ...
            self.take_screenshot("container_restart_error")
            raise
