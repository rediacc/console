#!/usr/bin/env python3
"""
Container Pause Test - Fixed Version
...
def run(playwright: Playwright) -> None:
    ...
    try:
        print("Starting Container Pause Test...")
        ...
        if not container_found:
            ...
            # Click container pause option
            print("10. Clicking container_pause...")
            time.sleep(0.5)  # Wait for menu
            
            pause_option = page.get_by_text("container_pause")
            if pause_option.is_visible():
                pause_option.click()
                print("    Container pause initiated!")
                time.sleep(2)  # Wait for pause to complete
                print("    Container should be paused now")
            else:
        ...
        if 'page' in locals():
            ...
            screenshot_path = Path(__file__).parent / "artifacts" / "screenshots" / "error_screenshot_pause.png"
