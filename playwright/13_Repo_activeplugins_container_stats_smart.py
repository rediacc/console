#!/usr/bin/env python3
"""
Container Stats Test - Smart Version
Tests the container stats functionality in Rediacc console with intelligent waits and config-based values
"""
...
class ContainerStatsTest:
    """Smart container stats test with configuration support"""
    ...
    def view_container_stats(self):
        """Find and view container stats with intelligent detection"""
        ...
        self._wait_for_element_state('text=container_stats', timeout=2000)
        
        stats_option = self.page.get_by_text("container_stats")
        if stats_option.is_visible():
            stats_option.click()
            print("    Container stats initiated!")
            
            # Wait for stats modal or view to appear
            stats_indicators = [
                '.ant-modal:has-text("Container Stats")',
                '.ant-modal:has-text("Statistics")',
                'text=CPU Usage',
                'text=Memory Usage',
                'text=Network I/O',
                '[data-testid*="stats"]',
            ...
            for indicator in stats_indicators:
                if self._wait_for_element_state(indicator, timeout=3000):
                    stats_found = True
                    print("    Stats view opened successfully!")
                    break
            
            if not stats_found:
                print("    Stats view opened (waiting for data to load)")
                # Wait a bit for stats to load
                self.page.wait_for_timeout(1000)
            
            # Capture stats information if available
            self._capture_stats_info()
    ...
    def _capture_stats_info(self):
        """Capture and log container stats information"""
        stats_captured = False
        
        # Wait for stats to load
        self.page.wait_for_timeout(500)
        
        # Try different patterns for CPU stats
        cpu_patterns = [
            'text=/CPU.*[0-9]+/',
            'text=/cpu.*[0-9]+/i',
            ':has-text("CPU"):has-text("%")',
            '.cpu-usage',
            '[data-testid*="cpu"]'
        ]
        ...
        for pattern in cpu_patterns:
            try:
                cpu_usage = self.page.locator(pattern).first
                if cpu_usage.count() > 0 and cpu_usage.is_visible():
                    cpu_text = cpu_usage.text_content()
                    print(f"    CPU Stats: {cpu_text}")
                    stats_captured = True
                    break
            except:
                continue
        
        # Try different patterns for Memory stats
        mem_patterns = [
            'text=/Memory.*[0-9]+/',
            'text=/mem.*[0-9]+/i',
            ':has-text("Memory"):has-text("MB")',
            ':has-text("Memory"):has-text("GB")',
            '.memory-usage',
            '[data-testid*="memory"]'
        ]
        ...
        for pattern in mem_patterns:
            try:
                mem_usage = self.page.locator(pattern).first
                if mem_usage.count() > 0 and mem_usage.is_visible():
                    mem_text = mem_usage.text_content()
                    print(f"    Memory Stats: {mem_text}")
                    stats_captured = True
                    break
            except:
                continue
        
        # Try different patterns for Network stats
        net_patterns = [
            'text=/Network.*[0-9]+/',
            'text=/net.*[0-9]+/i',
            ':has-text("Network"):has-text("KB/s")',
            ':has-text("Network"):has-text("MB/s")',
            '.network-io',
            '[data-testid*="network"]'
        ]
        ...
        for pattern in net_patterns:
            try:
                network_io = self.page.locator(pattern).first
                if network_io.count() > 0 and network_io.is_visible():
                    net_text = network_io.text_content()
                    print(f"    Network Stats: {net_text}")
                    stats_captured = True
                    break
            except:
                continue
        
        if not stats_captured:
            print("    Stats are loading or in a different format")
    ...
    def run(self, playwright: Playwright) -> None:
        """Main test execution"""
        try:
            print("Starting Container Stats Test (Smart Version)...")
            ...
            if not machine_expanded:
                ...
                self.take_screenshot("container_stats_skip")
                return
            ...
            if not repo_expanded:
                ...
                self.take_screenshot("container_stats_no_repos")
                return
            
            # View container stats
            success = self.view_container_stats()
            
            if success:
                print("\nTest completed successfully!")
                self.take_screenshot("container_stats_success")
                
                # Keep stats open for a moment to capture
                ...
                self.take_screenshot("container_stats_warning")
            ...
            self.take_screenshot("container_stats_error")
            raise
