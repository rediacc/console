#!/usr/bin/env python3
"""
Container Logs Test - Fixed Version
...
def run(playwright: Playwright) -> None:
    ...
    try:
        print("Starting Container Logs Test...")
        ...
        try:
            # Try to find repository with test-id
            repo_cell = page.get_by_test_id("machine-repo-list-table").get_by_role("cell", name=re.compile("repo", re.IGNORECASE))
            if repo_cell.is_visible():
                repo_cell.locator("span").first.click()
                print("   Repository expanded")
            else:
                raise Exception("Repository cell not found")
        except:
        ...
        if 'page' in locals():
            ...
            screenshot_path = Path(__file__).parent / "artifacts" / "screenshots" / "error_screenshot_logs.png"
