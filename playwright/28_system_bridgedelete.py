#!/usr/bin/env python3
"""
System Bridge Delete Test - Fixed Version
...
def run(playwright: Playwright) -> None:
    ...
    try:
        print("Starting System Bridge Delete Test...")
        ...
        screenshot_path = Path(__file__).parent / "artifacts" / "screenshots" / "system_bridgedelete_before_mode_switch.png"
            ...
            for selector in expert_radio_selectors:
                try:
                    ...
                    if expert_element.is_visible():
                        ...
                        screenshot_path_after = Path(__file__).parent / "artifacts" / "screenshots" / "system_bridgedelete_after_mode_switch.png"
        ...
        # Find and click delete button for specific bridge
        print("9. Looking for bridge delete button...")
        delete_found = False
        bridge_name = "testbridge012"
        
        try:
            # Try specific test-id first
            delete_button = page.get_by_test_id(f"system-bridge-delete-button-{bridge_name}")
            if delete_button.is_visible():
                delete_button.click()
                delete_found = True
                print(f"   Delete button clicked for bridge: {bridge_name}")
        except:
            pass
        ...
        if not delete_found:
            ...
            try:
                # Look for bridge row and delete button
                bridge_row = page.locator(f'tr:has-text("{bridge_name}")').first
                if bridge_row.is_visible():
                    delete_btn = bridge_row.locator('button:has-text("Delete")').first
                    if not delete_btn.is_visible():
                        delete_btn = bridge_row.locator('button[title*="delete"]').first
                    if not delete_btn.is_visible():
                        delete_btn = bridge_row.locator('button[title*="Delete"]').first
                    if not delete_btn.is_visible():
                        delete_btn = bridge_row.locator('[data-testid*="delete"]').first
                    
                    if delete_btn.is_visible():
                        delete_btn.click()
                        delete_found = True
                        print(f"   Delete button clicked for bridge in row")
                else:
        ...
        if not delete_found:
            ...
            print("   Bridge might not exist or delete option is not available")
        else:
        ...
        if 'page' in locals():
            ...
            screenshot_path = Path(__file__).parent / "artifacts" / "screenshots" / "error_screenshot_bridgedelete.png"
            page.screenshot(path=str(screenshot_path))
