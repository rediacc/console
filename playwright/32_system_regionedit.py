#!/usr/bin/env python3
"""
System Region Edit Test - Fixed Version
...
def run(playwright: Playwright) -> None:
    ...
    try:
        print("Starting System Region Edit Test...")
        ...
        screenshot_path = Path(__file__).parent / "artifacts" / "screenshots" / "system_regionedit_before_mode_switch.png"
            ...
            for selector in expert_radio_selectors:
                try:
                    ...
                    if expert_element.is_visible():
                        ...
                        screenshot_path_after = Path(__file__).parent / "artifacts" / "screenshots" / "system_regionedit_after_mode_switch.png"
        ...
        print("9. Looking for region edit button...")
        edit_button_found = False
        region_name = "test001"
        
        try:
            edit_button = page.get_by_test_id(f"system-region-edit-button-{region_name}")
            if edit_button.is_visible():
                edit_button.click()
                edit_button_found = True
                print(f"   Edit button clicked for region: {region_name}")
        except:
            pass
        ...
        if not edit_button_found:
            ...
            try:
                # Look for region row and edit button
                region_row = page.locator(f'tr:has-text("{region_name}")').first
                if region_row.is_visible():
                    edit_btn = region_row.locator('button:has-text("Edit")').first
                    if not edit_btn.is_visible():
                        edit_btn = region_row.locator('button[title*="edit"]').first
                    if not edit_btn.is_visible():
                        edit_btn = region_row.locator('button[title*="Edit"]').first
                    if not edit_btn.is_visible():
                        edit_btn = region_row.locator('[data-testid*="edit"]').first
                    
                    if edit_btn.is_visible():
                        edit_btn.click()
                        edit_button_found = True
                        print(f"   Edit button clicked for region in row")
                else:
        ...
        if not edit_button_found:
            ...
            try:
                ...
                if region_name_input.is_visible():
                    ...
                    region_name_input.fill("test002")
                    region_name_edited = True
                    print("   Region name changed to: test002")
            except:
                # Try alternative selectors
                ...
                for selector in region_name_selectors:
                    try:
                        ...
                        if region_name_input.is_visible():
                            ...
                            region_name_input.fill("test002")
                            region_name_edited = True
                            print("   Region name changed using alternative selector: test002")
                            break
                    except:
                        continue
                ...
                for selector in submit_selectors:
                    try:
                        ...
                        if submit_button.is_visible():
                            submit_button.click()
                            submit_found = True
                            print("   Region edit submitted using alternative selector")
                            break
                    except:
                        continue
            
            if not submit_found:
                print("   Warning: Could not submit region edit")
            else:
                time.sleep(2)  # Wait for region edit to complete
                print("   Region edit completed")
        ...
        if 'page' in locals():
            ...
            screenshot_path = Path(__file__).parent / "artifacts" / "screenshots" / "error_screenshot_regionedit.png"
            page.screenshot(path=str(screenshot_path))
